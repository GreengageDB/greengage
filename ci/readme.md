...
## pg_upgrade tests from Greengage 6

Change directory to gpdb sources destination. Make sure that directry doesn't contain binary objects from previous builds. Then run:

```bash
docker build -t gpdb7_u22_up67:latest -f ci/Dockerfile.ubuntu.gg67 .
```

```bash
 docker run --name gpdb_upgrade67 --rm -it \
 gpdb7_u22_up67:latest \
 /home/gpadmin/gpdb_src/concourse/scripts/test_upgrade67.bash
```