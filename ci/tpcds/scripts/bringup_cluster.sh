...
# install_gpdb just unpacks bin_gpdb into /usr/local/greenplum-db-devel; we don't
# need install_and_configure_gpdb (that also re-runs ./configure, which needs the
...
# our `set -u` doesn't abort, and so no proxy-interconnect branch is taken.
export MAKE_TEST_COMMAND="" BLDWRAP_POSTGRES_CONF_ADDONS=""
make_cluster
...
su - gpadmin -c '
  ...
  for h in localhost tpcds $(hostname) $(hostname -s); do ssh-keyscan "$h" 2>/dev/null || true; done >> ~/.ssh/known_hosts
