...
if [ "$RETVAL" -ne 0 ]; then
  echo "Error attempting to find Greengage executables in $GPSEARCH"
  exit 1
fi

if [ ! -x "$GPPATH" ]; then
  echo "No executables found for Greengage installation in $GPSEARCH"
  exit 1
fi
