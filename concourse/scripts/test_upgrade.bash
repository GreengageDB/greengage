...
# The host running the coordinator GPDB segment.
MASTER_HOST=

# The GPHOME directories (containing greengage_path.sh, the bin/ directory, and
...
# The old and new clusters' coordinator data directories.
...
validate_local_envvars() {
    ...
    if [ -n "${missing}" ]; then
        ...
        echo
        echo 'Please source greengage_path.sh and gpdemo-env.sh and try again.'
    ...
    if [[ $demodir != */gpAux/gpdemo ]]; then
        ...
        echo 'created using the gpdemo scripts in gpAux. Your coordinator data '
...
load_old_db_data() {
    # Copy the SQL dump over to the coordinator host and load it into the database.
    local dumpfile=$1
...
extract_gpdb_tarball() {
    ...
    ssh -ttn $node_hostname "sudo bash -c \"\
      ...
      sed -ie 's|^GPHOME=.*|GPHOME=${NEW_GPHOME}|' ${NEW_GPHOME}/greengage_path.sh ; \
    \""
}
...
gpinitsystem_for_upgrade() {
    ...
    # Greengage clusters should be upgraded when GUC settings' defaults change.
    ssh -ttn ${MASTER_HOST} '
        source '"${OLD_GPHOME}"'/greengage_path.sh
...
dump_old_master_query() {
    ...
    # run on the old coordinator, pre-upgrade.
    ssh -n ${MASTER_HOST} '
        source '"${OLD_GPHOME}"'/greengage_path.sh
...
if (( $CONCOURSE_MODE )); then
    MASTER_HOST=cdw
    OLD_GPHOME=/usr/local/greengage-db-devel
    NEW_GPHOME=/usr/local/gpdb_master
    DATADIR_PREFIX=/data/gpdata
    OLD_MASTER_DATA_DIRECTORY=/data/gpdata/coordinator/gpseg-1
    NEW_MASTER_DATA_DIRECTORY=/data/gpdata/coordinator-new/gpseg-1
    PSQL_ADDOPTS=
...
segment_gp_dbid=1; # starting at one, the coordinator value, will increment

# TODO: we need to switch the mode argument according to GPDB version
echo "Upgrading coordinator at ${MASTER_HOST}..."
