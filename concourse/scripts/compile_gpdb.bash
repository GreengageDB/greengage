...
function git_info() {
	...
	if [[ -d .git ]]; then
		...
		git log --abbrev-commit --date=relative "${PREV_TAG:+$PREV_TAG..}HEAD" >>${GREENGAGE_INSTALL_DIR}/etc/git-current-changelog.txt
	fi
	popd
}
...
function include_dependencies() {
	...
	mkdir -p ${GREENGAGE_INSTALL_DIR}/{lib/pkgconfig,include}
