...
class GpBuild:
    ...
    def create_demo_cluster(self, install_dir=INSTALL_DIR, no_mirrors=False):
        ...
        return subprocess.call(
            ...
            && {1} make {2} create-demo-cluster DEFAULT_QD_MAX_CONNECT=150"'.format(
                    install_dir, self.source_gcc_env_cmd, NO_MIRROR_STRING
                )
            ],
            cwd="gpdb_src/gpAux/gpdemo",
            shell=True,
        )

    def _run_gpdb_command(
        self, command, stdout=None, stderr=None, source_env_cmd="", print_command=True
    ):
    ...
    def run_explain_test_suite(self, dbexists):
        ...
        source_env_cmd = ""
        if dbexists:
            source_env_cmd = "source {0}/greengage_path.sh && source ~/.bash_profile && source gpdb_src/gpAux/gpdemo/gpdemo-env.sh".format(
                INSTALL_DIR
            )
        else:
            source_env_cmd = "source {0}/greengage_path.sh && source gpdb_src/gpAux/gpdemo/gpdemo-env.sh".format(
                INSTALL_DIR
            )
            ...
            fail_on_error(status)
            status = self._run_gpdb_command(
                "psql -f schema.sql", source_env_cmd=source_env_cmd
            )
            fail_on_error(status)

            with open("load_stats.txt", "w") as f:
                status = self._run_gpdb_command(
                    "psql -q -f stats.sql", stdout=f, source_env_cmd=source_env_cmd
                )
            if status:
        ...
        status = self._run_cmd(
            "{0} && cat gporca-commits-to-test/optional_gucs.txt >> $MASTER_DATA_DIRECTORY/postgresql.conf".format(
                source_env_cmd
            ),
            None,
        )
        ...
        status = self._run_cmd(
            "{0} && echo 'optimizer_segments=32\ngp_segments_for_planner=32' >> $MASTER_DATA_DIRECTORY/postgresql.conf".format(
                source_env_cmd
            ),
            None,
        )
        ...
        for fsql in sql_files:
            if fsql.endswith(".sql") and fsql not in ["stats.sql", "schema.sql"]:
                ...
                with open(output_fname, "w") as fout:
                    ...
                    current_status = self._run_gpdb_command(
                        "psql -a -f sql/{}".format(fsql),
                        stdout=fout,
                        stderr=fout,
                        source_env_cmd=source_env_cmd,
                        print_command=False,
                    )
                    if current_status != 0:
    ...
    def install_dependency(self, dependency_name, untar_dir="/usr/local"):
        ...
        subprocess.call(
            "tar -xzf {0}/*.tar.gz -C {1}".format(dependency_name, untar_dir),
            shell=True,
        )
