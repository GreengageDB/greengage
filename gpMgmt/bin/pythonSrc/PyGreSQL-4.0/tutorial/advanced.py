...
def inherit_demo(pgcnx):
    ...
    print(
        pgcnx.query("""SELECT name, altitude
        FROM cities
        WHERE altitude > 500""")
    )
    print()
    ...
    print(
        pgcnx.query("""SELECT c.name, c.altitude
        FROM cities* c
...
def array_demo(pgcnx):
    ...
    print()
    print(
        pgcnx.query("""SELECT name FROM sal_emp WHERE
    ...
    print(
        pgcnx.query(
            "SELECT sal_emp.schedule[1:2][1:1] FROM sal_emp WHERE sal_emp.name = 'Bill'"
        )
    )
