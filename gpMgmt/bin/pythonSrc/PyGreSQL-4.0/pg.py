...
class DB(object):
    ...
    _date_literals = frozenset(
        "current_date current_time current_timestamp localtime localtimestamp".split()
    )
    ...
    def pkey(self, cl, newpkey=None):
        ...
        if qcl not in self._pkeys:
            ...
            for r in self.db.query(
                ...
                " AND pg_attribute.attnum " + any_indkey
            ).getresult():
    ...
    def get_databases(self):
        ...
        return [
            s[0] for s in self.db.query("SELECT datname FROM pg_database").getresult()
        ]
    ...
    def get_relations(self, kinds=None):
        ...
        where = (
            kinds
            and "pg_class.relkind IN (%s) AND" % ",".join(["'%s'" % x for x in kinds])
            or ""
        )
        return map(
            _join_parts,
    ...
    def get_attnames(self, cl, newattnames=None):
        ...
        if isinstance(newattnames, dict):
            ...
            raise ProgrammingError("If supplied, newattnames must be a dictionary")
    ...
    def get(self, cl, arg, keyname=None):
        ...
        if keyname == "oid":
            ...
            where = " AND ".join([
                "%s = %s" % (k, self._quote(arg[k], attnames[k])) for k in keyname
            ])
    ...
    def update(self, cl, d=None, **kw):
        ...
        if qoid in d:
            ...
            try:
                where = " AND ".join([
                    "%s = %s" % (k, self._quote(d[k], attnames[k])) for k in keyname
                ])
            except KeyError:
