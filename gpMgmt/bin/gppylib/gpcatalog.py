...
class GPCatalog():
    ...
    def __init__(self, dbConnection):
        ...
        # Foreign key relationships are obtained from
    ...
    def _setForeignKeys(self):
        """
        Setup the foreign key relationships amongst the catalogs.
        """
        fk_query = """
select fktable::text, fkcols, pktable::text, pkcols, is_array from pg_get_catalog_foreign_keys();
                   """
        try:
            curs = self._query(fk_query)
            for row in curs.fetchall():
                (fktable, fkcols, pktable, pkcols, is_array) = row
                fk2 = GPCatalogTableForeignKey(fktable,
                                               fkcols,
                                               pktable,
                                               pkcols,
                                               is_array)
                self._tables[fktable]._addForeignKey(fk2)
            curs.close()
        except Exception as e:
...
class GPCatalogTableForeignKey():
    ...
    #   - isArray()           - Returns whether the foreign key is an oidvector
    #
    # --------------------------------------------------------------------
    def getTableName(self):
        return self._tname
    ...
    # Private Implementation functions
    # --------------------------------------------------------------------
    def __init__(self, tname, cols, pktablename, pkey, is_array):
        """
