...
class GPCatalog:
    ...
    def _setPrimaryKeys(self):
        ...
        if self._version >= "4.0":
            self._tables["pg_resqueuecapability"]._setPrimaryKey("resqueueid restypid")
        ...
        try:
            for tname, tdef in self._tidycat.iteritems():
                ...
                for fkdef in tdef["foreign_keys"]:
                    fk2 = GPCatalogTableForeignKey(tname, fkdef[0], fkdef[1], fkdef[2])
    ...
    def _validate(self):
        ...
        for relname in sorted(self._tables):
            ...
            if self._tables[relname].getPrimaryKey() == []:
                logger.warn(
                    "GPCatalogTable: unable to derive primary key for %s" % str(relname)
                )
...
class GPCatalogTable:
    ...
    def getTableColumns(self, with_oid=True, with_acl=True, excluding=None):
        ...
        return [
            ...
            if (
                (with_oid or x != "oid")
                and (with_acl or x != self._acl)
                and (x not in excluding)
            )
        ]
