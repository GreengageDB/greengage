...
class GpMasterEnvironment:
    ...
    def __init__(
        ...
        logger.debug(
            "Read from postgresql.conf max_connections=%s" % self.__masterMaxConnections
        )
        ...
        if readFromMasterCatalog:
            ...
            self.__pgVersion = dbconn.execSQLForSingletonRow(conn, "select version();")[
                0
            ]
    ...
    def getGpHome(self):
        return self.__gpHome

    def getGpVersion(self):
        return self.__gpVersion

    def getPgVersion(self):
        return self.__pgVersion

    def getMasterDataDir(self):
        return self.__masterDataDir

    def getMasterMaxConnections(self):
        return self.__masterMaxConnections
