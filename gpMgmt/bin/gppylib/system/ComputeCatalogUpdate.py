...
class ComputeCatalogUpdate:
    ...
    def __init__(self, gpArray, forceMap, useUtilityMode, allowPrimary):
        ...
        self.dbsegmap = dict([
            (seg.getSegmentDbId(), seg) for seg in gpArray.getSegmentsAsLoadedFromDb()
        ])

        # 'goalsegmap' reflects the desired state of the catalog
        self.goalsegmap = dict([
            (seg.getSegmentDbId(), seg)
            for seg in gpArray.getDbList(includeExpansionSegs=True)
        ])

        # find mirrors and primaries to remove
        self.mirror_to_remove = [
            seg
            for seg in self.dbsegmap.values()  # segment in database
            if seg.isSegmentMirror()  # segment is a mirror
            and (
                seg.getSegmentDbId() not in self.goalsegmap
            )  # but not in goal configuration
        ]
        self.debuglog("mirror_to_remove:          %s", self.mirror_to_remove)

        self.primary_to_remove = [
            seg
            for seg in self.dbsegmap.values()  # segment is database
        ...
        self.primary_to_add = [
            seg
            for seg in self.goalsegmap.values()  # segment in goal configuration
            if seg.isSegmentPrimary()  # segment is a primary
        ...
        initial_segment_to_update = [
            ...
            and (
                seg != self.dbsegmap[seg.getSegmentDbId()]
            )  # but some attributes differ
        ]
        ...
        self.segment_to_update = [
            seg
            for seg in initial_segment_to_update  # segments to update
            if seg.getSegmentDbId()
            not in removeandaddmap  # that don't require remove/add
        ]
        ...
        self.segment_unchanged = [
            ...
            and (
                seg == self.dbsegmap[seg.getSegmentDbId()]
            )  # and attribtutes are all the same
        ]
    ...
    def validate(self):
        ...
        if not self.allowPrimary:
            if len(self.primary_to_add) > 0:
                ...
                raise Exception(
                    "Internal error: Operation may not add primary: %s" % repr(p)
                )

            if len(self.primary_to_remove) > 0:
                p = self.primary_to_remove[0]
                raise Exception(
                    "Internal error: Operation may not remove primary: %s" % repr(p)
                )
        ...
        for seg in self.final_segments():
            ...
            if other is not None:
                raise Exception(
                    "Segments sharing a content id may not have same preferred role: %s and %s"
                    % (repr(seg), repr(other))
                )
        ...
        #
        only_contains_standby_mirror = (
            len(final[ROLE_MIRROR]) == 1 and final[ROLE_MIRROR].get(-1) != None
        )
        if len(final[ROLE_MIRROR]) > 0 and not only_contains_standby_mirror:
            for contentId in final[ROLE_PRIMARY]:
                if (
                    contentId != MASTER_CONTENT_ID
                    and final[ROLE_MIRROR].get(contentId) is None
                ):
                    seg = final[ROLE_PRIMARY][contentId]
                    raise Exception(
                        "Primary must have mirror when mirroring enabled: %s"
                        % repr(seg)
                    )
        ...
        for seg in self.mirror_to_remove_and_add:
            ...
            if seg.isSegmentMaster(current_role=True) or seg.isSegmentStandby(
                ...
                #
                raise Exception(
                    "Internal error: Can only change core details of segments, not masters"
            ...
            if not seg.isSegmentMirror(current_role=True):
                ...
                #
                raise Exception(
                    "Internal error: Can only change core details of mirrors, not primaries"
                    " (on segment %s) (seg %s vs original %s)"
                    % (seg.getSegmentDbId(), repr(seg), repr(originalSeg))
                )
...
if __name__ == "__main__":
    ...
    class xxx:
        def xxx():
            ...
            print(
                ...
                [
                    seg
                    for seg in dbsegmap.values()
                    if seg.getSegmentDbId() not in goalsegmap
                ],
            )
            print(
                "goal not db",
                [
                    seg
                    for seg in goalsegmap.values()
                    if seg.getSegmentDbId() not in dbsegmap
                ],
            )
    ...
    class GpArray1(GpArray):
        ...
        def getDbList(self, includeExpansionSegs):
            return [GpDb(1, 1, "p"), GpDb(2, 1, "m")]
    ...
    class GpArray1a(GpArray):
        ...
        def getDbList(self, includeExpansionSegs):
            return [GpDb(1, 1, "p"), GpDb(2, 1, "m", "a")]
    ...
    class GpArray2(GpArray):
        ...
        def getDbList(self, includeExpansionSegs):
            return [GpDb(1, 1, "p", "a"), GpDb(2, 1, "m", "a")]
    ...
    class GpArray4(GpArray):
        ...
        def getDbList(self, includeExpansionSegs):
            return [GpDb(1, 1, "p"), GpDb(2, 1, "m"), GpDb(3, 2, "p"), GpDb(4, 2, "m")]
    ...
    class GpArray6(GpArray):
        ...
        def getSegmentsAsLoadedFromDb(self):
            return [GpDb(1, 1, "p"), GpDb(2, 1, "m", misc="x")]
    ...
    class GpArray8(GpArrayBad):
        ...
        def getDbList(self, includeExpansionSegs):
            return [GpDb(0, -1, "p", misc="x"), GpDb(1, 1, "p"), GpDb(2, 1, "m")]

        def getSegmentsAsLoadedFromDb(self):
            return [GpDb(0, -1, "p"), GpDb(1, 1, "p"), GpDb(2, 1, "m")]
    ...
    class GpArray9(GpArrayBad):
        ...
        def __init__(s):
            GpArrayBad.__init__(s, [0])

        def getDbList(self, includeExpansionSegs):
            return [GpDb(0, -1, "p", rport=2), GpDb(1, 1, "p"), GpDb(2, 1, "m")]
    ...
    class GpArray10(GpArray):
        ...
        def getDbList(self, includeExpansionSegs):
            return [GpDb(1, 1, "p"), GpDb(2, 1, "m", rport=2)]
    ...
    class GpArray12(GpArrayBad):
        ...
        def __init__(s):
            GpArrayBad.__init__(s, allowPrimary=False)
    ...
    class GpArray14(GpArrayBad):
        ...
        def __init__(s):
            GpArrayBad.__init__(s)

        def getDbList(self, includeExpansionSegs):
            return [GpDb(1, 1, "p"), GpDb(2, 1, "p")]
    ...
    class GpArray16(GpArrayBad):
        ...
        def getDbList(self, includeExpansionSegs):
            return [GpDb(1, 1, "p"), GpDb(2, 1, "m"), GpDb(3, 2, "p")]
