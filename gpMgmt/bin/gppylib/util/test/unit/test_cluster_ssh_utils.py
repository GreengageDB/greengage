...
class SshUtilsTestCase(unittest.TestCase):
    ...
    def test04_exceptions(self, mock_stdout):
        ...
        with mock.patch.object(
            pxssh.pxssh, "login", side_effect=pxssh.ExceptionPxssh("foo")
        ) as mock_login:
            session1 = Session()
            session1.login(["localhost"], "gpadmin", 0.05, 1.0)
            self.assertEqual(
        ...
        with mock.patch.object(
            pxssh.pxssh, "login", side_effect=pxssh.EOF("foo")
        ) as mock_login:
            session2 = Session()
        ...
        with mock.patch.object(
            pxssh.pxssh, "login", side_effect=Exception("foo")
        ) as mock_login:
            session2 = Session()
            session2.login(["localhost"], "gpadmin", 0.05, 1.0)
            self.assertEqual(
    ...
    @patch("os.putenv")
    @patch("sys.stdout", new_callable=StringIO)
    def test05_login_retry_when_term_variable_is_set(
        self, mock_stdout, mock_putenv, mock_getenv
    ):
        """
    ...
    @patch("os.putenv")
    @patch("sys.stdout", new_callable=StringIO)
    def test06_login_does_not_retry_when_term_variable_is_not_set(
        self, mock_stdout, mock_putenv, mock_getenv
    ):
        """
        ...
        with mock.patch.object(
            ...
            self.assertNotIn(
                "Retrying by restoring the TERM env variable.\n", mock_stdout.getvalue()
            )
            mock_stdout.truncate(0)
