...
class WorkerPoolTestCase(unittest.TestCase):
    ...
    def test_RemoteExecutionContext_uses_provided_gphome_when_set(self):
        self.subject = RemoteExecutionContext(
            targetHost="myhost", stdin="my_stdin", gphome="other/gphome"
        )
        cmd = Command("dummy name", "echo 'foo'")
        self.subject.execute(cmd)
    ...
    def test_RemoteExecutionContext_uses_ampersand_multiple(self):
        ...
        self.subject.execute(cmd)
        self.assertEquals(
            "bar=1 && foo=1 && ssh -o StrictHostKeyChecking=no -o ServerAliveInterval=60 localhost "
            '". gphome/greengage_path.sh; bar=1 && foo=1 && ls /tmp"',
            cmd.cmdStr,
        )
