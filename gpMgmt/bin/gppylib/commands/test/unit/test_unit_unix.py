...
class UnixCommandTestCase(GpTestCase):
    ...
    def test_kill_9_segment_processes_info_msg(self):
        ...
        self.subject.logger.info.assert_called_once_with(
            "Terminating processes for segment /data/primary/gpseg0"
        )
    ...
    def test_kill_9_segment_processes_multiple_pids(self):
        ...
        self.subject.kill_9_segment_processes(
            "/data/primary/gpseg0", [123, 456], "sdw1"
        )

        call_expected = [
            call("kill -9 process", "kill -9 123", ctxt=2, remoteHost="sdw1"),
    ...
    def test_kill_9_segment_processes_some_pids_do_not_exist(self):
        ...
        self.subject.kill_9_segment_processes(
            "/data/primary/gpseg0", [123, 456, 789], "sdw1"
        )
        self.subject.logger.info.assert_called_once_with(
            "Terminating processes for segment /data/primary/gpseg0"
        )
    ...
    def test_kill_9_segment_processes_kill_error(self):
        ...
        self.subject.logger.error.assert_called_once_with(
            "Failed to kill process 789 for segment /data/primary/gpseg0: Kill Error"
        )

    @patch(
        "gppylib.commands.unix.get_rsync_version", return_value="rsync version 3.2.7"
    )
    ...
    @patch(
        "gppylib.commands.unix.get_rsync_version", return_value="rsync version 2.6.9"
    )
    @patch("gppylib.commands.unix.LooseVersion", side_effect=["2.6.9", "3.1.0"])
    def test_validate_rsync_version_false(
        self, mock_parse_version, mock_get_cmd_version
    ):
