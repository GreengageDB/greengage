...
from gppylib.commands.gp import (
    ...
    get_postgres_segment_processes,
    is_gprecoverseg_running,
)
...
class GpCommandTestCase(GpTestCase):
    ...
    @patch(
        "gppylib.commands.gp.Command.run", side_effect=[None, None, Exception("Error")]
    )
    @patch(
        "gppylib.commands.gp.Command.get_results",
        side_effect=[
    ...
    @patch(
        "gppylib.commands.gp.Command.get_results",
        return_value=CommandResult(0, "1234", "", True, False),
    )
    ...
    @patch(
        "gppylib.commands.gp.Command.run",
        return_value=CommandResult(0, "", "", True, False),
    )
    ...
    @patch(
        "gppylib.commands.gp.Command.get_results",
        return_value=CommandResult(0, b"\n111\n222\n333", b"", True, False),
    )
    ...
    def test_get_postgres_segment_processes_with_str_pgrep_output(
    ...
    @patch(
        "gppylib.commands.gp.Command.get_results",
        return_value=CommandResult(1, b"", b"", False, False),
    )
