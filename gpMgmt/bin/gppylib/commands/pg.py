    ...
    def validate(self):
        if not self.results.completed or self.results.halt:
            raise ExecutionError(
                "Command did not complete successfully rc: %d" % self.results.rc, self
            )
...
def getProcWithParent(host, targetParentPID, procname):
    ...
    logger.info(
        "couldn't find process with name: %s which is a child of PID: %s"
        % (procname, targetParentPID)
    )
    return (0, 0)
...
def killPgProc(db, procname, signal):
    ...
    if procPID == 0:
        raise Exception(
            "Invalid PID: '0' to kill.  parent postmaster PID: %s" % postmasterPID
        )
...
class PgReplicationSlot:
    ...
    def slot_exists(self):
        count = -1
        logger.debug(
            "Checking if slot {} exists for host:{}, port:{}".format(
                self.name, self.host, self.port
            )
        )
        sql = "SELECT count(*) FROM pg_catalog.pg_replication_slots WHERE slot_name = '{}'".format(
            self.name
        )
        try:
            ...
            raise Exception(
                "Failed to query pg_replication_slots for host:{}, port:{}: {}".format(
        ...
        if count == 0:
            logger.debug(
                "Slot {} does not exist for host:{}, port:{}".format(
                    self.name, self.host, self.port
                )
            )
            return False
    ...
    def drop_slot(self):
        logger.debug(
            "Dropping slot {} for host:{}, port:{}".format(
                self.name, self.host, self.port
            )
        )
        ...
        try:
            ...
            logger.exception(
                "Failed to query pg_drop_replication_slot for host:{}, port:{}: {}".format(
            ...
            raise Exception(
                "Failed to drop replication slot for host:{}, port:{} : {}".format(
                    self.host, self.port, str(ex)
                )
            )

        logger.debug(
            "Successfully dropped replication slot {} for host:{}, port:{}".format(
                self.name, self.host, self.port
            )
        )
    ...
    def create_slot(self):
        ...
        logger.debug(
            "Creating slot {} for host:{}, port:{}".format(
                self.name, self.host, self.port
            )
        )
        sql = "SELECT pg_create_physical_replication_slot('{}', true);".format(
            self.name
        )
        try:
            dburl = dbconn.DbURL(hostname=self.host, port=self.port)
            ...
            logger.exception(
                "Failed to query pg_create_physical_replication_slot for host:{}, port:{}: {}".format(
                    self.host, self.port, str(e)
                )
            )
            return False
            ...
            raise Exception(
                "Failed to create replication slot for host:{}, port:{} : {}".format(
                    self.host, self.port, str(ex)
                )
            )

        logger.debug(
            "Successfully created replication slot {} for host:{}, port:{}".format(
                self.name, self.host, self.port
            )
        )
        return True
...
class PgControlData(Command):
    def __init__(self, name, datadir, ctxt=LOCAL, remoteHost=None):
        ...
        self.data = None
        Command.__init__(
            self, name, "$GPHOME/bin/pg_controldata %s" % self.datadir, ctxt, remoteHost
        )
...
class PgRewind(Command):
    ...
    def __init__(self, name, target_datadir, source_host, source_port, progress_file):
        ...
        rewind_cmd = (
            '[ -f %s/recovery.conf ] || PGOPTIONS="-c gp_session_role=utility" $GPHOME/bin/pg_rewind '
            '--write-recovery-conf --slot="internal_wal_replication_slot" --source-server="%s" '
            "--target-pgdata=%s --progress"
            % (target_datadir, source_server, target_datadir)
        )
...
class PgBaseBackup(Command):
    def __init__(
        ...
        Command.__init__(
            self, "pg_basebackup", cmd_str, ctxt=ctxt, remoteHost=remoteHost
        )
