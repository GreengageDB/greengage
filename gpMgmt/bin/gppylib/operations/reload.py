...
class GpReload:
        ...
        with dbconn.connect(dbconn.DbURL(dbname=self.database, port=self.port)) as conn:
            ...
            for c in sort_column_list:
                if c[0] not in columns:
                    raise ExceptionNoStackTraceNeeded(
                        "Table {schema}.{table} does not have column {col}".format(
                            schema=schema_name, table=table_name, col=c[0]
                        )
                    )
    ...
    def validate_mid_level_partitions(self, schema_name, table_name):
        ...
        with dbconn.connect(dbconn.DbURL(dbname=self.database, port=self.port)) as conn:
            ...
            if (parent_schema, parent_table) == (schema_name, table_name):
                return
            try:
                max_level = dbconn.execSQLForSingleton(
                    conn,
            ...
            try:
                partition_level = dbconn.execSQLForSingleton(
                    conn,
    ...
    def validate_options(self):
        ...
        if not os.path.exists(self.table_file):
            raise ExceptionNoStackTraceNeeded(
                'Unable to find table file "{table_file}"'.format(
        ...
        if self.port is None:
            if "PGPORT" not in os.environ:
                raise ExceptionNoStackTraceNeeded(
                    "Please specify PGPORT using -p option or set PGPORT in the environment"
                )
        ...
        with open(self.table_file) as fp:
            for line in fp:
                ...
                try:
                    ...
                    raise ExceptionNoStackTraceNeeded(
                        "Line '{line}' is not formatted correctly: {ex}".format(
                            line=line, ex=e
                        )
                    )
    ...
    def get_row_count(self, table_name):
        with dbconn.connect(dbconn.DbURL(dbname=self.database, port=self.port)) as conn:
            c = execSQLForSingleton(
                conn, "SELECT count(*) FROM {table}".format(table=table_name)
            )
        return c
            ...
            c = execSQLForSingleton(
                ...
                """SELECT count(*)
                                             ...
                                             WHERE indrelid = (SELECT pg_class.oid
                                                               FROM pg_class, pg_namespace
                                                               WHERE pg_class.relname='{table}' AND pg_namespace.nspname='{schema}' AND pg_class.relnamespace = pg_namespace.oid)""".format(
                    table=table_name, schema=schema_name
                ),
            )
            if c != 0:
                if self.interactive:
                    return ask_yesno(
                        None,
                        "Table {schema}.{table} has indexes. This might slow down table reload. Do you still want to continue ?".format(
                            schema=schema_name, table=table_name
                        ),
                        "N",
                    )
    ...
    def get_table_size(self, schema_name, table_name):
        with dbconn.connect(dbconn.DbURL(dbname=self.database, port=self.port)) as conn:
            size = execSQLForSingleton(
                conn,
                """SELECT pg_size_pretty(pg_relation_size('{schema}.{table}'))""".format(
                    schema=schema_name, table=table_name
                ),
            )
        return size
    ...
    def reload_tables(self):
        with dbconn.connect(dbconn.DbURL(dbname=self.database, port=self.port)) as conn:
            ...
            for schema_name, table_name, sort_column_list in self.table_list:
                logger.info(
                    "Starting reload for table {schema}.{table}".format(
                        schema=schema_name, table=table_name
                    )
                )
                logger.info(
                    "Table {schema}.{table} has {rows} rows and {size} size".format(
                        schema=schema_name,
                        table=table_name,
                ...
                if not self.check_indexes(schema_name, table_name):
                    logger.info(
                        "Skipping reload for {schema}.{table}".format(
                            schema=schema_name, table=table_name
                        )
                    )
                    continue
                ...
                dbconn.execSQL(
                    conn,
                    """CREATE TEMP TABLE temp_{table} AS SELECT * FROM {schema}.{table}""".format(
                        schema=schema_name, table=table_name
                    ),
                )
                temp_row_count = dbconn.execSQLForSingleton(
                    conn,
                    """SELECT count(*) FROM temp_{table}""".format(table=table_name),
                )
                table_row_count = dbconn.execSQLForSingleton(
                    conn,
                    """SELECT count(*) from {schema}.{table}""".format(
                        table=table_name, schema=schema_name
                    ),
                )
                if temp_row_count != table_row_count:
                    raise Exception(
                        "Row count for temp table(%s) does not match(%s)"
                        % (temp_row_count, table_row_count)
                    )
                dbconn.execSQL(
                    conn,
                    "TRUNCATE TABLE {schema}.{table}".format(
                        schema=schema_name, table=table_name
                    ),
                )
                sort_order = ["%s %s" % (c[0], c[1]) for c in sort_column_list]
                parent_schema_name, parent_table_name = self.parent_partition_map[
                    (schema_name, table_name)
                ]
                dbconn.execSQL(
                    conn,
                    """INSERT INTO {parent_schema}.{parent_table} SELECT * FROM temp_{table} ORDER BY {column_list}""".format(
                        parent_schema=parent_schema_name,
                        parent_table=parent_table_name,
                        table=table_name,
                        column_list=",".join(sort_order),
                    ),
                )
                conn.commit()
                end = time.time()
                logger.info(
                    "Finished reload for table {schema}.{table} in time {sec} seconds".format(
                        schema=schema_name, table=table_name, sec=(end - start)
                    )
                )
    ...
    def run(self):
        ...
        logger.info(
            "Validating table file {table_file}".format(table_file=self.table_file)
        )
