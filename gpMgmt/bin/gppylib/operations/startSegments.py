...
class StartSegmentsOperation:
    ...
    def startSegments(self, gpArray, segments, startMethod, era):
        ...
        if numWorkers >= numNodes:
            ...
            assert (
                startMethod == START_AS_PRIMARY_OR_MIRROR
                or startMethod == START_AS_MIRRORLESS
            )
            self.__runStartCommand(
                segments, startMethod, numContentsInCluster, result, gpArray, era
            )
        else:
            ...
            if startMethod == START_AS_PRIMARY_OR_MIRROR:
                ...
                self.__runStartCommand(
                    mirrorDbs, startMethod, numContentsInCluster, result, gpArray, era
                )
                self.__runStartCommand(
                    primaryDbs, startMethod, numContentsInCluster, result, gpArray, era
                )
        ...
        # done!
        assert totalToAttempt == len(result.getFailedSegmentObjs()) + len(
            result.getSuccessfulSegments()
        )
        return result

    def __runStartCommand(
        self, segments, startMethod, numContentsInCluster, resultOut, gpArray, era
    ):
        """
        ...
        mirroringModePreTransition = (
            MIRROR_MODE_MIRRORLESS
            if startMethod == START_AS_MIRRORLESS
            else MIRROR_MODE_QUIESCENT
        )

        # launch the start
        for hostName, segments in GpArray.getSegmentsByHostName(segments).iteritems():
            logger.debug(
                "Dispatching command to start segments on host: %s, "
            ...
            #
            cmd = gp.GpSegStartCmd(
                "remote segment starts on host '%s'" % hostName,
                self.__gpHome,
                segments,
                self.__gpVersion,
                mirroringModePreTransition,
            ...
            if cmd.get_results().rc == 0 or cmd.get_results().rc == 1:
                ...
                for line in lines:
                    if line.startswith("STATUS"):
                        reasonCode, reasonStr, started, dir = parseStatusLine(
                            line, isStart=True
                        )
                ...
                for segment in cmd.dblist:
                    resultOut.addFailure(
                        segment, cmd.get_results(), gp.SEGSTART_ERROR_UNKNOWN_ERROR
                    )

    def __createPickledTransitionParameters(
        self,
        segmentsOnHost,
        targetModePerSegment,
