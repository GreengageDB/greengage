...
class ListRemoteFiles(RawRemoteOperation):
    def __init__(self, path, host):
        ...
        super(ListRemoteFiles, self).__init__(
            """ python -c "import os, pickle; print pickle.dumps(os.listdir('%s'))" """
            % path,
            host,
        )
...
class ListRemoteFilesByPattern(RawRemoteOperation):
    def __init__(self, path, pattern, host):
        ...
        super(ListRemoteFilesByPattern, self).__init__(
            """ python -c "import os, fnmatch, pickle; print pickle.dumps(fnmatch.filter(os.listdir('%s'), '%s'))" """
            % (path, pattern),
            host,
        )

    def __str__(self):
        return "ListRemoteFilesByPattern(%s, %s, %s)" % (
            self.path,
            self.pattern,
            self.host,
        )
...
class CheckRemoteDir(RawRemoteOperation):
    def __init__(self, path, host):
        self.path, self.host = path, host
        super(CheckRemoteDir, self).__init__(
            """ python -c "import os, pickle; print pickle.dumps(os.path.isdir('%s'))" """
            % path,
            host,
        )
...
class MakeRemoteDir(RawRemoteOperation):
    def __init__(self, path, host):
        self.path, self.host = path, host
        super(MakeRemoteDir, self).__init__(
            """ python -c "import os, pickle; print pickle.dumps(os.makedirs('%s'))" """
            % path,
            host,
        )
...
class CheckRemoteFile(RawRemoteOperation):
    def __init__(self, path, host):
        self.path, self.host = path, host
        super(CheckRemoteFile, self).__init__(
            """ python -c "import os, pickle; print pickle.dumps(os.path.isfile('%s'))" """
            % path,
            host,
        )
...
class CheckRemotePath(RawRemoteOperation):
    def __init__(self, path, host):
        self.path, self.host = path, host
        super(CheckRemotePath, self).__init__(
            """ python -c "import os, pickle; print pickle.dumps(os.path.exists('%s'))" """
            % path,
            host,
        )
...
class RemoveRemoteFile(RawRemoteOperation):
    def __init__(self, path, host):
        self.path, self.host = path, host
        super(RemoveRemoteFile, self).__init__(
            """ python -c "import os, pickle; print pickle.dumps(os.remove('%s'))" """
            % path,
            host,
        )
...
class RemoveRemoteTree(RawRemoteOperation):
    def __init__(self, path, host):
        self.path, self.host = path, host
        super(RemoveRemoteTree, self).__init__(
            """ python -c "import shutil, pickle; print pickle.dumps(shutil.rmtree('%s'))" """
            % path,
            host,
        )
    ...
    def execute(self):
        ...
        try:
            for seg in self.segments:
                ...
                if shared_mem:
                    cmd = Command(
                        "clean up shared memory", cmdStr="ipcrm -m %s" % shared_mem
                    )
                    pool.addCommand(cmd)
                pool.join()
