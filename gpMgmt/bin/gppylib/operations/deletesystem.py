...
def validate_pgport(master_data_dir):
    ...
    if env_pgport and pgport != int(env_pgport):
        raise Exception(
            "PGPORT value in %s/postgresql.conf does not match PGPORT environment variable"
            % master_data_dir
        )
    elif not env_pgport:
        logger.debug(
            "PGPORT not set in environment. PGPORT value in %s/postgresql.conf file will be used"
            % master_data_dir
        )
        env_pgport = pgport
