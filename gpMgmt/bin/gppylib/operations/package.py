...
try:
    ...
    from gppylib.operations.unix import (
        CheckFile,
        CheckDir,
        MakeDir,
        RemoveFile,
    ...
    sys.exit(
        "Operation: Cannot import modules.  Please check that you have sourced greengage_path.sh.  Detail: "
        + str(ex)
    )
...
class ArchCompatibilityError(Exception):
    ...
    def __init__(self, requiredarch, foundarch):
        Exception.__init__(
            self, "%s Arch required. %s Arch found" % (requiredarch, foundarch)
        )
...
class RemoteCommand(Operation):
    ...
    def execute(self):
        ...
        for host in self.host_list:
            cmd = Command(
                name="Remote Command", cmdStr=self.cmd_str, ctxt=REMOTE, remoteHost=host
            )
...
class ListPackages(Operation):
    ...
    def execute(self):
        ...
        package_list = ListFilesByPattern(
            GPPKG_ARCHIVE_PATH, "*" + GPPKG_EXTENSION
        ).run()
        ...
        for pkg in package_list:
            ...
            if len(pkg_name.split("-")) >= 2:
                package_name_list.append(
                    pkg_name[: pkg_name.index("-", pkg_name.index("-") + 1)]
                )
            else:
                try:
...
class IsVersionCompatible(Operation):
    ...
    def execute(self):
        ...
        if not required_gpdb_version.isVersionRelease(gpdb_version):
            logger.error(
                "%s requires Greengage Database version %s"
                % (gppkg.pkgname, required_gpdb_version)
            )
            return False
...
class ValidateInstallDebPackage(Operation):
    ...
    def execute(self):
        ...
        deb_packages_path = " ".join([
            os.path.join(TEMP_EXTRACTION_PATH, deb_path) for deb_path in deb_set
        ])

        if self.is_update:
            deb_test_command = (
                "fakeroot dpkg --dry-run --force-not-root --force-depends --log=/dev/null --admindir=%s "
                "--instdir=%s -i %s" % (DEB_DATABASE, GPHOME, deb_packages_path)
            )
        else:
            deb_test_command = (
                "fakeroot dpkg --dry-run --force-not-root --log=/dev/null --admindir=%s "
                "--instdir=%s -i %s" % (DEB_DATABASE, GPHOME, deb_packages_path)
            )
        ...
        archive_package_exists = CheckFile(
            os.path.join(GPPKG_ARCHIVE_PATH, self.gppkg.pkg)
        ).run()
    ...
    def check_existence(self):
        command = (
            "dpkg --force-not-root --log=/dev/null --admindir=%s --instdir=%s -l %s"
            % (DEB_DATABASE, GPHOME, self.gppkg.pkgname)
        )
    ...
    *and* the archive package already exists we declare the package is already installed.
    ...
    def execute(self):
        ...
        if self.gppkg.architecture.lower() != platform.machine().lower():
            raise ArchCompatibilityError(
                self.gppkg.architecture, platform.machine().lower()
            )

        rpm_set = set([self.gppkg.main_rpm] + self.gppkg.dependencies)
        rpm_install_string = " ".join([
            os.path.join(TEMP_EXTRACTION_PATH, rpm) for rpm in rpm_set
        ])
        if self.is_update:
        ...
        logger.info(
            cmd
        )  # TODO: AK: This should be debug(), but RMI cannot propagate a log level.
...
class ValidateUninstallDebPackage(Operation):
    ...
    def execute(self):
        ...
        deb_set = set([
            strip_extension_and_arch(deb_package) for deb_package in deb_list
        ])
        archive_package_exists = CheckFile(
            os.path.join(GPPKG_ARCHIVE_PATH, self.gppkg.pkg)
        ).run()
...
class ValidateUninstallPackage(Operation):
    ...
    *and* the archive package does not exist, we declare the package is not installed.
    ...
    def execute(self):
        ...
        rpm_uninstall_command = "rpm --test -e %s --dbpath %s" % (
            rpm_uninstall_string,
            RPM_DATABASE,
        )
    ...
    def resolve_shared_dependencies(self, rpm_set, dependency_lines):
        ...
        for dependency_line in dependency_lines:
            ...
            cmd = Command(
                "Discerning culprit rpms for %s" % violated_capability,
                "rpm -q --whatprovides %s --dbpath %s"
                % (violated_capability, RPM_DATABASE),
            )
            cmd.run(validateAfter=True)
            # remove the .x86_64 suffix for each rpm package to match the name in rpm_set
            culprit_rpms = set(
                dep.replace(".x86_64", "")
                for dep in cmd.get_results().stdout.splitlines()
            )
...
class ExtractPackage(Operation):
    ...
    def execute(self):
        ...
        if (
            os.path.exists(TEMP_EXTRACTION_PATH)
            and not CleanupDir(TEMP_EXTRACTION_PATH).run()
        ):
        ...
        if os.path.exists(path):
            for cur_file in os.listdir(path):
                shutil.move(
                    os.path.join(TEMP_EXTRACTION_PATH, DEPS_DIR, cur_file),
...
class InstallDebPackageLocally(Operation):
    ...
    def execute(self):
        ...
        if deb_set:
            deb_packages = " ".join([
                os.path.join(TEMP_EXTRACTION_PATH, deb_package)
                for deb_package in deb_set
            ])
            if self.is_update:
                deb_install_command = (
                    "fakeroot dpkg --force-not-root --force-depends --log=/dev/null --admindir=%s "
...
class UninstallDebPackageLocally(Operation):
    ...
    def execute(self):
        ...
        if deb_set:
            deb_uninstall_command = (
                "fakeroot dpkg --force-not-root --log=/dev/null --admindir=%s --instdir=%s -r %s"
...
class UninstallPackageLocally(Operation):
    ...
    def execute(self):
        ...
        if rpm_set:
            rpm_uninstall_command = "rpm -e %s --dbpath %s" % (
                " ".join(rpm_set),
                RPM_DATABASE,
            )
...
class SyncPackages(Operation):
    ...
    def execute(self):
        ...
        master_package_set = set(
            ListFilesByPattern(GPPKG_ARCHIVE_PATH, "*" + GPPKG_EXTENSION).run()
        )
        # set of packages on the remote host
        remote_package_set = set(
            ListRemoteFilesByPattern(
                GPPKG_ARCHIVE_PATH, "*" + GPPKG_EXTENSION, self.host
            ).run()
        )
        ...
        if install_package_set:
            logger.info(
                "The following packages will be installed on %s: %s"
                % (self.host, ", ".join(install_package_set))
            )
            for package in install_package_set:
                logger.debug("copying %s to %s" % (package, self.host))
                dstFile = os.path.join(GPHOME, package)
                Scp(
                    name="copying %s to %s" % (package, self.host),
        ...
        if uninstall_package_set:
            logger.info(
                "The following packages will be uninstalled on %s: %s"
                % (self.host, ", ".join(uninstall_package_set))
            )
            for package in uninstall_package_set:
                if platform.linux_distribution()[0] == "Ubuntu":
                    RemoteOperation(
                        UninstallDebPackageLocally(package), self.host
                    ).run()
                else:
    ...
    def execute(self):
        ...
        if platform.linux_distribution()[0] == "Ubuntu":
            ...
            if self.segment_host_list:
                ...
                HostOperation(
                    InstallDebPackageLocally(dstFile), self.segment_host_list
                ).run()

            # install package on standby
            if self.standby_host:
                Scp(
                    name="copying %s to %s" % (srcFile, self.standby_host),
                    srcFile=srcFile,
                    dstFile=dstFile,
                    dstHost=self.standby_host,
                ).run(validateAfter=True)
                RemoteOperation(
                    InstallDebPackageLocally(dstFile), self.standby_host
                ).run()
            ...
            if self.segment_host_list:
                GpScp(srcFile, dstFile, self.segment_host_list).run()
                HostOperation(
                    InstallPackageLocally(dstFile), self.segment_host_list
                ).run()
...
class UninstallPackage(Operation):
    ...
    def execute(self):
        ...
        if platform.linux_distribution()[0] == "Ubuntu":
            HostOperation(
                UninstallDebPackageLocally(self.gppkg.pkg), self.segment_host_list
            ).run()

            if self.standby_host:
                RemoteOperation(
                    UninstallDebPackageLocally(self.gppkg.pkg), self.standby_host
                ).run()
            ...
            HostOperation(
                UninstallPackageLocally(self.gppkg.pkg), self.segment_host_list
            ).run()

            if self.standby_host:
                RemoteOperation(
                    UninstallPackageLocally(self.gppkg.pkg), self.standby_host
                ).run()
        ...
        # perform any post-installation steps
        PerformHooks(
            hooks=self.gppkg.postuninstall,
            master_host=self.master_host,
            standby_host=self.standby_host,
            segment_host_list=self.segment_host_list,
        ).run()
...
class BuildGppkg(Operation):
    ...
    def execute(self):
        ...
        if self.filename is None:
            pkg = (
                pkg_path_details["pkgname"]
                + "-"
                + str(pkg_path_details["version"])
                + "-"
                + pkg_path_details["os"]
                + "-"
                + pkg_path_details["architecture"]
                + GPPKG_EXTENSION
            )
        else:
            pkg = self.filename
        ...
        with closing(tarfile.open(pkg, "w:gz")) as tarinfo:
            for cur_file in filelist:
                tarinfo.add(name=os.path.join(directory, cur_file), arcname=cur_file)
...
class UpdatePackageLocally(Operation):
    ...
    def execute(self):
        ...
        for archived_package_path in archived_package_paths:
            temp_package = Gppkg.from_package_path(
                os.path.join(GPPKG_ARCHIVE_PATH, archived_package_path)
            )
            if (
                temp_package.pkgname == current_package.pkgname
                and temp_package.version != current_package.version
            ):
                RemoveFile(
                    os.path.join(GPPKG_ARCHIVE_PATH, archived_package_path)
                ).run()
    ...
    def execute(self):
        if not os.path.samefile(self.to_gphome, GPHOME):
            raise ExceptionNoStackTraceNeeded(
                "The target GPHOME, %s, must match the current $GPHOME used to launch gppkg."
                % self.to_gphome
            )
        ...
        for package in packages:
            ...
            try:
                ...
                logger.exception(
                    "Failed to migrate %s from %s" % (old_archive_path, package)
                )
...
class GpScp(Operation):
    ...
    def execute(self):
        self.pool = WorkerPool()
        for host in self.host_list:
            self.pool.addCommand(
                Scp(
                    name="copying %s to %s" % (self.source_path, host),
