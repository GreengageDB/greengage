...
class QueryTestCase(GppkgTestCase):
    ...
    def test04_query_info_expanded(self):
        results = run_command(
            "gppkg --query --info %s" % self.alpha_spec.get_filename()
        )
        self.check_info_results(results, self.alpha_info)

        results = run_command(
            "gppkg --info --query %s" % self.alpha_spec.get_filename()
        )
        self.check_info_results(results, self.alpha_info)
    ...
    def test05_query_list_expanded(self):
        ...
        results = run_command(
            "gppkg --query --list %s" % self.alpha_spec.get_filename()
        )
        self.check_list_results(results, expected_list_result)

        results = run_command(
            "gppkg --list --query %s" % self.alpha_spec.get_filename()
        )
        self.check_list_results(results, expected_list_result)
    ...
    def test06_query_package(self):
        ...
        self.assertTrue(
            "%s is installed" % self.alpha_spec.get_filename().split("-")[0] in results
        )
        ...
        results = run_command("gppkg --query %s" % self.alpha_spec.get_filename())
        self.assertTrue(
            "%s is not installed" % self.alpha_spec.get_filename().split("-")[0]
            in results
        )
