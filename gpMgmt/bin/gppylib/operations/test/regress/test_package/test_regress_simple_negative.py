...
#    @unittest.expectedFailure
    def test00_wrong_os(self):
        ...
        with self.assertRaisesRe(ExecutionError , "%s OS required. %s OS found" % (os, OS)):
    ...
    def test01_wrong_arch(self):
        ...
        with self.assertRaisesRe(ExecutionError, "%s Arch required. %s Arch found" % (arch, ARCH)):
            self.install(gppkg_file)
    ...
    def test02_wrong_gpdbversion(self):
        ...
        with self.assertRaisesRe(ExecutionError, "requires Greengage Database version %s" % gpdb_version):
            self.install(gppkg_file)
    ...
    def test03_install_twice(self):
        ...
        with self.assertRaisesRe(ExecutionError, "%s is already installed" % gppkg_file):
            self.install(gppkg_file)
    ...
    def test04_update_gppkg_lower(self):
        ...
        with self.assertRaisesRe(ExecutionError, "Newer version of %s already installed" % update_gppkg_spec.get_package_name()):
    ...
    def test05_update_rpm_lower(self):
        ...
        with self.assertRaisesRe(ExecutionError, self.A_spec.get_filename()):
            self.update(update_gppkg_file)
        #Check that the original package is still installed and not updated
        assert self.check_install(self.alpha_spec.get_filename())
    ...
    def test06_uninstall_twice(self):
        ...
        with self.assertRaisesRe(ExecutionError, "%s has not been installed" % self.alpha_spec.get_package_name()):
            self.remove(self.alpha_spec.get_filename())

    def test07_invalid_gppkg_name(self):
        invalid_gppkg_name = "abcde-abc"
        with self.assertRaisesRe(ExecutionError, "Cannot find package %s" % invalid_gppkg_name):
