...
class CleanGppkgTestCase(GppkgTestCase):
    ...
    def test02_no_packages_on_cluster(self):
        ...
        for host in cluster_host_list:
            if host is not None:
                ...
                self.assertFalse(
                    CheckRemoteFile(
                        os.path.join(ARCHIVE_PATH, self.alpha_spec.get_filename()), host
                    ).run()
                )
    ...
    def test05_no_rpm_on_master(self):
        ...
        self.assertTrue(
            os.path.exists(os.path.join(ARCHIVE_PATH, self.alpha_spec.get_filename()))
        )
    ...
    def test07_no_package_on_standby(self):
        ...
        RemoveRemoteFile(
            os.path.join(ARCHIVE_PATH, self.alpha_spec.get_filename()), standby
        ).run()
