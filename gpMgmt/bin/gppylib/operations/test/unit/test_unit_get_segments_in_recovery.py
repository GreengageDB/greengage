...
class GetSegmentInrecoveryTestCase(GpTestCase):
    def setUp(self):
        ...
        self.apply_patches([
            patch(
                "gppylib.operations.get_segments_in_recovery.logger",
    ...
    def test_is_seg_in_backup_mode_conn_exception(self, mock1):
        ...
        self.assertEqual(
            "Failed to query pg_is_in_backup() for segment with hostname sdw1, port 6001, error: ",
            str(ex.exception),
        )
    ...
    def test_is_seg_in_backup_mode_returns_true(self, mock1, mock2):
        ...
        self.assertEqual(
            [
                call(
                    "Checking if backup is already in progress for the source server with host sdw1 and "
                    "port 6001"
                )
            ],
    ...
    @patch("gppylib.db.dbconn.connect", autospec=True)
    @patch("gppylib.db.dbconn.execSQL", return_value=FakeCursor(my_list=[[False]]))
