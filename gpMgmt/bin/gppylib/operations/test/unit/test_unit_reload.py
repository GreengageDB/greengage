...
class GpReloadTestCase(unittest.TestCase):
    ...
    def test_validate_options_table_file_not_specified(self):
        ...
        with self.assertRaisesRegexp(
            ExceptionNoStackTraceNeeded, "Please specify table file"
        ):
    ...
    def test_validate_options_table_file_does_not_exist(self, mock1):
        with self.assertRaisesRegexp(
            ExceptionNoStackTraceNeeded, 'Unable to find table file "/tmp/table_file"'
        ):
            self.reload.validate_options()
    ...
    def test_validate_options_database_not_specified(self, mock1):
        ...
        with self.assertRaisesRegexp(
            ExceptionNoStackTraceNeeded, "Please specify the correct database"
        ):
            self.reload.validate_options()
    ...
    def test_validate_table_file_empty_table_name(self):
        ...
        try:
            with self.assertRaisesRegexp(
                ExceptionNoStackTraceNeeded,
                "Line 'public.: a, b' is not formatted correctly",
            ):
    ...
    def test_validate_table_file_empty_schema_name(self):
        ...
        try:
            with self.assertRaisesRegexp(
                ExceptionNoStackTraceNeeded,
                "Line '.t1: a, b' is not formatted correctly",
            ):
                self.reload.validate_table_file()
        finally:
            if os.path.exists(self.reload.table_file):
    ...
    def test_validate_table_file_empty_column_list(self):
        ...
        try:
            with self.assertRaisesRegexp(
                ExceptionNoStackTraceNeeded,
                "Line 'public.t1:' is not formatted correctly",
            ):
                self.reload.validate_table_file()
        finally:
            if os.path.exists(self.reload.table_file):
    ...
    def test_validate_table_file_invalid_sort_order(self):
        ...
        try:
            with self.assertRaisesRegexp(
                ExceptionNoStackTraceNeeded,
                "Line 'public.t2: a, b foo' is not formatted correctly",
            ):
                self.reload.validate_table_file()
        finally:
            if os.path.exists(self.reload.table_file):
    ...
    def test_validate_table_file_valid_data1(self):
        ...
        try:
            expected_output = [
                ("public", "t1", [("a", "asc"), ("b", "asc")]),
                ("public", "t2", [("a", "asc"), ("b", "asc")]),
            ]
    ...
    def test_validate_table_file_valid_data2(self):
        ...
        try:
            expected_output = [
                ("public", "t1", [("a", "asc"), ("b", "desc")]),
                ("public", "t2", [("a", "desc"), ("b", "asc")]),
            ]
            self.assertEqual(expected_output, self.reload.validate_table_file())
        finally:
            if os.path.exists(self.reload.table_file):
    ...
    def test_validate_table_invalid_table(self, mock1, mock2):
        with self.assertRaisesRegexp(
            ExceptionNoStackTraceNeeded, "Table public.t1 does not exist"
        ):
    ...
    def test_validate_columns_invalid_column(self, mock1):
        ...
        with patch("gppylib.operations.reload.execSQL", return_value=m) as p:
            with self.assertRaisesRegexp(
                ExceptionNoStackTraceNeeded, "Table public.t1 does not have column c"
            ):
                self.reload.validate_columns(
                    "public", "t1", [("a", "x"), ("b", "y"), ("c", "z")]
                )
    ...
    @patch("gppylib.operations.reload.dbconn.connect")
    @patch(
        "gppylib.operations.reload.execSQL",
        side_effect=[[("public", "x1")], [("public", "x2")]],
    )
