...
class ConfigurationImplGpdbTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.acting_mirror0 = Segment.initFromString(
            "6|0|m|p|d|n|sdw2|sdw2|50002|/data/acting_mirror0"
        )
    ...
    @patch(
        "gppylib.system.configurationImplGpdb.GpConfigurationProviderUsingGpdbCatalog.fetchSingleOutputRow",
        return_value=[6],
    )
    @patch("gppylib.db.dbconn.executeUpdateOrInsert")
    def test_updateSystemConfigRemoveMirror_remove_acting_mirror(
        self, mockInsert, mockFetch
    ):
        addSQL = self.configProvider.updateSystemConfigRemoveMirror(
            self.conn, self.gpArray, self.acting_mirror0, "foo"
        )
        self.assertEqual(
            addSQL,
            "SELECT gp_add_segment(6::int2, 0::int2, 'm', 'p', 'n', 'd', 50002, 'sdw2', 'sdw2', '/data/acting_mirror0');\nINSERT INTO gp_configuration_history (time, dbid, \"desc\") VALUES(\n\tnow(),\n\t6,\n\t'gprecoverseg: segment config for backout: inserted segment configuration for full recovery or original dbid 6'\n)",
        )
        mockFetch.assert_called_with(
            self.conn, "SELECT gp_remove_segment_mirror(0::int2)"
        )
        mockInsert.assert_any_call(
            self.conn,
    ...
    @patch("gppylib.db.dbconn.executeUpdateOrInsert")
    def test_updateSystemConfigRemoveMirror_remove_actual_mirror(
        self, mockInsert, mockFetch
    ):
        addSQL = self.configProvider.updateSystemConfigRemoveMirror(
            self.conn, self.gpArray, self.mirror0, "foo"
        )
        self.assertEqual(
            addSQL,
            "SELECT gp_add_segment(4::int2, 0::int2, 'm', 'm', 'n', 'd', 50000, 'sdw2', 'sdw2', '/data/mirror0');\nINSERT INTO gp_configuration_history (time, dbid, \"desc\") VALUES(\n\tnow(),\n\t4,\n\t'gprecoverseg: segment config for backout: inserted segment configuration for full recovery or original dbid 4'\n)",
        )
        mockFetch.assert_called_with(
            self.conn, "SELECT gp_remove_segment_mirror(0::int2)"
        )
    ...
    def test_updateSystemConfigRemovePrimary(self, mockInsert, mockFetch):
        addSQL = self.configProvider.updateSystemConfigRemovePrimary(
            self.conn, self.gpArray, self.primary0, "foo"
        )
        self.assertEqual(
            addSQL,
            "SELECT gp_add_segment(2::int2, 0::int2, 'm', 'm', 'n', 'd', 40000, 'sdw1', 'sdw1', '/data/primary0');\nINSERT INTO gp_configuration_history (time, dbid, \"desc\") VALUES(\n\tnow(),\n\t2,\n\t'gprecoverseg: segment config for backout: inserted segment configuration for full recovery or original dbid 2'\n)",
        )
    ...
    def test_updateSystemConfigAddMirror_add_acting_mirror(self, mockInsert, mockFetch):
        removeSQL = self.configProvider.updateSystemConfigAddMirror(
            self.conn, self.gpArray, self.acting_mirror0, "foo"
        )
        self.assertEqual(
            removeSQL,
            "SELECT gp_remove_segment(6::int2);\nINSERT INTO gp_configuration_history (time, dbid, \"desc\") VALUES(\n\tnow(),\n\t6,\n\t'gprecoverseg: segment config for backout: inserted segment configuration for full recovery or original dbid 6'\n)",
        )
        mockFetch.assert_called_with(
            self.conn,
            "SELECT gp_add_segment(6::int2, 0::int2, 'm', 'm', 'n', 'd', 50002, 'sdw2', 'sdw2', '/data/acting_mirror0')",
        )
        mockInsert.assert_any_call(
            self.conn,
    ...
    def test_updateSystemConfigAddMirror_add_actual_mirror(self, mockInsert, mockFetch):
        ...
        self.assertEqual(
            removeSQL,
            "SELECT gp_remove_segment(4::int2);\nINSERT INTO gp_configuration_history (time, dbid, \"desc\") VALUES(\n\tnow(),\n\t4,\n\t'gprecoverseg: segment config for backout: inserted segment configuration for full recovery or original dbid 4'\n)",
        )
        mockFetch.assert_called_with(
            self.conn,
            "SELECT gp_add_segment(4::int2, 0::int2, 'm', 'm', 'n', 'd', 50000, 'sdw2', 'sdw2', '/data/mirror0')",
    ...
    def test_updateSystemConfigAddPrimary(self, mockInsert, mockFetch):
        removeSQL = self.configProvider.updateSystemConfigAddPrimary(
            self.conn, self.gpArray, self.primary0, "foo", {0: self.mirror0}
        )
        self.assertEqual(
            removeSQL,
            "SELECT gp_remove_segment_mirror(0::int2);\nINSERT INTO gp_configuration_history (time, dbid, \"desc\") VALUES(\n\tnow(),\n\t2,\n\t'gprecoverseg: segment config for backout: inserted segment configuration for full recovery or original dbid 2'\n)",
        )
        mockFetch.assert_any_call(
            self.conn,
            "SELECT content FROM pg_catalog.gp_segment_configuration WHERE dbId = 2",
        )
        mockFetch.assert_any_call(
            self.conn,
            "SELECT gp_add_segment(2::int2, 0::int2, 'p', 'p', 'n', 'u', 40000, 'sdw1', 'sdw1', '/data/primary0')",
        )
        mockInsert.assert_any_call(
            self.conn,
    ...
    @patch("gppylib.db.dbconn.executeUpdateOrInsert")
    def test_updateSystemConfigRemoveAddMirror_remove_acting_mirror(
        self, mockInsert, mockFetch
    ):
        addSQL, removeSQL = self.configProvider.updateSystemConfigRemoveAddMirror(
            self.conn, self.gpArray, self.acting_mirror0, "foo"
        )
        self.assertEqual(
            addSQL,
    ...
    @patch("gppylib.db.dbconn.executeUpdateOrInsert")
    def test_updateSystemConfigRemoveAddMirror_remove_actual_mirror(
        self, mockInsert, mockFetch
    ):
        addSQL, removeSQL = self.configProvider.updateSystemConfigRemoveAddMirror(
            self.conn, self.gpArray, self.mirror0, "foo"
