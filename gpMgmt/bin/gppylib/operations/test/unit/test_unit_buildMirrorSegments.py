...
from test.unit.gp_unittest import Contains, GpTestCase, SubTest
...
class BuildMirrorsTestCase(GpTestCase):
    ...
    def setUp(self):
        ...
        self.apply_patches([
            ...
            patch(
                "gppylib.operations.buildMirrorSegments.gplog.get_logger_dir",
                return_value="/tmp/logdir",
            ),
            patch(
                "gppylib.operations.buildMirrorSegments.gplog.logging_is_verbose",
                return_value=False,
            ),
            patch("gppylib.operations.buildMirrorSegments.read_era", self.mock_gp_era),
            patch(
                "gppylib.recoveryinfo.RecoveryResult.print_bb_rewind_differential_update_and_start_errors"
            ),
        ...
        self.recovery_info1 = RecoveryInfo(
            "/datadir2",
            7001,
            2,
            "source_host_for_dbid2",
            7002,
            "source_dir_for_dbid2",
            True,
            False,
            self.progress_file_dbid2,
        )
        self.recovery_info2 = RecoveryInfo(
            "/datadir3",
            7003,
            3,
            "source_host_for_dbid3",
            7004,
            "source_dir_for_dbid3",
            True,
            False,
            self.progress_file_dbid3,
        )
        self.recovery_info3 = RecoveryInfo(
            "/datadir4",
            7005,
            4,
            "source_host_for_dbid4",
            7006,
            "source_dir_for_dbid4",
            True,
            False,
            self.progress_file_dbid4,
        )
        self.recovery_info4 = RecoveryInfo(
            "/datadir5",
            7006,
            5,
            "source_host_for_dbid5",
            7007,
            "source_dir_for_dbid5",
            False,
            True,
            self.progress_file_dbid5,
        )
    ...
    def _setup_build_mirrors_mocks(self, build_mirrors_obj):
        ...
        build_mirrors_obj._run_recovery = Mock(
            return_value=RecoveryResult("action", [], None)
        )
        build_mirrors_obj._run_setup_recovery = Mock(
            return_value=RecoveryResult("action", [], None)
        )
    ...
    def _setup_recovery_mocks(self, host_errors=[]):
        host1_recovery_output = gp.GpSegRecovery(
            "test recover 1",
            "dummy_info1",
            "/tmp/log1/",
            False,
        ...
        host2_recovery_output = gp.GpSegRecovery(
            "test recover 2",
            "dummy_info2",
            "/tmp/log2/",
            True,
        ...
        if host_errors:
            host1_result = CommandResult(
                1, "failed 1".encode(), host_errors[0].encode(), True, False
            )
            host2_result = CommandResult(
                1, "failed 2".encode(), host_errors[1].encode(), True, False
            )
    ...
    def _common_asserts_with_stop_and_logger(
        self,
        build_mirrors_obj,
        expected_logger_msg,
        expected_segs_to_stop,
        expected_segs_to_markdown,
        expected_segs_to_update,
        cleanup_count,
        action_name,
        info_call_count=4,
    ):
        ...
        self.assertEqual(
            [call(expected_segs_to_stop)],
            build_mirrors_obj._get_running_postgres_segments.call_args_list,
        )
    ...
    def _common_asserts(
        ...
        self.assertEqual(
            [call(self.gpEnv, expected_segs_to_markdown)],
            build_mirrors_obj._wait_fts_to_mark_down_segments.call_args_list,
        )
        self.assertEqual(
            cleanup_count, build_mirrors_obj._clean_up_failed_segments.call_count
        )
        self.assertEqual(
            [call(self.default_action_name, ANY)],
            build_mirrors_obj._run_setup_recovery.call_args_list,
        )
        self.assertEqual(
            [call(self.default_action_name, ANY, self.gpEnv)],
            build_mirrors_obj._run_recovery.call_args_list,
        )
        self.assertEqual(
            [
                call(
                    self.gpArray,
                    ANY,
                    dbIdToForceMirrorRemoveAdd=expected_segs_to_update,
    ...
    def _create_mirror(
        self, dbid="2", contentid="0", state="n", status="u", host="sdw2"
    ):
    ...
    def test_recover_mirrors_noMirrors(self):
        build_mirrors_obj = GpMirrorListToBuild(
            toBuild=[], pool=None, quiet=True, parallelDegree=0, logger=self.mock_logger
        )
        self.assertTrue(build_mirrors_obj.recover_mirrors(None, None))

        self.assertEqual(
            [call("No segments to recover")], self.mock_logger.info.call_args_list
        )
    ...
    def test_add_mirrors_noMirrors(self):
        ...
        self.assertEqual(
            [call("No segments to add")], self.mock_logger.info.call_args_list
        )

    def test_add_recover_mirrors_no_failed_pass(self):
        for action in self.actions_to_test:
            with SubTest.subTest(action["action_name"]):
                tests = [
                    {
                        "name": "no_failed_full",
                ...
                self.assertEqual(
                    0, build_mirrors_obj._get_running_postgres_segments.call_count
                )
                self._common_asserts(
                    build_mirrors_obj,
                    [],
            ...
            with SubTest.subTest(action["action_name"]):
                tests = [
                    ...
                    },
                    {
                        ...
                        "failed": self._create_mirror(
                            status="d", dbid="4", host="sdw2"
                        ),
                        "live": self._create_primary(),
                        "forceFull": True,
                    },
                ...
                self._common_asserts_with_stop_and_logger(
                    build_mirrors_obj,
                    "Ensuring 4 failed segment(s) are stopped",
                    expected_segs_to_stop,
                    expected_segs_to_markdown,
                    {4: True, 30: True},
                    1,
            ...
            with SubTest.subTest(action["action_name"]):
                tests = [
                    ...
                    },
                    {
                        ...
                        "failed": self._create_primary(
                            dbid="9", status="d", unreachable=True
                        ),
                        "live": self._create_mirror(dbid="10", state="s"),
                        "failover": self._create_primary(dbid="9", status="d"),
                        "forceFull": False,
                ...
                self._common_asserts_with_stop_and_logger(
                    build_mirrors_obj,
                    "Ensuring 3 failed segment(s) are stopped",
                    expected_segs_to_stop,
                    expected_segs_to_markdown,
    ...
    def test_add_recover_mirrors_forceoverwrite_true(self):
        for action in self.actions_to_test:
            with SubTest.subTest(action["action_name"]):
                ...
                if action_name == "add":
                    self.assertTrue(
                        build_mirrors_obj.add_mirrors(self.gpEnv, self.gpArray)
                    )
                elif action_name == "recover":
                    self.assertTrue(
                        build_mirrors_obj.recover_mirrors(self.gpEnv, self.gpArray)
                    )

                self._common_asserts_with_stop_and_logger(
                    build_mirrors_obj,
                    "Ensuring 1 failed segment(s) are stopped",
                    [failed],
                    [],
                    {1: True},
                    0,
    ...
    def test_recover_mirrors_failed_seg_in_gparray_fail(self):
        ...
        for test in tests:
            mirror_to_build = GpMirrorToBuild(
                test["failed"],
                test["live"],
                test["failover"],
                test["forceFull"],
    ...
    def test_run_setup_recovery_errors(self):
        host1_error = (
            '[{"error_type": "validation", "error_msg":"some error for dbid 2", "dbid": 2, "datadir": "/datadir2", "port": 7001, "progress_file": "/tmp/progress2"}, '
        ...
        with self.assertRaises(ExceptionNoStackTraceNeeded):
            self.default_build_mirrors_obj._run_setup_recovery(
    ...
    def test_run_recovery_no_errors(self):
        ...
        self.default_build_mirrors_obj._run_recovery(
            self.default_action_name, recovery_info, self.gpEnv
        )

        self.assertEqual(
            [call(ANY, progressCmds=ANY, suppressErrorCheck=True)],
            self.default_build_mirrors_obj._GpMirrorListToBuild__runWaitAndCheckWorkerPoolForErrorsAndClear.call_args_list,
        )

        seg_recovery_cmds = self.default_build_mirrors_obj._GpMirrorListToBuild__runWaitAndCheckWorkerPoolForErrorsAndClear.call_args_list[
            0
        ][0][0]
        progress_cmds = self.default_build_mirrors_obj._GpMirrorListToBuild__runWaitAndCheckWorkerPoolForErrorsAndClear.call_args_list[
            0
        ][1]["progressCmds"]
        ...
        self.assertEqual(
            sorted(expected_recovery_cmd_strs[0]), sorted(seg_recovery_cmds[0].cmdStr)
        )
        self.assertEqual("host2", seg_recovery_cmds[1].remoteHost)
        self.assertEqual(
            sorted(expected_recovery_cmd_strs[1]), sorted(seg_recovery_cmds[1].cmdStr)
        )

        self.assertEqual(3, len(progress_cmds))
        self.assertEqual("host1", progress_cmds[0].remoteHost)
        self.assertEqual(
            "set -o pipefail; touch -a /tmp/progress_file2; tail -1 /tmp/progress_file2 | tr '\\r' '\\n' | tail -1",
            progress_cmds[0].cmdStr,
        )
        self.assertEqual("host2", progress_cmds[1].remoteHost)
        self.assertEqual(
            "set -o pipefail; touch -a /tmp/progress_file3; tail -1 /tmp/progress_file3 | tr '\\r' '\\n' | tail -1",
            progress_cmds[1].cmdStr,
        )
        self.assertEqual("host2", progress_cmds[2].remoteHost)
        self.assertEqual(
            "set -o pipefail; touch -a /tmp/progress_file4; tail -1 /tmp/progress_file4 | tr '\\r' '\\n' | tail -1",
    ...
    def test_run_recovery_invalid_errors(self):
        recovery_info = {
            "host1": [self.recovery_info1, self.recovery_info2],
            "host2": [self.recovery_info3, self.recovery_info4],
        }
    ...
    def test_run_recovery_all_dbids_fail_only_bb_rewind_errors(self):
        recovery_info = {
            "host1": [self.recovery_info1, self.recovery_info2],
            "host2": [self.recovery_info3],
        }
        host1_error = (
            '[{"error_type": "full", "error_msg":"some error for dbid 2", "dbid": 2, "datadir": "/datadir2", "port": 7001, "progress_file": "/tmp/progress2"}, '
    ...
    def test_run_recovery_some_dbids_fail_all_bb_rewind_errors(self):
        recovery_info = {
            "host1": [self.recovery_info1],
            "host2": [self.recovery_info2, self.recovery_info3],
        }
    ...
    def test_run_recovery_some_dbids_fail_all_bb_rewind_differential_errors(self):
        recovery_info = {
            "host1": [self.recovery_info1, self.recovery_info4],
            "host2": [self.recovery_info2, self.recovery_info3],
        }
        host1_error = (
            '[{"error_type": "full", "error_msg":"some error for dbid 2", "dbid": 2, "datadir": "/datadir2", "port": 7001, "progress_file": "/tmp/progress2"}, '
    ...
    def test_run_recovery_all_dbids_fail_all_start_errors(self):
        ...
        host1_error = (
            '[{"error_type": "start", "error_msg":"some error for dbid 2", "dbid": 2, "datadir": "/datadir2", "port": 7001, "progress_file": "/tmp/progress2"}, '
    ...
    def test_run_recovery_all_dbids_fail_bb_rewind_differential_and_start_errors(self):
        ...
        host2_error = (
            '[{"error_type": "full",  "error_msg":"some error for dbid 4", "dbid": 4, "datadir": "/datadir4", "port": 7005, "progress_file": "/tmp/progress4"},'
            '{"error_type": "differential", "error_msg":"some error for dbid 5", "dbid": 5, "datadir": "/datadir5", "port": 7006, "progress_file": "/tmp/progress5"}]'
        )
        self._setup_recovery_mocks([host1_error, host2_error])
    ...
    def test_run_recovery_all_dbids_fail_bb_rewind_differential_and_default_errors(
        self,
    ):
        recovery_info = OrderedDict()  # We use ordered dict to deterministically assert the side effects of _run_recovery
    ...
    def test_run_backout_some_dbids_have_errors(self):
        ...
        host1_result = CommandResult(
            1, "failed 1".encode(), host1_error.encode(), True, False
        )
        ...
        recovery_results = RecoveryResult(
            ...
            [host1_recovery_output, host2_recovery_output],
            self.mock_logger,
        )

        self.default_build_mirrors_obj._revert_config_update(
            recovery_results, self.test_backout_map
        )

        self.assertEqual(
            [
                call(Contains("Some mirrors failed during basebackup")),
                call(
                    Contains(
                        "Successfully reverted the gp_segment_configuration updates"
                    )
                ),
            ],
        ...
        self.assertEqual(
            [call(ANY, "BEGIN"), call(ANY, "COMMIT")],
            self.mock_dbconn.execSQL.call_args_list,
        )
        expected_sql = (
        ...
        self.assertEqual(
            [call(ANY, expected_sql, 1)],
            self.mock_dbconn.executeUpdateOrInsert.call_args_list,
        )
    ...
    def test_run_backout_all_dbids_have_errors(self):
        ...
        host2_result = CommandResult(
            1, "failed 2".encode(), host2_error.encode(), True, False
        )
        host1_recovery_output = Mock()
        host2_recovery_output = Mock()
        host1_recovery_output.get_results = Mock(return_value=host1_result)
    ...
    def test_run_backout_all_dbids_have_incremental_errors(self):
        host1_error = (
            '[{"error_type": "incremental", "error_msg":"some error for dbid 2", "dbid": 2, "datadir": "/datadir2", "port": 7001, "progress_file": "/tmp/progress2"}, '
            '{"error_type": "incremental", "error_msg":"some error for dbid 3", "dbid": 3, "datadir": "/datadir3", "port": 7003, "progress_file": "/tmp/progress3"}]'
        )
    ...
    def test_run_backout_all_dbids_have_differential_errors(self):
        host1_error = (
            '[{"error_type": "differential", "error_msg":"some error for dbid 2", "dbid": 2, "datadir": "/datadir2", "port": 7001, "progress_file": "/tmp/progress2"}, '
    ...
    def test_run_backout_no_errors(self):
        host1_recovery_output = Mock()
        host2_recovery_output = Mock()
        host1_recovery_output.get_results = Mock(
            return_value=CommandResult(0, b"", b"", True, False)
        )
        host2_recovery_output.get_results = Mock(
            return_value=CommandResult(0, b"", b"", True, False)
        )
    ...
    def test_run_backout_exception(self):
        self.mock_dbconn.executeUpdateOrInsert.side_effect = Exception(
            "running backout query failed"
        )
        ...
        recovery_results = RecoveryResult(
            self.default_action_name, [host1_recovery_output], self.mock_logger
        )
...
class BuildMirrorSegmentsTestCase(GpTestCase):
    ...
    @patch("gppylib.operations.buildMirrorSegments.get_pid_from_remotehost")
    @patch(
        "gppylib.operations.buildMirrorSegments.is_pid_postmaster", return_value=True
    )
    @patch(
        "gppylib.operations.buildMirrorSegments.check_pid_on_remotehost",
        return_value=True,
    )
    def test_get_running_postgres_segments_all_pid_postmaster(
    ...
    @patch("gppylib.operations.buildMirrorSegments.get_pid_from_remotehost")
    @patch(
        "gppylib.operations.buildMirrorSegments.is_pid_postmaster",
    ...
    def test_get_running_postgres_segments_some_pid_postmaster(
        self, mock1, mock2, mock3
    ):
        mock_segs = [Mock(), Mock()]
        expected_output = []
    ...
    def test_get_running_postgres_segments_one_pid_postmaster(
        self, mock1, mock2, mock3
    ):
        mock_segs = [Mock(), Mock()]
        expected_output = []
    ...
    @patch(
        ...
        return_value=base.CommandResult(
            rc=0, stdout="/tmp/seg0", stderr="", completed=True, halt=False
        ),
    )
    def test_dereference_remote_symlink_valid_symlink(self, mock1, mock2):
        ...
        host = "h1"
        self.assertEqual(
            self.buildMirrorSegs.dereference_remote_symlink(datadir, host), "/tmp/seg0"
        )
    ...
    @patch(
        "gppylib.commands.base.Command.get_results",
        return_value=base.CommandResult(
            rc=1, stdout="", stderr="", completed=True, halt=False
        ),
    )
    def test_dereference_remote_symlink_unable_to_determine_symlink(self, mock1, mock2):
        ...
        self.mock_logger.warning.assert_any_call(
            "Unable to determine if /tmp/seg0 is symlink. Assuming it is not symlink"
        )
    ...
    def test_checkForPortAndDirectoryConflicts__given_the_same_host_checks_ports_differ(
        self,
    ):
        ...
        with self.assertRaisesRegexp(
            Exception,
            r"Segment dbid's 2 and 1 on host samehost cannot have the same port 1111",
        ):
            self.buildMirrorSegs.checkForPortAndDirectoryConflicts(gpArray)

    def test_checkForPortAndDirectoryConflicts__given_the_same_host_checks_data_directories_differ(
        self,
    ):
        self.master.hostname = "samehost"
        self.primary.hostname = "samehost"
        ...
        with self.assertRaisesRegexp(
            Exception,
            r"Segment dbid's 2 and 1 on host samehost cannot have the same data directory '/data'",
        ):
            self.buildMirrorSegs.checkForPortAndDirectoryConflicts(gpArray)
...
class SegmentProgressTestCase(GpTestCase):
    ...
    def setUp(self):
        ...
        self.apply_patches([
            patch(
                "gppylib.recoveryinfo.gplog.get_logger_dir",
                return_value=self.tmp_log_dir,
            ),
        ...
        self.combined_progress_file = "{}/recovery_progress.file".format(
            self.tmp_log_dir
        )
    ...
    def test_command_output_is_displayed_once_after_worker_pool_completes(self):
        cmd1 = self.create_command(
            "localhost", 2, "./pg_basebackup.23432", "string 1\n"
        )
        ...
        self.buildMirrorSegs._join_and_show_segment_progress(
            [cmd1, cmd2, cmd3], outfile=outfile
        )
    ...
    def test_recovery_pattern_returned_matches_recovery_result(self):
        cmd = self.create_command(
            "localhost", 2, "./pg_basebackup.23432", "1164848/1371715 kB (84%)\n"
        )
        ...
        self.assertTrue(
            (re.search(pattern, cmd.get_results.return_value.stdout)) is not None
        )
    ...
    def test_recovery_pattern_returned_not_matches_recovery_result(self):
        ...
        self.assertTrue(
            (re.search(pattern, cmd.get_results.return_value.stdout)) is None
        )
    ...
    def test_inplace_display_uses_ansi_escapes_to_overwrite_previous_output(self):
        cmd1 = self.create_command(
            "localhost", 2, "./pg_basebackup.234324", "string 1\n"
        )
        cmd2 = self.create_command("host2", 4, "./pg_basebackup.234324", "string 2\n")

        cmd1.get_results.side_effect = [
            Mock(stdout="string 1"),
            Mock(stdout="string 2"),
        ]

        cmd2.get_results.side_effect = [
            Mock(stdout="string 3"),
            Mock(stdout="string 4"),
        ]

        outfile = StringIO.StringIO()
        self.pool.join.side_effect = [False, True]
        self.buildMirrorSegs._join_and_show_segment_progress(
            [cmd1, cmd2], inplace=True, outfile=outfile
        )
    ...
    def test_errors_during_command_execution_are_displayed(self):
        cmd1 = self.create_command(
            "localhost", 2, "./pg_basebackup.234324", stderr="some error\n"
        )
    ...
    def test_successful_command_execution_should_delete_the_recovery_progress_file(
        self,
    ):
        cmd1 = self.create_command(
            "host1", 1, "./pg_basebackup.23432", "1164848/1371715 kB (84%)\n"
        )
        cmd2 = self.create_command(
            "host2", 2, "./pg_rewind.23432", "1164858/1371715 kB (90%)\n"
        )
    ...
    def test_error_during_command_execution_should_delete_the_recovery_progress_file(
        self,
    ):
        cmd1 = self.create_command(
            "host1", 1, "./pg_basebackup.23432", stderr="some error\n"
        )
    ...
    def test_join_and_show_segment_progress_writes_progress_file(self):
        ...
        self.assertEqual(
            ...
            [
                ...
                "differential:3:117480 64% 8.34kB/s\n",
            ],
        )
    ...
    def test_join_and_show_segment_progress_overwrites(self):
        ...
        cmd1 = self.create_command(
            "host11", 11, "./pg_rewind.23432", "9964858/9971715 kB (1%)\n"
        )
        cmd2 = self.create_command(
            "host22", 22, "./pg_basebackup.23432", "9964848/9971715 kB (45%)\n"
        )
        cmd3 = self.create_command(
            "host33", 33, "./rsync.23432", "117480 64% 8.34kB/s\n"
        )
        self.buildMirrorSegs._join_and_show_segment_progress(
            [cmd1, cmd2, cmd3], outfile=outfile
        )
        ...
        with open(self.combined_progress_file, "r") as f:
            ...
            self.assertEqual(
                ...
                [
                    ...
                    "differential:33:117480 64% 8.34kB/s\n",
                ],
            )

    def test_verify_recovery_progress_file_when_command_results_are_not_in_expected_format(
        self,
    ):
    ...
    def test_skipping_pg_rewind_is_not_displayed_if_no_result_for_differential_recovery(
        self,
    ):
        cmd1 = self.create_command(
            "host1", 1, "./pg_basebackup.23432", "1164848/1371715 kB (84%)\n"
        ...
        with open(self.combined_progress_file, "r") as f:
            results = f.readlines()
        self.assertEqual(results, ["full:1:1164848/1371715 kB (84%)\n"])
        stdout_results = outfile.getvalue()
        self.assertEqual(
            stdout_results,
            (
                "2024-03-19 16:05:38.202000: host1 (dbid 1): 1164848/1371715 kB (84%)\n"
                "2024-03-19 16:05:38.202000: host2 (dbid 2): skipping pg_rewind on mirror as recovery.conf is present\n"
