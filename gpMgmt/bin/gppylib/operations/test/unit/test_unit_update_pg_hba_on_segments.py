...
class UpdatePgHBAConfTests(GpTestCase):
    def setUp(self):
        def _setup_gparray():
            master = Segment.initFromString("1|-1|p|p|s|u|cdw|cdw|5432|/data/master")
        ...
        self.apply_patches([
            patch(
                "gppylib.operations.update_pg_hba_on_segments.logger",
...
host replication gpadmin {ip_primary2}/32 trust"""
    ...
    @patch(
        "gppylib.operations.update_pg_hba_on_segments.gp.IfAddrs.list_addrs",
        return_value=["192.168.1.1", "192.168.2.1"],
    )
    def test_create_entries(self, mock_ifaddrs):
        ...
        entries_str = "\n".join(entries)
        entries_expected = self.entries_block.format(
            ip_primary1="192.168.1.1",
    ...
    @patch(
        "gppylib.operations.update_pg_hba_on_segments.create_entries",
