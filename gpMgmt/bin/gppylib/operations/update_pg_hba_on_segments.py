...
def create_entries(primary_hostname, mirror_hostname, hba_hostnames):
    ...
    entries.append(
        "host replication {username} samehost trust".format(username=unix.getUserName())
    )
    if hba_hostnames:
...
def update_pg_hba_on_segments(
    gpArray, hba_hostnames, batch_size, contents_to_update=None
):
    ...
    for segmentPair in gpArray.getSegmentList():
        ...
        update_cmds.append(
            SegUpdateHba(
                entries, segmentPair.primaryDB.datadir, remoteHost=primary_hostname
            )
        )
...
def update_pg_hba_for_new_mirrors(PgHbaEntriesToUpdate, hba_hostnames, batch_size):
    ...
    for primary_datadir, primary_hostname, newMirror_hostname in PgHbaEntriesToUpdate:
        ...
        update_cmds.append(
            SegUpdateHba(entries, primary_datadir, remoteHost=primary_hostname)
        )
        logger.info(
            "Updating pg_hba.conf entries on primary %s:%s with new mirror %s information"
