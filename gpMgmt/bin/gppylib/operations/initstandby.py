...
def cleanup_pg_hba_backup_on_segment(gparr):
    ...
    try:
        for host, data_dirs_list in host_to_seg_map.items():
            pickled_data_dirs_list = base64.urlsafe_b64encode(
                pickle.dumps(data_dirs_list)
            )
            cmdStr = (
                "$GPHOME/lib/python/gppylib/operations/initstandby.py -d %s -D"
                % pickled_data_dirs_list
            )
            cmd = Command(
        ...
        for item in pool.getCompletedItems():
            ...
            if result.rc != 0:
                logger.error(
                    "Unable to cleanup pg_hba.conf backup file %s" % str(result.stderr)
                )
...
def restore_pg_hba_on_segment(gparr):
    ...
    try:
        for host, data_dirs_list in host_to_seg_map.items():
            ...
            cmdStr = (
                "$GPHOME/lib/python/gppylib/operations/initstandby.py -d %s -r"
                % pickled_data_dirs_list
            )
            cmd = Command(
...
def update_pg_hba_conf_on_segments(gparr, standby_host, is_hba_hostnames=False):
    ...
    pickled_standby_pg_hba_info = base64.urlsafe_b64encode(
        pickle.dumps(standby_pg_hba_info)
    )
    ...
    try:
        for host, data_dirs_list in host_to_seg_map.items():
            ...
            cmdStr = (
                "$GPHOME/lib/python/gppylib/operations/initstandby.py -p %s -d %s"
                % (pickled_standby_pg_hba_info, pickled_data_dirs_list)
            )
            cmd = Command(
                "Update the pg_hba.conf on remote hosts",
                cmdStr=cmdStr,
                ctxt=REMOTE,
                remoteHost=host,
            )
...
def create_parser():
    parser = OptParser(
        option_class=OptChecker, description="update the pg_hba.conf on all segments"
    )
    ...
    parser.add_option(
        "-b", "--backup", action="store_true", help="Backup the pg_hba.conf file"
    )
    parser.add_option(
        "-r", "--restore", action="store_true", help="Restore the pg_hba.conf file"
    )
    parser.add_option(
        "-D",
        "--delete",
