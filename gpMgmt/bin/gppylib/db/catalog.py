...
class RemoteQueryCommand(Command):
    ...
    def run(self):
        ...
        with dbconn.connect(
            dbconn.DbURL(hostname=self.hostname, port=self.port, dbname=self.dbname),
            utility=True,
        ) as conn:
...
def doesSchemaExist(conn, schemaname):
    sql = (
        "SELECT nspname FROM pg_catalog.pg_namespace WHERE nspname = '%s'" % schemaname
    )
    cursor = None
    try:
        cursor = dbconn.execSQL(conn, sql)
        ...
        if numrows == 0:
            ...
            raise CatalogError(
                "more than one entry in pg_namespace for '%s'" % schemaname
            )
    finally:
        if cursor:
            cursor.close()
