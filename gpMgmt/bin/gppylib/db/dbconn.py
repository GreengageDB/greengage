...
class Pgpass:
    ...
    def get_password(self, username, hostname, port, database):
        for entry in self.entries:
            if (
                (entry["hostname"] == hostname or entry["hostname"] == "*")
                and (entry["port"] == str(port) or entry["port"] == "*")
                and (entry["database"] == database or entry["database"] == "*")
                and (entry["username"] == username or entry["username"] == "*")
            ):
    ...
    def __init__(
        ...
        if password is None:
            ...
            if pgpass.pgpass_valid():
                password = pgpass.get_password(
                    self.pguser, self.pghost, self.pgport, self.pgdb
                )
                if password:
...
def connect(
    ...
    if logConn == True:
        (logger.info if timeout is not None else logger.debug)(
            "Connecting to %s" % cstr
        )
    ...
    for i in range(retries):
        try:
            ...
            if "timeout expired" in str(e):
                logger.warning(
                    "Timeout expired connecting to %s, attempt %d/%d"
                    % (dbbase, i + 1, retries)
                )
                continue
            raise
...
class UnexpectedRowsError(Exception):
    def __init__(self, expected, actual, sql):
        ...
        Exception.__init__(
            self,
            "SQL retrieved %d rows but %d was expected:\n%s"
...
def execSQLForSingleton(conn, sql):
    ...
    if len(row) > 1:
        raise Exception(
            "SQL retrieved %d columns but 1 was expected:\n%s" % (len(row), sql)
        )
    return row[0]
...
def executeUpdateOrInsert(conn, sql, expectedRowUpdatesOrInserts):
    ...
    if cursor.rowcount != expectedRowUpdatesOrInserts:
        raise Exception(
            "SQL affected %s rows but %s were expected:\n%s"
