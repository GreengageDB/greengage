...
class GpRecoverSegmentProgram:
    ...
    def getRecoveryActionsBasedOnOptions(self, gpEnv, gpArray):
        if self.__options.rebalanceSegments:
            return GpSegmentRebalanceOperation(
                gpEnv,
                gpArray,
                self.__options.parallelDegree,
                self.__options.parallelPerHost,
                self.__options.replayLag,
            )
        instance = RecoveryTripletsFactory.instance(
            gpArray,
            self.__options.recoveryConfigFile,
            self.__options.newRecoverHosts,
        ...
        segs = [
            GpMirrorToBuild(
                ...
                self.__options.differentialResynchronization,
                t.recovery_type,
            )
    ...
    def syncPackages(self, new_hosts):
        ...
        try:
            ...
            self.logger.exception(
                "Syncing of Greengage Database extensions has failed."
            )
            self.logger.warning(
    ...
    def displayRecovery(self, mirrorBuilder, gpArray):
        ...
        if self.__options.rebalanceSegments:
            ...
            for toRebalance in gpArray.get_unbalanced_segdbs():
                ...
                tabLog.info([
                    "Balanced role",
                    "= Primary" if toRebalance.preferred_role == "p" else "= Mirror",
                ])
                tabLog.info([
                    "Current role",
            ...
            for toRecover in mirrorBuilder.getMirrorsToBuild():
                ...
                programIoUtils.appendSegmentInfoForOutput(
                    "Recovery Source", gpArray, toRecover.getLiveSegment(), tabLog
                )
    ...
    def _get_dblist(self):
        ...
        with dbconn.connect(dbconn.DbURL()) as conn:
            res = dbconn.execSQL(
                conn, "SELECT datname FROM PG_DATABASE WHERE datname != 'template0'"
            )
        return res.fetchall()

    def run(self):
        if (
            self.__options.parallelDegree < 1
            or self.__options.parallelDegree > gp.MAX_MASTER_NUM_WORKERS
        ):
        ...
        if (
            self.__options.parallelPerHost < 1
            or self.__options.parallelPerHost > gp.MAX_SEGHOST_NUM_WORKERS
        ):
            raise ProgramArgumentValidationException(
        ...
        if optionCnt > 1:
            raise ProgramArgumentValidationException(
                "Only one of -p and -r may be specified"
            )
        ...
        if optionCnt > 0 and self.__options.differentialResynchronization:
            raise ProgramArgumentValidationException(
                "Only one of -p, -r and --differential may be specified"
            )

        # verify "mode to recover" options
        if (
            self.__options.forceFullResynchronization
            and self.__options.differentialResynchronization
        ):
            raise ProgramArgumentValidationException(
                "Only one of -F and --differential may be specified"
            )

        # verify differential supported options
        if (
            self.__options.differentialResynchronization
            and self.__options.outputSampleConfigFile
        ):
            raise ProgramArgumentValidationException(
                "Invalid -o provided with --differential argument"
            )

        if self.__options.replayLag and not self.__options.rebalanceSegments:
            raise ProgramArgumentValidationException(
                "--replay-lag should be used only with -r"
            )
        ...
        if (
            self.__options.differentialResynchronization
            and not unix.validate_rsync_version(min_rsync_ver)
        ):
            raise ProgramArgumentValidationException(
                "To perform a differential recovery, a minimum rsync version "
        ...
        if self.__options.outputSampleConfigFile is not None:
            ...
            self.outputToFile(
                mirrorBuilder, gpArray, self.__options.outputSampleConfigFile
            )
            self.logger.info(
            ...
            if not mirrorBuilder.recover_mirrors(gpEnv, gpArray):
                if self.__options.differentialResynchronization:
                    self.logger.error(
                        "gprecoverseg differential recovery failed. Please check the gpsegrecovery.py log"
                        " file and rsync log file for more details."
                    )
                else:
                    self.logger.error(
                        "gprecoverseg failed. Please check the output for more details."
                    )
                sys.exit(1)
            ...
            self.logger.info(
                "Recovered mirror segments need to sync WAL with primary segments."
            )
            self.logger.info(
                "Use 'gpstate -e' to check progress of WAL sync remaining bytes"
            )
    ...
    def validate_heap_checksum_consistency(self, gpArray, mirrorBuilder):
        live_segments = [
            target.getLiveSegment() for target in mirrorBuilder.getMirrorsToBuild()
        ]
        failed_segments = [
            target.getFailedSegment() for target in mirrorBuilder.getMirrorsToBuild()
        ]
        ...
        heap_checksum = HeapChecksum(
            gpArray,
            num_workers=min(self.__options.parallelDegree, len(live_segments)),
            logger=self.logger,
        )
        successes, failures = heap_checksum.get_segments_checksum_settings(
            live_segments + failed_segments
        )
        ...
        consistent, inconsistent, master_checksum_value = (
            heap_checksum.check_segment_consistency(successes)
        )
        if len(inconsistent) > 0:
            ...
            for gpdb in inconsistent:
                ...
                self.logger.fatal(
                    "%s checksum set to %s differs from master checksum set to %s"
                    % (segment_name, checksum, master_checksum_value)
                )
            raise Exception("Heap checksum setting differences reported on segments")
        self.logger.info(
            "Heap checksum setting is consistent between master and the segments that are candidates "
            "for recoverseg"
        )
    ...
    def createParser():
        ...
        addTo.add_option(
            ...
            dest="replayLag",
            metavar="<replayLag>",
            help="Allowed replay lag on mirror, lag should be provided in GBs",
        )
        addTo.add_option(
            "",
            "--hba-hostnames",
