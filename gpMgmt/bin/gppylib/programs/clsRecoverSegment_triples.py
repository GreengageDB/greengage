...
def is_pg_rewind_running(hostname, port):
    ...
    """
    logger.debug(
        "Checking for running instances of pg_rewind with host {} and port {} as source server".format(
            hostname, port
        )
    )
...
def extract_recovery_config_info(parts):
    ...
    if len(parts) == 5:
        if parts[0] not in {"I", "D", "F", "i", "d", "f"}:
            raise Exception(
                "Invalid recovery type provided, please provide any of I,D,F,i,d,f as recovery_type"
            )
        hostname = parts[1]
...
class RecoveryTriplet:
    ...
    def __repr__(self):
        return "Failed: {0} Live: {1} Failover: {2}".format(
            self.failed, self.live, self.failover
        )
    ...
    def validate(failed, live, failover):
        msg = (
            "liveSegment"
            if not failed
            else "No peer found for dbid {}. liveSegment".format(
                failed.getSegmentDbId()
            )
        )
        ...
        if failed is not None:
            ...
            if failed.getSegmentDbId() == live.getSegmentDbId():
                raise ExceptionNoStackTraceNeeded(
                    ...
                    "A segment may not be recovered from itself" % live.getSegmentDbId()
                )
        ...
        if failover is not None:
            if failover.getSegmentContentId() != live.getSegmentContentId():
                raise ExceptionNoStackTraceNeeded(
                    ...
                    "mirror content %d"
                    % (live.getSegmentContentId(), failover.getSegmentContentId())
                )
            if failover.getSegmentDbId() == live.getSegmentDbId():
                raise ExceptionNoStackTraceNeeded(
                    "For content %d, the dbid values are the same.  "
                    "A segment may not be built from itself" % live.getSegmentDbId()
                )
            if failover.unreachable:
                raise ExceptionNoStackTraceNeeded(
                    "The recovery target segment %s (content %s) is unreachable."
                    % (failover.getSegmentHostName(), failover.getSegmentContentId())
                )
...
class RecoveryTripletRequest:
    def __init__(
        self,
        failed,
        failover_host_name=None,
        failover_host_address=None,
        failover_port=None,
        failover_datadir=None,
        is_new_host=False,
        recovery_type=None,
    ):
        self.failed = failed
...
class RecoveryTripletsFactory:
    @staticmethod
    def instance(
        gpArray, config_file=None, new_hosts=[], outputConfigFile=None, paralleldegree=1
    ):
        """
        ...
        if config_file:
            ...
            if not new_hosts:
                return RecoveryTripletsInplace(
                    gpArray, outputConfigFile, paralleldegree
                )
            else:
                return RecoveryTripletsNewHosts(
                    gpArray, new_hosts, outputConfigFile, paralleldegree
                )
...
class RecoveryTriplets:
    ...
    def _get_unreachable_failover_hosts(self, requests):
        hostlist = {
            req.failover_host_address for req in requests if req.failover_host_address
        }
        return get_unreachable_segment_hosts(
            hostlist, min(self.paralleldegree, len(hostlist))
        )
    ...
    def _convert_requests_to_triplets(self, requests):
        ...
        for req in requests:
            ...
            if peer is None:
                raise Exception(
                    "No peer found for dbid {}. liveSegment is None".format(
                        failed_segment_dbid
                    )
                )
            ...
            if req.failover_host_address:
                ...
                if req.failover_host_name != req.failover_host_address:
                    ...
                    if not validateHostnameAddress(
                        req.failover_host_name, req.failover_host_address
                    ):
                        logger.warning(
                            "Not able to co-relate hostname:{0} with address:{1}. "
                            "Skipping recovery for segments with contentId {2}".format(
                                req.failover_host_name,
                                req.failover_host_address,
                                peer_contentid,
                            )
                        )
                        continue
                ...
                failover.unreachable = (
                    failover.getSegmentHostName() in unreachable_failover_hosts
                )
            else:
            ...
            triplets.append(
                RecoveryTriplet(req.failed, peer, failover, req.recovery_type)
            )
...
class RecoveryTripletsInplace(RecoveryTriplets):
    def __init__(self, gpArray, outputConfigFile, paralleldegree):
        super(RecoveryTripletsInplace, self).__init__(
            gpArray, outputConfigFile, paralleldegree
        )
...
class RecoveryTripletsNewHosts(RecoveryTriplets):
    def __init__(self, gpArray, newHosts, outputConfigFile, paralleldegree):
        super(RecoveryTripletsNewHosts, self).__init__(
            gpArray, outputConfigFile, paralleldegree
        )
    ...
    def getTriplets(self):
        ...
        requests = []
        for failedHost, failoverHost in zip(
            sorted(failedSegments.keys()), self.newHosts
        ):
            for failed in failedSegments[failedHost]:
                failoverPort = self.portAssigner.findAndReservePort(
                    failoverHost, failoverHost
                )
                req = RecoveryTripletRequest(
                    failed,
                    failover_host_name=failoverHost,
    ...
    class _PortAssigner:
        ...
        def findAndReservePort(self, hostName, address):
            ...
            for port in range(
                minPort, RecoveryTripletsNewHosts._PortAssigner.MAX_PORT_EXCLUSIVE
            ):
    ...
    def getTriplets(self):
        def _find_failed_from_row():
            ...
            for segment in self.gpArray.getDbList():
                ...
                if (
                    row["hostname_check_required"]
                    and segment.getSegmentHostName() != row["failedHostname"]
                ):
                    continue
                if (
                    segment.getSegmentAddress() == row["failedAddress"]
        ...
        with open(config_file) as f:
            for lineno, line in line_reader(f):
                ...
                (
                    ...
                    datadir,
                    recovery_type,
                    hostname_check_required,
                ) = extract_recovery_config_info(parts)
                check_values(
                    lineno,
                    hostname=hostname,
                    address=address,
                ...
                if len(groups) == 2:
                    ...
                    if len(parts2) not in [3, 4] or len(parts) != len(parts2):
                        msg = (
                            "line {0} of file {1}: expected equal parts, either 3 or 4 on both segment group, obtained {2} on "
    ...
    def _validate(rows):
        ...
        for row in rows:
            ...
            if address + datadir in failed:
                msg = (
                    "config file lines {0} and {1} conflict: "
                    "Cannot recover the same failed segment {2} and data directory {3} twice.".format(
                        failed[address + datadir], lineno, address, datadir
                    )
                )
                raise ExceptionNoStackTraceNeeded(msg)
            ...
            if "newAddress" not in row:
                if address + datadir in new:
                    msg = (
                        "config file lines {0} and {1} conflict: "
                        "Cannot recover segment {2} with data directory {3} in place if it is used as a recovery segment.".format(
                            new[address + datadir], lineno, address, datadir
                        )
                    )
                    raise ExceptionNoStackTraceNeeded(msg)
            else:
                address2, port2, datadir2 = (
                    row["newAddress"],
                    row["newPort"],
                    row["newDataDirectory"],
                )

                if address2 + datadir2 in new:
                    msg = (
                        "config file lines {0} and {1} conflict: "
                        "Cannot recover to the same segment {2} and data directory {3} twice.".format(
                            new[address2 + datadir2], lineno, address2, datadir2
                        )
                    )
                    raise ExceptionNoStackTraceNeeded(msg)
