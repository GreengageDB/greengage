...
class GpExpandTestCase(unittest.TestCase):
        ...
        with dbconn.connect(dbconn.DbURL(), unsetSearchPath=False) as conn:
            ...
            if next_mir_port is None or next_mir_port == " " or next_mir_port == 0:
                ...
                select_mirror = (
                    "select distinct hostname \
                                 from gp_segment_configuration \
                                 where content >= 0 and preferred_role = 'm' and hostname != '%s'"
                    % self.primary_host_name
                )
                    ...
                    if mirroring_on:  # The content number for mirror is same as the primary segment's content number
                        ...
                        outfile.write(
                            ...
                            % (
                                ...
                                self.mirror_host_address,
                                next_mir_port,
                                mir_datadir,
                                next_dbid,
                                next_content,
                                "m",
                                str(next_mir_replication_port),
                            )
                        )
                        next_mir_port += 1
    ...
    def _get_dist_policies(self):
        ...
        with dbconn.connect(dbconn.DbURL(), unsetSearchPath=False) as conn:
            cursor = dbconn.execSQL(
                conn, "select * from gp_distribution_policy;"
            ).fetchall()
            for row in cursor:
                policies.append(row)
    ...
    def test00_pg_hba_conf_file(self):
        ...
        cmd = Command(
            name="run gpexpand", cmdStr="gpexpand -i %s" % (self.EXPANSION_INPUT_FILE)
        )
        ...
        cmd = Command(
            name="get the temp pg_hba.conf file",
            cmdStr="ls %s"
            % os.path.join(
                os.path.dirname(self.MASTER_DATA_DIRECTORY), "gpexpand*", "pg_hba.conf"
            ),
        )
        cmd.run(validateAfter=True)
        ...
        with open(temp_pg_hba_conf) as f:
            for line in f:
                if (
                    line.strip() == "# %s" % self.primary_host_name
                    or line.strip() == "# %s" % self.mirror_host_name
                ):
        ...
        GpStart(name="start the database in master only mode", masterOnly=True).run(
            validateAfter=True
        )
        Command(name="rollback the expansion", cmdStr="gpexpand -r").run(
            validateAfter=True
        )
    ...
    def test01_distribution_policy(self):
        ...
        try:
            ...
            cmd = Command(
                name="run gpexpand",
                cmdStr="gpexpand -i %s" % (self.EXPANSION_INPUT_FILE),
            )
            with self.assertRaisesRegexp(ExecutionError, "Fault Injection"):
                cmd.run(validateAfter=True)
