...
class UpdatePgHBAConfTests(GpTestCase):
    def setUp(self):
        self.apply_patches([patch("os.rename"), patch("os.path.abspath")])
    ...
    def test_remove_dup_add_entries(self):
        ...
        test_entries = "test2\ntest3"
        result_out_entries = seg_update_pg_hba.remove_dup_add_entries(
            pg_hba_entries, test_entries
        )
...
# host   replication   gpadmin   {ip_mirror1}/32   trust"""
        pg_hba_existing_entries = entries_block.format(
            ip_mirror1="10.0.0.1", ip_mirror2="10.0.0.2"
        ).split("\n")

        test_entries = "host all gpadmin {ip_mirror1}/32 trust\nhost all gpadmin {ip_mirror3}/32 trust".format(
            ip_mirror1="10.0.0.1", ip_mirror3="10.0.0.3"
        )

        result_out_entries = seg_update_pg_hba.remove_dup_add_entries(
            pg_hba_existing_entries, test_entries
        )
...
host all gpadmin {ip_mirror3}/32 trust"""
        expected_out_entries = expected_block.format(
            ip_mirror1="10.0.0.1", ip_mirror2="10.0.0.2", ip_mirror3="10.0.0.3"
        ).split("\n")
        self.assertEqual(result_out_entries, expected_out_entries)
