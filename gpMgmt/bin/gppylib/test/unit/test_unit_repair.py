...
class RepairTestCase(GpTestCase):
    def setUp(self):
        ...
        self.context.report_cfg = {
            0: {"segname": "seg1", "hostname": "somehost", "port": 25432},
    ...
    def test_create_repair_extra__normal(self, mock_repair):
        ...
        extra_missing_repair_obj = Mock(
            spec=["get_segment_to_oid_mapping", "get_delete_sql"]
        )
        ...
        repair_dir = self.subject.create_repair_for_extra_missing(
            catalog_table_obj, issues=None, pk_name=None, segments=self.context.cfg
        )
        self.assertEqual(repair_dir, self.repair_dir_path)
        bash_contents = [
            "#!/bin/bash\n",
        ...
        self.verify_repair_dir_contents(
            "run_somedb_extra_{}_timestamp.sh".format(catalog_table_name), bash_contents
        )
    ...
    def test_create_segment_repair_scripts(self):
        ...
        repair_dir = self.subject.create_segment_repair_scripts(
            segments=segments_with_repair_statements
        )
        self.assertEqual(repair_dir, self.repair_dir_path)
        bash_contents = [
            "#!/bin/bash\n",
    ...
    def test_append_content_to_bash_script__with_catalog_table(self):
        ...
        self.subject.append_content_to_bash_script(
            self.repair_dir_path, script, catalog_table_name
        )
        bash_contents = [
            "#!/bin/bash\n",
            "set -o errexit\n",
        ...
        self.verify_repair_dir_contents(
            "run_somedb_issuetype_{}_timestamp.sh".format(catalog_table_name),
            bash_contents,
        )
