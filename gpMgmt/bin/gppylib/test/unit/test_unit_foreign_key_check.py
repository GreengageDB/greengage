...
class GpCheckCatTestCase(GpTestCase):
    def setUp(self):
        ...
        self.subject = ForeignKeyCheck(
            self.db_connection, self.logger, False, self.autoCast
        )
    ...
    def test_get_fk_query_left_join_returns_the_correct_query(self):
        ...
        result_query = self.subject.get_fk_query_left_join(
            "catname",
            "pkcatname",
            "fkeystr",
            "pkeystr",
            ["pkeys-1", "pkeys-2"],
            ["cat1pkeys-1", "cat1pkeys-2"],
        )
    ...
    def test_get_fk_query_full_join_returns_the_correct_query(self):
        expected_query = """
              ...
              GROUP BY pkeys-1, pkeys-2, pkcatname_pkeystr, missing_catalog, present_key
              """
        result_query = self.subject.get_fk_query_full_join(
            "catname",
            "pkcatname",
            "fkeystr",
            "pkeystr",
    ...
    def test_runCheck(self, mock):
        tables = [
            self._get_mock_for_catalog_table("table1"),
            self._get_mock_for_catalog_table("table2"),
        ]
        self.subject.runCheck(tables)

        self.assertEquals(
            len(self.subject.checkTableForeignKey.call_args_list), len(tables)
        )
    ...
    @patch(
        "gpcheckcat_modules.foreign_key_check.ForeignKeyCheck.get_fk_query_full_join"
    )
    @patch(
        "gpcheckcat_modules.foreign_key_check.ForeignKeyCheck.get_fk_query_left_join"
    )
    @patch("gpcheckcat_modules.foreign_key_check.log_literal")
    def test_checkTableForeignKey__returns_correct_join_query(
        ...
        for table_name in cat_tables_to_validate:
            foreign_key_mock_1 = self._get_mock_for_foreign_key(
                pkey_tablename="pg_class", cat_table_name=table_name
            )
            foreign_key_mock_2 = self._get_mock_for_foreign_key(
                pkey_tablename="arbitrary_catalog_table", cat_table_name=table_name
            )
            catalog_table_mock = self._get_mock_for_catalog_table(
                table_name, [foreign_key_mock_1, foreign_key_mock_2]
            )
            ...
            self.assertEquals(
                issue_list[0],
                ("pg_class", ["pkey1", "pkey2"], [("r1", "r2"), ("r3", "r4")]),
            )
            self.assertEquals(
                issue_list[1],
                (
                    "arbitrary_catalog_table",
                    ["pkey1", "pkey2"],
                    [("r1", "r2"), ("r3", "r4")],
                ),
            )
            ...
            def __generate_pg_class_call(
                table, primary_key_cat_name, col_type, with_filter=True
            ):
                if with_filter:
                    return call(
                        table_name,
                        "%s" % primary_key_cat_name,
                        "%s" % col_type.keys()[0],
                        "oid",
            ...
            if table_name in self.full_join_cat_tables:
                pg_class_call = __generate_pg_class_call(
                    table_name, "pg_class", col_type, with_filter=True
                )
                foreign_key_mock_calls.append(pg_class_call)

                pg_class_call = __generate_pg_class_call(
                    table_name, "arbitrary_catalog_table", col_type, with_filter=False
                )
                foreign_key_mock_calls_left.append(pg_class_call)
            else:
                pg_class_call = __generate_pg_class_call(
                    table_name, "pg_class", col_type, with_filter=False
                )
                foreign_key_mock_calls_left.append(pg_class_call)
            ...
            if table_name in self.full_join_cat_tables:
                ...
                fk_query_full_join_mock.assert_has_calls(
                    foreign_key_mock_calls, any_order=False
                )
            else:
                arbitrary_catalog_table_call = __generate_pg_class_call(
                    table_name, "arbitrary_catalog_table", col_type, with_filter=False
                )
                ...
                fk_query_left_join_mock.assert_has_calls(
                    foreign_key_mock_calls_left, any_order=False
                )
    ...
    def _get_filter(self, table_name):
        ...
        query_filters["pg_constraint"] = (
            "((relchecks>0 or relhaspkey='t') and relkind = 'r')"
        )
        query_filters[
            "gp_distribution_policy"
        ] = """(relnamespace not in(select oid from pg_namespace where nspname ~ 'pg_')
    ...
    def _get_mock_for_catalog_table(self, table_name, foreign_keys=None):
        ...
        catalog_table_mock.getTableColtypes.return_value = self._get_col_types(
            table_name
        )
