...
class GpStop(GpTestCase):
    ...
    def test_option_master_success_without_auto_accept(self, mock_userinput):
        ...
        mock_userinput.ask_yesno.assert_called_once_with(
    ...
    def test_host_missing_from_config(self):
        ...
        self.assertIn(
            "host 'nothere' is not found in gp_segment_configuration", error_msgs
        )
    ...
    def test_happypath_in_interactive_mode(self, mock_userinput):
        ...
        self.assertEquals(
            self.mock_GpSegStopCmdInit.call_args_list[0][1]["dbs"][0], self.primary0
        )
        self.assertEquals(
            self.mock_GpSegStopCmdInit.call_args_list[0][1]["dbs"][1], self.primary1
        )
        self.assertEquals(
            self.mock_GpSegStopCmdInit.call_args_list[1][1]["dbs"][0], self.mirror2
        )
        self.assertEquals(
            self.mock_GpSegStopCmdInit.call_args_list[1][1]["dbs"][1], self.mirror3
        )
    ...
    def test_host_option_segment_in_change_tracking_mode_fails(self):
        ...
        with self.assertRaisesRegexp(
            Exception, "Segment '%s' not synchronized. Aborting." % self.primary0
        ):
            gpstop.run()
    ...
    def test_host_option_with_master_option_fails(self):
        ...
        with self.assertRaisesRegexp(
            ...
            "Incompatible flags. Cannot mix '--host' option with '-m' for master-only.",
        ):
    ...
    def test_host_option_with_restart_option_fails(self):
        ...
        with self.assertRaisesRegexp(
            ProgramArgumentValidationException,
            "Incompatible flags. Cannot mix '--host' option with '-r' for restart.",
        ):
            self.subject.GpStop.createProgram(options, args)
    ...
    def test_reload_config_use_local_context(self):
        ...
        self.assertEquals(
            None, self.mock_workerpool.addCommand.call_args_list[0][0][0].remoteHost
        )
        self.assertEquals(
            "sdw1", self.mock_workerpool.addCommand.call_args_list[1][0][0].remoteHost
        )
        self.assertEquals(
            "sdw1", self.mock_workerpool.addCommand.call_args_list[2][0][0].remoteHost
        )
    ...
    def test_host_option_with_request_sighup_option_fails(self):
        ...
        with self.assertRaisesRegexp(
            ProgramArgumentValidationException,
            "Incompatible flags. Cannot mix '--host' "
    ...
    def test_gpstop_fast_if_previous_gpstop_was_interrupted(self, mockGpStart):
        ...
        self.subject.logger.warning.assert_any_call(
            "The database is currently in the process of shutting down."
        )
        self.subject.logger.info.assert_any_call("Your choice was 'f'")
        self.subject.logger.info.assert_any_call(
            "Stopping the master to finish the database shutdown"
        )
    ...
    def test_gpstop_fails_to_start_master_after_interrupt(self, mockGpstart):
        ...
        gpstop = self.subject.GpStop.createProgram(options, args)
        with self.assertRaisesRegexp(
            Exception, "Unable to start the cluster in master_only mode: error"
        ):
            gpstop.run()

    @patch("gpstop.gp.MasterStop")
    def test_gpstop_fails_to_stop_master_processes_after_interrupt(
        self, mockMasterStop
    ):
        sys.argv = ["gpstop", "-am"]
        parser = self.subject.GpStop.createParser()
    ...
    def test_gpstop_sighup_option_after_interrupt(self):
        ...
        self.subject.logger.fatal.assert_any_call(
            "Cannot send SIGHUP to postmaster as the database is shutting down. "
            "Please run 'gpstop' to complete the shutdown process."
        )

    @patch("gpstop.GpArray.initFromCatalog")
    def test_gpstop_raise_exception_other_than_database_system_shutting_down(
        self, mock_initFromCatalog
    ):
        sys.argv = ["gpstop", "-a"]
        parser = self.subject.GpStop.createParser()
        options, args = parser.parse_args()
...
class GpStopSmartModeTestCase(unittest.TestCase):
    ...
    def _setup_subprocess(self, subprocess_call):
        ...
        def _call(args, **kwargs):
            if args[0] != "pg_ctl":
                self.fail(
                    "Expected only pg_ctl calls to be made during subprocess.call()."
                )
    ...
    def test_stop_master_smart_issues_pg_ctl_stop(
        self, subprocess_call, dbconn_connect
    ):
        self._setup_subprocess(subprocess_call)

        self.gpstop.master_datadir = "datadir"
        self.gpstop._stop_master_smart()
        subprocess_call.assert_any_call([
            "pg_ctl",
            "stop",
            "-W",
            "-m",
    ...
    def test_stop_master_smart_raises_exception_if_stop_fails(
        self, subprocess_call, dbconn_connect
    ):
        self._setup_subprocess(subprocess_call)
        self.stop_retval = 1
    ...
    def test_stop_master_smart_calls_pg_ctl_status_until_server_stops(
        self, subprocess_call, dbconn_connect
    ):
        self._setup_subprocess(subprocess_call)
        ...
        self.assertEqual(
            self.i,
            3,
            "expected _stop_master_smart to return after the third call to pg_ctl status",
        )

    def test_stop_master_smart_raises_exception_if_status_fails(
        self, subprocess_call, dbconn_connect
    ):
        self._setup_subprocess(subprocess_call)
    ...
    def test_SigIntHandler_must_have_SIGINT_ignored_on_entry(
        self, subprocess_call, dbconn_connect
    ):
    ...
    def test_SigIntHandler_catches_SIGINT_when_enabled(
        self, subprocess_call, dbconn_connect
    ):
        with gpstop.GpStop._SigIntHandler() as handler:
    ...
    def test_SigIntHandler_ignores_SIGINT_when_disabled(
        self, subprocess_call, dbconn_connect
    ):
        with gpstop.GpStop._SigIntHandler() as handler:
            self.assertFalse(handler.interrupted)
