...
class GpSegStart(GpTestCase):
    def setUp(self):
        ...
        local_version = gp.GpVersion.local(
            "local GP software version check", gp.get_gphome()
        )
        ...
        self.args_list = [
            ...
            "3",
            "--era",
            "5c105ee373d42194_180105120208",
            "-t",
            "600",
            "-p",
            "pickled transition data",
            "-D",
            "2|0|p|p|s|u|aspen|aspen|25432|/doesnt/exist/datadirs/dbfast1/demoDataDir0",
            "-D",
            "5|0|m|m|s|u|aspen|aspen|25435|" + self.failing_data_dir,
            "-D",
            "3|1|p|p|s|u|aspen|aspen|25433|/doesnt/exist/datadirs/dbfast2/demoDataDir1",
            "-D",
            "6|1|m|m|s|u|aspen|aspen|25436|/doesnt/exist/datadirs/dbfast_mirror2/demoDataDir1",
            "-D",
            "4|2|p|p|s|u|aspen|aspen|25434|/doesnt/exist/datadirs/dbfast3/demoDataDir2",
            "-D",
            "7|2|m|m|s|u|aspen|aspen|25437|/doesnt/exist/datadirs/dbfast_mirror3/demoDataDir2",
        ]
        self.apply_patches([
            patch("os.path.isdir"),
    ...
    @patch.object(
        PgControlData,
        "get_results",
        return_value=CommandResult(0, "/tmp/f1", "", True, False),
    )
    ...
    @patch.object(
        PgControlData,
        "get_results",
        return_value=CommandResult(1, "/tmp/f1", "", True, False),
    )
    def test_startSegments_when_pg_controldata_failed(self, mock1):
        ...
        for result in gpsegstart.overall_status.results:
            self.assertTrue(
                result.reasoncode == gp.SEGSTART_ERROR_PG_CONTROLDATA_FAILED
            )
    ...
    def test_startSegments_when_pg_ctl_failed(self, mock1, mock2, mock_get_results):
        ...
        mock_get_results.segment.getSegmentDataDirectory.return_value = (
            self.failing_data_dir
        )
        ...
        gpsegstart = self.subject.GpSegStart.createProgram(options, args)
        with patch(
            "gpsegstart.base.WorkerPool.return_value.getCompletedItems",
            return_value=[mock_get_results],
        ):
            exitCode = gpsegstart.run()
