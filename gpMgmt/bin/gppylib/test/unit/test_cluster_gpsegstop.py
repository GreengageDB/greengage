...
class SegStopTestCase(unittest.TestCase):
    def setUp(self):
        self.segstop = SegStop(name="Segment Stop", db=None, mode=None, timeout=None)
    ...
    @patch(
        "gpsegstop.pg.ReadPostmasterTempFile.getResults",
        return_value=(True, 1234, "/tmp/gpseg0"),
    )
    ...
    @patch(
        "gppylib.commands.base.Command.get_results",
        return_value=CommandResult(1, "", "", True, False),
    )
    @patch(
        "gpsegstop.pg.ReadPostmasterTempFile.getResults",
        return_value=(True, 1234, "/tmp/gpseg0"),
    ...
    @patch("gpsegstop.unix.kill_9_segment_processes")
    @patch("gpsegstop.unix.check_pid", return_value=False)
    def test_run_with_pg_controldata_error(
        self, mock1, mock2, mock3, mock4, mock5, mock6, mock7
    ):
        self.segstop.db = "/tmp/gpseg0:1234"
        self.segstop.mode = "smart"
        self.segstop.timeout = "10"
