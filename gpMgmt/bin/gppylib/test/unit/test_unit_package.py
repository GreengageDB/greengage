...
from gppylib.operations.package import (
    IsVersionCompatible,
    ListPackages,
    MigratePackages,
...
class IsVersionCompatibleTestCase(GpTestCase):
    def setUp(self):
        self.gppkg_mock_values = {
            ...
            "dependencies": [],
            "file_list": ["deps", "gppkg_spec.yml", "plperl-1.1-2.x86_64.rpm"],
            "gpdbversion": Mock(),
            "preuninstall": [],
            "os": "rhel5",
            "architecture": "x86_64",
        ...
        self.apply_patches([
            patch(
                "gppylib.operations.package.logger",
...
class ListPackagesTestCase(GpTestCase):
    ...
    def test__execute_happy_list_all_packages(self):
        self.mock_list_files_by_pattern_run.return_value = [
            "sample.gppkg",
            "sample-version-random_OS-arch_type.gppkg",
        ]
        package_name_list = self.subject.execute()
...
class MigratePackagesTestCase(GpTestCase):
    def setUp(self):
        ...
        if platform.linux_distribution()[0] == "Ubuntu":
            self.mock_install_package_locally = self.get_mock_from_apply_patch(
                "InstallDebPackageLocally"
            )
        else:
            self.mock_install_package_locally = self.get_mock_from_apply_patch(
    ...
    def test__execute_finds_no_packages(self):
        ...
        self.mock_logger.info.assert_called_with(
            "There are no packages to migrate from %s." % self.args["from_gphome"]
        )
    ...
    def test__execute_finds_some_packages(self):
        ...
        self.assertIn(
            ...
            % ", ".join(["sample.gppkg", "sample2.gppkg", "another-long-one.gppkg"]),
            log_messages,
        )
    ...
    def test__execute_catches_AlreadyInstalledError(self):
        ...
        self.mock_install_package_locally.return_value.run.side_effect = (
            AlreadyInstalledError("sample.gppkg")
        )
    ...
    def test__execute_catches_Exception_failed_to_migrate(self):
        ...
        self.mock_install_package_locally.return_value.run.side_effect = [
            None,
            Exception("foobar something bad"),
            None,
        ]
        ...
        self.assertIn(
            ...
            % (os.path.join(self.args["from_gphome"], ARCHIVE_PATH), "sample2.gppkg"),
            log_messages,
        )
        self.assertIn("The package migration has completed.", log_messages)
...
class SyncPackagesTestCase(GpTestCase):
    def setUp(self):
        ...
        self.mock_install_packages_locally = self.get_mock_from_apply_patch(
            "InstallPackageLocally"
        )
...
class CleanGppkgTestCase(GpTestCase):
    def setUp(self):
        ...
        self.parallel_operation_mock = self.get_mock_from_apply_patch(
            "ParallelOperation"
        )

    def test_two_segments_unreachable(self):
        self.sync_packages_mock.return_value.get_ret.side_effect = [
