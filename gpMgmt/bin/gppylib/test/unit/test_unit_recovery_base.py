...
class RecoveryBaseTestCase(GpTestCase):
    def setUp(self):
        ...
        self.apply_patches([
            patch(
                "recovery_base.gplog.setup_tool_logging", return_value=self.mock_logger
            ),
        ...
        self.confinfo = gppylib.recoveryinfo.serialize_list([
            self.full_r1,
            self.incr_r2,
        ])
    ...
    def _asserts_for_passing_tests(
        self, stderr_buf, ex, enable_verbose_count=0, warn_count=0
    ):
        self.assertEqual("", stderr_buf.getvalue().strip())
        self.assertEqual(0, ex.exception.code)

        self.assertEqual(
            enable_verbose_count, self.mock_enable_verbose_logging.call_count
        )
    ...
    def _asserts_for_failing_tests(
        self, ex, stderr_buf, expected_message, info_count=1
    ):
        self.assertEqual(1, ex.exception.code)
        self.assertRegexpMatches(expected_message, stderr_buf.getvalue().strip())
        self.assertTrue(
            any(
                re.search(expected_message, call_args[0])
                for call_args, _ in self.mock_logger.error.call_args_list
            )
        )
    ...
    def _assert_workerpool_calls(self, mock_workerpool):
        ...
        self.assertEqual(
            [call(self.mock_cmd1), call(self.mock_cmd2)],
    ...
    def test_confinfo_passed_as_blank_fails(self):
        ...
        stderr_buf, ex = self.run_recovery_base_get_stderr()
        self._assert_exception_from_parseargs(
            ex, stderr_buf, "recovery_base: error: -c option requires an argument\n"
        )
    ...
    def test_invalid_option_fails(self):
        ...
        stderr_buf, ex = self.run_recovery_base_get_stderr()
        self._assert_exception_from_parseargs(
            ex, stderr_buf, "recovery_base: error: no such option: -z\n"
        )

    def test_invalid_type_batchsize_fails(self):
        sys.argv = ["recovery_base", "-c {}".format(self.confinfo), "-b", "foo"]
        stderr_buf, ex = self.run_recovery_base_get_stderr()
        self._assert_exception_from_parseargs(
            ex,
            stderr_buf,
    ...
    def test_default_batch_size_is_used_passes(self, mock_workerpool):
        ...
        sys.argv = ["recovery_base", "-l", "/tmp/logdir", "-c {}".format(new_confinfo)]
        stderr_buf, ex = self.run_recovery_base_get_stderr()
        self._asserts_for_passing_tests(stderr_buf, ex)
    ...
    def test_valid_cmd_default_options_passes(self, mock_workerpool):
        ...
        self._asserts_for_passing_tests(stderr_buf, ex)
        self.assertEqual(
            [call("test_file.py", ANY, ANY, logdir="/tmp/logdir")],
    ...
    def test_invalid_cmd_fails(self):
        ...
        stderr_buf, ex = self.run_recovery_base_get_stderr()
        self._asserts_for_failing_tests(
            ex,
            stderr_buf,
            '[{"progress_file": null, "dbid": null, "error_type": "default", "port": null, '
            '"datadir": null, "error_msg": "^/bin/bash.*invalid_cmd_str: command not found$"}, '
            '{"progress_file": null, "dbid": null, "error_type": "default", "port": null, '
            '"datadir": null, "error_msg": "^/bin/bash.*invalid_cmd_str: command not found$"}]',
        )
    ...
    def test_valid_failing_cmd_fails(self):
        ...
        self._asserts_for_failing_tests(
            ex,
            stderr_buf,
            '[{"progress_file": null, "dbid": null, "error_type": "default",'
            ' "port": null, "datadir": null, "error_msg": ""}, '
            '{"progress_file": null, "dbid": null, "error_type": "default", '
            '"port": null, "datadir": null, "error_msg": ""}]',
        )
...
class SetCmdResultsTestCase(GpTestCase):
    ...
    def test_set_recovery_cmd_results_catch_exception(self):
        ...
        expected_stderr = (
            '{"error_type": 10, "error_msg": "running the cmd failed", "dbid": 2, '
    ...
    def test_set_recovery_cmd_results_catch_exception_none_error_type(self):
        ...
        expected_stderr = (
            '{"error_type": "default", "error_msg": "running the cmd failed", "dbid": 2, '
            '"datadir": "/tmp/datadir2", "port": 7002, "progress_file": "/tmp/progress_file2"}'
        )
        self._assert_cmd_failed(test_cmd, expected_stderr)
