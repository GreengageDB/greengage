...
class GucCollectionTest(GpTestCase):
    ...
    def test_when_non_matching_file_value_yields_failure_format(self):
        ...
        self.assertIn(
            "[context: -1] [dbid: dbid1] [name: guc_name] [value: master_value | file: master_file_value]"
            "\n[context: 0] [dbid: dbid2] [name: guc_name] [value: value | file: file_value]",
            self.subject.report(),
        )

    def test_when_the_master_is_different_from_segment_yet_all_segments_same_yields_success_format(
        self,
    ):
        ...
        self.assertIn(
            "Master  value: master_value | file: master_value", self.subject.report()
        )
    ...
    def test_less_than_two_segments_raises(self):
        ...
        with self.assertRaisesRegexp(
            Exception, "Collections must have at least a master and segment value"
        ):
            self.subject.report()
    ...
    def test_update_after_having_full_comparison_for_a_given_contentid_succeeds(self):
        ...
        self.assertIn(
            "Master  value: master_value | file: master_value\nSegment value: value | file: value",
            self.subject.report(),
        )
    ...
    def test_when_file_value_empty_file_compare_succeeds(self):
        ...
        self.assertIn(
            "Master  value: master_value | not set in file", self.subject.report()
        )
    ...
    def test_update_when_file_segment_has_same_dbid_overwrites_and_succeeds(self):
        ...
        self.assertEquals(
            self.subject.gucs["-1"].primary_file_seg_guc, primary_file_seg
        )
    ...
    def test_update_when_only_file_segments_primary_and_mirror_fails(self):
        ...
        self.assertIn(
            "[context: -1] [dbid: dbid1] [name: guc_name] [not set in file]\n",
            self.subject.report(),
        )
        self.assertIn(
    ...
    def test_for_sorting_based_on_context_and_dbid_with_file_option_succeeds(self):
        ...
        self.assertEquals(
            ...
            "[context: 1] [dbid: dbid6] [name: guc_name] [value: value]",
            self.subject.report(),
        )
    ...
    def test_for_sorting_based_on_context_and_dbid_with_file_compare_option_succeeds(
        ...
        self.assertEquals(
            "[context: -1] [dbid: dbid1] [name: guc_name] [value: master_value | file: master_value]\n"
