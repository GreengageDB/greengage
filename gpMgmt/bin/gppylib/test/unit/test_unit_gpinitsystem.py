...
class GpInitSystemTest(GpTestCase):
    def setUp(self):
        ...
        self.gpinitsystem_path = os.path.realpath(
            os.path.join(
                os.path.dirname(os.path.realpath(__file__)), "../../../gpinitsystem"
            )
        )
    ...
    def test_option_cluster_configfile_and_input_configfile_should_error(self):
        ...
        self.assertIn(
            "[FATAL]:-Options [-c] and [-I] cannot be used at the same time.", output
        )
        self.assertNotIn("Creates a new Greengage Database instance", output)
        self.assertNotIn(
            "Initializes a Greengage Database system by using configuration", output
        )

    def test_option_without_neither_cluster_configfile_and_input_configfile_should_error(
        self,
    ):
        p = Popen([self.gpinitsystem_path], stdout=PIPE)
        output = p.stdout.read()
        self.assertIn(
            "[FATAL]:-At least one of two options, [-c] or [-I], is required", output
        )
        self.assertNotIn("Creates a new Greengage Database instance", output)
        self.assertNotIn(
            "Initializes a Greengage Database system by using configuration", output
    ...
    def test_option_with_input_configfile_should_work(self):
        ...
        output = p.stdout.read()
        self.assertIn(
            "[INFO]:-Checking configuration parameters, please wait...", output
        )
