...
class GpInitSystemTest(GpTestCase):
    ...
    def test_option_cluster_configfile_and_input_configfile_should_error(self):
        p = gpsubprocess.Popen([self.gpinitsystem_path, '-c', 'cluster_configfile', '-I', 'input_configfile'], stdout=PIPE)
        output = p.stdout.read()
        self.assertIn("[FATAL]:-Options [-c] and [-I] cannot be used at the same time.", output)
        self.assertNotIn("Creates a new Greengage Database instance", output)
        self.assertNotIn("Initializes a Greengage Database system by using configuration", output)

    def test_option_without_neither_cluster_configfile_and_input_configfile_should_error(self):
        p = gpsubprocess.Popen([self.gpinitsystem_path], stdout=PIPE)
        output = p.stdout.read()
    ...
    def test_option_with_input_configfile_should_work(self):
        p = gpsubprocess.Popen([self.gpinitsystem_path, '-I', 'input_configfile'], stdout=PIPE)
        output = p.stdout.read()
    ...
    def test_option_with_cluster_configfile_should_work(self):
        p = gpsubprocess.Popen([self.gpinitsystem_path, '-c', 'cluster_configfile'], stdout=PIPE)
        output = p.stdout.read()
        self.assertIn("[INFO]:-Checking configuration parameters, please wait...", output)

    def test_option_help_prints_docs_usage(self):
        p = gpsubprocess.Popen([self.gpinitsystem_path, '--help'], stdout=PIPE)
        output = p.stdout.read()
        self.assertIn("Initializes a Greengage Database system by using configuration", output)
        self.assertNotIn("Creates a new Greengage Database instance", output)

    def test_invalid_option_prints_raw_usage(self):
        p = gpsubprocess.Popen([self.gpinitsystem_path, '--unknown-option'], stdout=PIPE)
        output = p.stdout.read()
        self.assertIn("[ERROR]:-Unknown option --unknown-option", output)
        self.assertIn("Creates a new Greengage Database instance", output)
        self.assertNotIn("Initializes a Greengage Database system by using configuration", output)
