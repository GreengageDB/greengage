...
class FileSegmentGucTest(GpTestCase):
    ...
    def test_when_segment_report_success_format_file(self):
        ...
        self.assertEquals(
            self.subject.report_success_format(), "Segment value: value_from_file"
        )

    def test_when_master_report_success_format_file(self):
        self.subject.context = "-1"
        self.assertEquals(
            self.subject.report_success_format(), "Master  value: value_from_file"
        )
    ...
    def test_init_with_insufficient_file_values_raises(self):
        ...
        with self.assertRaisesRegexp(
            Exception, "must provide \['context', 'guc name', 'value', 'dbid'\]"
        ):
            FileSegmentGuc(row)
    ...
    def test_when_value_none_report_success_prints_message(self):
        ...
        self.assertEquals(
            self.subject.report_success_format(), "No value is set on segments"
        )
