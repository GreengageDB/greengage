...
class IncrementalRecoveryTestCase(GpTestCase):
    def setUp(self):
        ...
        self.mock_pgrewind_modifyconfsetting = self.get_mock_from_apply_patch(
            "ModifyConfSetting"
        )
        ...
        self.seg_recovery_info = RecoveryInfo(
            m.getSegmentDataDirectory(),
            m.getSegmentPort(),
            m.getSegmentDbId(),
    ...
    def _assert_cmd_failed(self, expected_stderr):
        ...
        self.assertItemsEqual(
            expected_stderr, self.incremental_recovery_cmd.get_results().stderr
        )
        self.assertEqual(
            False, self.incremental_recovery_cmd.get_results().wasSuccessful()
        )
    ...
    def test_incremental_run_passes(self):
        ...
        expected_init_args = call(
            "rewind dbid: 2", "/data/mirror0", "sdw1", 40000, "/tmp/test_progress_file"
        )
        ...
        logger_call_args = [
            call(
                "Running pg_rewind with progress output temporarily in /tmp/test_progress_file"
            ),
        ...
        gpsegrecovery.start_segment.assert_called_once_with(
            self.seg_recovery_info, self.mock_logger, self.era
        )
    ...
    def test_incremental_run_exception(self):
        ...
        self._assert_cmd_failed(
            '{"error_type": "incremental", "error_msg": "pg_rewind failed", "dbid": 2, '
    ...
    def test_logger_info_exception(self):
        ...
        self.assertEqual(0, gpsegrecovery.start_segment.call_count)
        self._assert_cmd_failed(
            '{"error_type": "default", "error_msg": "logger exception", "dbid": 2, '
            '"datadir": "/data/mirror0", "port": 50000, "progress_file": "/tmp/test_progress_file"}'
        )
    ...
    def test_incremental_start_segment_exception(self):
        ...
        self._assert_cmd_failed(
            '{"error_type": "start", "error_msg": "pg_ctl start failed", "dbid": 2, '
            '"datadir": "/data/mirror0", "port": 50000, "progress_file": "/tmp/test_progress_file"}'
        )

    def test_incremental_modify_conf_setting_exception(self):
        self.mock_pgrewind_modifyconfsetting.side_effect = [
            Exception("modify conf port failed"),
            Mock(),
        ]
        self.incremental_recovery_cmd.run()
        ...
        self.assertEqual(1, self.mock_pgrewind_init.call_count)
        self.assertEqual(1, self.mock_pgrewind_run.call_count)
        self.mock_pgrewind_modifyconfsetting.assert_called_once_with(
            ...
            "port",
            self.seg_recovery_info.target_port,
            optType="number",
        )
        self.assertEqual(1, self.mock_pgrewind_modifyconfsetting.call_count)
        self.assertEqual(0, gpsegrecovery.start_segment.call_count)
        self._assert_cmd_failed(
            '{"error_type": "update", "error_msg": "modify conf port failed", "dbid": 2, '
            '"datadir": "/data/mirror0", "port": 50000, "progress_file": "/tmp/test_progress_file"}'
        )
...
class FullRecoveryTestCase(GpTestCase):
    ...
    def _assert_basebackup_runs(self, expected_init_args):
        ...
        expected_logger_info_args = [
            call(
                "Running pg_basebackup with progress output temporarily in /tmp/test_progress_file"
            ),
            call("Successfully ran pg_basebackup for dbid: 2"),
            call("Updating /data/mirror0/postgresql.conf"),
        ]
        self.assertEqual(
            expected_logger_info_args, self.mock_logger.info.call_args_list
        )
    ...
    def _assert_cmd_failed(self, expected_stderr):
        ...
        self.assertEqual("", self.full_recovery_cmd.get_results().stdout)
        self.assertItemsEqual(
            expected_stderr, self.full_recovery_cmd.get_results().stderr
        )
    ...
    def test_basebackup_run_exception(self):
        ...
        self.assertEqual(
            [call(validateAfter=True)], self.mock_pgbasebackup_run.call_args_list
        )
        self.assertEqual(
            [
                call(
        ...
        self.assertEqual(0, gpsegrecovery.start_segment.call_count)
        self._assert_cmd_failed(
            '{"error_type": "full", "error_msg": "backup failed once", "dbid": 2, '
            '"datadir": "/data/mirror0", "port": 50000, "progress_file": "/tmp/test_progress_file"}'
        )
    ...
    def test_basebackup_init_exception(self):
        ...
        self._assert_cmd_failed(
            '{"error_type": "full", "error_msg": "backup init failed", "dbid": 2, '
            '"datadir": "/data/mirror0", "port": 50000, "progress_file": "/tmp/test_progress_file"}'
        )

    def test_basebackup_start_segment_exception(self):
        gpsegrecovery.start_segment.side_effect = [
            Exception("pg_ctl start failed"),
            Mock(),
        ]
    ...
    def test_basebackup_modify_conf_setting_exception(self):
        self.mock_pgbasebackup_modifyconfsetting.side_effect = [
            Exception("modify conf port failed"),
            Mock(),
        ]
        self.full_recovery_cmd.run()
...
class SegRecoveryTestCase(GpTestCase):
    ...
    def test_get_recovery_cmds_is_called(
        self, mock_get_recovery_cmds, mock_recovery_base_main, mock_logger
    ):
        mix_confinfo = gppylib.recoveryinfo.serialize_list([self.full_r1, self.incr_r2])
        sys.argv = [
            "gpsegrecovery",
        ...
        mock_get_recovery_cmds.assert_called_once_with(
            [self.full_r1, self.incr_r2], True, mock_logger.return_value, self.era
        )
        mock_recovery_base_main.assert_called_once_with(
            mock_get_recovery_cmds.return_value
        )

    def _assert_validation_full_call(
        self, cmd, expected_recovery_info, expected_forceoverwrite=False
    ):
        self.assertTrue(isinstance(cmd, gpsegrecovery.FullRecovery))
    ...
    def _assert_setup_incr_call(self, cmd, expected_recovery_info):
        self.assertTrue(isinstance(cmd, gpsegrecovery.IncrementalRecovery))
    ...
    def _assert_setup_diff_call(self, cmd, expected_recovery_info):
        self.assertTrue(isinstance(cmd, gpsegrecovery.DifferentialRecovery))
    ...
    def test_get_recovery_cmds_full_recoveryinfo(self):
        cmd_list = SegRecovery().get_recovery_cmds(
            [self.full_r1, self.full_r2], False, self.mock_logger, self.era
        )
    ...
    def test_get_recovery_cmds_incr_recoveryinfo(self):
        cmd_list = SegRecovery().get_recovery_cmds(
            [self.incr_r1, self.incr_r2], False, self.mock_logger, self.era
        )
    ...
    def test_get_recovery_cmds_diff_recoveryinfo(self):
        cmd_list = SegRecovery().get_recovery_cmds(
            [self.diff_r1, self.diff_r2], False, self.mock_logger, self.era
        )
    ...
    def test_get_recovery_cmds_mix_recoveryinfo(self):
        cmd_list = SegRecovery().get_recovery_cmds(
            [self.full_r1, self.incr_r2, self.diff_r1],
            False,
            self.mock_logger,
            self.era,
        )
    ...
    def test_get_recovery_cmds_mix_recoveryinfo_forceoverwrite(self):
        cmd_list = SegRecovery().get_recovery_cmds(
            [self.full_r1, self.incr_r2, self.diff_r1], True, self.mock_logger, self.era
        )
        self._assert_validation_full_call(
            cmd_list[0], self.full_r1, expected_forceoverwrite=True
        )
        self._assert_setup_incr_call(cmd_list[1], self.incr_r2)
        self._assert_setup_diff_call(cmd_list[2], self.diff_r1)
...
class DifferentialRecoveryClsTestCase(GpTestCase):
    def setUp(self):
        ...
        self.diff_recovery_cmd = gpsegrecovery.DifferentialRecovery(
            name="test differential recovery",
            recovery_info=self.seg_recovery_info,
            logger=self.mock_logger,
            era=self.era,
        )
    ...
    def test_pg_start_backup_conn_exception(self, mock1):
        ...
        self.assertTrue(
            "Failed to query pg_start_backup() for segment with host sdw1 and port 40000"
            in str(ex.exception)
        )
    ...
    def test_pg_start_backup_success(self, mock1, mock2):
        ...
        self.assertEqual(
            [
                call(
                    "Successfully ran pg_start_backup for segment on host sdw1, port 40000"
                )
            ],
    ...
    def test_pg_stop_backup_conn_exception(self, mock1):
        ...
        self.assertTrue(
            "Failed to query pg_stop_backup() for segment with host sdw1 and port 40000"
            in str(ex.exception)
        )
    ...
    def test_pg_stop_backup_success(self, mock1, mock2):
        ...
        self.assertEqual(
            [
                call(
                    "Successfully ran pg_stop_backup for segment on host sdw1, port 40000"
                )
            ],
            self.mock_logger.debug.call_args_list,
        )
    ...
    @patch(
        "gppylib.db.catalog.RemoteQueryCommand.get_results",
        return_value=[["1111", "/data/mytblspace1"], ["2222", "/data/mytblspace2"]],
    )
    @patch("gpsegrecovery.get_remote_link_path", return_value="/data/mytblspace1/2")
    @patch("os.listdir")
    @patch("os.symlink")
    def test_sync_tablespaces_outside_data_dir(self, mock1, mock2, mock3, mock4):
        ...
        self.assertEqual(
            [call("Syncing tablespaces of dbid 2 which are outside of data_dir")],
            self.mock_logger.debug.call_args_list,
        )
    ...
    def test_sync_xlog_and_control_file_success(self):
        self.diff_recovery_cmd.sync_xlog_and_control_file()
        self.assertEqual(2, self.mock_rsync_init.call_count)
        self.assertEqual(2, self.mock_rsync_run.call_count)
        self.assertEqual(call(validateAfter=True), self.mock_rsync_run.call_args)
        self.assertEqual(
            [
                call("Syncing pg_xlog directory of dbid 2"),
    ...
    def test_sync_pg_data_success(self):
        ...
        self.assertEqual(call(validateAfter=True), self.mock_rsync_run.call_args)
        self.assertEqual(
            [call("Syncing pg_data of dbid 2")], self.mock_logger.debug.call_args_list
        )
...
class DifferentialRecoveryRunTestCase(GpTestCase):
    def setUp(self):
        ...
        self.apply_patches([
            patch(
                "gpsegrecovery.DifferentialRecovery.sync_tablespaces",
                return_value=Mock(),
            ),
            patch(
                "gpsegrecovery.DifferentialRecovery.pg_start_backup",
                return_value=Mock(),
            ),
            patch(
                "gpsegrecovery.DifferentialRecovery.pg_stop_backup", return_value=Mock()
            ),
            patch("gpsegrecovery.ModifyConfSetting", return_value=Mock()),
            patch("gpsegrecovery.start_segment", return_value=Mock()),
            patch(
                "gppylib.commands.pg.PgReplicationSlot.slot_exists", return_value=Mock()
            ),
            patch(
                "gppylib.commands.pg.PgReplicationSlot.drop_slot", return_value=Mock()
            ),
            patch(
                "gppylib.commands.pg.PgReplicationSlot.create_slot", return_value=Mock()
            ),
            patch("gppylib.commands.pg.PgBaseBackup.__init__", return_value=None),
            patch("gppylib.commands.pg.PgBaseBackup.run"),
            patch("gppylib.db.dbconn.connect", autospec=True),
            patch(
                "gppylib.db.dbconn.execSQL", return_value=FakeCursor(my_list=[[True]])
            ),
        ])
    ...
    def _assert_rsync_runs(self):
        ...
        expected_logger_info_args = [
            call(
                "Running differential recovery with progress output temporarily in /tmp/test_progress_file"
            ),
    ...
    @patch("gpsegrecovery.DifferentialRecovery.sync_pg_data", return_value=Mock())
    @patch(
        "gpsegrecovery.DifferentialRecovery.write_conf_files", side_effect=Exception()
    )
    ...
    @patch("gpsegrecovery.DifferentialRecovery.sync_pg_data", return_value=Mock())
    @patch(
        "gpsegrecovery.DifferentialRecovery.sync_xlog_and_control_file",
        side_effect=Exception(),
    )
    @patch("gpsegrecovery.DifferentialRecovery.write_conf_files", return_value=Mock())
    def test_diff_recovery_sync_xlog_and_control_file_exception(
    ...
    def test_diff_recovery_modify_conf_setting_exception(self, mock1, mock2):
        ...
        self.assertEqual(
            [
                call("Writing recovery.conf and internal.auto.conf files for dbid 2"),
    ...
    def test_diff_recovery_basebackup_run_one_exception(self, mock1, mock2):
        self.mock_pgbasebackup_run.side_effect = [
            Exception("backup failed once"),
            Mock(),
        ]
        ...
        self.assertEqual(1, self.mock_pgbasebackup_init.call_count)
        self.assertEqual(
            [expected_init_args], self.mock_pgbasebackup_init.call_args_list
        )
        self.assertEqual(1, self.mock_pgbasebackup_run.call_count)
        self.assertEqual(
            [call(validateAfter=True)], self.mock_pgbasebackup_run.call_args_list
        )
        self.mock_logger.info.any_call(
            "Running pg_basebackup failed: backup failed once"
        )
        self.assertEqual(0, gpsegrecovery.start_segment.call_count)
        self.assertEqual(2, self.mock_logger.debug.call_count)
        self.assertEqual(
