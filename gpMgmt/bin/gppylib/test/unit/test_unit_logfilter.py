...
class GppylibLogFilterTestCase(unittest.TestCase):
    def test_parse_log_name(self):
        testData = [
            ...
            (
                ...
                lf.LogNameInfo(
                    "gpdb-2023-10-05_090100.csv", datetime(2023, 10, 5, 9, 1), True
                ),
            ),
            (
                "gpdb-2023-10-05_09050.csv",
                lf.LogNameInfo(
                    "gpdb-2023-10-05_09050.csv", datetime(2023, 10, 5, 9, 5), True
                ),
            ),
            (
                "gpdb-2023-10-05.csv",
                lf.LogNameInfo(
                    "gpdb-2023-10-05.csv", datetime(2023, 10, 5, 0, 0), True
                ),
            ),
            (
                "gpdb-2023-10-05_09010.csv",
                lf.LogNameInfo(
                    "gpdb-2023-10-05_09010.csv", datetime(2023, 10, 5, 9, 1), True
                ),
            ),
            (
                "gpdb-23-10-05_09011.csv",
                lf.LogNameInfo("gpdb-23-10-05_09011.csv", None, True),
            ),  # invalid format %Y should have 4 digits
            (
                "gpdb-2023-1-05_2000.csv",
                lf.LogNameInfo(
                    "gpdb-2023-1-05_2000.csv", datetime(2023, 1, 5, 20, 0), True
                ),
            ),
            (
                "gpdb-2023-1-5_0000.csv",
                lf.LogNameInfo(
                    "gpdb-2023-1-5_0000.csv", datetime(2023, 1, 5, 0, 0), True
                ),
            ),
        ]
    ...
    def test_getOrderedLogNameInfoArrByNameTS(self):
        ...
        self.assertEqual(
            lf._getOrderedLogNameInfoArrByNameTS(fNamesWithoutTS),
            expectedOrderWithoutTS,
        )
        ...
        fNamesWithTS = [
            "gpdb-2023-10-05_090100.csv",
            "gpdb-2023-01-05_090100.csv",
            "gpdb-2023-2-05_080000.csv",
        ]
        expectedOrderWithTS = [
            lf.LogNameInfo(
                "gpdb-2023-01-05_090100.csv", datetime(2023, 1, 5, 9, 1), True
            ),
            lf.LogNameInfo(
                "gpdb-2023-2-05_080000.csv", datetime(2023, 2, 5, 8, 0), True
            ),
            lf.LogNameInfo(
                "gpdb-2023-10-05_090100.csv", datetime(2023, 10, 5, 9, 1), True
            ),
        ...
        self.assertEqual(
            lf._getOrderedLogNameInfoArrByNameTS(fNamesWithTS), expectedOrderWithTS
        )
    ...
    def test_getLogInfoArrayByNamesOrderedAndMarkedInTSRange_corner(self):
        ...
        expectedArr = [
            lf.LogNameInfo("gpdb-2023-10-05_000000.csv", datetime(2023, 10, 5), True)
        ]
        ...
        expectedArr = [
            lf.LogNameInfo(
                "gpdb-2023-1-5_0000.csv", datetime(2023, 1, 5, 0, 0), True
            ),  # format changed - first number of day is skipped and time doesn't contain seconds
            lf.LogNameInfo(
                "gpdb-2023-1-05_2000.csv", datetime(2023, 1, 5, 20, 0), True
            ),  # format changed - time doesn't contain seconds
            lf.LogNameInfo(
                "gpdb-2023-10-05.csv", datetime(2023, 10, 5, 0, 0), True
            ),  # format changed - no time prefix
            lf.LogNameInfo(
                "gpdb-2023-10-5.csv", datetime(2023, 10, 5, 0, 0), True
            ),  # format changed - first number of day is skipped
            lf.LogNameInfo(
                "gpdb-2023-10-05_09010.csv", datetime(2023, 10, 5, 9, 1), True
            ),  # format changed - last second is skipped
            lf.LogNameInfo(
                "gpdb-2023-10-05_090100.csv", datetime(2023, 10, 5, 9, 1), True
            ),
            lf.LogNameInfo(
                "gpdb-2023-10-05_09050.csv", datetime(2023, 10, 5, 9, 5), True
            ),  # format changed - last second skipped
            lf.LogNameInfo(
                "gpdb-9999-12-31_235959.csv", datetime(9999, 12, 31, 23, 59, 59), True
            ),  # timestamp is around datetime.max
        ...
        self.assertEqual(expectedArr, fnToTest(testArr, datetime.min, datetime.max))
        self.assertEqual(
            expectedArr, fnToTest(testArr, datetime(2023, 1, 5, 10), datetime.max)
        )

        # first element doesn't match the `begin` date
        expectedArr[0].belongsToTimeRangeFilter = False
        self.assertEqual(
            expectedArr, fnToTest(testArr, datetime(2023, 1, 5, 20), datetime.max)
        )
        self.assertEqual(
            expectedArr, fnToTest(testArr, datetime(2023, 1, 5, 21), datetime.max)
        )
