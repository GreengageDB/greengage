...
class GpArrayTestCase(GpTestCase):
    ...
    def setUp(self):
        ...
        self.apply_patches([
            ...
            patch(
                "gppylib.system.configurationImplGpdb.GpConfigurationProviderUsingGpdbCatalog",
                return_value=self.gpConfigMock,
            ),
        ])
    ...
    def test_get_segment_list(self):
        ...
        expansion_hosts = []
        self._validate_get_segment_list(
            gparray, hostlist, expansion_hosts, primary_list
        )
        ...
        offset = len(hostlist) * len(
            primary_list
        )  # need to continue numbering where the last createSegmentRows left off
        for row in rows:
            gparray.addExpansionSeg(
                row.content + offset,
                "p" if convert_bool(row.isprimary) else "m",
                row.dbid + offset,
                "p" if convert_bool(row.isprimary) else "m",
                row.host,
                row.address,
    ...
    @patch(
        "gppylib.system.environment.GpMasterEnvironment",
        return_value=Mock(),
        autospec=True,
    )
    def test_get_gparray_from_config(
        self, gpMasterEnvironmentMock, getConfigProviderFunctionMock
    ):
    ...
    def _validate_get_segment_list(
        self, gparray, hostlist, expansion_hosts, primary_list
    ):
        ...
        for segment in segments:
            ...
            datadir = primary.datadir[
                0 : primary.datadir.rindex("/")
            ]  # strip off the "/gpseg##" portion of the primary name for comparison
    ...
    @patch(
        ...
        return_value=[
            [
                "PostgreSQL 8.3.23 (Greenplum Database 5.0.0 build dev) on x86_64-pc-linux-gnu, compiled by GCC gcc (GCC) 4.4.7 20120313 (Red Hat 4.4.7-17) compiled on Feb  9 2017 23:06:31"
            ]
        ],
    )
    ...
    def test_initFromCatalog_mismatched_versions(self, mock_connect, mock_execSQL):
        with self.assertRaisesRegexp(
            Exception, "Cannot connect to GPDB version 5 from installed version 6"
        ):
