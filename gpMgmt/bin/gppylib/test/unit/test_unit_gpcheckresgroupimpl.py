...
class GpCheckResGroupImplCGroup(unittest.TestCase):
    def setUp(self):
        ...
        self.touch(
            os.path.join(self.cgroup_mntpnt, "cpu", "gpdb", "cgroup.procs"), 0o600
        )
        self.touch(
            os.path.join(self.cgroup_mntpnt, "cpu", "gpdb", "cpu.cfs_period_us"), 0o600
        )
        self.touch(
            os.path.join(self.cgroup_mntpnt, "cpu", "gpdb", "cpu.cfs_quota_us"), 0o600
        )
        ...
        self.touch(
            os.path.join(self.cgroup_mntpnt, "cpuacct", "gpdb", "cgroup.procs"), 0o600
        )
        self.touch(
            os.path.join(self.cgroup_mntpnt, "cpuacct", "gpdb", "cpuacct.usage"), 0o400
        )
        self.touch(
            os.path.join(self.cgroup_mntpnt, "cpuacct", "gpdb", "cpuacct.stat"), 0o400
        )

        self.touch(
            os.path.join(self.cgroup_mntpnt, "memory", "memory.limit_in_bytes"), 0o400
        )
        self.touch(
            os.path.join(self.cgroup_mntpnt, "memory", "memory.memsw.limit_in_bytes"),
            0o400,
        )

        os.mkdir(os.path.join(self.cgroup_mntpnt, "memory", "gpdb"), 0o700)
        self.touch(
            os.path.join(self.cgroup_mntpnt, "memory", "gpdb", "memory.limit_in_bytes"),
            0o600,
        )
        self.touch(
            os.path.join(self.cgroup_mntpnt, "memory", "gpdb", "memory.usage_in_bytes"),
            0o400,
        )

        os.mkdir(os.path.join(self.cgroup_mntpnt, "cpuset", "gpdb"), 0o700)
        self.touch(
            os.path.join(self.cgroup_mntpnt, "cpuset", "gpdb", "cgroup.procs"), 0o600
        )
        self.touch(
            os.path.join(self.cgroup_mntpnt, "cpuset", "gpdb", "cpuset.cpus"), 0o600
        )
        self.touch(
            os.path.join(self.cgroup_mntpnt, "cpuset", "gpdb", "cpuset.mems"), 0o600
        )
    ...
    def test_when_cpu_gpdb_dir_missing(self):
        ...
        with self.assertRaisesRegexp(
            AssertionError, "directory '.*/cpu/gpdb/' does not exist"
        ):
            self.cgroup.validate_all()
    ...
    def test_when_cpu_gpdb_dir_bad_permission(self):
        ...
        with self.assertRaisesRegexp(
            AssertionError,
            "directory '.*/cpu/gpdb/' permission denied: require permission 'rwx'",
        ):
            self.cgroup.validate_all()
    ...
    def test_when_cpu_gpdb_cgroup_procs_missing(self):
        ...
        with self.assertRaisesRegexp(
            AssertionError, "file '.*/cpu/gpdb/cgroup.procs' does not exist"
        ):
            self.cgroup.validate_all()
    ...
    def test_when_cpu_gpdb_cgroup_procs_bad_permission(self):
        ...
        with self.assertRaisesRegexp(
            AssertionError,
            "file '.*/cpu/gpdb/cgroup.procs' permission denied: require permission 'rw'",
        ):
            self.cgroup.validate_all()
    ...
    def test_when_cpu_gpdb_cpu_cfs_period_us_missing(self):
        ...
        with self.assertRaisesRegexp(
            AssertionError, "file '.*/cpu/gpdb/cpu.cfs_period_us' does not exist"
        ):
            self.cgroup.validate_all()

    def test_when_cpu_gpdb_cpu_cfs_period_us_bad_permission(self):
        os.chmod(
            os.path.join(self.cgroup_mntpnt, "cpu", "gpdb", "cpu.cfs_period_us"), 0o100
        )
        with self.assertRaisesRegexp(
            AssertionError,
            "file '.*/cpu/gpdb/cpu.cfs_period_us' permission denied: require permission 'rw'",
        ):
            self.cgroup.validate_all()
    ...
    def test_when_cpu_gpdb_cpu_cfs_quota_us_missing(self):
        ...
        with self.assertRaisesRegexp(
            AssertionError, "file '.*/cpu/gpdb/cpu.cfs_quota_us' does not exist"
        ):
            self.cgroup.validate_all()

    def test_when_cpu_gpdb_cpu_cfs_quota_us_bad_permission(self):
        os.chmod(
            os.path.join(self.cgroup_mntpnt, "cpu", "gpdb", "cpu.cfs_quota_us"), 0o100
        )
        with self.assertRaisesRegexp(
            AssertionError,
            "file '.*/cpu/gpdb/cpu.cfs_quota_us' permission denied: require permission 'rw'",
        ):
            self.cgroup.validate_all()
    ...
    def test_when_cpu_gpdb_cpu_shares_missing(self):
        ...
        with self.assertRaisesRegexp(
            AssertionError, "file '.*/cpu/gpdb/cpu.shares' does not exist"
        ):
            self.cgroup.validate_all()
    ...
    def test_when_cpu_gpdb_cpu_shares_bad_permission(self):
        ...
        with self.assertRaisesRegexp(
            AssertionError,
            "file '.*/cpu/gpdb/cpu.shares' permission denied: require permission 'rw'",
        ):
            self.cgroup.validate_all()
    ...
    def test_when_cpuacct_gpdb_dir_missing(self):
        shutil.rmtree(os.path.join(self.cgroup_mntpnt, "cpuacct", "gpdb"))
        with self.assertRaisesRegexp(
            AssertionError, "directory '.*/cpuacct/gpdb/' does not exist"
        ):
            self.cgroup.validate_all()
    ...
    def test_when_cpuacct_gpdb_dir_bad_permission(self):
        ...
        with self.assertRaisesRegexp(
            AssertionError,
            "directory '.*/cpuacct/gpdb/' permission denied: require permission 'rwx'",
        ):
            self.cgroup.validate_all()
        # restore permission for the dir to be removed in tearDown()
        os.chmod(os.path.join(self.cgroup_mntpnt, "cpuacct", "gpdb"), 0o700)
    ...
    def test_when_cpuacct_gpdb_cgroup_procs_missing(self):
        ...
        with self.assertRaisesRegexp(
            AssertionError, "file '.*/cpuacct/gpdb/cgroup.procs' does not exist"
        ):
            self.cgroup.validate_all()

    def test_when_cpuacct_gpdb_cgroup_procs_bad_permission(self):
        os.chmod(
            os.path.join(self.cgroup_mntpnt, "cpuacct", "gpdb", "cgroup.procs"), 0o100
        )
        with self.assertRaisesRegexp(
            AssertionError,
            "file '.*/cpuacct/gpdb/cgroup.procs' permission denied: require permission 'rw'",
        ):
            self.cgroup.validate_all()
    ...
    def test_when_cpuacct_gpdb_cpuacct_usage_missing(self):
        ...
        with self.assertRaisesRegexp(
            AssertionError, "file '.*/cpuacct/gpdb/cpuacct.usage' does not exist"
        ):
            self.cgroup.validate_all()

    def test_when_cpuacct_gpdb_cpuacct_usage_bad_permission(self):
        os.chmod(
            os.path.join(self.cgroup_mntpnt, "cpuacct", "gpdb", "cpuacct.usage"), 0o100
        )
        with self.assertRaisesRegexp(
            AssertionError,
            "file '.*/cpuacct/gpdb/cpuacct.usage' permission denied: require permission 'r'",
        ):
            self.cgroup.validate_all()
    ...
    def test_when_cpuacct_gpdb_cpuacct_stat_missing(self):
        ...
        with self.assertRaisesRegexp(
            AssertionError, "file '.*/cpuacct/gpdb/cpuacct.stat' does not exist"
        ):
            self.cgroup.validate_all()

    def test_when_cpuacct_gpdb_cpuacct_stat_bad_permission(self):
        os.chmod(
            os.path.join(self.cgroup_mntpnt, "cpuacct", "gpdb", "cpuacct.stat"), 0o100
        )
        with self.assertRaisesRegexp(
            AssertionError,
            "file '.*/cpuacct/gpdb/cpuacct.stat' permission denied: require permission 'r'",
        ):
            self.cgroup.validate_all()
    ...
    def test_when_memory_limit_in_bytes_missing(self):
        ...
        with self.assertRaisesRegexp(
            AssertionError, "file '.*/memory/memory.limit_in_bytes' does not exist"
        ):
            self.cgroup.validate_all()

    def test_when_memory_limit_in_bytes_bad_permission(self):
        os.chmod(
            os.path.join(self.cgroup_mntpnt, "memory", "memory.limit_in_bytes"), 0o100
        )
        with self.assertRaisesRegexp(
            AssertionError,
            "file '.*/memory/memory.limit_in_bytes' permission denied: require permission 'r'",
        ):
            self.cgroup.validate_all()
    ...
    def test_when_memory_gpdb_dir_missing(self):
        ...
        if gpver.version >= [6, 0, 0]:
            with self.assertRaisesRegexp(
                AssertionError, "directory '.*/memory/gpdb/' does not exist"
            ):
                self.cgroup.validate_all()
        else:
            self.cgroup.validate_all()
    ...
    def test_when_memory_gpdb_dir_bad_permission(self):
        ...
        if gpver.version >= [6, 0, 0]:
            with self.assertRaisesRegexp(
                AssertionError,
                "directory '.*/memory/gpdb/' permission denied: require permission 'rwx'",
            ):
                self.cgroup.validate_all()
        else:
            self.cgroup.validate_all()
    ...
    def test_when_memory_gpdb_limit_in_bytes_missing(self):
        ...
        if gpver.version >= [6, 0, 0]:
            with self.assertRaisesRegexp(
                AssertionError,
                "file '.*/memory/gpdb/memory.limit_in_bytes' does not exist",
            ):
                self.cgroup.validate_all()
        else:
            self.cgroup.validate_all()
    ...
    def test_when_memory_gpdb_usage_in_bytes_missing(self):
        ...
        if gpver.version >= [6, 0, 0]:
            with self.assertRaisesRegexp(
                AssertionError,
                "file '.*/memory/gpdb/memory.usage_in_bytes' does not exist",
            ):
                self.cgroup.validate_all()
        else:
            self.cgroup.validate_all()
    ...
    def test_when_memory_gpdb_usage_in_bytes_bad_permission(self):
        ...
        if gpver.version >= [6, 0, 0]:
            with self.assertRaisesRegexp(
                AssertionError,
                "file '.*/memory/gpdb/memory.usage_in_bytes' permission denied: require permission 'r'",
            ):
                self.cgroup.validate_all()
        else:
            self.cgroup.validate_all()
    ...
    def test_when_cpuset_gpdb_dir_missing(self):
        ...
        if gpver.version >= [6, 0, 0]:
            with self.assertRaisesRegexp(
                AssertionError, "directory '.*/cpuset/gpdb/' does not exist"
            ):
                self.cgroup.validate_all()
        else:
            self.cgroup.validate_all()
    ...
    def test_when_cpuset_gpdb_dir_bad_permission(self):
        ...
        if gpver.version >= [6, 0, 0]:
            with self.assertRaisesRegexp(
                AssertionError,
                "directory '.*/cpuset/gpdb/' permission denied: require permission 'rwx'",
            ):
                self.cgroup.validate_all()
        else:
            self.cgroup.validate_all()
    ...
    def test_when_cpuset_gpdb_cgroup_procs_missing(self):
        ...
        if gpver.version >= [6, 0, 0]:
            with self.assertRaisesRegexp(
                AssertionError, "file '.*/cpuset/gpdb/cgroup.procs' does not exist"
            ):
                self.cgroup.validate_all()
        else:
            self.cgroup.validate_all()
    ...
    def test_when_cpuset_gpdb_cgroup_procs_bad_permission(self):
        os.chmod(
            os.path.join(self.cgroup_mntpnt, "cpuset", "gpdb", "cgroup.procs"), 0o100
        )
        if gpver.version >= [6, 0, 0]:
            with self.assertRaisesRegexp(
                AssertionError,
                "file '.*/cpuset/gpdb/cgroup.procs' permission denied: require permission 'rw'",
            ):
                self.cgroup.validate_all()
        else:
            self.cgroup.validate_all()
    ...
    def test_when_cpuset_gpdb_cpuset_cpus_missing(self):
        ...
        if gpver.version >= [6, 0, 0]:
            with self.assertRaisesRegexp(
                AssertionError, "file '.*/cpuset/gpdb/cpuset.cpus' does not exist"
            ):
                self.cgroup.validate_all()
        else:
            self.cgroup.validate_all()
    ...
    def test_when_cpuset_gpdb_cpuset_cpus_bad_permission(self):
        os.chmod(
            os.path.join(self.cgroup_mntpnt, "cpuset", "gpdb", "cpuset.cpus"), 0o100
        )
        if gpver.version >= [6, 0, 0]:
            with self.assertRaisesRegexp(
                AssertionError,
                "file '.*/cpuset/gpdb/cpuset.cpus' permission denied: require permission 'rw'",
            ):
                self.cgroup.validate_all()
        else:
            self.cgroup.validate_all()
    ...
    def test_when_cpuset_gpdb_cpuset_mems_missing(self):
        ...
        if gpver.version >= [6, 0, 0]:
            with self.assertRaisesRegexp(
                AssertionError, "file '.*/cpuset/gpdb/cpuset.mems' does not exist"
            ):
                self.cgroup.validate_all()
        else:
            self.cgroup.validate_all()
    ...
    def test_when_cpuset_gpdb_cpuset_mems_bad_permission(self):
        os.chmod(
            os.path.join(self.cgroup_mntpnt, "cpuset", "gpdb", "cpuset.mems"), 0o100
        )
        if gpver.version >= [6, 0, 0]:
            with self.assertRaisesRegexp(
                AssertionError,
                "file '.*/cpuset/gpdb/cpuset.mems' permission denied: require permission 'rw'",
            ):
                self.cgroup.validate_all()
        else:
            self.cgroup.validate_all()
