...
class GpAddMirrorsTest(GpTestCase):
    def setUp(self):
        ...
        self.apply_patches([
            ...
            patch(
                "gppylib.programs.clsAddMirrors.logger",
                return_value=Mock(spec=["log", "info", "debug", "error"]),
            ),
            patch(
                "gppylib.programs.clsAddMirrors.log_to_file_only", return_value=Mock()
            ),
            patch(
                "gppylib.programs.clsAddMirrors.GpMasterEnvironment",
                return_value=Mock(),
                spec=GpMasterEnvironment,
            ),
            patch("gppylib.system.faultProberInterface.getFaultProber"),
            patch(
                "gppylib.programs.clsAddMirrors.configInterface.getConfigurationProvider",
                return_value=Mock(),
            ),
            patch("gppylib.programs.clsAddMirrors.heapchecksum.HeapChecksum"),
            patch(
                "gppylib.gparray.GpArray.getSegmentsByHostName",
                return_value=self.gparray_get_segments_by_hostname,
            ),
        ])
        self.raw_input_mock = self.get_mock_from_apply_patch("raw_input")
        self.mock_logger = self.get_mock_from_apply_patch("logger")
        self.gpMasterEnvironmentMock = self.get_mock_from_apply_patch(
            "GpMasterEnvironment"
        )
        ...
        self.mock_heap_checksum.return_value.are_segments_consistent.return_value = (
            False
        )
    ...
    def test_validate_heap_checksum_succeeds_if_cluster_consistent(self):
        ...
        self.mock_logger.info.assert_any_call(
            "Heap checksum setting consistent across cluster"
        )
    ...
    def test_run_calls_validate_heap_checksum(self):
        ...
        with self.assertRaisesRegexp(
            Exception, "Segments have heap_checksum set inconsistently to master"
        ):
            command_obj.run()
    ...
    def test_generated_file_contains_default_port_offsets(self):
        ...
        self.config_provider_mock.loadSystemConfig.return_value = GpArray([
            self.master,
            self.primary0,
            self.primary1,
        ])
    ...
    def test_datadir_interview(self):
        self.raw_input_mock.side_effect = [
            "/tmp/datadirs/mirror1",
            "/tmp/datadirs/mirror2",
            "/tmp/datadirs/mirror3",
        ]
        ...
        self.subject = GpAddMirrorsProgram(options)
        directories = self.subject._GpAddMirrorsProgram__getDataDirectoriesForMirrors(
            3, None
        )
