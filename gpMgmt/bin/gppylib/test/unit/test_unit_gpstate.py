...
class RecoveryProgressTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.gpArrayMock.getSegDbList.return_value = [
            self.primary1,
            self.primary2,
            self.primary3,
        ]

    def check_recovery_fields(
        self, segment, type, completed, total, percentage, stage=None
    ):
        self.assertEqual(type, self.data.getStrValue(segment, VALUE_RECOVERY_TYPE))
        self.assertEqual(
            completed, self.data.getStrValue(segment, VALUE_RECOVERY_COMPLETED_BYTES)
        )
        self.assertEqual(
            percentage, self.data.getStrValue(segment, VALUE_RECOVERY_PERCENTAGE)
        )
        if type == "differential":
            self.assertEqual(
                stage, self.data.getStrValue(segment, VALUE_RECOVERY_STAGE)
            )
        else:
            self.assertEqual(
                total, self.data.getStrValue(segment, VALUE_RECOVERY_TOTAL_BYTES)
            )
    ...
    def test_parse_recovery_progress_data_returns_empty_when_file_does_not_exist(self):
        self.assertEqual(
            ...
            GpSystemStateProgram._parse_recovery_progress_data(
                self.data, "/file/does/not/exist", self.gpArrayMock
            ),
        )
    ...
    def test_parse_recovery_progress_data_adds_recovery_progress_data_during_recovery(
        self,
    ):
        with tempfile.NamedTemporaryFile() as f:
            f.write(
                "full:1: 1164848/1371715 kB (84%), 0/1 tablespace (...t1/demoDataDir0/base/16384/40962)".encode(
                    "utf-8"
                )
            )
            f.flush()
            self.assertEqual(
                [self.primary1],
                GpSystemStateProgram._parse_recovery_progress_data(
                    self.data, f.name, self.gpArrayMock
                ),
            )

            self.check_recovery_fields(
                self.primary1, "full", "1164848", "1371715", "84%"
            )
            self.check_recovery_fields(self.primary2, "", "", "", "")
            self.check_recovery_fields(self.primary3, "", "", "", "")

    def test_parse_recovery_progress_data_adds_recovery_progress_data_during_multiple_recoveries(
        self,
    ):
        with tempfile.NamedTemporaryFile() as f:
            f.write(
                "full:1: 1164848/1371715 kB (0%), 0/1 tablespace (...t1/demoDataDir0/base/16384/40962)\n".encode(
                    "utf-8"
                )
            )
            f.write(
                "incremental:2: 1171384/1371875 kB (85%)anything can appear here".encode(
                    "utf-8"
                )
            )
            f.write(
                "incremental:2: 1171384/1371875 kB (85%)anything can appear here\n".encode(
                    "utf-8"
                )
            )
            f.write(
            ...
            f.flush()
            self.assertEqual(
                [self.primary1, self.primary2, self.primary3],
                GpSystemStateProgram._parse_recovery_progress_data(
                    self.data, f.name, self.gpArrayMock
                ),
            )
            ...
            self.check_recovery_fields(
                self.primary1, "full", "1164848", "1371715", "0%"
            )
            self.check_recovery_fields(
                self.primary2, "incremental", "1171384", "1371875", "85%"
            )
            self.check_recovery_fields(
                self.primary3,
                "differential",
                ...
                "",
                "74%",
                "Syncing pg_data of dbid 1",
            )

    def test_parse_recovery_progress_data_doesnt_adds_recovery_progress_data_only_for_completed_recoveries(
        self,
    ):
        with tempfile.NamedTemporaryFile() as f:
            f.write(
                "full:1: 1164848/1371715 kB (84%), 0/1 tablespace (...t1/demoDataDir0/base/16384/40962)\n".encode(
                    "utf-8"
                )
            )
            f.write(
                "full:2: 1164848/1371715 kB (100%), 0/1 tablespace (...t1/demoDataDir0/base/16384/40962)\n".encode(
                    "utf-8"
                )
            )
            f.write(
                "full:3: 1164848/1371715 kB (100#), 0/1 tablespace (...t1/demoDataDir0/base/16384/40962)\n".encode(
                    "utf-8"
                )
            )
            f.write(
                "full:3: 1164848/1371715 kB 84%, 0/1 tablespace (...t1/demoDataDir0/base/16384/40962)\n".encode(
                    "utf-8"
                )
            )
            f.write(
            ...
            f.write(
                "full:3: /1371715 kB (100%, 0/1 tablespace (...t1/demoDataDir0/base/16384/40962)\n".encode(
                    "utf-8"
                )
            )
            f.write(
                "full:3: 1/1371715 KB (100%), 0/1 tablespace (...t1/demoDataDir0/base/16384/40962)\n".encode(
                    "utf-8"
                )
            )
            f.write(
            ...
            f.write(
                "full:3: 1/1371715 MB (100%), 0/1 tablespace (...t1/demoDataDir0/base/16384/40962)\n".encode(
                    "utf-8"
                )
            )
            f.write(
                "full:3: 1/1371715 B (100%), 0/1 tablespace (...t1/demoDataDir0/base/16384/40962)\n".encode(
                    "utf-8"
                )
            )
            f.write(
            ...
            f.write(
                "full:3: 1164848/1371715 kB (84%a), 0/1 tablespace (...t1/demoDataDir0/base/16384/40962)\n".encode(
                    "utf-8"
                )
            )
            f.write(
                "full:3: 1164848/1371715 kB (a84%), 0/1 tablespace (...t1/demoDataDir0/base/16384/40962)\n".encode(
                    "utf-8"
                )
            )
            f.write(
            ...
            self.check_recovery_fields(
                self.primary2, "full", "1164848", "1371715", "100%"
            )
            self.check_recovery_fields(self.primary3, "", "", "", "")

    def test_parse_recovery_progress_data_adds_differential_recovery_progress_data_during_single_recovery(
        self,
    ):
        with tempfile.NamedTemporaryFile() as f:
            f.write(
                "differential:1:     38,861,653   7%   43.45MB/s    0:00:00 (xfr#635, ir-chk=9262/9919) :Syncing pg_data of dbid 1\n".encode(
                    "utf-8"
                )
            )
            f.flush()
    ...
    def test_parse_recovery_progress_data_adds_differential_recovery_progress_data_during_multiple_recovery(
        self,
    ):
        with tempfile.NamedTemporaryFile() as f:
            f.write(
            ...
            f.write(
                "differential:2:    122,017,543  74%   74.02MB/s    0:00:01 (xfr#1994, to-chk=963/2979) :Syncing tablespace of dbid 2 for oid 17934\n".encode(
                    "utf-8"
                )
            )
            f.write(
                "differential:3:    122,017,543  (74%)   74.02MB/s    0:00:01 (xfr#1994, to-chk=963/2979) :Invalid format\n".encode(
                    "utf-8"
                )
            )
            f.flush()
            ...
            self.check_recovery_fields(
                ...
                "",
                "74%",
                "Syncing tablespace of dbid 2 for oid 17934",
            )
            self.check_recovery_fields(self.primary3, "", "", "", "")
...
class ReplicationInfoTestCase(unittest.TestCase):
    ...
    def stub_activity_entry(self, **kwargs):
        ...
        return (kwargs.get("backend_start", None),)
    ...
    def test_add_replication_info_adds_unknowns_if_primary_is_down(self):
        ...
        self.assertEqual(
            "Unknown", self.data.getStrValue(self.mirror, VALUE__REPL_SENT_LOCATION)
        )
        self.assertEqual(
            "Unknown", self.data.getStrValue(self.mirror, VALUE__REPL_FLUSH_LOCATION)
        )
        self.assertEqual(
            "Unknown", self.data.getStrValue(self.mirror, VALUE__REPL_REPLAY_LOCATION)
        )

    @mock.patch("gppylib.db.dbconn.connect", autospec=True)
    def test_add_replication_info_adds_unknowns_if_connection_cannot_be_made(
        self, mock_connect
    ):
        # Simulate a connection failure in dbconn.connect().
        mock_connect.side_effect = pgdb.InternalError(
            "connection failure forced by unit test"
        )
        GpSystemStateProgram._add_replication_info(self.data, self.primary, self.mirror)
    ...
    @mock.patch("gppylib.db.dbconn.connect", autospec=True)
    def test_add_replication_info_adds_unknowns_if_pg_stat_replication_has_no_entries(
        self, mock_connect, mock_execSQL
    ):
    ...
    @mock.patch("gppylib.db.dbconn.execSQL", autospec=True)
    @mock.patch("gppylib.db.dbconn.connect", autospec=True)
    def test_add_replication_info_adds_unknowns_if_pg_stat_replication_has_too_many_mirrors(
        self, mock_connect, mock_execSQL
    ):
        self.mock_pg_stat_replication(
            mock_execSQL,
            [
    ...
    @mock.patch("gppylib.db.dbconn.execSQL", autospec=True)
    @mock.patch("gppylib.db.dbconn.connect", autospec=True)
    def test_add_replication_info_populates_correctly_from_pg_stat_replication(
        self, mock_connect, mock_execSQL
    ):
        ...
        self.assertEqual(
            "1024", self.data.getStrValue(self.primary, VALUE__REPL_SENT_LEFT)
        )

        self.assertEqual(
            "Streaming", self.data.getStrValue(self.mirror, VALUE__MIRROR_STATUS)
        )
        self.assertEqual(
            "0/1000", self.data.getStrValue(self.mirror, VALUE__REPL_SENT_LOCATION)
        )
        self.assertEqual(
            "0/0800", self.data.getStrValue(self.mirror, VALUE__REPL_FLUSH_LOCATION)
        )
        self.assertEqual(
            "0/0000", self.data.getStrValue(self.mirror, VALUE__REPL_REPLAY_LOCATION)
        )
        self.assertEqual(
            "2048", self.data.getStrValue(self.mirror, VALUE__REPL_FLUSH_LEFT)
        )
        self.assertEqual(
            "4096", self.data.getStrValue(self.mirror, VALUE__REPL_REPLAY_LEFT)
        )

    @mock.patch("gppylib.db.dbconn.execSQL", autospec=True)
    @mock.patch("gppylib.db.dbconn.connect", autospec=True)
    def test_add_replication_info_omits_lag_info_if_WAL_locations_are_identical(
        self, mock_connect, mock_execSQL
    ):
        # Set up the row definition.
        self.mock_pg_stat_replication(
        ...
        self.assertEqual(
            "0", self.data.getStrValue(self.primary, VALUE__REPL_SENT_LEFT)
        )
        ...
        self.assertEqual(
            "0/1000", self.data.getStrValue(self.mirror, VALUE__REPL_FLUSH_LOCATION)
        )
        self.assertEqual(
            "0/1000", self.data.getStrValue(self.mirror, VALUE__REPL_REPLAY_LOCATION)
        )
        self.assertEqual(
            "0", self.data.getStrValue(self.mirror, VALUE__REPL_FLUSH_LEFT)
        )
        self.assertEqual(
            "0", self.data.getStrValue(self.mirror, VALUE__REPL_REPLAY_LEFT)
        )

    @mock.patch("gppylib.db.dbconn.execSQL", autospec=True)
    @mock.patch("gppylib.db.dbconn.connect", autospec=True)
    def test_add_replication_info_adds_unknowns_if_pg_stat_replication_is_incomplete(
        self, mock_connect, mock_execSQL
    ):
        # Set up the row definition.
        self.mock_pg_stat_replication(
        ...
        self.assertEqual(
            "Unknown", self.data.getStrValue(self.primary, VALUE__REPL_SENT_LEFT)
        )
        ...
        self.assertEqual(
            "Unknown", self.data.getStrValue(self.mirror, VALUE__REPL_FLUSH_LEFT)
        )
        self.assertEqual(
            "Unknown", self.data.getStrValue(self.mirror, VALUE__REPL_REPLAY_LEFT)
        )
    ...
    @mock.patch("gppylib.db.dbconn.execSQL", autospec=True)
    @mock.patch("gppylib.db.dbconn.connect", autospec=True)
    def test_add_replication_info_displays_full_backup_state_on_primary(
        self, mock_connect, mock_execSQL
    ):
        self.mock_pg_stat_replication(
            mock_execSQL,
        ...
        self.assertEqual(
            "Copying files from primary",
            self.data.getStrValue(self.primary, VALUE__MIRROR_STATUS),
        )

    @mock.patch("gppylib.db.dbconn.execSQL", autospec=True)
    @mock.patch("gppylib.db.dbconn.connect", autospec=True)
    def test_add_replication_info_displays_full_backup_start_timestamp_on_primary(
        self, mock_connect, mock_execSQL
    ):
        self.mock_pg_stat_replication(
            mock_execSQL,
        ...
        self.assertEqual(
            ...
            self.data.getStrValue(self.primary, VALUE__MIRROR_RECOVERY_START),
        )

    @mock.patch("gppylib.db.dbconn.execSQL", autospec=True)
    @mock.patch("gppylib.db.dbconn.connect", autospec=True)
    def test_add_replication_info_displays_simultaneous_backup_and_replication(
        self, mock_connect, mock_execSQL
    ):
        self.mock_pg_stat_replication(
            mock_execSQL,
    ...
    @mock.patch("gppylib.db.dbconn.execSQL", autospec=True)
    @mock.patch("gppylib.db.dbconn.connect", autospec=True)
    def test_add_replication_info_displays_status_when_pg_rewind_is_active_and_mirror_is_down(
        self, mock_connect, mock_execSQL
    ):
        self.mock_pg_stat_replication(mock_execSQL, [])
        self.mock_pg_current_xlog_location(mock_execSQL, [])
        ...
        self.assertEqual(
            "Rewinding history to match primary timeline",
            self.data.getStrValue(self.primary, VALUE__MIRROR_STATUS),
        )

    @mock.patch("gppylib.db.dbconn.execSQL", autospec=True)
    @mock.patch("gppylib.db.dbconn.connect", autospec=True)
    def test_add_replication_info_does_not_update_mirror_status_when_mirror_is_down_and_there_is_no_recovery_underway(
        self, mock_connect, mock_execSQL
    ):
        self.mock_pg_stat_replication(mock_execSQL, [])
        self.mock_pg_current_xlog_location(mock_execSQL, [])
        ...
        self.assertEqual(
            "previous value", self.data.getStrValue(self.primary, VALUE__MIRROR_STATUS)
        )

    @mock.patch("gppylib.db.dbconn.execSQL", autospec=True)
    @mock.patch("gppylib.db.dbconn.connect", autospec=True)
    def test_add_replication_info_displays_start_time_when_pg_rewind_is_active_and_mirror_is_down(
        self, mock_connect, mock_execSQL
    ):
        mock_date = "1970-01-01 00:00:00.000000-00"
        self.mock_pg_stat_replication(mock_execSQL, [self.stub_replication_entry()])
        self.mock_pg_current_xlog_location(mock_execSQL, [])
        self.mock_pg_stat_activity(
            mock_execSQL, [self.stub_activity_entry(backend_start=mock_date)]
        )
        self.mirror.status = gparray.STATUS_DOWN
        ...
        self.assertEqual(
            mock_date, self.data.getStrValue(self.primary, VALUE__MIRROR_RECOVERY_START)
        )

    @mock.patch("gppylib.db.dbconn.execSQL", autospec=True)
    @mock.patch("gppylib.db.dbconn.connect", autospec=True)
    def test_add_replication_info_does_not_query_pg_stat_activity_when_mirror_is_up(
        self, mock_connect, mock_execSQL
    ):
        self.mock_pg_stat_replication(mock_execSQL, [])
        self.mock_pg_current_xlog_location(mock_execSQL, [])
    ...
    def test_set_mirror_replication_values_complains_about_incorrect_kwargs(self):
        with self.assertRaises(TypeError):
            GpSystemStateProgram._set_mirror_replication_values(
                self.data, self.mirror, badarg=1
            )

    # test the condition when netstat is not found but ss is found
    @mock.patch(
        "gppylib.commands.unix.findCmdInPath", mock.MagicMock(side_effect=find_ss)
    )
    def test_PgPortIsActive_no_netstat(self):
        port = 6000
        port_active = unix.PgPortIsActive.local(
            "check netstat for postmaster port", "/tmp/.s.PGSQL.%d" % port, port
        )
        self.assertTrue(port_active, "PgPortIsActive.local should be true")

    # test the condition when netstat and ss are not found
    @mock.patch(
        "gppylib.commands.unix.findCmdInPath",
        mock.MagicMock(side_effect=dont_find_ss_netstat),
    )
...
class SegmentsReqAttentionTestCase(GpTestCase):
    ...
    def test_WalSyncRemainingBytes_multiple_segments_both_catchup(self):
        ...
        with mock.patch("gppylib.db.dbconn.execSQL", return_value=m) as mock_execSQL:
            GpSystemStateProgram._get_unsync_segs_add_wal_remaining_bytes(
                self.data, self.gpArray
            )
            self.assertEqual(mock_execSQL.call_count, 2)
            self.assertEqual(
                "100",
                self.data.getStrValue(self.primary1, VALUE__REPL_SYNC_REMAINING_BYTES),
            )
            self.assertEqual(
                "10",
                self.data.getStrValue(self.primary2, VALUE__REPL_SYNC_REMAINING_BYTES),
            )
