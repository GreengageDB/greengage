...
class GpExpand(GpTestCase):
    def setUp(self):
        ...
        gpexpand_file = os.path.abspath(
            os.path.dirname(__file__) + "/../../../gpexpand"
        )
    ...
    def test_validate_heap_checksums_aborts_when_cluster_inconsistent(self, mock1):
        ...
        self.subject.logger.fatal.assert_any_call(
            "Cluster heap checksum setting differences reported"
        )
        self.subject.logger.fatal.assert_any_call(
            "Heap checksum settings on 1 of 2 segment instances "
            "do not match master <<<<<<<<"
        )
        self.subject.logger.error.assert_called_with(
            "gpexpand failed: Segments have heap_checksum set "
    ...
    @patch("gpexpand.is_cluster_up_and_balanced", return_value=True)
    def test_validate_heap_checksums_completes_when_cluster_consistent(
        ...
        self.subject.logger.error.assert_called_with(
            "gpexpand failed: Invalid input file: No expansion "
            "segments defined \n\nExiting..."
        )
    ...
    def test_nonstandard_gpArray_user_aborts(self):
        ...
        with patch("sys.stdout", new=io.BytesIO()) as mock_stdout:
            with self.assertRaises(SystemExit):
                self.subject.interview_setup(self.gparray, self.options)
            self.assertIn(
                "The current system appears to be non-standard.", mock_stdout.getvalue()
            )
