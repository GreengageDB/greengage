...
class ValidationForFullRecoveryTestCase(GpTestCase):
    def setUp(self):
        ...
        self.validation_recovery_cmd = gpsegsetuprecovery.ValidationForFullRecovery(
            name="test validation for full recovery",
    ...
    def _assert_passed(self):
        ...
        self.assertEqual(
            True, self.validation_recovery_cmd.get_results().wasSuccessful()
        )
        self.mock_logger.info.assert_called_with(
            "Validation successful for segment with dbid: 2"
        )
    ...
    def _assert_failed(self, expected_error):
        ...
        self.assertItemsEqual(
            expected_error, self.validation_recovery_cmd.get_results().stderr
        )
        self.assertEqual(
            False, self.validation_recovery_cmd.get_results().wasSuccessful()
        )
    ...
    def test_no_forceoverwrite_dir_exists_not_empty(self):
        ...
        error_str = "for segment with port 50000: Segment directory '{}' exists but is not empty!".format(
            os.path.dirname(d)
        )
        expected_error = (
            '{{"error_type": "validation", "error_msg": "{}", "dbid": 2, "datadir": "{}", "port": 50000, '
            '"progress_file": "/tmp/test_progress_file"}}'.format(
                error_str, os.path.dirname(d)
            )
        )
    ...
    def test_no_forceoverwrite_mkdir_exception(self, mock_os_mkdir):
        ...
        self._assert_failed(
            '{"error_type": "validation", "error_msg": "mkdirs failed", "dbid": 2, '
...
class SetupForIncrementalRecoveryTestCase(GpTestCase):
    def setUp(self):
        ...
        self.apply_patches([
            patch("gpsegsetuprecovery.dbconn.connect", return_value=self.mock_conn_val),
        ...
        self.setup_for_incremental_recovery_cmd = (
            gpsegsetuprecovery.SetupForIncrementalRecovery(
    ...
    def _assert_cmd_passed(self):
        ...
        self.assertEqual(
            "", self.setup_for_incremental_recovery_cmd.get_results().stdout
        )
        self.assertEqual(
            "", self.setup_for_incremental_recovery_cmd.get_results().stderr
        )
        self.assertEqual(
            True, self.setup_for_incremental_recovery_cmd.get_results().wasSuccessful()
        )
    ...
    def _assert_cmd_failed(self, expected_stderr):
        ...
        self.assertTrue(
            expected_stderr
            in self.setup_for_incremental_recovery_cmd.get_results().stderr
        )
        self.assertEqual(
            False, self.setup_for_incremental_recovery_cmd.get_results().wasSuccessful()
        )

    def _assert_checkpoint_query(self):
        gpsegsetuprecovery.dbconn.DbURL.assert_called_once_with(
            hostname="sdw1", port=40000, dbname="template1"
        )
        gpsegsetuprecovery.dbconn.connect.assert_called_once_with(
            self.mock_dburl_val, utility=True
        )
        gpsegsetuprecovery.dbconn.execSQL.assert_called_once_with(
            self.mock_conn_val, "CHECKPOINT"
        )
    ...
    @patch(
        "gppylib.commands.pg.Command",
        return_value=Command("rc1_cmd", "echo 1 | grep 2"),
    )
...
class SetupForDifferentialRecoveryTestCase(GpTestCase):
    def setUp(self):
        ...
        self.setup_for_differential_recovery_cmd = (
            gpsegsetuprecovery.SetupForDifferentialRecovery(
                name="setup for differential recovery",
                recovery_info=self.seg_recovery_info,
                logger=self.mock_logger,
            )
        )
    ...
    def _assert_cmd_passed(self):
        ...
        self.assertEqual(
            "", self.setup_for_differential_recovery_cmd.get_results().stdout
        )
        self.assertEqual(
            "", self.setup_for_differential_recovery_cmd.get_results().stderr
        )
        self.assertEqual(
            True, self.setup_for_differential_recovery_cmd.get_results().wasSuccessful()
        )
    ...
    def _assert_cmd_failed(self, expected_stderr):
        ...
        self.assertTrue(
            expected_stderr
            in self.setup_for_differential_recovery_cmd.get_results().stderr
        )
        self.assertEqual(
            False,
            self.setup_for_differential_recovery_cmd.get_results().wasSuccessful(),
        )
...
class SegSetupRecoveryTestCase(GpTestCase):
    ...
    def _assert_validation_full_call(
        ...
        self.assertTrue(isinstance(cmd, gpsegsetuprecovery.ValidationForFullRecovery))
    ...
    def _assert_setup_incr_call(self, cmd, expected_recovery_info):
        self.assertTrue(isinstance(cmd, gpsegsetuprecovery.SetupForIncrementalRecovery))
    ...
    @patch(
        "gpsegsetuprecovery.ValidationForFullRecovery.validate_failover_data_directory"
    )
    @patch("gpsegsetuprecovery.dbconn.connect")
    @patch("gpsegsetuprecovery.dbconn.DbURL")
    @patch("gpsegsetuprecovery.dbconn.execSQL")
    def test_complete_workflow(
        self, mock_execsql, mock_dburl, mock_connect, mock_validate_datadir
    ):
        ...
        self.assertRegexpMatches(gplog.get_logfile(), "/gpsegsetuprecovery.py_\d+\.log")
    ...
    def test_complete_workflow_exception(
        ...
        self.assertItemsEqual(
            '[{"error_type": "validation", "error_msg": "connect failed", "dbid": 4, "datadir": "target_data_dir4", '
    ...
    def test_get_recovery_cmds_is_called(
        self, mock_get_setup_cmds, mock_recovery_base_main, mock_logger
    ):
        mix_confinfo = gppylib.recoveryinfo.serialize_list([
            self.full_r1,
            self.incr_r2,
        ...
        SegSetupRecovery().main()
        mock_get_setup_cmds.assert_called_once_with(
            [self.full_r1, self.incr_r2, self.diff_r1], True, mock_logger.return_value
        )
        mock_recovery_base_main.assert_called_once_with(
            mock_get_setup_cmds.return_value
        )
