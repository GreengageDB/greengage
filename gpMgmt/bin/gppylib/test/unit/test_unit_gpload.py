...
class GpLoadTestCase(unittest.TestCase):
    ...
    def help_test_with_config(
        self, gpload_param, expected_begin_value, expected_commit_value
    ):
        print(gpload_param)
    ...
    def test_reuse_exttbl_with_logerrors_and_limit(self):
        ...
        sql = gploader.get_reuse_exttable_query(
            "csv", "header", None, {}, None, False, None
        )
    ...
    def test_case_merge_transaction(self):
        self.help_test_with_config(
            ["-f", os.path.join(os.path.dirname(__file__), "gpload_merge.yml")],
            True,
            True,
        )
    ...
    def test_case_insert_transaction(self):
        self.help_test_with_config(
            ["-f", os.path.join(os.path.dirname(__file__), "gpload_insert.yml")],
            True,
            True,
        )
    ...
    def test_case_insert_without_transaction(self):
        self.help_test_with_config(
            [
                "--no_auto_trans",
                "-f",
                os.path.join(os.path.dirname(__file__), "gpload_insert.yml"),
            ],
            False,
            False,
        )
    ...
    def test_case_update_with_transaction(self):
        self.help_test_with_config(
            ["-f", os.path.join(os.path.dirname(__file__), "gpload_update.yml")],
            True,
            True,
        )
    ...
    def test_case_update_without_transaction(self):
        self.help_test_with_config(
            [
                "--no_auto_trans",
                "-f",
                os.path.join(os.path.dirname(__file__), "gpload_update.yml"),
            ],
            False,
            False,
        )
    ...
    def test_case_configvalue(self):
        gploader = gpload([
            "-f",
            os.path.join(os.path.dirname(__file__), "allconfig.yml"),
        ])
        ...
        self.assertEqual(
            False, gploader.getconfig("gpload:input:log_errors", bool, False)
        )
        self.assertEqual(
            True, gploader.getconfig("gpload:input:fully_qualified_domain_name", bool)
        )
