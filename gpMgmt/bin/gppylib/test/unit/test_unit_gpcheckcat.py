...
class GpCheckCatTestCase(GpTestCase):
    def setUp(self):
        ...
        self.apply_patches([
            ...
            patch(
                "gpcheckcat.UniqueIndexViolationCheck",
                return_value=self.unique_index_violation_check,
            ),
        ...
        issues_list["cat2"] = [
            ("pg_type", ["pkey1", "pkey2"], [("r1", "r2"), ("r3", "r4")]),
            (
                "arbitrary_catalog_table",
                ["pkey1", "pkey2"],
                [("r1", "r2"), ("r3", "r4")],
    ...
    def test_running_unique_index_violation_check__makes_the_check(self):
        ...
        self.unique_index_violation_check.runCheck.assert_called_with(
            self.db_connection
        )

    def test_running_unique_index_violation_check__when_no_violations_are_found__passes_the_check(
        self,
    ):
        self.subject.runOneCheck("unique_index_violation")
    ...
    def test_running_unique_index_violation_check__when_violations_are_found__fails_the_check(
        self,
    ):
        self.unique_index_violation_check.runCheck.return_value = [
            dict(
                table_oid=123,
                table_name="stephen_table",
                index_name="finger",
                column_names="c1, c2",
    ...
    def test_checkcat_report__after_running_unique_index_violations_check__reports_violations(
        self,
    ):
        self.unique_index_violation_check.runCheck.return_value = [
            dict(
        ...
        expected_message1 = (
            "    Table stephen_table has a violated unique index: finger"
        )
    ...
    def test_drop_leaked_schemas____when_leaked_schemas_exist__finds_and_drops_leaked_schemas(
        self,
    ):
        self.leaked_schema_dropper.drop_leaked_schemas.return_value = [
            "schema1",
        ...
        self.leaked_schema_dropper.drop_leaked_schemas.assert_called_once_with(
            self.db_connection
        )
    ...
    def test_drop_leaked_schemas__when_leaked_schemas_exist__reports_which_schemas_are_dropped(
        self,
    ):
        self.leaked_schema_dropper.drop_leaked_schemas.return_value = [
            "schema1",
    ...
    @patch(
        "gpcheckcat_modules.repair.Repair.create_repair_for_extra_missing",
        return_value="/tmp",
    )
    ...
    def test_checkForeignKey__with_arg_gp_fastsequence(
        self, process_foreign_key_mock, fast_seq_mock
    ):
        cat_mock = Mock()
        self.subject.GV.catalog = cat_mock

        gp_fastsequence_issue = dict()
        gp_fastsequence_issue["gp_fastsequence"] = [
            ("pg_class", ["pkey1", "pkey2"], [("r1", "r2"), ("r3", "r4")]),
            (
                "arbitrary_catalog_table",
                ["pkey1", "pkey2"],
    ...
    @patch(
        "gpcheckcat.checkTableMissingEntry", return_value={("pg_clas", "oid"): "extra"}
    )
    ...
    @patch(
        "gpcheckcat.checkTableMissingEntry",
        return_value={("pg_operator", "typename, typenamespace"): "extra"},
    )
    ...
    def test_RelationObject_reportAllIssues_handles_None_fields(self):
        ...
        uut.reportAllIssues()
        log_messages = [
            args[0][1].strip() for args in self.subject.logger.log.call_args_list
        ]
    ...
    def test_checkMixDistPolicy_with_error_on_execution(self):
        ...
        self.db_connection.query.side_effect = Exception(
