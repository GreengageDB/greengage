...
class GpCheckPerf(GpTestCase):
    def setUp(self):
        gpcheckperf_file = os.path.abspath(
            os.path.dirname(__file__) + "/../../../gpcheckperf"
        )
    ...
    def test_parseCommandLine_when_get_memory_fails(self, mock_get_memory):
        ...
        self.assertEqual(
            e.exception.code,
            "[Error] could not get system memory size. Instead, you can use the -S option to provide the file size value",
        )
    ...
    def test_scp_not_enabled(self, mock_hostlist, mock_gpsync, mock_isScpEnabled):
        src = "%s/lib/multidd" % os.path.abspath(
