...
def write_config_file(
    ...
    port="",
    table="lineitem",
    file="lineitem.tbl.small",
):
    if (not user or user == "") and (
        not os.environ.get("PGUSER") or os.environ.get("PGUSER") == ""
    ):
    ...
    if platform.system() not in ["Windows", "Microsoft"]:
        ...
        if os.path.isfile(os.environ["GPHOME"] + "/docs/cli_help/gpload_help.bak"):
            os.system(
                ...
                + " "
                + os.environ["GPHOME"]
                + "/docs/cli_help/gpload_help.bak "
                + os.environ["GPHOME"]
                + "/docs/cli_help/gpload_help"
            )
    if platform.system() in ["Windows", "Microsoft"]:
        if os.path.isfile(
            "gpAdminLogs/gpload_" + datetime.date.today().strftime("%Y%m%d") + ".log"
        ):
            os.system(
                windows_path(
                    remove_command
