...
def test_64_gpload_multi_input_file():
    ...
    write_config_file(
        format="text", file=["data_file.txt", "data_file1.txt"], table="texttable"
    )
...
def test_65_gpload_multi_input_file_with_a_notexist():
    ...
    write_config_file(
        format="text",
        file=["data_file.txt", "data_file1.txt", "data_file99.txt"],
        table="texttable",
    )
...
def test_66_gpload_multi_input_file_gz_bz2():
    ...
    write_config_file(
        format="text",
        file=["data_file.txt.gz", "data_file1.txt.bz2"],
        table="texttable",
    )
...
def test_71_gpload_chinese_column_name_merge_mode():
    ...
    f.write(
        """\\! psql -d reuse_gptest -c 'select * from chinese表 order by "列#2";'"""
    )
    f.close()
...
def test_74_gpload_insert_partial_columns():
    ...
    f.write(
        """\\! psql -d reuse_gptest -c 'select count(*) from chinese表 where "列four" is NULL;'"""
    )
    f.close()
...
def test_75_gpload_merge_chinese_standard_conforming_str_on():
    ...
    write_config_file(
        ...
        delimiter="';'",
        sql=True,
        before='"set standard_conforming_strings =on;"',
    )
    write_config_file(
        mode="merge",
        update_columns=update_columns,
...
def test_76_gpload_merge_chinese_standard_conforming_str_off():
    ...
    write_config_file(
        ...
        delimiter="';'",
        sql=True,
        before='"set standard_conforming_strings =off;"',
    )
    write_config_file(
        mode="merge",
        update_columns=update_columns,
...
def test_78_gpload_merge_upper_case():
    ...
    for i in range(6):
        write_config_file(
            mode="insert",
            file="data_file.txt",
            config="config/config_file" + str(i),
            reuse_tables=True,
            fast_match=True,
