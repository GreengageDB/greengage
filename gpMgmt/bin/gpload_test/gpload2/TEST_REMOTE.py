...
def write_config_file(
    ...
    host="localhost",
    formatOpts="text",
    file=os.path.join("data", "external_file_01.txt"),
    table="texttable",
    format="text",
    delimiter="'|'",
    escape="",
...
def isFileEqual(f1, f2, optionalFlags="", outputPath="", myinitfile=""):
    ...
    if os.path.exists(os.path.join(suitePath, "init_file")):
        (ok, out) = run(
            "gpdiff.pl -w " + optionalFlags + " --gp_init_file=%s --gp_init_file=%s "
...
class GPLoad_FormatOpts_TestCase(unittest.TestCase):
    ...
    def doTest(self, num, query=""):
        ...
        commands = (
            "gpload -f "
            + mkpath(os.path.join("config", "config_file"))
            + " -d reuse_gptest"
            + os.linesep
            + "gpload -f "
            + mkpath(os.path.join("config", "config_file"))
    ...
    def test_15_gpload_reuse_table_merge_mode_with_different_columns_order(self):
        ...
        write_config_file(
            "merge",
            "true",
            file=os.path.join("data", "data_file.tbl"),
            columns_flag="1",
            mapping="1",
        )
        self.doTest(15)
