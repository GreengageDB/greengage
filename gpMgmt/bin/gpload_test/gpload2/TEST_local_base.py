...
def isFileEqual(f1, f2, optionalFlags="", outputPath="", myinitfile=""):
    ...
    if os.path.exists(suitePath + "/init_file"):
        (ok, out) = run(
            gphome
            + "/lib/postgresql/pgxs/src/test/regress/gpdiff.pl -w "
            + optionalFlags
...
def doTest(num):
    ...
    if (
        num in Modify_Output_Case
    ):  # some cases need to modify output file to avoid compareing fail with ans file
        ...
        ModifyOutFile(
            ...
            [newpat1, newpat2, newpat3, newpat4, newpat5],
        )
