...
from .TEST_local_base import copy_data
import pytest
...
def test_02_gpload_formatOpts_delimiter():
    ...
    write_config_file(
        reuse_tables=True, format="text", file="data_file.txt", delimiter="'\\t'"
    )
...
def test_30_gpload_reuse_table_update_mode_with_fast_match():
    ...
    copy_data("external_file_04.txt", "data_file.txt")
    write_config_file(
        mode="update", reuse_tables=True, fast_match=True, file="data_file.txt"
    )
...
def test_32_gpload_update_mode_without_reuse_table_with_fast_match():
    ...
    copy_data("external_file_08.txt", "data_file.txt")
    write_config_file(
        mode="update", reuse_tables=False, fast_match=True, file="data_file.txt"
    )
...
def test_35_gpload_reuse_table_merge_mode_with_fast_match_default_encoding():
    ...
    write_config_file(
        mode="merge", reuse_tables=True, fast_match=True, file="data_file.txt"
    )
...
def test_42_gpload_update_condition():
    ...
    f.close()
    write_config_file(
        mode="insert", format="text", file="data_file.txt", table="texttable"
    )
    update_columns = ["s2"]
    match_columns = ["n1"]
    write_config_file(
