...
def test_215_gpload_formatOpts_escape_default():
    ...
    runfile(file)
    write_config_file(
        reuse_tables=True, format="text", file="data_file.txt", table="texttable2"
    )
...
def test_216_gpload_formatOpts_escape_default_csv():
    ...
    runfile(file)
    write_config_file(
        reuse_tables=True, format="csv", file="data_file.csv", table="texttable2"
    )
...
def test_232_gpload_null_as_escape():
    ...
    f.write(
        "\\! psql -d reuse_gptest -c 'SELECT COUNT(*) FROM texttable2 WHERE s1 is NULL'"
    )
    f.close()
    write_config_file(
        reuse_tables=True,
...
def test_235_gpload_default_null_as_csv():
    ...
    write_config_file(
        reuse_tables=True, format="csv", file="data_file.csv", table="texttable"
    )
...
def test_242_gpload_GBK_format_text_GBK_encoding():
    ...
    write_config_file(
        reuse_tables=True, file="data_file.txt", table="texttable2", encoding="GBK"
    )
...
def test_261_gpload_custom_format():
    ...
    write_config_file(
        ...
        error_limit=2,
        sql=True,
        before="set dataflow.prefer_custom_text = false;",
        file="data_file.txt",
        table="texttable2",
    )
