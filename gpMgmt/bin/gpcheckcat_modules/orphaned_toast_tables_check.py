...
"""
    ...
    def runCheck(self, db_connection):
        orphaned_toast_tables = db_connection.query(
            self.orphaned_toast_tables_query
        ).dictresult()
        ...
        for row in orphaned_toast_tables:
            if row["expected_table_oid"] is None and row["dependent_table_oid"] is None:
                ...
                table = OrphanedTable(
                    row["dependent_table_oid"], row["dependent_table_name"]
                )
                issue_type = ReferenceOrphanToastTableIssue

            elif row["dependent_table_oid"] is None:
                table = OrphanedTable(
                    row["expected_table_oid"], row["expected_table_name"]
                )
    ...
    def add_repair_statements(self, segments):
        ...
        for issue in self._issues:
            if issue.repair_script:
                content_id_to_segment_map[issue.row["content_id"]][
                    "repair_statements"
                ].append(issue.repair_script)

        segments_with_repair_statements = filter(
            lambda segment: len(segment["repair_statements"]) > 0,
            content_id_to_segment_map.values(),
        )
