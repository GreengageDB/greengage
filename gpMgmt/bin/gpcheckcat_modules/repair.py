...
class Repair:
    ...
    def create_repair(self, sql_repair_contents):
        ...
        master_segment = next(
            segment
            for segment in self._context.cfg.values()
            if segment["content"] == -1
        )

        sql_filename = self._create_sql_file_in_repair_dir(
            repair_dir, sql_repair_contents, master_segment
        )
        self._create_bash_script_in_repair_dir(repair_dir, sql_filename, master_segment)

        return repair_dir

    def create_repair_for_extra_missing(
        self, catalog_table_obj, issues, pk_name, segments
    ):
        catalog_name = catalog_table_obj.getTableName()
        extra_missing_repair_obj = RepairMissingExtraneous(
            catalog_table_obj=catalog_table_obj, issues=issues, pk_name=pk_name
        )
        repair_dir = self.create_repair_dir()
        segment_to_oids_map = extra_missing_repair_obj.get_segment_to_oid_mapping(
            map(lambda config: config["content"], segments.values())
        )
        ...
        for segment_id, oids in segment_to_oids_map.iteritems():
            segment = next(
                segment
                for segment in self._context.cfg.values()
                if segment["content"] == segment_id
            )

            sql_content = extra_missing_repair_obj.get_delete_sql(oids)
            self._create_bash_script_in_repair_dir(
                repair_dir, sql_content, segment=segment, catalog_name=catalog_name
            )
    ...
    def create_segment_repair_scripts(self, segments):
        ...
        for segment in segments:
            sql_filename = self._create_sql_file_in_repair_dir(
                repair_dir, segment["repair_statements"], segment
            )
    ...
    def _create_bash_script_in_repair_dir(
        self, repair_dir, sql, segment, catalog_name=None
    ):
        if not sql:
            return

        bash_script_content = self._get_bash_script_content(sql, segment)
        self.append_content_to_bash_script(
            repair_dir, bash_script_content, catalog_name
        )

    def _get_bash_or_out_filename(self):
        return "%s_%s_%s" % (
            self._context.dbname,
            self._issue_type,
    ...
    def _get_bash_filepath(self, repair_dir, catalog_name=None):
        ...
        if self._issue_type and catalog_name:
            bash_filename = "run_{0}_{1}_{2}_{3}.sh".format(
                self._context.dbname,
                self._issue_type,
                catalog_name,
                self._context.timestamp,
    ...
    def _get_psql_command(self, segment, sql, out_filename):
        ...
        psql_cmd += "psql -X -v ON_ERROR_STOP=1 -a -h {hostname} -p {port} ".format(
            hostname=segment["hostname"], port=segment["port"]
        )
        ...
        psql_cmd += '"{sql}" "{dbname}" >> {out_filename} 2>&1\n'.format(
            sql=sql, dbname=self._context.dbname, out_filename=out_filename
        )
        return psql_cmd
