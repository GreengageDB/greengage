...
class DoubleOrphanToastTableIssue(OrphanToastTableIssue):
    def __init__(self, row, table):
        ...
        if row["double_orphan_parent_reltoastrelid"] is None:
            ...
            cause = cause % dict(
                toast_table_oid=row["toast_table_oid"],
            ...
            row["double_orphan_parent_oid"]
            and row["double_orphan_parent_toast_oid"] is None
        ):
...
class ReferenceOrphanToastTableIssue(OrphanToastTableIssue):
    def __init__(self, row, table):
        cause = """has an orphaned TOAST table '%s' (OID: %s).""" % (
    ...
    def repair_script(self):
        return """UPDATE "pg_class" SET reltoastrelid = %d WHERE oid = %s;""" % (
            self.row["toast_table_oid"],
            self.row["dependent_table_oid"],
        )
...
class MismatchOrphanToastTableIssue(OrphanToastTableIssue):
    def __init__(self, row, table):
        cause = (
            """has an orphaned TOAST table '%s' (OID: %s). Expected dependent table to be '%s' (OID: %s)."""
            % (
                row["toast_table_name"],
                row["toast_table_oid"],
