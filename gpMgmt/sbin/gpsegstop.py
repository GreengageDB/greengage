...
class SegStopStatus:
    ...
    def __str__(self):
        return "STATUS--DIR:%s--STOPPED:%s--REASON:%s" % (
            self.datadir,
            self.stopped,
            self.reason,
        )
...
class SegStop(base.Command):
    def __init__(self, name, db, mode, timeout):
        ...
        self.datadir = None
        self.logger = logger
        base.Command.__init__(
            self,
            name=self.name,
            cmdStr="Stop an individual segment on the host",
            ctxt=None,
            remoteHost=None,
        )
    ...
    def run(self):
        try:
            ...
            logger.debug(
                "Postgres processes running for segment %s: %s",
                self.datadir,
                postgres_pids,
            )

            cmd = gp.SegmentStop(
                "segment shutdown", self.datadir, mode=self.mode, timeout=self.timeout
            )
            cmd.run()
            ...
            logger.exception(e)
            self.result = SegStopStatus(
                self.datadir, False, "Shutdown failed! %s" % str(e)
            )
            return self.result
...
class GpSegStop:
    ...
    def run(self):
        ...
        for db in self.dblist:
            cmd = SegStop(
                "segment shutdown", db=db, mode=self.mode, timeout=self.timeout
            )
    ...
    def createProgram(options, args):
        ...
        return GpSegStop(
            ...
            segment_batch_size=options.segment_batch_size,
        )
