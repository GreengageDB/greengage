...
def parseargs():
    parser = OptParser(
        option_class=OptChecker, description=" ".join(description.split())
    )
...
def read_from_hba_file_and_get_empty_tempfile(hba_filename):
    ...
    (tempFD, temp_hba_filename) = tempfile.mkstemp(
        prefix="pg_hba",
        suffix=".tmp",
        dir=os.path.abspath(os.path.dirname(hba_filename)),
    )

    os.chmod(
        os.path.abspath(temp_hba_filename),
        os.stat(os.path.abspath(hba_filename)).st_mode,
    )
