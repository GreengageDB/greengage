...
def before_all(context):
    if map(int, behave.__version__.split(".")) < [1, 2, 6]:
        raise Exception(
            "Requires at least behave version 1.2.6 (found %s)" % behave.__version__
        )
...
def before_feature(context, feature):
    ...
    if "analyzedb" in feature.tags:
        ...
        context.conn = dbconn.connect(
            dbconn.DbURL(dbname="incr_analyze"), unsetSearchPath=False
        )
    ...
    if "gpreload" in feature.tags:
        ...
        context.conn = dbconn.connect(
            dbconn.DbURL(dbname="gpreload_db"), unsetSearchPath=False
        )
    ...
    if "minirepro" in feature.tags:
        ...
        context.conn = dbconn.connect(
            dbconn.DbURL(dbname=minirepro_db), unsetSearchPath=False
        )
        ...
        dbconn.execSQL(
            context.conn, "create view v1 as select a, b from t1, t3 where t1.a=t3.e"
        )
        dbconn.execSQL(
            context.conn, "create view v2 as select c, d from t2, t3 where t2.c=t3.f"
        )
        dbconn.execSQL(
            context.conn, "create view v3 as select a, d from v1, v2 where v1.a=v2.c"
        )
        ...
        dbconn.execSQL(
            context.conn,
            "create or replace function select_one() returns integer as $$ select 1 $$ language sql",
        )
        context.conn.commit()
...
def before_scenario(context, scenario):
    ...
    if "concourse_cluster" in scenario.effective_tags and not hasattr(
        context, "concourse_cluster_created"
    ):
    ...
    if "analyzedb" not in context.feature.tags:
        ...
        drop_database_if_exists(context, "testdb")
    if (
        "gp_bash_functions.sh" in context.feature.tags
        or "backup_restore_bashrc" in scenario.effective_tags
    ):
        backup_bashrc()
...
def after_scenario(context, scenario):
    ...
    if (
        set(context.feature.tags).intersection(tags_to_cleanup)
        and "skip_cleanup" not in scenario.effective_tags
    ):
    ...
    if not set(context.feature.tags).intersection(tags_to_not_restart_db):
        ...
        if (
            not check_user_permissions(home_dir, "write")
            and hasattr(context, "orig_write_permission")
    ...
    if (
        hasattr(context, "path_for_which_to_restore_the_permissions")
        and hasattr(context, "permissions_to_restore_path_to")
        and hasattr(context, "host_to_restore_path_to")
    ):
        cmd_str = "sudo chmod {0} {1}".format(
        ...
        cmd = Command(
            ...
            cmdStr=cmd_str,
            ctxt=REMOTE,
            remoteHost=context.host_to_restore_path_to,
        )
        cmd.run(validateAfter=True)
    elif hasattr(context, "path_for_which_to_restore_the_permissions"):
        raise Exception(
            "Missing permissions_to_restore_path_to for %s"
            % context.path_for_which_to_restore_the_permissions
        )
    ...
    if "remove_rsync_bash" in scenario.effective_tags:
        for host in context.hosts_with_rsync_bash:
            cmd = Command(
                name="remove /usr/local/bin/rsync",
                cmdStr="sudo rm /usr/local/bin/rsync",
                remoteHost=host,
                ctxt=REMOTE,
            )
            cmd.run(validateAfter=True)
