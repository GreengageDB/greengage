...
def _generate_input_config(spread=False):
    ...
    cmd_str = "gpaddmirrors -a -o %s -m %s" % (
        mirror_config_output_file,
        datadir_config,
    )
    if spread:
        cmd_str += " -s"
...
def add_three_mirrors_with_one_mirror_not_empty(context):
    ...
    datadir_cmd = "cat {} | tail -1 | awk '-F|' '{{print $4}}'".format(
        mirror_config_output_file
    )
    datadir = (
        subprocess.check_output(["bash", "-c", datadir_cmd]).decode("utf-8").strip()
    )
    ...
    with tempfile.NamedTemporaryFile(dir=datadir):
        cmd = Command(
            "gpaddmirrors ", "gpaddmirrors -a -i %s" % (mirror_config_output_file)
        )
...
def add_mirrors(context, options):
    ...
    cmd = Command(
        "gpaddmirrors ", "gpaddmirrors -a -i %s %s" % (context.mirror_config, options)
    )
...
# for the item
@given(
    'pg_hba file "{filename}" on host "{host}" contains entries for "{search_items}"'
)
    ...
    for search_item in search_item_list:
        ...
        for entry in pghba_contents:
            ...
            if contents.startswith("host") and contents.endswith("trust"):
                ...
                if (
                    (search_item == hostname)
                    or (search_hostname == hostname)
                    or (search_ip_addr[0] in hostname)
                ):
                    found = True
                    break
        if not found:
            raise Exception(
                "entry for expected item %s, ip_addr[0] %s not existing in pg_hba.conf '%s'"
...
def impl(context, expected_mirror_count):
    expected_mirror_count = (
        0 if expected_mirror_count == "no" else int(expected_mirror_count)
    )
    actual_mirror_count = _get_mirror_count()
    if actual_mirror_count != expected_mirror_count:
        raise Exception(
            "Expected {} mirrors but found {}".format(
                expected_mirror_count, actual_mirror_count
            )
        )
...
def impl(context):
    ...
    try:
        cmd = Command(
            "gpaddmirrors ",
            "gpaddmirrors -a -i %s -d %s" % (context.mirror_config, mdd),
        )
        cmd.run(validateAfter=True)
    finally:
...
def impl(context):
    ...
    # and the current one.
    for array, hostMap in [
        (context.gparray, old_content_to_host),
        (gparray, curr_content_to_host),
    ]:
    ...
    for key in old_content_to_host.keys():
        if curr_content_to_host[key] != old_content_to_host[key]:
            raise Exception(
                "Mirror host doesn't match for content %s (old host=%s) (new host=%s)"
...
def impl(context):
    ...
    context.expected_segs.append(
        "sdw1-1|21500|/tmp/gpmovemirrors/data/mirror/gpseg2_moved"
    )
    ...
    with open(input_filename, "w") as fd:
        fd.write(
            "sdw1-1|21500|/tmp/gpmovemirrors/data/mirror/gpseg2 %s\n"
            % context.expected_segs[0]
        )
        fd.write(
            "sdw1-1|21501|/tmp/gpmovemirrors/data/mirror/gpseg3 %s"
            % context.expected_segs[1]
        )
...
def impl(context):
    ...
    if len(context.expected_segs) != len(actual_segs):
        raise Exception(
            "expected number of segs to be %d, but got %d"
            % (len(context.expected_segs), len(actual_segs))
        )
    if context.expected_segs != actual_segs:
        raise Exception(
            "expected segs to be %s, but got %s" % (context.expected_segs, actual_segs)
        )
...
def impl(context, mirror_config):
    if mirror_config not in ["group", "spread"]:
        raise Exception(
            '"%s" is not a valid mirror configuration for this step; options are "group" and "spread".'
        )
    ...
    for segmentPair in gparray.segmentPairs:
        ...
        if primary_host == mirror_host:
            raise Exception(
                "Host %s has both primary and mirror for content %d"
                % (primary_host, pair_content)
            )
    ...
    if mirror_config == "spread":
        ...
        for primary_host in primary_to_mirror_host_map:
            ...
            if other_host_set != mirror_host_set:
                raise Exception(
                    "Expected primaries on %s to be mirrored to %s, but they are mirrored to %s"
        ...
        for primary_host in primary_to_mirror_host_map:
            ...
            if num_mirror_hosts != 1:
                raise Exception(
                    "Expected primaries on %s to all be mirrored to the same host, but they are mirrored to %d different hosts"
...
@given(
    "edit the input file to recover mirror with content {content_ids} to a new directory with mode {mode}"
)
@given(
    "edit the input file to move mirror with content {content_ids} to a new directory with mode {mode}"
)
    ...
    for content in [int(c) for c in content_ids.split(",")]:
        ...
        for seg in segments:
            if seg.mirrorDB.getSegmentContentId() == content:
                ...
                context.old_data_dirs[content] = [
                    mirror.getSegmentHostName(),
                    mirror.getSegmentDataDirectory(),
                ]

                valid_config_new = "{}|{}|{}".format(
                    mirror.getSegmentHostName(), mirror.getSegmentPort(), target_datadir
                )
                with open(context.mirror_context.input_file_path(), "a") as fd:
                    fd.write("{} {}\n".format(valid_config, valid_config_new))
                context.data_dirs_created[seg.mirrorDB.getSegmentHostName()].append(
                    target_datadir
                )
...
@given(
    "edit the input file to add mirror with content {content_ids} to a new {directory} with mode {mode}"
)
    ...
    for content in [int(c) for c in content_ids.split(",")]:
        ...
        for seg in segments:
            if seg.primaryDB.getSegmentContentId() == content:
                ...
                valid_config_new = "{}|{}|{}|{}".format(
                    content, seg.primaryDB.getSegmentHostName(), new_port, new_datadir
                )
                with open(context.mirror_context.input_file_path(), "a") as fd:
                    fd.write("{}\n".format(valid_config_new))
                context.data_dirs_created[seg.primaryDB.getSegmentHostName()].append(
                    new_datadir
                )
                new_location = "{}|{}|{}".format(
                    seg.primaryDB.getSegmentHostName(), new_port, new_datadir
                )
...
@given(
    "edit the input file to add mirror on host {host} with contents {content_ids} to a new directory with mode {mode}"
)
    ...
    for content in [int(c) for c in content_ids.split(",")]:
        ...
        for seg in segments:
            if seg.primaryDB.getSegmentContentId() == content:
                new_port = seg.primaryDB.getSegmentPort() + 100 * (content + 1)
                valid_config_new = "{}|{}|{}|{}".format(
                    seg.primaryDB.getSegmentContentId(), host, new_port, new_datadir
                )
                with open(context.mirror_context.input_file_path(), "a") as fd:
                    fd.write("{}\n".format(valid_config_new))
...
@given(
    "edit the input file to recover mirror with content {content} to a new directory on remote host with mode {mode}"
)
def impl(context, content, mode):
    ...
    for seg in segments:
        if seg.mirrorDB.getSegmentContentId() == content:
            ...
            new_datadir = context.temp_base_dir_remote
            valid_config_new = "{}|{}|{}".format(
                mirror.getSegmentHostName(), mirror.getSegmentPort(), new_datadir
            )
            with open(context.mirror_context.input_file_path(), "a") as fd:
                fd.write("{} {}\n".format(valid_config, valid_config_new))
            context.data_dirs_created[seg.mirrorDB.getSegmentHostName()].append(
...
def impl(context, content_ids):
    for content in [int(c) for c in content_ids.split(",")]:
        ...
        rm_cmd = Command(
            name="Remove old datadirs on remote host",
            cmdStr="rm -rf {}".format(old_datadir),
            remoteHost=old_host,
            ctxt=REMOTE,
        )
...
def make_temp_dir_on_remote(context, hostname, tmp_base_dir_remote, mode="700"):
    ...
    tempfile_cmd = Command(
        name="Create temp directory on remote host",
        cmdStr=""" python -c "import tempfile; t=tempfile.mkdtemp(dir='{}');print(t)" """.format(
            tmp_base_dir_remote
        ),
        remoteHost=hostname,
        ctxt=REMOTE,
...
@given(
    'datadirs from "{config}" configuration for "{old_host}" are created on "{new_host}" with mode {mode}'
)
@then(
    'datadirs from "{config}" configuration for "{old_host}" are created on "{new_host}" with mode {mode}'
)
def impl(context, config, old_host, new_host, mode):
    ...
    for seg in gparray_before.getDbList():
        if seg.hostname == old_host:
            mkdir_cmd = Command(
                name="create directory remote",
                cmdStr="mkdir -p {}; chmod {} {}".format(
                    seg.datadir, mode, seg.datadir
                ),
                remoteHost=new_host,
                ctxt=REMOTE,
            )
...
@given(
    "edit the hostsname input file to recover segment with content {content} full inplace"
)
def impl(context, content):
    content = int(content)
    segments = GpArray.initFromCatalog(dbconn.DbURL()).getSegmentList()
...
@given(
    "edit the hostsname input file to recover segment with content {content} with invalid hostname"
)
def impl(context, content):
    content = int(content)
    segments = GpArray.initFromCatalog(dbconn.DbURL()).getSegmentList()
    for seg in segments:
        if seg.mirrorDB.getSegmentContentId() == content:
            mirror = seg.mirrorDB
            valid_config = "{}|{}|{}|{} invalid_host|{}|{}|{}".format(
                mirror.getSegmentHostName(),
                mirror.getSegmentAddress(),
                mirror.getSegmentPort(),
                mirror.getSegmentDataDirectory(),
...
@given(
    "edit the input file to recover with content id {content_id} to host {recovery_host}"
)
...
@given(
    "a temporary directory with mode '{mode}' is created under data_dir of primary with content {content}"
)
@when(
    "a temporary directory with mode '{mode}' is created under data_dir of primary with content {content}"
)
...
def impl(context, mode, content):
    ...
    primary_segment = list(
        filter(
            lambda seg: seg.getSegmentRole() == ROLE_PRIMARY
            and seg.getSegmentContentId() == int(content),
            all_segments,
        )
    )[0]
    mirror_segment = list(
...
@then(
    "check if mirrors on content {content_ids} are moved to new location on input file"
)
def impl(context, content_ids):
    if content_ids == "None":
        return
    ...
    for seg in segments:
        ...
        if not (expected_config == actual_config):
            raise Exception(
                "Expected mirror location '{}' and found: '{}'".format(
...
def impl(context, content_ids):
    ...
    segments_to_test = filter(
        lambda seg: seg.getSegmentContentId()
        in [int(c) for c in content_ids.split(",")],
        all_segments_actual,
    )

    for seg in segments_to_test:
        original_seg = context.original_seg_info[
            "{}_{}".format(seg.getSegmentContentId(), seg.getSegmentPreferredRole())
        ]
        original_config = "{}|{}|{}".format(
            original_seg.getSegmentHostName(),
            original_seg.getSegmentPort(),
            original_seg.getSegmentDataDirectory(),
        )
        actual_config = "{}|{}|{}".format(
            seg.getSegmentHostName(),
            seg.getSegmentPort(),
...
def run_gpmovemirrors(context, extra_args=""):
    cmd = "gpmovemirrors --input=%s %s" % (
        context.mirror_context.input_file_path(),
        extra_args,
    )
...
def impl(context, utility, extra_args=""):
    cmd = "{} -i {} {}".format(
        utility, context.mirror_context.input_file_path(), extra_args
    )
    run_gpcommand(context, cmd)


@when(
    'the user asynchronously runs {utility} with input file and additional args "{extra_args}" and the process is saved'
)
@given(
    'the user asynchronously runs {utility} with input file and additional args "{extra_args}" and the process is saved'
)
...
@given(
    'a sample gpmovemirrors input file is created in "{mirror_config}" configuration'
)
def impl(context, mirror_config):
    if mirror_config not in ["group", "spread"]:
        raise Exception(
    ...
    with open(input_filename, "w") as fd:
        for content in [1, 2, 4, 5]:
            ...
            mirrors = map(
                lambda segmentPair: segmentPair.mirrorDB, gparray.getSegmentList()
            )
            mirror = next(
                iter(
                    filter(
                        lambda mirror: mirror.getSegmentContentId() == content, mirrors
                    )
                ),
                None,
            )
...
@then(
    'verify the tablespace directories on host "{host}" for content "{content}" are {status}'
)
def impl(context, host, content, status):
    ...
    with dbconn.connect(dbconn.DbURL(dbname="postgres"), unsetSearchPath=False) as conn:
        ...
        oids = [row[0] for row in dbconn.execSQL(conn, oids_query).fetchall()]
        dbid = dbconn.execSQLForSingleton(
            conn,
            "SELECT dbid FROM gp_segment_configuration WHERE content = %s AND preferred_role = 'm'"
            % content,
        )
        for oid in oids:
            locations.append(
                dbconn.execSQLForSingleton(conn, location_query % (oid, content))
            )

    for location in locations:
        cmd = Command(
            name="check tablespace dirs",
            cmdStr="find %s -name %s -type d -print 2> /dev/null ||true"
            % (location, dbid),
            ctxt=REMOTE,
            remoteHost=host,
        )
    ...
    if status == "deleted":
        if existing_dirs:
            raise Exception(
                "One or more directories have not been deleted:\n%s" % existing_dirs
            )
    else:
        if existing_dirs != locations:
            missing_dirs = [d for d in locations if d not in existing_dirs]
            raise Exception(
                "One or more directories are not present on %s: %s"
                % (host, missing_dirs)
            )


@then(
    "the user executes steps required for rerunning gpmovemirrors for contents {contents}"
)
...
@given(
    "the user executes steps required for running in place full recovery for all failed contents"
)
@then(
    "the user executes steps required for running in place full recovery for all failed contents"
)
