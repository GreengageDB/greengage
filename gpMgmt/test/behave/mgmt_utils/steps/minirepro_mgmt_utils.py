...
@then(
    'the output file "{output_file}" should contain "{str_before}" before "{str_after}"'
)
...
@then(
    'the output file "{output_file}" should be loaded to database "{db_name}" without error'
)
def impl(context, output_file, db_name):
    ...
    if "ERROR:" in context.error_message:
        raise Exception(
            "Database %s failed to run %s, error message: %s"
            % (db_name, output_file, context.error_message)
        )


@then(
    'the file "{query_file}" should be executed in database "{db_name}" without error'
)
