...
def change_hostname(content, preferred_role, hostname):
    with dbconn.connect(
        ...
        dbconn.execSQL(
            conn,
            "UPDATE gp_segment_configuration SET hostname = '{0}', address = '{0}' WHERE content = {1} AND preferred_role = '{2}'".format(
                hostname, content, preferred_role
            ),
        )
        conn.commit()
...
def impl(context, seg_type, content):
    if seg_type == "primary":
        ...
        raise Exception(
            "Invalid segment type %s (options are primary and mirror)" % seg_type
        )
...
def impl(context):
    ...
    for dbid in sorted(context.down_segment_dbids):
        context.execute_steps(
            'Then gpstart should print "Marking segment %s down because invalid_host is unreachable" to stdout'
            % dbid
        )
...
def must_have_expected_status(content, preferred_role, expected_status):
    ...
    if status != expected_status:
        raise Exception(
            "Expected status for role %s to be %s, but it is %s"
...
def get_guc_value(guc):
    with dbconn.connect(
        ...
        value = dbconn.execSQLForSingleton(conn, "show %s" % guc)
    return value
...
@given(
    'the status of the {seg_type} on content {content} should be "{expected_status}"'
)
