...
def ensure_temp_directory_is_empty(context, temp_directory):
    run_command(
        context, "rm -rf /tmp/{temp_directory}".format(temp_directory=temp_directory)
    )
...
def step_impl(context):
    ...
    cmd = (
        "ps aux | grep '[p]ostgres .* %s' | awk '{print \$2}' | xargs kill -9" % datadir
    )
...
def step_impl(context):
    ...
    if result_cursor.rowcount != context.current_cluster_size:
        raise Exception(
            "expected all %d primaries to have replication slots, only %d have slots"
            % (context.current_cluster_size, results.rowcount)
        )
...
def step_impl(context):
    result_cursor = execute_sql(
        "postgres", "select datadir from gp_segment_configuration where role='m';"
    )
