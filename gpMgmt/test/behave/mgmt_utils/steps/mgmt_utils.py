...
def show_all_installed(gphome):
    ...
    if "ubuntu" in name:
        return (
            "dpkg --get-selections --admindir=%s/share/packages/database/deb | awk '{print \$1}'"
            % gphome
        )
    elif (
        "centos" in name
        or "red hat enterprise linux" in name
        or "oracle linux server" in name
        or "rocky linux"
        or "ol" in name
    ):
...
def remove_native_package_command(gphome, full_gppkg_name):
    ...
    if "ubuntu" in name:
        return (
            "fakeroot dpkg --force-not-root --log=/dev/null --instdir=%s --admindir=%s/share/packages/database/deb -r %s"
            % (gphome, gphome, full_gppkg_name)
        )
    elif (
        "centos" in name
        or "red hat enterprise linux" in name
        ...
        return "rpm -e %s --dbpath %s/share/packages/database" % (
            full_gppkg_name,
            gphome,
        )
    else:
...
def impl(context):
    if (
        check_database_is_running(context)
...
def impl(context, hba_hostnames_toggle):
    extra_config = (
        'env EXTRA_CONFIG="HBA_HOSTNAMES={}" ONLY_PREPARE_CLUSTER_ENV=true'.format(
            hba_hostnames_toggle
        )
    )
...
def impl(context, checksum_toggle):
    extra_config = (
        'env EXTRA_CONFIG="HEAP_CHECKSUM={}" ONLY_PREPARE_CLUSTER_ENV=true'.format(
            checksum_toggle
        )
    )
    create_local_demo_cluster(context, extra_config=extra_config)
...
def impl(context, num_primaries):
    ...
    create_local_demo_cluster(
        context, with_mirrors="false", with_standby="false", num_primaries=num_primaries
    )
...
@given(
    'the user runs sql "{query}" in "{db}" on primary segment with content {contentids}'
)
@when(
    'the user runs sql "{query}" in "{db}" on primary segment with content {contentids}'
)
...
def impl(context, query, db, contentids):
    ...
    for content in content_ids:
        ...
        psql_cmd = (
            "PGDATABASE='%s' PGOPTIONS='-c gp_session_role=utility' psql -h %s -p %s -c \"%s\"; "
            % (db, host, port, query)
        )
        Command(name="Running Remote command: %s" % psql_cmd, cmdStr=psql_cmd).run(
            validateAfter=True
        )
...
def impl(context, dbname, cname):
    ...
    context.named_conns[cname] = dbconn.connect(
...
def impl(conetxt, tabname):
    ...
    with dbconn.connect(dbconn.DbURL(dbname=dbname), unsetSearchPath=False) as conn:
        sql = (
            "create writable external table {tabname}(a int) location "
            "('gpfdist://host.invalid:8000/file') format 'text'"
...
def impl(conetxt, tabname):
    ...
    with dbconn.connect(dbconn.DbURL(dbname=dbname), unsetSearchPath=False) as conn:
        sql = (
            "create external table {tabname}(i int, j int) location "
            "('gpfdist://host.invalid:8000/file') format 'text'"
        ).format(tabname=tabname)
        dbconn.execSQL(conn, sql)
        ...
        dbconn.execSQL(conn, sql)
        sql = (
            "alter table t exchange partition for (2018) with table {tabname} without validation"
        ).format(tabname=tabname)
        dbconn.execSQL(conn, sql)
        conn.commit()
...
def impl(conetxt, tabname):
    dbname = "gptest"
    with dbconn.connect(dbconn.DbURL(dbname=dbname), unsetSearchPath=False) as conn:
        sql = "create table {tabname}(i int) distributed by (i) partition by range(i) (start(0) end(10001) every(1000))".format(
            tabname=tabname
        )
        dbconn.execSQL(conn, sql)
        sql = "INSERT INTO {tabname} SELECT generate_series(1, 10000)".format(
            tabname=tabname
        )
        dbconn.execSQL(conn, sql)
        conn.commit()
...
def impl(context, tabname, schemaname):
    dbname = "gptest"
    with dbconn.connect(dbconn.DbURL(dbname=dbname), unsetSearchPath=False) as conn:
        sql = "ALTER TABLE {tabname} SET SCHEMA {schemaname}".format(
            tabname=tabname, schemaname=schemaname
        )
        dbconn.execSQL(conn, sql)
        conn.commit()
...
def impl(context, checksum_toggle):
    ...
    if is_ok:
        ...
        if context.ret_code != 0:
            raise Exception(
                "cannot run gpconfig: %s, stdout: %s"
                % (context.error_message, context.stdout_message)
            )
...
@given(
    'database "{dbname}" is created if not exists on host "{HOST}" with port "{PORT}" with user "{USER}"'
)
@then(
    'database "{dbname}" is created if not exists on host "{HOST}" with port "{PORT}" with user "{USER}"'
)
...
@when(
    'the database "{dbname}" does not exist on host "{HOST}" with port "{PORT}" with user "{USER}"'
)
@given(
    'the database "{dbname}" does not exist on host "{HOST}" with port "{PORT}" with user "{USER}"'
)
...
@given(
    "all files in pg_xlog directory are deleted from data directory of preferred primary of content {content_ids}"
)
def impl(context, content_ids):
    all_segments = GpArray.initFromCatalog(dbconn.DbURL()).getDbList()
    segments = filter(
        lambda seg: seg.getSegmentPreferredRole() == ROLE_PRIMARY
        and seg.getSegmentContentId() in [int(c) for c in content_ids.split(",")],
        all_segments,
    )
    for seg in segments:
        cmd = Command(
            name="Remove pg_xlog files",
            cmdStr="rm -rf {}".format(
                os.path.join(seg.getSegmentDataDirectory(), "pg_xlog")
            ),
...
def impl(context, action, segment, content):
    ...
    inject_fault_query = (
        "SELECT gp_inject_fault_infinite('wal_sender_loop', '%s', dbid) FROM gp_segment_configuration WHERE content=%s AND role=%s;"
        % (action, content, role)
    )
...
def impl(context, segment, content):
    ...
    inject_fault_query = (
        "SELECT gp_inject_fault_infinite('walrecv_skip_flush', 'skip', dbid) FROM gp_segment_configuration WHERE content=%s AND role=%s;"
        % (content, role)
    )
    execute_sql("postgres", inject_fault_query)
    return
...
def impl(context, content):
    host, port = get_primary_segment_host_port_for_content(content)
    query = (
        "SELECT pg_current_xlog_location() - sent_location FROM pg_stat_replication;"
    )
    desired_result = 0
...
@given(
    "the user waits until recovery_progress.file is created in {logdir} and verifies its format"
)
@when(
    "the user waits until recovery_progress.file is created in {logdir} and verifies its format"
)
        ...
        if os.path.exists(recovery_progress_file):
            ...
            for line in context.recovery_lines:
                ...
                progress_pattern = re.compile(
                    get_recovery_progress_pattern(recovery_type)
                )
                # TODO: assert progress line in the actual hosts bb/rewind progress file
                if (
                    re.search(progress_pattern, progress)
                    and dbid.isdigit()
                    and recovery_type in ["full", "differential", "incremental"]
                ):
                    return
                else:
                    raise Exception(
                        'File present but incorrect format line "{}"'.format(line)
                    )
        time.sleep(0.1)
...
@then(
    "verify that lines from recovery_progress.file are present in segment progress files in {logdir}"
)
def impl(context, logdir):
    all_progress_lines_by_dbid = {}
    for line in context.recovery_lines:
        recovery_type, dbid, line_from_combined_progress_file = line.strip().split(
            ":", 2
        )
        all_progress_lines_by_dbid[int(dbid)] = [
            recovery_type,
            line_from_combined_progress_file,
        ]
    ...
    for seg in all_segments:
        ...
        if seg_dbid in all_progress_lines_by_dbid:
            recovery_type, line_from_combined_progress_file = (
                all_progress_lines_by_dbid[seg_dbid]
            )
            ...
            seg_progress_file = "{}/{}.*.dbid{}.out".format(
                log_dir, process_name, seg_dbid
            )
            check_cmd_str = 'grep "{}" {}'.format(
                line_from_combined_progress_file, seg_progress_file
            )
            check_cmd = Command(
                name="check line in segment progress file",
            ...
            if check_cmd.get_return_code() != 0:
                raise Exception(
                    "Expected line {} in segment progress file {} on host {} but not found.".format(
                        line_from_combined_progress_file,
                        seg_progress_file,
...
def impl(context, logdir):
    ...
    if os.path.exists("{}/recovery_progress.file".format(log_dir)):
        raise Exception(
            "recovery_progress.file is still present under {}".format(log_dir)
        )
...
def backup_bashrc():
    ...
    if os.path.isfile(file):
        ...
        if result[0] != 0:
            raise Exception(
                "Error while backing up bashrc file. STDERR:%s" % (result[2])
            )
    return
...
def impl(context):
    file = "~/.bashrc"
    command = (
        "echo 'echo -e \"banner test1\\nbanner test2\\nbanner test-3\\nbanner test4\"' >> %s; source %s"
        % (file, file)
    )
    result = run_cmd(command)
    if result[0] != 0:
...
def impl(context):
    ...
    command = 'source %s; REMOTE_EXECUTE_AND_GET_OUTPUT localhost "echo %s"' % (
        gp_bash_functions,
        message,
    )
    result = run_cmd(command)
    if result[0] != 0:
        raise Exception(
            "Expected error code is 0. Command returned error code:%s.\nStderr:%s\n"
            % (result[0], result[2])
        )
    if result[1].strip() != message:
        raise Exception(
            "Expected output is: [%s] while received output is: [%s] Return code:%s"
...
def impl(context):
    ...
    message = "Hello World. This is a simple command output"
    command = (
        'source %s; REMOTE_EXECUTE_AND_GET_OUTPUT localhost "echo %s; hostname | wc -w | xargs"'
        % (gp_bash_functions, message)
    )
    result = run_cmd(command)
    if result[0] != 0:
        raise Exception(
...
def impl(context):
    ...
    token = "GP_DELIMITER_FOR_IGNORING_BASH_BANNER"
    command = (
        'source %s; REMOTE_EXECUTE_AND_GET_OUTPUT localhost "echo %s; echo %s; echo %s"'
        % (gp_bash_functions, message, token, message)
    )
    result = run_cmd(command)
    if result[0] != 0:
        raise Exception(
...
@given(
    "the user asynchronously sets up to end {process_name} process in {secs} seconds"
)
...
def impl(context, process_name, secs):
    if process_name == "that":
        ...
        command = "sleep %d; ps ux | grep %s | awk '{print $2}' | xargs kill" % (
            int(secs),
            process_name,
        )
...
@when(
    "the user asynchronously sets up to end {process_name} process when {log_msg} is printed in the logs"
)
...
@then(
    "the user asynchronously sets up to end {kill_process_name} process when {log_msg} is printed in the {logfile_name} logs"
)
...
def impl(context, ret_code):
    ...
    if rc != int(ret_code):
        raise Exception(
            "return code of the async proccess didn't match:\n"
            "rc: %s\n"
            "stdout: %s\n"
...
def impl(context, process_name):
    wait_process_command = (
        "while ps ux | grep %s | grep -v grep; do sleep 0.1; done;" % process_name
    )
...
@given(
    'the user modifies the external_table.sql file "{filepath}" with host "{HOST}" and port "{port}"'
)
@when(
    'the user modifies the external_table.sql file "{filepath}" with host "{HOST}" and port "{port}"'
)
...
@given(
    'the user starts the gpfdist on host "{HOST}" and port "{port}" in work directory "{dir}" from remote "{ctxt}"'
)
@then(
    'the user starts the gpfdist on host "{HOST}" and port "{port}" in work directory "{dir}" from remote "{ctxt}"'
)
def impl(context, HOST, port, dir, ctxt):
    ...
    gpfdist = Gpfdist(
        "gpfdist on host %s" % host,
        dir,
        port,
        os.path.join(dir, "gpfdist.pid"),
        int(ctxt),
        host,
        gp_source_file,
    )
...
@given(
    'the user stops the gpfdist on host "{HOST}" and port "{port}" in work directory "{dir}" from remote "{ctxt}"'
)
@then(
    'the user stops the gpfdist on host "{HOST}" and port "{port}" in work directory "{dir}" from remote "{ctxt}"'
)
...
def impl(context, command, out_msg, num):
    ...
    if match_count != int(num):
        raise Exception(
            "Expected %s to occur %s times. Found %d. stdout: %s"
            % (out_msg, num, count, msg_list)
        )
...
def impl(context, msg):
    ...
    for seg in segments:
        if seg.isSegmentPrimary():
            ...
            sql = (
                "select * from gp_toolkit.__gp_log_segment_ext where logsegment='%s' and logtime > (select * from log_timestamp) and logmessage like '%s'"
                % (segname, msg)
            )
            try:
                ...
                if cursor.fetchone():
                    raise Exception(
                        "Fatal message exists in pg_log file on primary segment %s"
                        % segname
                    )
            finally:
                pass
...
def impl(context, command, called_command, num, args):
    ...
    if len(matches) != int(num):
        raise Exception(
            "Expected %s to occur with %s args %s times. Found %d. \n %s"
...
def impl(context, command, num):
    ...
    for matched_line in matches:
        ...
        if init_workers > int(num):
            raise Exception(
                "Expected Workerpool for %s to be initialized with %d workers. Found %d. \n %s"
...
def impl(context, table_type, tablename, dbname):
    if not check_table_exists(
        context, dbname=dbname, table_name=tablename, table_type=table_type
    ):
        raise Exception(
            "Table '%s' of type '%s' does not exist when expected"
            % (tablename, table_type)
        )


@then(
    'verify that there is a "{table_type}" table "{tablename}" in "{dbname}" with "{numrows}" rows'
)
def impl(context, table_type, tablename, dbname, numrows):
    ...
    try:
        rowcount = dbconn.execSQLForSingleton(
            conn, "SELECT count(*) FROM %s" % tablename
        )
        if rowcount != int(numrows):
            raise Exception(
                "Expected to find %d rows in table %s, found %d"
                % (int(numrows), tablename, rowcount)
            )
    finally:
        conn.close()
...
def impl(context, dbname):
    with dbconn.connect(
        dbconn.DbURL(dbname="template1"), unsetSearchPath=False
    ) as conn:
...
def impl(context, filepath):
    ...
    try:
        ...
        raise Exception(
            "file '%s' should not exist in standby master data directory" % fullfilepath
        )
...
@then(
    'validate that first column of first stored row has "{numlines}" lines of raw output'
)
def impl(context, numlines):
    ...
    if raw_lines_count != numlines:
        raise Exception(
            "Found %d of stored query result but expected %d records"
            % (raw_lines_count, numlines)
        )
...
def get_standby_host():
    ...
    segments = gparray.getDbList()
    standby_master = [
        seg.getSegmentHostName() for seg in segments if seg.isSegmentStandby()
    ]
...
def run_gpinitstandby(
    context, hostname, port, standby_data_dir, options="", remote=False
):
    if "-n" in options:
        cmd = "gpinitstandby -a"
    elif remote:
...
@when(
    "the user initializes a standby on the same host as master and the same data directory"
)
def impl(context):
    hostname = get_master_hostname("postgres")[0][0]
...
@when(
    'running gpinitstandby on host "{master}" to create a standby on host "{standby}"'
)
@given(
    'running gpinitstandby on host "{master}" to create a standby on host "{standby}"'
)
...
def impl(context, options):
    dbname = "postgres"
    with dbconn.connect(
        dbconn.DbURL(port=os.environ.get("PGPORT"), dbname=dbname),
...
def impl(context, options):
    context.execute_steps(
        """Then the user runs command "gpactivatestandby -a %s" from standby master"""
        % options
    )
...
def impl(context, contain):
    string_to_find = (
        "Run command bash .*backout_gpinitsystem.* on master to remove these changes$"
    )
    ...
    if contain == "contain":
        if has_exception(context):
            raise context.exception
        context.gpinit_backout_command = re.search(
            "Run command(.*)on master", context.stdout_message
        ).group(1)
        ...
        raise Exception(
            "Incorrect step name, only use 'should contain' and 'should not contain'"
        )
...
def impl(context):
    ...
    if context.master_hostname != context.standby_hostname:
        ...
        cmd = "gpinitstandby -a -s %s -P %s -S %s" % (
            context.master_hostname,
            context.master_port,
            master_data_dir,
        )
    ...
    context.execute_steps(
        """Then the user runs command "%s" from standby master""" % cmd
    )
...
def impl(context, path, perm, host):
    ...
    test_permissions = int(
        perm, 8
    )  # accept string input with octal semantics and convert to a raw number
...
def impl(context):
    print(
        "go look in environment.py to see how it uses the path and permissions on context to make sure it's cleaned up"
    )
...
def stop_all_primary_or_mirror_segments(context, segment_type):
    if segment_type not in ("primary", "mirror"):
        raise Exception(
            "Expected segment_type to be 'primary' or 'mirror', but found '%s'."
            % segment_type
        )

    role = ROLE_PRIMARY if segment_type == "primary" else ROLE_MIRROR
    stop_segments(
        context, lambda seg: seg.getSegmentRole() == role and seg.content != -1
    )
...
def stop_all_primary_or_mirror_segments_on_hosts(context, segment_type, hosts):
    ...
    stop_segments(
        context,
        lambda seg: seg.getSegmentRole() == role
        and seg.content != -1
        and seg.getSegmentHostName() in hosts,
    )
...
def stop_segments_on_contentID(context, role, contentID):
    if role not in ("primary", "mirror"):
        raise Exception(
            "Expected segment_type to be 'primary' or 'mirror', but found '%s'." % role
        )

    role = ROLE_PRIMARY if role == "primary" else ROLE_MIRROR
    stop_segments(
        context,
        lambda seg: seg.getSegmentRole() == role and seg.content == int(contentID),
    )
...
def stop_segments_on_contentID(context, role, contents):
    ...
    role = ROLE_PRIMARY if role == "primary" else ROLE_MIRROR
    stop_segments_immediate(
        context, lambda seg: seg.getSegmentRole() == role and seg.content in content_ids
    )
...
def impl(context, content_ids, expected_status):
    ...
    for content in content_ids.split(","):
        if expected_status == "up" and not is_segment_running(
            ROLE_MIRROR, int(content)
        ):
            raise Exception("mirror for content {} is not up".format(content))
        elif expected_status == "down" and is_segment_running(
            ROLE_MIRROR, int(content)
        ):
...
def impl(context, seg):
    ...
    filename = os.path.join(
        os.getcwd(), "./test/behave/mgmt_utils/steps/data/pid_background_script.py"
    )

    cmd = Command(
        name="Remove background script on remote host",
        cmdStr="rm -f /tmp/pid_background_script.py",
        remoteHost=hostname,
        ctxt=REMOTE,
    )
    cmd.run(validateAfter=True)
    ...
    cmd = Command(
        name="Copy background script to remote host",
        cmdStr="scp %s %s:/tmp" % (filename, hostname),
    )
    cmd.run(validateAfter=True)

    cmd = Command(
        name="Run Bg process to save pid",
        cmdStr='sh -c "python /tmp/pid_background_script.py /tmp/bgpid" &>/dev/null &',
        remoteHost=hostname,
        ctxt=REMOTE,
    )
    cmd.run(validateAfter=True)
    ...
    cmd = Command(
        name="get Bg process PID",
        cmdStr="sleep 1; until [ -f /tmp/bgpid ]; do sleep 1; done; cat /tmp/bgpid",
        remoteHost=hostname,
        ctxt=REMOTE,
    )
    cmd.run(validateAfter=True)
    ...
    if not context.bg_pid:
        raise Exception(
            "Unable to obtain the pid of the background script. Seg Host: %s, get_results: %s"
...
def impl(context, seg):
    ...
    cmd = Command(
        name="get bg pid",
        cmdStr="ps ux | grep pid_background_script.py | grep -v grep | awk '{print \$2}'",
        remoteHost=hostname,
        ctxt=REMOTE,
    )
    cmd.run(validateAfter=True)
    ...
    cmd = Command(
        name="remove pid", cmdStr="rm -rf /tmp/bgpid", remoteHost=hostname, ctxt=REMOTE
    )
    cmd.run(validateAfter=True)
...
def impl(context, process, contentids):
    ...
    for host in hosts:
        cmd = Command(
            name="kill process {}".format(process),
            cmdStr="pkill -9 {}".format(process),
            remoteHost=host,
            ctxt=REMOTE,
        )
        cmd.run(validateAfter=True)
...
def impl(context, seg):
    ...
    cmd = Command(
        name="Copy pid file",
        cmdStr="cp %s %s" % (pid_file_orig, pid_file),
        remoteHost=hostname,
        ctxt=REMOTE,
    )
    cmd.run(validateAfter=True)
    ...
    cpCmd = Command(
        name="copy pid file to master for editing",
        cmdStr="scp %s:%s /tmp" % (hostname, pid_file),
    )
    ...
    cpCmd = Command(
        name="copy pid file to segment after editing",
...
@when(
    'we generate the postmaster.pid file with a non running pid on the same "{seg}" segment'
)
def impl(context, seg):
    if seg == "primary":
        data_dir = context.pseg_data_dir
...
def impl(context, seg):
    ...
    segStartCmd = SegmentStart(
        name="Starting new segment dbid %s on host %s." % (str(dbid), hostname),
        gpdb=segment,
...
def impl(context, seg):
    ...
    cmd = Command(
        name="Copy pid file",
        cmdStr="cp %s %s" % (pid_file, pid_file_orig),
        remoteHost=hostname,
        ctxt=REMOTE,
    )
    cmd.run(validateAfter=True)
...
def impl(context, seg):
    ...
    cmd = Command(
        ...
        cmdStr="rm -f %s" % (os.path.join(data_dir, "postmaster.pid.orig")),
        remoteHost=hostname,
        ctxt=REMOTE,
    )
    cmd.run(validateAfter=True)
...
def impl(context):
    check_segment_config_query = (
        "SELECT * FROM gp_segment_configuration WHERE content = -1 AND role = 'm'"
    )
...
def impl(context):
    check_segment_config_query = (
        "SELECT * FROM gp_segment_configuration WHERE content = -1 AND role = 'p' AND preferred_role = 'p' AND dbid = %s"
        % context.standby_dbid
    )
    with dbconn.connect(
        dbconn.DbURL(
            hostname=context.standby_hostname,
            dbname="postgres",
            port=context.standby_port,
        ),
        unsetSearchPath=False,
    ) as conn:
        segconfig = dbconn.execSQL(conn, check_segment_config_query).fetchall()
    ...
    if len(segconfig) != 1:
        raise Exception(
            "gp_segment_configuration did not have standby master acting as new master"
        )
...
def impl(context, schema_name, dbname):
    ...
    if not schema_exists:
        raise Exception(
            "Schema '%s' does not exist in the database '%s'" % (schema_name, dbname)
        )


@then(
    'verify that the utility {utilname} ever does logging into the user\'s "{dirname}" directory'
)
...
@then(
    'verify that sequence "{seq_name}" last value is "{last_value}" in database "{dbname}"'
)
@when(
    'verify that sequence "{seq_name}" last value is "{last_value}" in database "{dbname}"'
)
...
def impl(context, seq_name, last_value, dbname):
    ...
    if lv != int(last_value):
        raise Exception(
            'Sequence %s last value is not %s in %s"' % (seq_name, last_value, dbname)
        )
...
@then(
    'verify that the last line of the file "{filename}" in the master data directory {contain} the string "{output}"'
)
...
@then(
    'verify that the last line of the file "{filename}" in the master data directory {contain} the string "{output}"{escape}'
)
def verify_master_file_last_line(context, filename, contain, output, escape):
    if contain == "should contain":
        ...
        raise Exception(
            "only 'should contain' and 'should not contain' are valid inputs"
        )

    find_string_in_master_data_directory(
        context, filename, output, valuesShouldExist, (escape == " escaped")
    )


def find_string_in_master_data_directory(
    context, filename, output, valuesShouldExist, escapeStr=False
):
    contents = ""
    ...
    if (not valuesShouldExist) and pat.search(contents):
        err_str = "Did not expect stdout string '%s' but found: '%s'" % (
            output,
            contents,
        )
        raise Exception(err_str)
...
@given(
    'verify that the file "{filename}" in the master data directory has "{some}" line starting with "{output}"'
)
@then(
    'verify that the file "{filename}" in the master data directory has "{some}" line starting with "{output}"'
)
def impl(context, filename, some, output):
    ...
    with open(file_path) as fr:
        for line in fr:
            ...
            if not valuesShouldExist:
                if match:
                    err_str = "Expected no stdout string '%s' and found: '%s'" % (
                        regexStr,
                        contents,
                    )
                    raise Exception(err_str)
    ...
    if valuesShouldExist:
        err_str = "xx Expected stdout string '%s' and found: '%s'" % (
            regexStr,
            contents,
        )
        raise Exception(err_str)
...
@given(
    'verify that the file "{filename}" in each segment data directory has "{some}" line starting with "{output}"'
)
@then(
    'verify that the file "{filename}" in each segment data directory has "{some}" line starting with "{output}"'
)
    ...
    for info in segment_info:
        ...
        try:
            ...
            if not valuesShouldExist:
                if val:
                    raise Exception(
                        'File %s on host %s does start with "%s"(val error: %s)'
                        % (filepath, host, output, val)
                    )
            else:
                if not val:
                    raise Exception(
                        'File %s on host %s does start not with "%s"(val error: %s)'
                        % (filepath, host, output, val)
                    )
        except:
            raise Exception(
                'File %s on host %s does start with "%s"(parse error)'
                % (filepath, host, output)
            )


@then(
    'verify that the last line of the file "{filename}" in each segment data directory {contain} the string "{output}"'
)
def impl(context, filename, contain, output):
    if contain == "should contain":
        valuesShouldExist = True
    ...
    for info in segment_info:
        ...
        if valuesShouldExist and (output not in actual):
            raise Exception(
                'File %s on host %s does not contain "%s"' % (filepath, host, output)
            )
        if (not valuesShouldExist) and (output in actual):
            raise Exception(
                'File %s on host %s contains "%s"' % (filepath, host, output)
            )


@given(
    'verify that the path "{filename}" in each segment data directory does not exist'
)
...
def impl(context, filename):
    ...
    for info in segment_info:
        ...
        cmd = Command(
            name="check exists directory or not",
            cmdStr=cmd_str,
            ctxt=REMOTE,
            remoteHost=host,
        )
        ...
        try:
            ...
            if val:
                raise Exception(
                    'Path %s on host %s exists (val %s) (cmd "%s")'
                    % (filepath, host, val, cmd_str)
                )
        except:
            raise Exception(
                'Path %s on host %s exists (cmd "%s")' % (filepath, host, cmd_str)
            )
...
def imp(context):
    ...
    if rc:
        raise Exception(
            "Error executing grep on gpcheckcat logs while finding ACL: %s" % error
        )

    rc, error, output = run_cmd("grep 'Default skipping test: owner' %s" % pattern)
    if rc:
        raise Exception(
            "Error executing grep on gpcheckcat logs while finding Owner: %s" % error
        )
...
@given(
    'there is a "{tabletype}" table "{tablename}" in "{dbname}" with "{numrows}" rows'
)
def impl(context, tabletype, tablename, dbname, numrows):
    populate_regular_table_data(
        context,
        tabletype,
        tablename,
        dbname,
...
def impl(context, tabletype, tablename, dbname):
    populate_regular_table_data(
        context, tabletype, tablename, dbname, compression_type=None, with_data=True
    )


@given(
    'there is a "{tabletype}" table "{tablename}" in "{dbname}" with data and description'
)
@then(
    'there is a "{tabletype}" table "{tablename}" in "{dbname}" with data and description'
)
...
@given(
    'there is a "{tabletype}" partition table "{table_name}" in "{dbname}" with data'
)
...
@then(
    'an attribute of table "{table}" in database "{dbname}" is deleted on segment with content id "{segid}"'
)
@when(
    'an attribute of table "{table}" in database "{dbname}" is deleted on segment with content id "{segid}"'
)
def impl(context, table, dbname, segid):
    local_cmd = (
        "psql %s -t -c \"SELECT port,hostname FROM gp_segment_configuration WHERE content=%s and role='p';\""
        % (dbname, segid)
    )
...
@when(
    'a table "{table}" in database "{dbname}" has its relnatts inflated on segment with content id "{segid}"'
)
def impl(context, table, dbname, segid):
    local_cmd = (
        "psql %s -t -c \"SELECT port,hostname FROM gp_segment_configuration WHERE content=%s and role='p';\""
...
@then(
    'The user runs sql "{query}" in "{dbname}" on specified segment {host}:{port} in utility mode'
)
@when(
    'The user runs sql "{query}" in "{dbname}" on specified segment {host}:{port} in utility mode'
)
...
@when(
    'The user runs psql "{psql_cmd}" against database "{dbname}" when utility mode is set to {utility_mode}'
)
@then(
    'The user runs psql "{psql_cmd}" against database "{dbname}" when utility mode is set to {utility_mode}'
)
...
def impl(context, psql_cmd, dbname, utility_mode):
    if utility_mode == "True":
        cmd = "PGOPTIONS='-c gp_session_role=utility' psql -d '{}' {};".format(
            dbname, psql_cmd
        )
    else:
...
def impl(context, path, num):
    ...
    if result != int(num):
        raise Exception(
            "expected %s items but found %s items in path %s" % (num, result, path)
        )
...
def impl(context, dir):
    ...
    for file in bash_files:
        ...
        if context.ret_code != 0:
            raise Exception(
                "Error running repair script %s: %s" % (file, context.stdout_message)
            )
...
def impl(context, user_table, catalog_table, primary_key, db_name):
    delete_qry = "delete from %s where %s='%s'::regclass::oid;" % (
        catalog_table,
        primary_key,
        user_table,
    )
...
def impl(context, user_table, catalog_table, primary_key, db_name):
    ...
    with dbconn.connect(
        dbconn.DbURL(dbname=db_name, port=port, hostname=host),
        utility=True,
...
def impl(context, proc):
    ...
    while (current_time - start_time).seconds < 120:
        ...
        if cmd.get_return_code() > 1:
            raise Exception(
                "unexpected problem with gprep, return code: %s" % cmd.get_return_code()
            )
...
def impl(context):
    ...
    cmd = "gpperfmon_install --port {master_port} --enable --password foo".format(
...
def impl(context):
    ...
    gpperfmon_config_file = "%s/gpperfmon/conf/gpperfmon.conf" % os.getenv(
...
@given(
    'the setting "{variable_name}" is NOT set in the configuration file "{path_to_file}"'
)
@when(
    'the setting "{variable_name}" is NOT set in the configuration file "{path_to_file}"'
)
...
@given(
    'the setting "{setting_string}" is placed in the configuration file "{path_to_file}"'
)
@when(
    'the setting "{setting_string}" is placed in the configuration file "{path_to_file}"'
)
...
@given(
    "the latest gpperfmon gpdb-alert log is copied to a file with a fake (earlier) timestamp"
)
@when(
    "the latest gpperfmon gpdb-alert log is copied to a file with a fake (earlier) timestamp"
)
def impl(context):
    gpdb_alert_file_path_src = sorted(
        glob.glob(
            os.path.join(
                os.getenv("MASTER_DATA_DIRECTORY"), "gpperfmon", "logs", "gpdb-alert*"
            )
        )
    )[-1]
...
def impl(context, gppkg_name):
    ...
    for hostname in set(hostlist):
        cmd = Command(
            name="check if internal gppkg is installed",
...
def impl(context, command):
    ...
    for hostname in set(hostlist):
        cmd = Command(
            name="running command:%s" % command,
            cmdStr=command,
            ctxt=REMOTE,
            remoteHost=hostname,
        )
...
def impl(context, gppkg_name):
    ...
    for hostname in set(hostlist):
        ...
        if gppkg_name in cmd.get_stdout():
            raise Exception(
                '"%s" gppkg is installed on host: %s. \nInstalled packages: %s'
                % (gppkg_name, hostname, cmd.get_stdout())
            )
...
def _remove_gppkg_from_host(context, gppkg_name, is_master_host):
    ...
    full_gppkg_name = next(
        (gppkg for gppkg in installed_gppkgs if gppkg_name in gppkg), None
    )
    if not full_gppkg_name:
        raise Exception(
            "Found no matches for gppkg '%s'\n"
            "gppkgs installed:\n%s" % (gppkg_name, installed_gppkgs)
        )

    remove_command = remove_native_package_command(remote_gphome, full_gppkg_name)
    cmd = Command(
        name="Cleanly remove from the remove host",
...
def impl(context, location):
    ...
    subprocess.check_call([
        ...
        "-e",
        r"s|^GPHOME=.*$|GPHOME={}|".format(location),
        greengage_path,
    ])
    ...
    try:
        ...
        subprocess.check_call(["mv", "{}.bak".format(greengage_path), greengage_path])
...
def impl(context, hosts):
    ...
    for host in host_list:
        rm_cmd = Command(
            name="remove files in gpAdminLogs",
...
def impl(context, dir):
    ...
    for host in host_list:
        rm_cmd = Command(
            name="remove files in {}".format(dir),
...
def impl(context, has, expected_file):
    ...
    if files_found and (has == "has no"):
        raise Exception(
            "expected no %s files in %s, but found %s"
            % (expected_file, log_dir, files_found)
        )
    if (not files_found) and (has == "has"):
        raise Exception(
            "expected %s file in %s, but not found" % (expected_file, log_dir)
        )


@then(
    'gpAdminLogs directory has "{expected_file}" files on respective hosts only for content {content_ids}'
)
def impl(context, expected_file, content_ids):
    content_list = [int(c) for c in content_ids.split(",")]
    all_segments = GpArray.initFromCatalog(dbconn.DbURL()).getDbList()
    segments = filter(
        lambda seg: seg.getSegmentRole() == ROLE_MIRROR and seg.content in content_list,
        all_segments,
    )
    host_to_seg_dbids = {}
    for seg in segments:
    ...
    for segHost, expected_files_on_host in host_to_seg_dbids.items():
        ...
        if not files_found:
            raise Exception(
                "expected {} files in {} on host {}, but not found".format(
                    expected_file, log_dir, segHost
                )
            )

        if len(files_found) != len(expected_files_on_host):
            raise Exception(
                "expected {} {} files in {} on host {}, but found {}: {}".format(
                    len(expected_files_on_host),
                    expected_file,
                    log_dir,
                    segHost,
                    len(files_found),
...
@then(
    "all previous progress files are removed from gpAdminLogs directory on respective hosts only for content {content_ids}"
)
def impl(context, content_ids):
    content_list = [int(c) for c in content_ids.split(",")]
    all_segments = GpArray.initFromCatalog(dbconn.DbURL()).getDbList()
        ...
        for file in context.existing_progress_files[segHost]:
            if file_substring in file:
                ...
                if checkfile_cmd.get_return_code() == 0:
                    raise Exception(
                        "Did not expect {} file in {} on host {}, but found".format(
                            file, log_dir, segHost
                        )
                    )
...
def impl(context, has, expected_file, hosts):
    ...
    for seg_host in all_segment_hosts:
        ...
        if files_found > 0 and (has == "has no"):
            raise Exception(
                "expected no {} files in {} on host {}, but found".format(
                    expected_file, log_dir, seg_host
                )
            )
...
def impl(context, role_name, dbname):
    ...
    try:
        ...
        if result != role_name:
            raise Exception(
                "Role %s does not exist in database %s." % (role_name, dbname)
            )
    except:
...
def impl(context):
    cmd = Command(name="Get system timezone", cmdStr='date +"%Z"')
    cmd.run(validateAfter=True)
...
def impl(context):
    cmd = Command(
        name="Get database timezone", cmdStr='psql -d template1 -c "show time zone" -t'
    )
    cmd.run(validateAfter=True)
    tz = cmd.get_stdout()
    cmd = Command(
        name="Get abbreviated database timezone",
        cmdStr="psql -d template1 -c \"select abbrev from pg_timezone_names where name='%s';\" -t"
        % tz,
    )
    cmd.run(validateAfter=True)
...
def step_impl(context):
    if context.database_timezone != context.system_timezone:
        raise Exception(
            "Expected database timezone to be %s, but it was %s"
            % (context.system_timezone, context.database_timezone)
        )
...
def step_impl(context, abbreviated_timezone):
    if context.database_timezone != abbreviated_timezone:
        raise Exception(
            "Expected database timezone to be %s, but it was %s"
            % (abbreviated_timezone, context.database_timezone)
        )
...
def step_impl(context):
    if context.startup_timezone != context.system_timezone:
        raise Exception(
            "Expected timezone in startup.log to be %s, but it was %s"
            % (context.system_timezone, context.startup_timezone)
        )
...
def step_impl(context, abbreviated_timezone):
    if context.startup_timezone != abbreviated_timezone:
        raise Exception(
            "Expected timezone in startup.log to be %s, but it was %s"
            % (abbreviated_timezone, context.startup_timezone)
        )
...
def _create_cluster(
    ...
    try:
        with dbconn.connect(
            ...
            curs = dbconn.execSQL(
                conn, "select count(*) from gp_segment_configuration where role='m';"
            )
            count = curs.fetchall()[0][0]
            if not with_mirrors and count == 0:
                print(
                    "Skipping creating a new cluster since the cluster is primary only already."
                )
                return
            elif with_mirrors and count > 0:
                print(
                    "Skipping creating a new cluster since the cluster has mirrors already."
                )
                return
    except:
        pass
    ...
    testcluster = TestCluster(
        hosts=[master_host] + segment_host_list,
        base_dir=context.working_directory,
        hba_hostnames=hba_hostnames,
    )
    testcluster.reset_cluster()
    testcluster.create_cluster(
        with_mirrors=with_mirrors, mirroring_configuration=mirroring_configuration
    )
    context.gpexpand_mirrors_enabled = with_mirrors


@then(
    'a cluster is created with no mirrors on "{master_host}" and "{segment_host_list}"'
)
@given(
    'a cluster is created with no mirrors on "{master_host}" and "{segment_host_list}"'
)
...
@given(
    'with HBA_HOSTNAMES "{hba_hostnames}" a cluster is created with no mirrors on "{master_host}" and "{segment_host_list}"'
)
@when(
    'with HBA_HOSTNAMES "{hba_hostnames}" a cluster is created with no mirrors on "{master_host}" and "{segment_host_list}"'
)
@when(
    'with HBA_HOSTNAMES "{hba_hostnames}" a cross-subnet cluster without a standby is created with no mirrors on "{master_host}" and "{segment_host_list}"'
)
def impl(context, master_host, segment_host_list, hba_hostnames):
    _create_cluster(
        context, master_host, segment_host_list, hba_hostnames, with_mirrors=False
    )


@given(
    'a cross-subnet cluster without a standby is created with mirrors on "{master_host}" and "{segment_host_list}"'
)
...
@given(
    'a cluster is created with "{mirroring_configuration}" segment mirroring on "{master_host}" and "{segment_host_list}"'
)
def impl(context, mirroring_configuration, master_host, segment_host_list):
    _create_cluster(
        ...
        segment_host_list,
        with_mirrors=True,
        mirroring_configuration=mirroring_configuration,
    )


@given(
    'the user runs gpexpand interview to add {num_of_segments} new segment and {num_of_hosts} new host "{hostnames}"'
)
@when(
    'the user runs gpexpand interview to add {num_of_segments} new segment and {num_of_hosts} new host "{hostnames}"'
)
def impl(context, num_of_segments, num_of_hosts, hostnames):
    ...
    if num_of_hosts > 0:
        ...
        if num_of_hosts != len(hosts):
            raise Exception(
                "Incorrect amount of hosts. number of hosts:%s\nhostnames: %s"
                % (num_of_hosts, hosts)
            )
    ...
    output, returncode = gpexpand.do_interview(
        hosts=hosts,
...
def impl(context):
    if len(glob.glob("{}/*.ts".format(context.working_directory))) != 1:
        raise Exception(
            "expected gpexpand tablespace input configuration file to be created"
        )


@when(
    "the user runs gpexpand with the latest gpexpand_inputfile with additional parameters {additional_params}"
)
def impl(context, additional_params=""):
    ...
    if ret_code != 0:
        raise Exception(
            "gpexpand exited with return code: %d.\nstderr=%s\nstdout=%s"
            % (ret_code, std_err, std_out)
        )


@when(
    "the user runs gpexpand with the latest gpexpand_inputfile without ret code check"
)
def impl(context):
    gpexpand = Gpexpand(context, working_directory=context.working_directory)
...
def _gpexpand_redistribute(context, duration=False, endtime=False):
    ...
    if duration or endtime:
        if ret_code != 0:
            ...
            raise Exception(
                "gpexpand didn't stop at duration / endtime.\nstderr=%s\nstdout=%s"
                % (std_err, std_out)
            )
    if ret_code != 0:
        raise Exception(
...
@given(
    "the user runs gpexpand with a static inputfile for a two-node cluster with mirrors"
)
def impl(context):
    inputfile_contents = """
...
@when(
    "the user runs gpexpand with a static inputfile for a single-node cluster with mirrors"
)
def impl(context):
...
@when(
    "the user runs gpexpand with a static inputfile for a single-node cluster with mirrors without ret code check"
)
def impl(context):
...
def impl(context):
    data_dir = os.path.join(context.working_directory, "data/master/gpseg-1")
...
def impl(context):
    ...
    raise Exception(
        "The master pid has been changed.\nprevious: %s\ncurrent: %s"
        % (context.master_pid, current_master_pid)
    )
...
def impl(context, tabname, numsegments):
    dbname = "gptest"
    with dbconn.connect(dbconn.DbURL(dbname=dbname), unsetSearchPath=False) as conn:
        query = "select numsegments from gp_distribution_policy where localoid = '{tabname}'::regclass".format(
            tabname=tabname
        )
    ...
    raise Exception(
        "The numsegments of the writable external table {tabname} is {ns} (expected to be {numsegments})".format(
            tabname=tabname, ns=str(ns), numsegments=str(numsegments)
        )
    )
...
def impl(context, table_name):
    ...
    if (
        xid != context.long_run_select_only_xid
        or data_result != context.long_run_select_only_data_result
    ):
        ...
        raise Exception(
            error_str
            % (
                context.long_run_select_only_xid,
                xid,
                context.long_run_select_only_data_result,
                data_result,
            )
        )
...
@then(
    "verify that long-run transaction aborted for changing the catalog by creating table {table_name}"
)
def impl(context, table_name):
    dbname = "gptest"
    ...
    if context.long_run_xid != xid:
        raise Exception(
            "Incorrect xid of long run transaction: before %s, after %s"
            % (context.long_run_xid, xid)
        )
...
def impl(context, table_name):
    dbname = "gptest"
    with closing(
        dbconn.connect(dbconn.DbURL(dbname=dbname), unsetSearchPath=False)
    ) as conn:
        ...
        query = (
            """select count(distinct(gp_segment_id)) from gp_dist_random('%s')"""
            % table_name
        )
    ...
    if no_of_segments != no_segments_table_present:
        raise Exception(
            "Table %s is not present on newly expanded segments" % table_name
        )
...
def impl(context, hostnames):
    ...
    for host in hosts:
        cmd = Command(
            name="create data directories for expansion",
            cmdStr="mkdir -p %s/data/primary; mkdir -p %s/data/mirror"
            % (base_dir, base_dir),
            ctxt=REMOTE,
            remoteHost=host,
        )
...
def make_temp_dir(context, tmp_base_dir, mode=""):
    ...
    if mode:
        os.chmod(
            path.normpath(path.join(tmp_base_dir, context.temp_base_dir)), int(mode, 8)
        )
...
def impl(context, table_name, expansion_state):
    ...
    conn = dbconn.connect(dbconn.DbURL(dbname=dbname), unsetSearchPath=False)
    try:
        query = """select status from gpexpand.status_detail where fq_name='public.{0}'""".format(
            table_name
        )
...
def step_impl(context, options):
    if "-f" in options:
        if (
            context.standby_hostname not in context.stdout_message
            or context.standby_data_dir not in context.stdout_message
            or str(context.standby_port) not in context.stdout_message
        ):
            raise Exception(
                "gpstate -f output is missing expected standby master information"
            )
    elif "-s" in options:
        if (
            context.standby_hostname not in context.stdout_message
        ...
        for stdout_line in context.stdout_message.split("\n"):
            if "up segments, from configuration table" in stdout_line:
                segments_up = int(
                    re.match(
                        ".*of up segments, from configuration table\s+=\s+([0-9]+)",
                        stdout_line,
                    ).group(1)
                )
                if segments_up <= 1:
                    raise Exception(
                        "gpstate -Q output does not match expectations of more than one segment up"
                    )

            if "down segments, from configuration table" in stdout_line:
                segments_down = int(
                    re.match(
                        ".*of down segments, from configuration table\s+=\s+([0-9]+)",
                        stdout_line,
                    ).group(1)
                )
                if segments_down != 0:
                    raise Exception(
                        "gpstate -Q output does not match expectations of all segments up"
                    )
        ...
        for i in range(cursor.rowcount):
            ...
            if (
                datadir not in context.stdout_message
                or str(port) not in context.stdout_message
            ):
                raise Exception(
                    "gpstate -m output missing expected mirror info, datadir %s port %d"
                    % (datadir, port)
                )
    else:
...
def impl(context, config_file):
    run_gpcommand(
        context,
        "gpinitsystem -a -I %s -l /tmp/gpinitsystemtest -s localhost -P 21100 -S $MASTER_DATA_DIRECTORY/newstandby -h ../gpAux/gpdemo/hostfile"
        % config_file,
    )
    check_return_code(context, 0)
...
def impl(context, config_file):
    run_gpcommand(
        context,
        "gpinitsystem -a -c ../gpAux/gpdemo/clusterConfigFile -O %s" % config_file,
    )
    check_return_code(context, 0)
...
def step_impl(context):
    ...
    try:
        ...
        for segment in segments:
            ...
            local_conf_copy = os.path.join(
                os.getenv("MASTER_DATA_DIRECTORY"),
                "%s.%s" % ("postgresql.conf", hostname),
            )
            cmd = Command(
                name="Copy remote conf to local to diff",
                cmdStr="scp %s:%s %s"
                % (hostname, remote_postgresql_conf, local_conf_copy),
            )
            cmd.run(validateAfter=True)
            ...
            if str(dic["port"]) != port:
                raise Exception(
                    "port value in postgresql.conf of %s is incorrect. Expected:%s, given:%s"
...
def impl(context, table_name, dbname):
    ...
    if table_name not in context.table_row_count:
        context.table_row_count[table_name] = _get_row_count_per_segment(
            table_name, dbname
        )
...
def impl(context, table, dbname):
    ...
    context.pre_redistribution_dist_policy = _get_dist_policy_per_partition(
        table, dbname
    )


@then(
    'distribution information from table "{table}" with data in "{dbname}" is verified against saved data'
)
def impl(context, table, dbname):
    ...
    if len(pre_distribution_row_count) >= len(post_distribution_row_count):
        raise Exception(
            "Failed to redistribute table. Expected to have more than %d segments, got %d segments"
            % (len(pre_distribution_row_count), len(post_distribution_row_count))
        )
    ...
    if len(post_distribution_row_count) != post_distribution_num_segments:
        raise Exception(
            "Failed to redistribute table %s. Expected table to have data on %d segments, but found %d segments"
            % (table, post_distribution_num_segments, len(post_distribution_row_count))
        )

    if sum(pre_distribution_row_count) != sum(post_distribution_row_count):
        raise Exception(
            "Redistributed data does not match pre-redistribution data. Actual: %d, Expected: %d"
            % (sum(post_distribution_row_count), sum(pre_distribution_row_count))
        )

    mean = sum(post_distribution_row_count) / len(post_distribution_row_count)
    variance = sum(
        pow(row_count - mean, 2) for row_count in post_distribution_row_count
    ) / len(post_distribution_row_count)
    ...
    if relative_std_error > tolerance:
        raise Exception(
            "Unexpected variance for redistributed data in table %s. Relative standard error %f exceeded tolerance factor of %f."
            % (table, relative_std_error, tolerance)
        )

    for i in range(len(post_distribution_dist_policy)):
        if (
            post_distribution_dist_policy[i][0] == pre_redistribution_dist_policy[i][0]
            or post_distribution_dist_policy[i][1]
            != pre_redistribution_dist_policy[i][1]
            or post_distribution_dist_policy[i][2]
            != pre_redistribution_dist_policy[i][2]
        ):
            raise Exception(
                """Redistributed policy does not match pre-redistribution policy.
            before expanded: %s, after expanded: %s"""
                % (
                    ",".join(map(str, pre_redistribution_dist_policy[i])),
...
@then(
    'the row count from table "{table_name}" in "{dbname}" is verified against the saved data'
)
def impl(context, table_name, dbname):
    ...
    if saved_row_count != current_row_count:
        raise Exception(
            "%s table in %s has %d rows, expected %d rows."
            % (table_name, dbname, current_row_count, saved_row_count)
        )


@then(
    'distribution information from table "{table1}" and "{table2}" in "{dbname}" are the same'
)
def impl(context, table1, table2, dbname):
    ...
    if distribution_row_count_tbl1 != distribution_row_count_tbl2:
        raise Exception(
            "%s and %s have different distribution. Row count of %s is %s and row count of %s is %s"
            % (
                table1,
                table2,
                table1,
                distribution_row_count_tbl1,
                table2,
                distribution_row_count_tbl2,
            )
        )
...
def _get_row_count_per_segment(table, dbname):
    with dbconn.connect(dbconn.DbURL(dbname=dbname), unsetSearchPath=False) as conn:
        query = (
            "SELECT gp_segment_id,COUNT(*) FROM %s GROUP BY gp_segment_id ORDER BY gp_segment_id;"
            % table
        )
        cursor = dbconn.execSQL(conn, query)
        ...
        ]  # indices are the gp segment id's, so no need to store them explicitly


def _get_dist_policy_per_partition(table, dbname):
    with dbconn.connect(dbconn.DbURL(dbname=dbname), unsetSearchPath=False) as conn:
        query = (
            "select * from gp_distribution_policy where localoid::regclass::text like '%s%%' order by localoid;"
            % table
        )
        cursor = dbconn.execSQL(conn, query)
        rows = cursor.fetchall()
...
def impl(context):
    ...
    if ret_code != 0:
        raise Exception(
            "rollback exited with return code: %d.\nstderr=%s\nstdout=%s"
            % (ret_code, std_err, std_out)
        )
...
@given(
    'the database "{dbname}" is broken with "{broken}" orphaned toast tables only on segments with content IDs "{contentIDs}"'
)
...
UPDATE pg_class SET reltoastrelid = 0 WHERE relname = 'double_orphan_invalid_parent';"""
    ...
    if contentIDs:
        ...
        seg_config_sql = (
            """SELECT port,hostname FROM gp_segment_configuration WHERE role='p' AND content IN (%s);"""
            % contentIDs
        )
    ...
    for dbURL in dbURLs:
        ...
        with dbconn.connect(
            dbURL, allowSystemTableMods=True, utility=utility, unsetSearchPath=False
        ) as conn:
            dbconn.execSQL(conn, sql)
            conn.commit()
...
def impl(context, dbname):
    ...
    with dbconn.connect(
        master, allowSystemTableMods=True, unsetSearchPath=False
    ) as conn:
        dbconn.execSQL(
            conn,
            """
    ...
    with dbconn.connect(
        seg0, utility=True, allowSystemTableMods=True, unsetSearchPath=False
    ) as conn:
        dbconn.execSQL(
            conn,
            """
    ...
    with dbconn.connect(
        seg1, utility=True, allowSystemTableMods=True, unsetSearchPath=False
    ) as conn:
        dbconn.execSQL(
            conn,
            """
...
def impl(context):
    ...
    standby_remote_statusfile = "%s:%s/%s" % (
        standby_host,
        standby_datadir,
        status_file,
    )
    ...
    standby_remote_gp_segment_configuration_file = "%s:%s/%s" % (
        standby_host,
        standby_datadir,
        gp_segment_configuration_backup,
    )
    ...
    cmd = Command(
        name="Copy standby file to master",
        cmdStr="scp %s %s" % (standby_remote_statusfile, standby_local_statusfile),
    )
    cmd.run(validateAfter=True)
    cmd = Command(
        name="Copy standby file to master",
        cmdStr="scp %s %s"
        % (
            standby_remote_gp_segment_configuration_file,
            standby_local_gp_segment_configuration_file,
        ),
    )
    cmd.run(validateAfter=True)
    ...
    if not os.path.exists(standby_local_gp_segment_configuration_file):
        raise Exception(
            'file "%s" is not exist' % standby_remote_gp_segment_configuration_file
        )
...
def are_on_different_subnets(primary_hostname, mirror_hostname):
    ...
    name = x[0].lower()
    if "ubuntu" in name:
        primary_broadcast = check_output([
            "ssh",
            "-n",
            primary_hostname,
            "/sbin/ip addr show ens4 | grep 'inet .* brd' | awk '{ print $4 }'",
        ])
        mirror_broadcast = check_output([
            "ssh",
            "-n",
            mirror_hostname,
            "/sbin/ip addr show ens4 | grep 'inet .* brd' | awk '{ print $4 }'",
        ...
        primary_broadcast = check_output([
            ...
            "-n",
            primary_hostname,
            "/sbin/ip addr show | grep 'inet .* brd' | awk '{ print $4 }'",
        ])
        mirror_broadcast = check_output([
            "ssh",
            "-n",
    ...
    if not primary_broadcast:
        raise Exception(
            "primary hostname %s has no broadcast address" % primary_hostname
        )
...
def impl(context, including):
    ...
    if including == "including":
        ...
        if not are_on_different_subnets(
            gparray.master.hostname, gparray.standbyMaster.hostname
        ):
            raise Exception(
                "master %s and its standby %s are on same the subnet"
                % (gparray.master, gparray.standbyMaster)
            )
    ...
    for segPair in gparray.segmentPairs:
        ...
        if not are_on_different_subnets(
            segPair.primaryDB.hostname, segPair.mirrorDB.hostname
        ):
...
def impl(context, content, desired_state):
    ...
    with dbconn.connect(
        ...
        rows = dbconn.execSQL(
            conn,
            "SELECT role, preferred_role FROM gp_segment_configuration WHERE content = %s and preferred_role %s role"
            % (content, role_operator),
        ).fetchall()
...
@given(
    "the user asynchronously runs pg_basebackup with {segment} of content {contentid} as source and the process is saved"
)
@when(
    "the user asynchronously runs pg_basebackup with {segment} of content {contentid} as source and the process is saved"
)
...
def impl(context, slot):
    ...
    segments = gparray.getDbList()
    dbname = "template1"
    query = "SELECT count(*) FROM pg_catalog.pg_replication_slots WHERE slot_name = '{}' and active = 't'".format(
        slot
    )
    ...
    for seg in segments:
        if seg.isSegmentPrimary(current_role=True):
            ...
            with closing(
                dbconn.connect(
                    dbconn.DbURL(dbname=dbname, port=port, hostname=host),
                    utility=True,
                    unsetSearchPath=False,
                )
            ) as conn:
                ...
                if result == 0:
                    raise Exception(
                        "Slot either does not exist or is inactive for host:{}, port:{}".format(
                            host, port
                        )
                    )
...
def impl(context, contentid):
    ...
    if str(contentid) not in segments_with_running_basebackup:
        raise Exception(
            "pg_basebackup entry was not found for content %s in gp_stat_replication"
            % contentid
        )


@given(
    "user waits until gp_stat_replication table has no pg_basebackup entries for content {contentids}"
)
@when(
    "user waits until gp_stat_replication table has no pg_basebackup entries for content {contentids}"
)
...
def impl(context, contentids):
    ...
    sql = (
        "select count(*) from gp_stat_replication where application_name = 'pg_basebackup' and gp_segment_id in (%s)"
        % (content_ids)
    )
    no_basebackup = False
    ...
    if not no_basebackup:
        raise Exception(
            "pg_basebackup entry was found for contents %s in gp_stat_replication after %d retries"
            % (contentids, retries)
        )
...
def impl(context):
    ...
    cmd = Command(
        name="backup the hosts file", cmdStr="sudo cp /etc/hosts /tmp/hosts_orig"
    )
    cmd.run(validateAfter=True)
    # Get the host-name
    ...
    cmd = Command(
        name="update hostlist with new hostname",
        cmdStr="sudo sed 's/%s/%s__1 %s/g' </etc/hosts >> /tmp/hosts; sudo cp -f /tmp/hosts /etc/hosts;rm /tmp/hosts"
...
def impl(context):
    ...
    cmd = Command(
        name="update temp hosts file",
        cmdStr="sed 's/%s/%s__1/g' < ../gpAux/gpdemo/hostfile >> /tmp/hostfile--1"
        % (hostname, hostname),
    )
    cmd.run(validateAfter=True)
...
def impl(context):
    ...
    cmd = Command(
        name="update master hostname in config file",
...
def impl(context):
    ...
    with dbconn.connect(dbconn.DbURL(), unsetSearchPath=False) as conn:
        sql = (
            "SELECT count(*) FROM gp_segment_configuration WHERE hostname='%s'"
            % hostname_new
        )
        num_matching = dbconn.execSQL(conn, sql).fetchone()
        if num_matching[0] != 0:
            raise Exception(
                "Found entries in gp_segment_configuration is host-address popoulated as host-name"
            )
    # Verify correct host-name is populated in the config
    with dbconn.connect(dbconn.DbURL(), unsetSearchPath=False) as conn:
        sql = (
            "SELECT count( distinct hostname) FROM gp_segment_configuration WHERE hostname='%s'"
            % hostname_orig
        )
        num_matching = dbconn.execSQL(conn, sql).fetchone()
        if num_matching[0] != 1:
            raise Exception(
                "Found no entries in gp_segment_configuration is host-address popoulated as host-name"
            )
...
def impl(context):
    ...
    cmd_str = "rm -f ~/.ssh/id_rsa ~/.ssh/id_rsa.pub ~/.ssh/known_hosts; $GPHOME/bin/gpssh-exkeys -h {}".format(
        hostname
    )
...
def impl(context):
    cmd = Command(
        name="create input host file",
        cmdStr="echo sdw1 > /tmp/hostfile1;echo cdw >> /tmp/hostfile1;",
    )
    cmd.run(validateAfter=True)
...
def impl(context):
    ...
    for row in rows:
        ...
        if (
            context.ret_code == 2
            and "FATAL:  connections to primary segments are not allowed"
            in context.error_message
        ):
            continue
        else:
            raise Exception(
                "segment process not running in execute mode for DBID:{0}".format(dbid)
            )


@given(
    "user creates a new executable rsync script which inserts data into table and runs checkpoint along with doing "
    "rsync"
)
def impl(context):
    rsync_script = """
...
"""
    ...
    with closing(
        dbconn.connect(
            dbconn.DbURL(port=os.environ.get("PGPORT")), unsetSearchPath=False
        )
    ) as conn:
        query = (
            "select distinct hostname from gp_segment_configuration where status='d';"
        )
    ...
    cmd = Command(
        name="create a file for new rsync script",
    ...
    for host in host_list:
        ...
        cmd = Command(
            name="update rsync bash script",
            cmdStr="scp /usr/local/bin/rsync {}:/usr/local/bin/rsync".format(host),
        )
        cmd.run(validateAfter=True)

        cmd = Command(
            name="create script to clear cmd_cache",
            cmdStr=clear_cmd_cache_script,
            remoteHost=host,
            ctxt=REMOTE,
        )
        cmd.run(validateAfter=True)
        ...
        cmd = Command(
            name="run script to clear cmd_cache",
            cmdStr="chmod +x /tmp/clear_cmd_cache.py;/tmp/clear_cmd_cache.py",
            remoteHost=host,
            ctxt=REMOTE,
        )
        cmd.run(validateAfter=True)
...
def impl(context, table, dbname, count):
    ...
    if int(count) != sum(current_row_count):
        raise Exception(
            "%s table in %s has %d rows, expected %d rows."
            % (table, dbname, sum(current_row_count), int(count))
        )
...
def impl(context, command, num):
    ...
    for expected_pattern in expected_lines:
        match_count = len(
            re.findall(re.escape(expected_pattern), context.stdout_message)
        )
        if match_count != int(num):
            raise Exception(
...
def impl(context, dbname):
    with dbconn.connect(
        ...
        query = """SELECT datname,oid  FROM pg_database WHERE datname='{0}';""".format(
            dbname
        )
...
@then(
    "the created config file {output_config_file} contains the commented row for unreachable failed segment"
)
def impl(context, output_config_file):
    ...
    if set(expected_seg_rows) != set(actual_seg_rows):
        raise Exception(
            "created config file {} does not contain all of the expected rows".format(
                output_config_file
            )
        )
...
@then(
    "the user waits until recovery_progress.file is created in {logdir} and verifies that all dbids progress with {stage} are present"
)
...
def set_ic_proxy_and_address(context, new_addr, proxy_mode):
    ...
    with closing(dbconn.connect(dbconn.DbURL(), unsetSearchPath=False)) as conn:
        ...
        if context.ret_code != 0:
            raise Exception(
                "cannot run %s: %s, stdout: %s"
                % (cmd, context.error_message, context.stdout_message)
            )
...
@given("the cluster is running in IC proxy mode")
def step_impl(context):
    set_ic_proxy_and_address(context, "", True)


@given("the cluster is not running in IC proxy mode, but proxy_addresses has been set")
def step_impl(context):
    set_ic_proxy_and_address(context, "", False)


@given("the cluster is running in IC proxy mode with new proxy address {address}")
...
def impl(context, second):
    cmd = Command(
        name="psql",
        cmdStr="-c 'SELECT * from generate_series(1, %s) a where pg_sleep(1) is not null;'"
        % second,
    )
    cmd.runNoWait()
