...
def impl(context, contents):
    ...
    for seg in all_segments:
        ...
        with closing(dbconn.connect(dbconn.DbURL(), unsetSearchPath=False)) as conn:
            dbid = dbconn.execSQLForSingleton(
                conn,
                "SELECT dbid FROM gp_segment_configuration WHERE content = %s AND "
                "preferred_role = '%s'" % (content, preferred_role),
            )
        with closing(dbconn.connect(dbconn.DbURL(), unsetSearchPath=False)) as conn:
            num_tuples = dbconn.execSQLForSingleton(
                conn,
                "SELECT count(*) FROM gp_configuration_history WHERE dbid = %d AND "
        ...
        with closing(dbconn.connect(dbconn.DbURL(), unsetSearchPath=False)) as conn:
            context.original_config_history_backout_count = dbconn.execSQLForSingleton(
                conn,
                "SELECT count(*) FROM gp_configuration_history WHERE "
                "gp_configuration_history.desc LIKE 'gprecoverseg: segment config for backout%%';",
            )
...
def impl(context):
    ...
    for seg in gparray.getDbList():
        if (
            ...
            and seg.getSegmentContentId() == context.remote_mirror_segcid
        ):
...
def impl(context, seg):
    ...
    qry = (
        """select count(*) from gp_segment_configuration where status='d' and hostname='%s' and dbid=%s"""
        % (seghost, dbid)
    )
    row_count = getRows("postgres", qry)[0][0]
    if row_count != 1:
        raise Exception(
            "Expected %s segment %s on host %s to be down, but it is running."
            % (seg, datadir, seghost)
        )
...
def impl(context, seg):
    ...
    raise Exception(
        "Expected %s segment %s on host %s to be down, but it is still running after %d seconds."
        % (seg, datadir, seghost, timeout)
    )
...
@then(
    'the saved primary segment reports the same value for sql "{sql_cmd}" db "{dbname}" as was saved'
)
def impl(context, sql_cmd, dbname):
    psql_cmd = (
        "PGDATABASE='%s' PGOPTIONS='-c gp_session_role=utility' psql -t -h %s -p %s -c \"%s\"; "
        % (
            dbname,
            context.remote_pair_primary_host,
            context.remote_pair_primary_port,
            sql_cmd,
        )
    )
...
def isSegmentUp(context, dbid):
    qry = (
        """select count(*) from gp_segment_configuration where status='d' and dbid=%s"""
        % dbid
    )
...
def getPrimaryDbIdFromCid(context, cid):
    dbid_from_cid_sql = (
        "SELECT dbid FROM gp_segment_configuration WHERE content=%s and role='p';" % cid
    )
...
def getMirrorDbIdFromCid(context, cid):
    dbid_from_cid_sql = (
        "SELECT dbid FROM gp_segment_configuration WHERE content=%s and role='m';" % cid
    )
    result = getRow("template1", dbid_from_cid_sql)
    return result[0]
...
def runCommandOnRemoteSegment(context, cid, sql_cmd):
    local_cmd = (
        "psql template1 -t -c \"SELECT port,hostname FROM gp_segment_configuration WHERE content=%s and role='p';\""
        % cid
    )
    ...
    host = host.strip()
    psql_cmd = (
        "PGDATABASE='template1' PGOPTIONS='-c gp_session_role=utility' psql -h %s -p %s -c \"%s\"; "
        % (host, port, sql_cmd)
    )
...
def impl(context, utility, output, segment_type):
    ...
    all_segments = GpArray.initFromCatalog(dbconn.DbURL()).getDbList()
    segments = filter(
        lambda seg: seg.getSegmentRole() == role and seg.content >= 0, all_segments
    )
...
@then(
    '{utility} should print "{recovery_type}" errors to stdout for content {content_ids}'
)
@when(
    '{utility} should print "{recovery_type}" errors to stdout for content {content_ids}'
)
def impl(context, utility, recovery_type, content_ids):
    ...
    if recovery_type not in ("incremental", "full", "differential", "start"):
        raise Exception(
            "Expected recovery_type to be 'incremental', 'full' or 'start, but found '%s'."
            % recovery_type
        )
    ...
    for segment in segments:
        if recovery_type == "incremental" or recovery_type == "full":
            expected = r"hostname: {}; port: {}; logfile: {}/gpAdminLogs/pg_{}.\d{{8}}_\d{{6}}.dbid{}.out; recoverytype: {}".format(
                segment.getSegmentHostName(),
                segment.getSegmentPort(),
                os.path.expanduser("~"),
                "rewind" if recovery_type == "incremental" else "basebackup",
        ...
        if recovery_type == "differential":
            ...
            expected = r"hostname: {}; port: {}; datadir: {}".format(
                segment.getSegmentHostName(),
                segment.getSegmentPort(),
...
def impl(context, num):
    ...
    expected_args = "-l {} -v -b 64 --force-overwrite --era={}".format(
        _get_gpAdminLogs_directory(), era
    )
    matches = lines_matching_both(gprecoverseg_output, expected_command, expected_args)

    if len(matches) != int(num):
        raise Exception(
            "Expected gpsegrecovery.py to occur with %s args %s times. Found %d. \n %s"
            % (expected_args, num, len(matches), gprecoverseg_output)
        )


@then(
    "check if gprecoverseg ran gpsegsetuprecovery.py {num} times with the expected args"
)
def impl(context, num):
    gprecoverseg_output = context.stdout_message

    expected_command = "Running Command: $GPHOME/sbin/gpsegsetuprecovery.py"
    expected_args = "-l {} -v -b 64 --force-overwrite".format(
        _get_gpAdminLogs_directory()
    )
    matches = lines_matching_both(gprecoverseg_output, expected_command, expected_args)
...
@then(
    "check if {recovery_type} recovery was successful for mirrors with content {content_ids}"
)
def impl(context, recovery_type, content_ids):
    if recovery_type == "incremental":
        ...
        raise Exception(
            "Expected recovery_type to be 'incremental', 'full', 'differential' but found '%s'."
            % recovery_type
        )
...
@then(
    "check if {recovery_type} recovery failed for mirrors with content {content_ids} for {utility}"
)
...
@when(
    "check if start failed for contents {content_ids} during full recovery for {utility}"
)
@then(
    "check if start failed for contents {content_ids} during full recovery for {utility}"
)
...
def impl(context, host):
    all_segments = GpArray.initFromCatalog(dbconn.DbURL()).getDbList()
    segments = filter(
        lambda seg: seg.getSegmentRole() == ROLE_MIRROR
        and seg.getSegmentHostName() == host,
        all_segments,
    )
...
def impl(context, original_host, new_host):
    ...
    segments = filter(
        lambda seg: seg.getSegmentRole() == ROLE_MIRROR
        and seg.getSegmentHostName() == original_host,
        all_segments,
    )
    content_ids_on_host = [seg.content for seg in segments]
    content_id_str = ",".join(str(i) for i in content_ids_on_host)
    ...
    for segment in segments:
        ...
        expected = (
            r"hostname: {}; port: \d{{5}}; logfile: {}/gpAdminLogs/pg_basebackup.\d{{8}}_\d{{6}}.dbid{}.out; "
            r"recoverytype: full".format(
                new_host, os.path.expanduser("~"), segment.getSegmentDbId()
            )
        )
        check_stdout_msg(context, expected)
...
@then(
    'gprecoverseg should print "{output}" to stdout for mirrors with content {content_ids}'
)
...
@then(
    'the "{before}" and "{after}" cluster configuration matches for gprecoverseg newhost'
)
def impl(context, before, after):
    if (
        not hasattr(context, "saved_array")
        or (before not in context.saved_array)
        or (after not in context.saved_array)
    ):
        raise Exception("before_array or after_array not saved prior to call")

    compare_gparray_with_expected(
        context.saved_array[before], context.saved_array[after]
    )
...
@then(
    'the "{before}" and "{after}" cluster configuration matches with the expected for gprecoverseg newhost'
)
def impl(context, before, after):
    if (
        not hasattr(context, "saved_array")
...
"""
    ...
    expected[
        "two_hosts_down"
    ] = """1|-1|p|p|n|u|cdw|cdw|5432|/data/gpdata/master/gpseg-1
10|0|p|m|s|u|sdw2|sdw2|21000|/data/gpdata/mirror/gpseg0
11|1|p|m|s|u|sdw2|sdw2|21001|/data/gpdata/mirror/gpseg1
4|2|p|p|s|u|sdw2|sdw2|20000|/data/gpdata/primary/gpseg2
5|3|p|p|s|u|sdw2|sdw2|20001|/data/gpdata/primary/gpseg3
...
"""
    ...
    compare_gparray_with_expected(
        context.saved_array[before], expected_before_gparray, array_name=before
    )
    compare_gparray_with_expected(
        context.saved_array[after], expected_after_gparray, array_name=after
    )
...
def compare_gparray_with_expected(actual_gparray, expected_gparray, array_name=""):
    def _sortedSegs(gparray):
        ...
        for host in segs_by_host:
            segs_by_host[host] = sorted(
                segs_by_host[host], key=lambda seg: seg.getSegmentDbId()
            )
        return segs_by_host
...
@when(
    'a gprecoverseg input file "{filename}" is created with a different data directory for content {content}'
)
def impl(context, filename, content):
    line = ""
    with closing(
        dbconn.connect(dbconn.DbURL(dbname="template1"), unsetSearchPath=False)
    ) as conn:
        result = dbconn.execSQLForSingletonRow(
            conn,
            "SELECT port, hostname, datadir FROM gp_segment_configuration WHERE preferred_role='p' AND content = %s;"
            % content,
        )
...
def impl(context, content_ids, expected_status):
    ...
    for content in contents:
        ...
        wait_for_desired_query_result(
            dbconn.DbURL(), query, "u" if expected_status == "up" else "d"
        )


@then(
    "the contents {contents} should have their original data directory in the system configuration"
)
def impl(context, contents):
    ...
    for content in contents:
        with closing(
            ...
            for role in [ROLE_PRIMARY, ROLE_MIRROR]:
                actual_datadir = dbconn.execSQLForSingleton(
                    conn,
                    "SELECT datadir FROM gp_segment_configuration WHERE preferred_role='{}' AND "
                    "content = {};".format(role, content),
                )
                expected_datadir = context.original_seg_info[
                    "{}_{}".format(content, role)
...
@then(
    "the gp_configuration_history table should contain a backout entry for the {seg} segment for contents {contents}"
)
def impl(context, seg, contents):
    ...
    for content in contents:
        ...
        with closing(dbconn.connect(dbconn.DbURL(), unsetSearchPath=False)) as conn:
            actual_tuples = dbconn.execSQLForSingleton(
                conn,
                "SELECT count(*) FROM gp_configuration_history WHERE dbid = %d AND gp_configuration_history.desc LIKE 'gprecoverseg: segment config for backout%%';"
                % dbid,
            )

        original_tuples = context.original_config_history_info[
            "{}_{}".format(content, role)
        ]
        if (
            actual_tuples != original_tuples + 1
        ):  # Running the backout script should have inserted exactly 1 entry
            raise Exception(
...
@then(
    "the gp_configuration_history table should contain {expected_additional_entries} additional backout entries"
)
def impl(context, expected_additional_entries):
    ...
    expected_total_entries = int(context.original_config_history_backout_count) + int(
        expected_additional_entries
    )
    if actual_backout_entries != expected_total_entries:
        raise Exception(
            "Expected configuration history table to have {} backout entries, found {}".format(
                context.original_config_history_backout_count
                + expected_additional_entries,
                actual_backout_entries,
            )
        )


@when(
    'a gprecoverseg input file "{filename}" is created with added parameter hostname to recover the failed segment on new host'
)
def impl(context, filename):
    with closing(
        dbconn.connect(dbconn.DbURL(dbname="template1"), unsetSearchPath=False)
    ) as conn:
        failed_port, failed_hostname, failed_datadir, failed_address = (
            context.remote_pair_primary_port,
            context.remote_pair_primary_host,
            context.remote_pair_primary_datadir,
            context.remote_pair_primary_address,
        )
        result = dbconn.execSQL(
            conn,
            "SELECT hostname FROM gp_segment_configuration WHERE preferred_role='p' and status = 'u' and content != -1;",
        ).fetchall()
        failover_port, failover_hostname, failover_datadir = (
            23000,
            result[0][0],
            failed_datadir,
        )
...
@when(
    "check hostname and address updated on segment configuration with the saved information"
)
def impl(context):
    with closing(
        dbconn.connect(dbconn.DbURL(dbname="template1"), unsetSearchPath=False)
        ...
        result = dbconn.execSQL(
            conn,
            "SELECT content, hostname, address FROM gp_segment_configuration WHERE dbid = {};".format(
        ...
        if (
            address != context.recovery_host_address
            or hostname != context.recovery_host_name
        ):
            raise Exception(
                "Host name and address could not updated on segment configuration for dbId {0}".format(
                    context.remote_pair_primary_segdbId
                )
            )
...
@when(
    'a gprecoverseg input file "{filename}" is created with hostname parameter to recover the failed segment on same host'
)
def impl(context, filename):
    port, hostname, datadir, address = (
        context.remote_pair_primary_port,
        context.remote_pair_primary_host,
        context.remote_pair_primary_datadir,
        context.remote_pair_primary_address,
    ...
    line = "{0}|{1}|{2}|{3} {4}|{5}|{6}|/tmp/newdir".format(
        hostname, address, port, datadir, hostname, host_address, port
    )
...
@when(
    'a gprecoverseg input file "{filename}" is created with hostname parameter matches with segment configuration table for incremental recovery of failed segment'
)
def impl(context, filename):
    port, hostname, datadir, address = (
        context.remote_pair_primary_port,
...
@when(
    'a gprecoverseg input file "{filename}" is created with invalid format for inplace full recovery of failed segment'
)
def impl(context, filename):
    port, hostname, datadir, address = (
        context.remote_pair_primary_port,
    ...
    line = "{0}|{1}|{2}|{3} {4}|{5}|/tmp/newdir".format(
        hostname, address, port, datadir, host_address, port
    )
...
@when(
    'a gprecoverseg input file "{filename}" created with invalid failover hostname for full recovery of failed segment'
)
def impl(context, filename):
    port, hostname, datadir, address = (
        context.remote_pair_primary_port,
    ...
    line = "{0}|{1}|{2}|{3} {4}_1|{5}|{6}|/tmp/newdir".format(
        hostname, address, port, datadir, hostname, host_address, port
    )
...
@when(
    'a gprecoverseg input file "{filename}" is created with and without parameter hostname to recover all the failed segments'
)
def impl(context, filename):
    lines = []
    with closing(
        ...
        rows = dbconn.execSQL(
            conn,
            "SELECT port, hostname, datadir, content, address FROM gp_segment_configuration WHERE  status = 'd' and content != -1;",
        ).fetchall()
    for i, row in enumerate(rows):
        ...
        if content == 0:
            ...
            output_str += "{0}|{1}|{2} {3}|{4}|/tmp/newdir{5}".format(
                address, port, datadir, address, port, i
            )
        else:
            output_str += "{0}|{1}|{2}|{3} {4}|{5}|{6}|/tmp/newdir{7}".format(
                hostname, address, port, datadir, hostname, host_address, port, i
            )
...
@when(
    'a gprecoverseg input file "{filename}" is created with invalid hostname parameter that does not matches with the segment configuration table hostname'
)
def impl(context, filename):
    port, hostname, datadir, address = (
        context.remote_pair_primary_port,
...
def impl(context):
    ...
    cmd = Command(
        "list directory",
        cmdStr="test -d {}".format(db_data_dir),
        ctxt=REMOTE,
        remoteHost=hostname,
    )
    cmd.run()
    ...
    if rc == 0:
        raise Exception(
            'Orphaned directory:"{0}" of dropped database:"{1}" exists on host:"{2}"'.format(
                db_data_dir, context.db_name, hostname
            )
        )


@when(
    'a gprecoverseg input file "{filename}" is created with all the failed segments and {category} recovery type'
)
def impl(context, filename, category):
    ...
    for seg in failed_segments:
        ...
        if content == 0:
            if category == "valid":
                output_str += "I|{0}|{1}|{2}|{3}".format(
                    hostname, address, port, datadir
                )
            else:
                output_str += "Invalid_rtype|{0}|{1}|{2}|{3}".format(
                    hostname, address, port, datadir
                )
        elif content == 1:
