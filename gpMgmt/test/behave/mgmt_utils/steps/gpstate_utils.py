...
def impl(context):
    with open(
        "{}/gpAdminLogs/recovery_progress.file".format(os.path.expanduser("~")), "w+"
    ) as fp:
        fp.writelines(context.recovery_lines)


@given(
    "a sample recovery_progress.file is created with ongoing recoveries in gpAdminLogs"
)
def impl(context):
    with open(
        "{}/gpAdminLogs/recovery_progress.file".format(os.path.expanduser("~")), "w+"
        ...
        fp.write(
            "full:5: 1164848/1371715 kB (84%), 0/1 tablespace (...t1/demoDataDir0/base/16384/40962)\n"
        )
        fp.write("incremental:6: 1/1371875 kB (1%)")


@then(
    "a sample {lock_file} directory is created using the background pid in master_data_directory"
)
@given(
    "a sample {lock_file} directory is created using the background pid in master_data_directory"
)
def impl(context, lock_file):
    if "bg_pid" in context:
        ...
        if not unix.check_pid(bg_pid):
            raise Exception(
                "The background process with PID {} is not running.".format(bg_pid)
            )
    else:
        bg_pid = ""
...
@given(
    "a sample recovery_progress.file is created with completed recoveries in gpAdminLogs"
)
def impl(context):
    with open(
        "{}/gpAdminLogs/recovery_progress.file".format(os.path.expanduser("~")), "w+"
        ...
        fp.write(
            "full:6: 1164848/1371715 kB (84%), 0/1 tablespace (...t1/demoDataDir0/base/16384/40962)\n"
        )
...
@then(
    'gpstate output contains "{recovery_types}" entries for mirrors of content {contents}'
)
def impl(context, recovery_types, contents):
    ...
    for seg in all_segments:
        if (
            seg.getSegmentContentId() in contents
            and seg.getSegmentRole() == ROLE_MIRROR
        ):
    ...
    for index, seg_to_display in enumerate(segments_to_display):
        ...
        if recovery_types[index] == "differential":
            expected_msg = (
                "{}[ \t]+{}[ \t]+{}[ \t]+(.+?)[ \t]+([\d,]+)[ \t]+[0-9]+\%".format(
                    hostname, port, recovery_types[index]
                )
            )
        else:
            expected_msg = (
...
def check_stdout_msg_in_order(context, msg):
    ...
    if not match:
        err_str = (
            ...
            msg,
            context.stdout_message[context.stdout_position :],
...
@given(
    "a sample recovery_progress.file is created with ongoing differential recoveries in gpAdminLogs"
)
def impl(context):
    with open(
        "{}/gpAdminLogs/recovery_progress.file".format(os.path.expanduser("~")), "w+"
        ...
        fp.write(
            "differential:6:          8,192 100%    7.81MB/s    0:00:00 (xfr#1, to-chk=0/1) :Syncing tablespace of "
