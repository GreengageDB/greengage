...
def run_exkeys(hosts, capture=False):
    ...
    with (
        ...
        tempfile.NamedTemporaryFile() as temp_err,
    ):
        ...
        with (
            pipe_out.open("/dev/stdout", "w") as out,
            pipe_err.open("/dev/stderr", "w") as err,
        ):
...
def impl(context, works):
    ...
    if should_work != did_work:
        ...
        raise Exception(
            "actual result of ssh cdw: %s (expected: %s)", result, expected_code
        )
