...
def run_command_remote(
    context, command, host, source_file, export_mdd, validateAfter=True
):
    cmd = Command(
        name="run command %s" % command,
        cmdStr="gpssh -h %s -e 'source %s; %s; %s'"
        % (host, source_file, export_mdd, command),
    )
...
def run_gpcommand(context, command, cmd_prefix=""):
    ...
    if cmd_prefix:
        cmd = Command(
            name="run %s" % command, cmdStr="%s;$GPHOME/bin/%s" % (cmd_prefix, command)
        )
    try:
        cmd.run(validateAfter=True)
    except ExecutionError as e:
...
def check_return_code(context, ret_code):
    if context.ret_code != int(ret_code):
        ...
        if context.stdout_message:
            emsg += "STDOUT:\n%s\n" % context.stdout_message
        raise Exception(
            "expected return code '%s' does not equal actual return code '%s' \n%s"
            % (ret_code, context.ret_code, emsg)
        )
...
def check_database_is_running(context):
    ...
    running_status = chk_local_db_running(
        os.environ.get("MASTER_DATA_DIRECTORY"), pgport
    )
    gpdb_running = (
        running_status[0]
        and running_status[1]
        and running_status[2]
        and running_status[3]
    )
...
def stop_primary(context, content_id):
    get_psegment_sql = (
        "select datadir, hostname from gp_segment_configuration where content=%i and role='p';"
        % content_id
    )
    with dbconn.connect(
...
def check_db_exists(dbname, host=None, port=0, user=None):
    with dbconn.connect(
        dbconn.DbURL(hostname=host, username=user, port=port, dbname="template1"),
        unsetSearchPath=False,
    ) as conn:
        count = dbconn.execSQLForSingleton(
            conn, "SELECT count(*) FROM pg_database WHERE datname='{}'".format(dbname)
        )
        if count == 0:
            return False
...
def create_database_if_not_exists(context, dbname, host=None, port=0, user=None):
    ...
    context.conn = dbconn.connect(
        dbconn.DbURL(dbname=context.dbname), unsetSearchPath=False
    )
...
def create_database(context, dbname=None, host=None, port=0, user=None):
    ...
    if host is None or port == 0 or user is None:
        ...
        createdb_cmd = 'psql -h %s -p %d -U %s -d template1 -c "create database %s"' % (
            host,
            port,
            user,
            dbname,
    ...
    raise Exception(
        "create database for '%s' failed after %d attempts" % (dbname, LOOPS)
    )
...
def check_table_exists(
    context, dbname, table_name, table_type=None, host=None, port=0, user=None
):
    with dbconn.connect(
        dbconn.DbURL(hostname=host, port=port, username=user, dbname=dbname),
        unsetSearchPath=False,
    ) as conn:
        if "." in table_name:
            ...
            SQL_format = """
                ...
                """
            SQL = SQL_format % (
                escape_string(tablename, conn=conn),
                escape_string(schemaname, conn=conn),
            )
        else:
            SQL_format = """
...
def drop_external_table_if_exists(context, table_name, dbname):
    if check_table_exists(
        context, table_name=table_name, dbname=dbname, table_type="external"
    ):
...
def drop_table(context, table_name, dbname, host=None, port=0, user=None):
    ...
    with dbconn.connect(
        dbconn.DbURL(hostname=host, username=user, port=port, dbname=dbname),
        unsetSearchPath=False,
    ) as conn:
        dbconn.execSQL(conn, SQL)
        conn.commit()
    ...
    if check_table_exists(
        context, table_name=table_name, dbname=dbname, host=host, port=port, user=user
    ):
        raise Exception("Unable to successfully drop the table %s" % table_name)
...
def get_partition_tablenames(tablename, dbname, part_level=1):
    child_part_sql = (
        "select partitiontablename from pg_partitions where tablename='%s' and partitionlevel=%s;"
...
def validate_part_table_data_on_segments(context, tablename, part_level, dbname):
    ...
    for part_tablename in rows:
        seg_data_sql = (
            "select gp_segment_id, count(*) from gp_dist_random('%s') group by gp_segment_id;"
            % part_tablename[0]
        )
...
def create_external_partition(context, tablename, dbname, port, filename):
    ...
    create_table_str = (
        "Create table %s (%s) Distributed randomly \
    ...
    create_ext_table_str = (
        "Create readable external table %s_ret (%s) \
    ...
    alter_table_str = (
        "Alter table %s exchange partition p_2 \
...
def create_partition(
    ...
    if storage_type == "heap":
        ...
        create_table_str = (
            create_table_str
            + " WITH(appendonly = true, orientation = %s) "
            % storage_type_dict[storage_type]
        )
        if compression_type is not None:
            create_table_str = (
                create_table_str[:-2] + ", compresstype = " + compression_type + ") "
            )
        if partition:
            create_table_str = create_table_str + part_table
    ...
    if with_data:
        populate_partition(
            tablename, PARTITION_START_DATE, dbname, 0, rowcount, host, port, user
        )
...
def populate_partition(
    ...
    insert_sql_str = (
        "insert into %s select i+%d, 'backup', i + date '%s' from generate_series(0,%d) as i"
        % (tablename, data_offset, start_date, rowcount)
    )
    insert_sql_str += (
        "; insert into %s select i+%d, 'restore', i + date '%s' from generate_series(0,%d) as i"
        % (tablename, data_offset, start_date, rowcount)
    )
...
def create_indexes(context, table_name, indexname, dbname):
    btree_index_sql = "create index btree_%s on %s using btree(column1);" % (
        indexname,
        table_name,
    )
    bitmap_index_sql = "create index bitmap_%s on %s using bitmap(column3);" % (
        indexname,
        table_name,
    )
...
def validate_index(context, table_name, dbname):
    index_sql = (
        "select count(indexrelid::regclass) from pg_index, pg_class where indrelid = '%s'::regclass group by indexrelid;"
        % table_name
    )
    rows = getRows(dbname, index_sql)
    if len(rows) != 2:
        raise Exception(
            "Index creation was not successful. Expected 2 rows does not match %d rows"
            % len(rows)
        )
...
def create_int_table(context, table_name, table_type="heap", dbname="testdb"):
    ...
    if table_type == "AO":
        CREATE_TABLE_SQL = (
            "create table %s WITH(APPENDONLY=TRUE) as select generate_series(1,%d) as c1"
            % (table_name, NROW)
        )
    elif table_type == "CO":
        CREATE_TABLE_SQL = (
            "create table %s WITH(APPENDONLY=TRUE, orientation=column) as select generate_series(1, %d) as c1"
            % (table_name, NROW)
        )
...
def drop_database(context, dbname, host=None, port=0, user=None):
    ...
    if host is None or port == 0 or user is None:
        ...
        dropdb_cmd = 'psql -h %s -p %d -U %s -d template1 -c "drop database %s"' % (
            host,
            port,
            user,
            dbname,
...
def check_row_count(context, tablename, dbname, nrows):
    ...
    if (
        hasattr(context, "standby_was_activated")
        and context.standby_was_activated is True
    ):
        dburl = dbconn.DbURL(
            dbname=dbname, port=context.standby_port, hostname=context.standby_hostname
        )
    else:
    ...
    if result != nrows:
        raise Exception(
            "%d rows in table %s.%s, expected row count = %d"
            % (result, dbname, tablename, nrows)
        )
...
def is_segment_running(role, contentid):
    ...
    for seg in segments:
        if (
            seg.getSegmentRole() == role
            and seg.content == contentid
            and seg.status != "u"
        ):
            print(("segment is not up - %s" % seg))
            return False
...
def check_count_for_specific_query(dbname, query, nrows):
    ...
    if result != nrows:
        raise Exception(
            "%d rows in query: %s. Expected row count = %d" % (result, query, nrows)
        )
...
def get_primary_segment_host_port():
    ...
    get_psegment_sql = (
        "select hostname, port from gp_segment_configuration where dbid=%i;"
        % FIRST_PRIMARY_DBID
    )
    with dbconn.connect(
        dbconn.DbURL(dbname="template1"), unsetSearchPath=False
...
def get_primary_segment_host_port_for_content(content="0"):
    ...
    """
    get_psegment_sql = (
        "SELECT hostname, port FROM gp_segment_configuration WHERE content=%s AND role='p';"
        % content
    )
    with dbconn.connect(
        dbconn.DbURL(dbname="template1"), unsetSearchPath=False
...
def populate_regular_table_data(
    context,
    tabletype,
    table_name,
    dbname,
...
def is_process_running(proc_name, host=None):
    ...
    if cmd.get_return_code() > 1:
        raise Exception(
            "unexpected problem with pgrep, return code: %s" % cmd.get_return_code()
        )
