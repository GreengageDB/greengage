...
class GpDeleteSystem(Command):
    ...
    def __init__(self, mdd=None):
        ...
        cmd_str = (
            'export MASTER_DATA_DIRECTORY=%s; echo -e "y\\ny\\n" | gpdeletesystem -d %s'
            % (mdd, mdd)
        )
...
class TestCluster:
    def __init__(
        self, hosts=None, base_dir="/tmp/default_gpinitsystem", hba_hostnames="0"
    ):
        """
        ...
        self.gpinitconfig_mirror_template = local_path(
            "configs/gpinitconfig_mirror_template"
        )
    ...
    def _generate_gpinit_config_files(self):
        ...
        config_template = (
            self.gpinitconfig_mirror_template
            if self.mirror_enabled
            else self.gpinitconfig_template
        )
    ...
    def create_cluster(self, with_mirrors=False, mirroring_configuration="group"):
        ...
        segment_mirroring_option = (
            "--mirror-mode=spread" if mirroring_configuration == "spread" else ""
        )
        gpinitsystem_cmd = clean_env + "gpinitsystem -a -c  %s %s" % (
            self.init_file,
            segment_mirroring_option,
        )
...
def substitute_strings_in_file(input_file, output_file, sub_dictionary):
    ...
    print(
        "input_file: %s ; output_file: %s ; sub_dictionary: %s"
        % (input_file, output_file, str(sub_dictionary))
    )
    substituted = False
...
def reset_hosts(hosts, test_base_dir):
    ...
    reset_primary_dirs_cmd = "gpssh %s -e 'rm -rf %s; mkdir -p %s'" % (
        host_args,
        primary_dir,
        primary_dir,
    )
    ...
    reset_mirror_dirs_cmd = "gpssh %s -e 'rm -rf %s; mkdir -p %s'" % (
        host_args,
        mirror_dir,
        mirror_dir,
    )
    ...
    reset_master_dirs_cmd = "gpssh %s -e 'rm -rf %s; mkdir -p %s'" % (
        host_args,
        master_dir,
        master_dir,
    )
