...
class Gpexpand:
    ...
    def do_interview(
        self, hosts=None, num_of_segments=1, directory_pairs=None, has_mirrors=False
    ):
        """
        ...
        p1 = Popen(["gpexpand"], stdout=PIPE, stdin=PIPE, cwd=self.working_directory)
    ...
    def initialize_segments(self, additional_params=""):
        ...
        return run_gpcommand(
            self.context, "gpexpand -i %s %s" % (input_files[-1], additional_params)
        )
...
if __name__ == "__main__":
    gpexpand = Gpexpand()
    gpexpand.do_interview(
        num_of_segments=2, directory_pairs=[("/tmp/foo", ""), ("/tmp/bar", "")]
    )
