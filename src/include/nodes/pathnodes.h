...
typedef struct PlannerGlobal
{
	...
	/* slice IDs for SubPlan nodes */
	int		   *subplan_sliceIds pg_node_attr(read_write_ignore);
	...
	/* redo plan on every execution? */
	bool		oneoffPlan;

	/* if valid, query can be used with CURRENT OF for this rel */
	Oid			simplyUpdatableRel;

	/* workspace for GPDB plan sharing */
	ApplyShareInputContext share pg_node_attr(read_write_ignore);
	 ...
	 */
	/* don't expand foreign partitions */
	bool		skip_foreign_partitions;
	/* foreign partition was skipped */
	bool		foreign_partition_was_skipped;
	 ...
	 */
	int			numSlices;
	struct PlanSlice *slices pg_node_attr(read_write_ignore);
} PlannerGlobal;
...
struct PlannerInfo
{
	...
	/* list of non-equivalence clauses */
	List	   *non_eq_clauses;
	...
	/* list of CtePlanInfo, one for each CTE */
	List	   *list_cteplaninfo pg_node_attr(read_write_ignore);
	...
	/* hint on where the result of the query will be needed; unset if not known */
	...
	List	   *curOuterParams;
	int			numMotions;
	PlanSlice  *curSlice pg_node_attr(read_write_ignore);
	/* Planner configuration */
	PlannerConfig *config pg_node_attr(read_write_ignore);
	 ...
	 */
	List	   *partition_selector_candidates pg_node_attr(read_write_ignore);
 ...
 * totals across all segments. Divide by cdbpolicy->numsegments to get the
...
typedef enum RelOptKind
{
	...
	RELOPT_DEADREL				/* GPDB: relation eliminated from the plan */
} RelOptKind;
...
typedef struct RelOptInfo
{
	...
	/* GPDB: list of RT indexes */
	List	   *partitioned_child_rels;

	/*
	 * GPDB: In a subquery, if this base relation contains quals that must be
	 * evaluated at "outerquery" locus, and the base relation has a different
	 * locus, they are kept here in 'upperrestrictinfo' (RestrictInfo
	 * structures, if base rel), instead of 'baserestrictinfo'.
	 */
	List	   *upperrestrictinfo;
} RelOptInfo;
...
typedef struct DistributionKey
{
	...
	/* the values that are distributed (EquivalenceClasses) */
	List	   *dk_eclasses pg_node_attr(copy_as_scalar, equal_as_scalar);
 ...
 *
 * GPDB: The 'rows' estimate, as well as all the costs, are *per node* values.
 ...
 * are for the whole relation, across all segments! So you cannot generally
...
typedef struct Path
{
	...
	/* executor RAM needed for Path + kids */
	EstimatedBytes memory;

	/* distribution of the result tuples */
	CdbPathLocus locus;

	/*
	 * true => path contains a CdbMotion operator without a slackening operator
	 * above it
	 */
	bool		motionHazard;

	/* CDB: true => path can accept ExecRescan call */
	bool		rescannable;
	 ...
	 * the same slice, if this path is chosen. It is used in partition
	 * planning, to decide if it's safe to create a PartitionSelector node that
	 * affects other nodes at a distance. That can only be done if the
	 * PartitionSelector would be executed in the same slice.
	 *
...
typedef struct RestrictInfo
{
	 ...
	 * it must be evaluated in the same slice as the parent query)
	 */
	bool		contain_outer_query_references pg_node_attr(equal_ignore);
