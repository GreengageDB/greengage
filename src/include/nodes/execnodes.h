...
typedef struct BitmapHeapScanState
{
	...
	/*
	 * GPDB: the bitmap is a generic Node (TIDBitmap or StreamBitmap), and the
	 * executor drives its own iteration/prefetch (nodeBitmapHeapscan.c), since
	 * GPDB bitmap scans also cover AO tables and streaming bitmaps that do not
	 * go through the upstream heapam read_stream path.
	 */
	Node	   *tbm;
	...
	/* upstream per-node instrumentation, copied into sinstrument for parallel */
...
typedef struct HashJoinState
{
	...
	List	   *hj_Collations;
	ExprState  *hj_OuterHash;	/* upstream: precompiled outer hash expr */
