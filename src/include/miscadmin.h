...
typedef enum BackendType
{
	...
	B_WAL_SENDER,
	B_WAL_WRITER,
	/* GPDB-specific backend types; keep these last */
	B_FTS_HANDLER_PROCESS,
	B_FAULT_HADLER_PROCESS,
} BackendType;

/*
 * GPDB: upstream defines this as (B_WAL_WRITER + 1); it must count the
 * GPDB-specific backend types above, since it sizes arrays indexed by
 * MyBackendType (e.g. pg_stat_io).
 */
#define BACKEND_NUM_TYPES (B_FAULT_HADLER_PROCESS + 1)
