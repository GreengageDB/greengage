...
/*
 * GPDB: one above upstream's ID (PG18 = 0x01A5BCB7), because
 * PGSTAT_KIND_RESQUEUE changes the stats file contents vs vanilla; a
 * pre-existing vanilla-format stats file must be discarded on startup.
 */
...
/* The collector's data per resource queue (cdb extension) */
...
/*
 * Each backend maintains a hash with a PgStat_StatPortalEntry per portal so we
 * can track elapsed exec/wait times (to the nearest second) for resource-queue
 * statistics.  The collector only ever sees the embedded PgStat_StatQueueEntry.
 */
...
extern PgStat_StatPortalEntry *pgstat_getportalentry(uint32 portalid,
													 Oid queueid);
extern void pgstat_report_queuestat(void);
...
/* QD: combine per-table statistics reported back by the QEs */
struct CdbDispatchResults;
struct pg_result;
extern void pgstat_send_qd_tabstats(void);
extern void pgstat_combine_one_qe_result(List **oidList,
										 struct pg_result *pgresult,
										 int nest_level,
										 int32 segindex);
extern void pgstat_combine_from_qe(struct CdbDispatchResults *results,
