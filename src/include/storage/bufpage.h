...
/*
 * GPDB: Retrieving LSN of a shared buffer is safe only if: (1) exclusive lock
 * on the buffer's contents is held OR (2) shared lock on the buffer's contents
 * and the buffer header spinlock is held.  The Assert() validates that a
...
PageGetLSN(const PageData *page)
{
	...
	Assert(BufferLockHeldByMe((Page) page));
#endif
