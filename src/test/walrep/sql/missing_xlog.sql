...
-- PG16+ replaced the walsender "walread" error (WalSndCtl->error, surfaced as
-- gp_stat_replication.sync_error) with replication-slot invalidation
-- (RS_INVAL_WAL_REMOVED) when a mirror's required WAL has been removed: the
-- invalidated slot is now rejected at StartReplication before the walsender
-- ever tries to read the missing segment.  Detect that condition through the
-- primary slot's pg_replication_slots.invalidation_reason instead.
create or replace function wait_for_slot_invalidation (expected_reason text, segment_id int, retries int) returns bool as
$$
declare
	i int;
begin
	...
	loop
		if exists (select 1 from
			(select gp_execution_segment() as segid, slot_name, invalidation_reason
			   from gp_dist_random('pg_replication_slots')) s
			where s.segid = segment_id
			  and s.slot_name = 'internal_wal_replication_slot'
			  and coalesce(s.invalidation_reason, 'none') = expected_reason) then
			return true;
		end if;
		if i >= retries then
			return false;
...
\! gprecoverseg -av
-- end_ignore
-- we expect the primary's replication slot to be invalidated with
-- 'wal_removed', since the WAL files required by the mirror were removed on
-- the corresponding primary (PG16+ invalidates the slot instead of the
-- walsender reporting a 'walread' error via gp_stat_replication.sync_error).
select wait_for_slot_invalidation('wal_removed', 0, 500);
select coalesce(invalidation_reason, 'none') from
	(select gp_execution_segment() as segid, slot_name, invalidation_reason
	   from gp_dist_random('pg_replication_slots')) s
	where s.segid = 0 and s.slot_name = 'internal_wal_replication_slot';
...
-- the invalidation should go away after full recovery recreates the slot
select wait_for_slot_invalidation('none', 0, 500);
select coalesce(invalidation_reason, 'none') from
	(select gp_execution_segment() as segid, slot_name, invalidation_reason
	   from gp_dist_random('pg_replication_slots')) s
	where s.segid = 0 and s.slot_name = 'internal_wal_replication_slot';
