...
insert into reindex_dropindex_crtab_part_heap_btree (id, owner, description, property, poli, target) select i, 'user' || i, 'Testing GiST Index', '((3, 1300), (33, 1330))','( (22,660), (57, 650), (68, 660) )', '( (76, 76), 76)' from generate_series(1,1000) i ;
create index on reindex_dropindex_crtab_part_heap_btree(id);
create index on reindex_dropindex_crtab_part_heap_btree(owner);

-- start_ignore
create table before_reindex_dropindex_crtab_part_heap_btree as select oid as c_oid, gp_segment_id as c_gp_segment_id, relfilenode as c_relfilenode, relname as c_relname from gp_dist_random('pg_class') where relname like 'reindex_dropindex_crtab_part_heap_btree%_id_idx';
-- end_ignore
...
select 1 AS index_exists_on_all_segs from gp_dist_random('pg_class') WHERE relname = 'reindex_dropindex_crtab_part_heap_btree_owner_idx' GROUP BY oid having count(*) = (SELECT count(*) FROM gp_segment_configuration WHERE role='p' AND content > -1);
...
1: BEGIN;
1: LOCK reindex_dropindex_crtab_part_heap_btree IN ACCESS EXCLUSIVE MODE;
2&: REINDEX TABLE  reindex_dropindex_crtab_part_heap_btree;
3&: DROP INDEX reindex_dropindex_crtab_part_heap_btree_owner_idx;
1: COMMIT;
2<:
3<:
...
3: select 1-count(1) as index_dropped from (select * from pg_class union all select * from gp_dist_random('pg_class')) t where t.relname = 'reindex_dropindex_crtab_part_heap_btree_owner_idx';
