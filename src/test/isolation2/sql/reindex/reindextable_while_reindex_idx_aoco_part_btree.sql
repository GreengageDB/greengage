DROP TABLE IF EXISTS reindex_crtab_part_aoco_btree;

CREATE TABLE reindex_crtab_part_aoco_btree ( id INTEGER, owner VARCHAR, description VARCHAR, property BOX, poli POLYGON, target CIRCLE, v VARCHAR, t TEXT, f FLOAT, p POINT, c CIRCLE, filler VARCHAR DEFAULT 'Big data is difficult to work with using most relational database management systems and desktop statistics and visualization packages, requiring instead massively parallel software running on tens, hundreds, or even thousands of servers.What is considered big data varies depending on the capabilities of the organization managing the set, and on the capabilities of the applications.This is here just to take up space so that we use more pages of data and sequential scans take a lot more time. ') with (appendonly=true,orientation=column) DISTRIBUTED BY (id) PARTITION BY RANGE (id) ( DEFAULT PARTITION de_fault );
insert into reindex_crtab_part_aoco_btree (id, owner, description, property, poli, target) select i, 'user' || i, 'Testing GiST Index', '((3, 1300), (33, 1330))','( (22,660), (57, 650), (68, 660) )', '( (76, 76), 76)' from generate_series(1,1000) i ;
insert into reindex_crtab_part_aoco_btree (id, owner, description, property, poli, target) select i, 'user' || i, 'Testing GiST Index', '((3, 1300), (33, 1330))','( (22,660), (57, 650), (68, 660) )', '( (76, 76), 76)' from generate_series(1,1000) i ;
create index on reindex_crtab_part_aoco_btree(id);
-- @Description Ensures that a reindex table during reindex index operations is ok
-- 

DELETE FROM reindex_crtab_part_aoco_btree  WHERE id < 128;
1: create temp table old_relfilenodes as
   (select gp_segment_id as dbid, relfilenode, oid, relname from gp_dist_random('pg_class')
    where relname like 'reindex_crtab_part_aoco_btree%_idx'
    union all
    select gp_segment_id as dbid, relfilenode, oid, relname from pg_class
    where relname like 'reindex_crtab_part_aoco_btree%_idx');
-- By default there is one index creation operation per segment because CREATE INDEX was run.
1: select distinct count(distinct relfilenode), relname from old_relfilenodes group by dbid, relname;

select gp_inject_fault('reindex_relation', 'suspend', '' /* DDL */, '' /* database name */,
    '' /* table name */, 1 /* start occurrence */, 1 /* end_occurrence */, 0 /* extra_arg */,
    dbid, -1 /* any gp_session_id */, 0 /* nesting level */)
    from gp_segment_configuration where role='p' and content = -1;

-- The fault injector should suspend during reindexing of the default partition.
-- Lock of this partition should be held.
1&: reindex table reindex_crtab_part_aoco_btree;

2: select gp_wait_until_triggered_fault('reindex_relation', 1,
    (select dbid from gp_segment_configuration where role='p' and content=-1));

2: begin;

-- Should wait until REINDEX TABLE releases the lock
2&: reindex index reindex_crtab_part_aoco_btree_1_prt_de_fault_id_idx;

select gp_inject_fault('reindex_relation','reset',
       (select dbid from gp_segment_configuration where role='p' and content=-1));

-- After resetting the fault injector, session 1 finishes REINDEX TABLE, releases all locks, and then session 2 finishes.
1<:
2<:
-- Session 2 has not committed yet. Session 1 should see the effects of
-- its own REINDEX command in pg_class. The following query
-- validates that session 1's REINDEX generated a new relfilenode for the default-partition index.
-- Reindexing of the root table is not performed during REINDEX TABLE, so its counter is not increased.
1: insert into old_relfilenodes
   (select gp_segment_id as dbid, relfilenode, oid, relname from gp_dist_random('pg_class')
    where relname like 'reindex_crtab_part_aoco_btree%_idx'
    union all
    select gp_segment_id as dbid, relfilenode, oid, relname from pg_class
    where relname like 'reindex_crtab_part_aoco_btree%_idx');
-- Expect two distinct relfilenodes for one segment in old_relfilenodes table.
1: select distinct count(distinct relfilenode), relname from old_relfilenodes group by dbid, relname;
2: commit;
-- After session 2 commits, session 1 should see that the relfilenode count for default-partition index increased.
1: insert into old_relfilenodes
   (select gp_segment_id as dbid, relfilenode, oid, relname from gp_dist_random('pg_class')
    where relname like 'reindex_crtab_part_aoco_btree%_idx'
    union all
    select gp_segment_id as dbid, relfilenode, oid, relname from pg_class
    where relname like 'reindex_crtab_part_aoco_btree%_idx');
1: select distinct count(distinct relfilenode), relname from old_relfilenodes group by dbid, relname;

select count(*) from reindex_crtab_part_aoco_btree where id = 998;
set enable_seqscan=false;
set enable_indexscan=true;
select count(*) from reindex_crtab_part_aoco_btree where id = 999;
