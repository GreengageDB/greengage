...
1: commit;
select gp_wait_until_triggered_fault('before_xlog_xact_prepare', 1, dbid)
  from gp_segment_configuration where role = 'p' and content = 0;
...
1: commit;
select gp_wait_until_triggered_fault('start_performDtxProtocolCommitOnePhase', 1, dbid)
  from gp_segment_configuration where role = 'p' and content = 0;
