...
-- pg_stat_file is volatile, so its function scan is pinned to the
-- coordinator; combined with EXECUTE ON MASTER the paths captured on
-- segno -1 are always resolved against the coordinator's data directory.
