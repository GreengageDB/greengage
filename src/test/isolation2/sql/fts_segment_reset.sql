...
1:select gp_inject_fault('backend_abort_handling', 'skip', dbid)
from gp_segment_configuration where role = 'p' and content = 0;
1&:select gp_wait_until_triggered_fault('backend_abort_handling', 1, dbid)
from gp_segment_configuration where role = 'p' and content = 0;
...
2:select gp_inject_fault('start_prepare', 'panic', dbid)
from gp_segment_configuration where role = 'p' AND content = 0;
2&:create table fts_reset_t(a int);
...
3&:create table fts_reset_t2(a int);
...
4:set gp_gang_creation_retry_timer = 10000;
4:create table fts_reset_t3(a int);
