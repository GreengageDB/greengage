...
def parse_include_statement(sql):
    ...
    if stripped_command.endswith(";"):
        ...
        raise SyntaxError(
            "expected 'include: %s' to end with a semicolon." % stripped_command
        )
...
class ConnectionInfo(object):
    ...
    def get_hostname_port(name, role_name):
        ...
        real_content_id = (
            content_id % max_content_id
            if content_id >= 0
            else content_id % (-max_content_id)
        )
        for content, host, port, role in conn_map:
            if real_content_id == content and role == role_name:
                return (host, port)
        raise Exception(
            "Cannot find a connection with content_id=%d, role=%c"
...
class GlobalShellExecutor(object):
    ...
    def terminate(self, with_error=False):
        ...
        if (
            not with_error
            and self.initfile_prefix != None
            and len(self.initfile_prefix) > 1
        ):
            output_init_file = "%s.initfile" % self.initfile_prefix
            cmd = (
                """ [ ! -z "${MATCHSUBS}" ] && echo "-- start_matchsubs ${NL} ${MATCHSUBS} ${NL}-- end_matchsubs" > %s """
                % output_init_file
            )
    ...
    def __run_command(self, sh_cmd):
        ...
        while self.sh_proc.poll() is None:
            ...
            if e:
                ...
                self.terminate(True)
                raise GlobalShellExecutor.ExecutionError(
                    "Error happened to the bash process, see %s for details."
                    % self.bash_log_file.name
                )
            ...
            if not r and not e:
                self.terminate(True)
                raise GlobalShellExecutor.ExecutionError(
                    "Timeout happened to the bash process, see %s for details."
                    % self.bash_log_file.name
                )

        self.terminate(True)
        raise GlobalShellExecutor.ExecutionError(
            "Bash process has been stopped, see %s for details."
            % self.bash_log_file.name
        )
    ...
    def exec_global_shell(self, sh_cmd, is_trip_output_end_blanklines):
        if self.sh_proc == None:
            raise GlobalShellExecutor.ExecutionError(
                "The bash process has been terminated abnormally, see %s for details."
                % self.bash_log_file.name
            )
    ...
    def exec_global_shell_with_orig_str(
        self, input, sh_cmd, is_trip_output_end_blanklines
    ):
        self.v_cnt = 1 + self.v_cnt
        escape_in = input.replace("'", "'\\''")
        # send shell cmd and set the temp RAW_STR
        cmd = (
            """ export RAW_STR%d='%s' && export RAW_STR=$RAW_STR%d && %s ; unset RAW_STR """
...
class SQLIsolationExecutor(object):
    ...
    class SQLConnection(object):
        def __init__(self, out_file, name, mode, dbname, user=None, passwd=None):
            ...
            self.p = multiprocessing.Process(
                target=self.session_process, args=(child_conn,)
            )
        ...
        def session_process(self, pipe):
            sp = SQLIsolationExecutor.SQLSessionProcess(
                self.name,
                self.mode,
                pipe,
                self.dbname,
        ...
        def query(self, command, post_run_cmd=None, global_sh_executor=None):
            ...
            if post_run_cmd != None:
                new_out = global_sh_executor.exec_global_shell_with_orig_str(
                    r.rstrip(), post_run_cmd, True
                )
                for line in new_out:
        ...
        def fork(self, command, blocking, global_sh_executor):
            ...
            if blocking:
                ...
                if self.pipe.poll(0):
                    p = self.pipe.recv()
                    raise Exception(
                        "Forked command is not blocking; got output: %s" % p.strip()
                    )
            self.has_open = True
    ...
    class SQLSessionProcess(object):
        ...
        def connectdb(
            ...
            while retry:
                try:
                    ...
                    if self.mode == "retrieve" and (
                        "auth token is invalid" in str(e)
                        or "Authentication failure" in str(e)
                        or "does not exist" in str(e)
                    ):
                        self.create_exception = e
                        break
                    elif (
                        "the database system is starting up" in str(e)
                        or "the database system is resetting" in str(e)
                        or "the database system is in recovery mode" in str(e)
                    ) and retry > 1:
                        retry -= 1
                        time.sleep(0.1)
                    else:
        ...
        def get_hostname_port(self, contentid, role):
            ...
            """
            query = (
                "SELECT hostname, port FROM gp_segment_configuration WHERE"
                " content = %s AND role = '%s'"
            ) % (contentid, role)
        ...
        def do(self):
            ...
            while c:
                ...
                if self.create_exception:
                    ...
                    self.pipe.send(
                        "#%s%s> %s" % (self.name, self.mode, str(self.create_exception))
                    )
                else:
    ...
    def get_process(self, out_file, name, mode="", dbname="", user=None, passwd=None):
        ...
        if (name, mode) not in self.processes:
            ...
            self.processes[(name, mode)] = SQLIsolationExecutor.SQLConnection(
                out_file, name, mode, dbname, user, passwd
            )
    ...
    def get_all_primary_contentids(self, dbname):
        ...
        result = con.query(
            "SELECT content FROM gp_segment_configuration WHERE role = 'p' order by content"
        ).getresult()
        if len(result) == 0:
    ...
    def __preprocess_sql(self, name, pre_run_cmd, sql, global_sh_executor):
        ...
        sqls = global_sh_executor.exec_global_shell_with_orig_str(
            sql, pre_run_cmd, True
        )
        if len(sqls) != 1:
    ...
    def process_command(self, command, output_file, global_sh_executor):
        ...
        if m:
            ...
            if sql.startswith("@db_name"):
                ...
                if not len(sql_parts) == 2:
                    raise Exception(
                        "Invalid syntax with dbname, should be of the form 1:@db_name <db_name>: <sql>"
                    )
                ...
                (found_hd, pre_run_cmd, ex_sql) = global_sh_executor.extract_sh_cmd(
                    "@pre_run", sql
                )
                if found_hd:
                    sql = ex_sql
                else:
                    (found_hd, post_run_cmd, ex_sql) = (
                        global_sh_executor.extract_sh_cmd("@post_run", sql)
                    )
                    if found_hd:
                        sql = ex_sql
        ...
        if not flag:
            if sql.startswith("!"):
                ...
                if not bg_mode:
                    ...
                    if mode == "\\retcode":
                        ...
                        print(
                            "(exited with code {})".format(cmd_output.returncode),
                            file=output_file,
                        )
            elif sql.startswith("include:"):
                helper_file = parse_include_statement(sql)

                self.get_process(output_file, process_name, dbname=dbname).query(
                    load_helper_file(helper_file)
                )
            else:
                sql_new = self.__preprocess_sql(
                    process_name, pre_run_cmd, sql.strip(), global_sh_executor
                )
                self.get_process(
                    output_file, process_name, con_mode, dbname=dbname
                ).query(sql_new, post_run_cmd, global_sh_executor)
        elif flag == "&":
            self.get_process(output_file, process_name, con_mode, dbname=dbname).fork(
                sql.strip(), True, global_sh_executor
            )
        elif flag == ">":
            self.get_process(output_file, process_name, con_mode, dbname=dbname).fork(
                sql.strip(), False, global_sh_executor
            )
        elif flag == "<":
            if len(sql) > 0:
            ...
            if process_name == "*":
                process_names = [
                    str(content) for content in self.get_all_primary_contentids(dbname)
                ]
            else:
            ...
            for name in process_names:
                sql_new = self.__preprocess_sql(
                    name, pre_run_cmd, sql.strip(), global_sh_executor
                )
                self.get_process(output_file, name, con_mode, dbname=dbname).query(
                    sql_new, post_run_cmd, global_sh_executor
                )
        elif flag == "U&":
            sql_new = self.__preprocess_sql(
                process_name, pre_run_cmd, sql.strip(), global_sh_executor
            ...
            self.get_process(output_file, process_name, con_mode, dbname=dbname).fork(
                sql_new, True, global_sh_executor
            )
        elif flag == "U<":
            if len(sql) > 0:
                raise Exception("No query should be given on join")
            ...
            self.get_process(output_file, process_name, con_mode, dbname=dbname).query(
                sql_new, post_run_cmd, global_sh_executor
            )
        elif flag == "R":
            if process_name == "*":
            ...
            for name in process_names:
                try:
                    ...
                    (retrieve_user, retrieve_token) = self.__get_retrieve_user_token(
                        name, global_sh_executor
                    )
                    self.get_process(
                        output_file,
                        name,
            ...
            (retrieve_user, retrieve_token) = self.__get_retrieve_user_token(
                process_name, global_sh_executor
            )
            self.get_process(
                output_file,
                process_name,
                ...
                user=retrieve_user,
                passwd=retrieve_token,
            ).fork(sql_new, True, global_sh_executor)
        elif flag == "R<":
            if len(sql) > 0:
                raise Exception("No query should be given on join")
            (retrieve_user, retrieve_token) = self.__get_retrieve_user_token(
            ...
            self.get_process(output_file, process_name, con_mode, dbname=dbname).query(
                sql.strip(), post_run_cmd, global_sh_executor
            )
        elif flag == "Mq":
            if len(sql) > 0:
        ...
        try:
            ...
            for line in sql_file:
                ...
                if command_part == "" or command_part == "\n":
                    ...
                    re.match(r".*;\s*$", command_part)
                    or re.match(r"^\d+[qt\\<]:\s*$", line)
                    or re.match(r"^\*R[qt]:$", line)
                    or re.match(r"^-?\d+[SUMR][qt\\<]:\s*$", line)
                ):
    ...
    *U: UPDATE pg_catalog.<table> SET <column> = <value> WHERE <cond>;
    ...
    def run_sql_file(self, sql_file, out_file=None, out_dir=None, optimizer=None):
        ...
        if optimizer is None:
            ...
            gucs_sql_file = os.path.join(
                out_dir,
                os.path.basename(sql_file).replace(
...
if __name__ == "__main__":
    parser = OptionParser()
    parser.add_option(
        "--dbname", dest="dbname", help="connect to database DBNAME", metavar="DBNAME"
    )
    parser.add_option(
        "--initfile_prefix",
        dest="initfile_prefix",
        help="The file path prefix for automatically generated initfile",
        metavar="INITFILE_PREFIX",
    )
