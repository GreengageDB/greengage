...
begin
    ...
    loop
        -- the GGDB per-slice "Executor Memory" line is emitted only on some runs;
        -- drop it so the row count is deterministic
        continue when ln ~ 'Executor Memory';
...
-- start_ignore
-- GPDB: lateral PlaceHolderVar under nullable outer join has no MPP motion plan (deferred)
explain (verbose, costs off)
select t1.q1, x from
  int8_tbl t1 left join
  (int8_tbl t2 left join
