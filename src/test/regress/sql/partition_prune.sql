...
-- s/segment \d+/segment ###/
-- m/->  Partial Aggregate \(/
-- s/actual rows=\d+ loops=\d+/actual rows=N loops=N/
-- m/->  Seq Scan on lprt_a/
-- s/actual rows=\d+ loops=\d+/actual rows=N loops=N/
-- m/(Partial Aggregate|Seq Scan on lprt_a).* \(never executed\)/
-- s/\(never executed\)/(actual rows=N loops=N)/
-- end_matchsubs
-- start_matchignore
-- m/^ +Partitions scanned:/
-- m/^ +Extra Text: /
-- m/^ +Executor Memory: /
-- end_matchignore
