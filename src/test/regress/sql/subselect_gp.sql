...
-- A window function in the subquery is evaluated over the aggregate's
-- single-row result: no join row can carry the value a no-match row would
-- see, and a WindowFunc cannot be evaluated outside the subquery, so the
-- pull-up must bail out and the sublink runs as a SubPlan.
set optimizer=off;

explain (costs off) select * from t_out
where a > (select count(*) + count(*) over () from t_in where t_in.a = t_out.a);
...
-- Substituting 0 for count(*) must not turn 1/count(*) into a constant that
-- the planner folds at plan time: "1/0" raises "division by zero" while
-- planning, even for a query that returns no rows at all. The pull-up must
-- bail out and the sublink runs as a SubPlan.
set optimizer=off;

create table t_empty(a int) distributed by (a);

explain (costs off) select * from t_empty
where a > (select 1/count(*) from t_in where t_in.a = t_empty.a);
