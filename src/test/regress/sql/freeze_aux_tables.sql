...
declare
  rec rel_ages;
begin
  for rec in select -1 gp_segment_id, replace(relname, testrelid::oid::text, '<oid>'), age(relfrozenxid)
    from pg_class c join pg_am a on a.oid = relam
    where relkind in ('r','t','o','b','M') and amname in ('heap', 'ao_row', 'ao_column')
    and (relname like '%\_' || testrelid::oid or c.oid = testrelid::oid )
    and not relfrozenxid = 0
  loop
    return next rec;
  end loop;

  for rec in select c.gp_segment_id, replace(relname, testrelid::oid::text, '<oid>'), age(relfrozenxid)
    from gp_dist_random('pg_class') c join pg_am a on a.oid = relam
    where relkind in ('r','t','o','b','M') and amname in ('heap', 'ao_row', 'ao_column')
    and (relname like '%\_' || testrelid::oid or c.oid = testrelid::oid )
    and not relfrozenxid = 0
  loop
