...
class CFile(object):
    ...
    attribute_pat = re.compile(
        r"__attribute__\s*\(\((format\s*\([^\)]+\)\s*|format_arg\s*\(\d+\)\s*|.+?)\)\)"
    )
    # #include <filename>.c
...
class FuncSignature(object):
    ...
    def make_body(self):
        ...
        for arg in self.args:
            ...
            argname = subscript.sub("", argname)
            buf.append(
                "\toptional_assignment({ref}{arg});".format(ref=ref, arg=argname)
            )
        ...
        if special.ByValStructs.has(self.rettype):
            ret = (
                "\t{rettype} *ret = ({rettype} *) mock();\n" + "\treturn *ret;"
            ).format(rettype=self.rettype)
...
def main():
    ...
    try:
        ...
        parser.add_option("--out-dir", dest="out_dir", default=os.path.join(mydir, "."))
        parser.add_option(
            "--src-dir", dest="src_dir", default=os.path.join(mydir, "../../..")
        )
