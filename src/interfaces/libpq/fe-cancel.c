 ...
 *
 * GGDB: internal_cancel is the shared worker behind PQcancel and
 * PQrequestFinish.  When requestFinish is true it sends FINISH_REQUEST_CODE
 * (a graceful query-finish request) instead of CANCEL_REQUEST_CODE.
 */
static int
...
retry3:
	...
	/*
	 * GGDB: the cancel request packet was pre-built by PQgetCancel with
	 * CANCEL_REQUEST_CODE.  When this is a graceful query-finish request,
	 * override the request code in place before sending it.
	 */
	if (requestFinish)
	{
		CancelRequestPacket *req = (CancelRequestPacket *) &cancel->cancel_req;

		req->cancelRequestCode = (MsgType) pg_hton32(FINISH_REQUEST_CODE);
	}
...
retry5:
	...
	/*
	 * GGDB: Wait for at most 660 seconds, which is the sum of max values of
	 * authentication_timeout and pre_auth_delay. Most likely, it's long
	 * enough to make sure the process forked by the postmaster on a segment
	 * is finished.
	 */
	pollRet = poll(pollFds, 1, 660 * 1000);
	if (pollRet == 0)
	{
		/* timeout - reopen the connection and resend the request */
		closesocket(tmpsock);
		tmpsock = PGINVALID_SOCKET;
		goto retry2;
	}
	else if (pollRet < 0)
	{
		int			olderrno = SOCK_ERRNO;

		if (olderrno == EAGAIN || olderrno == EINTR)
			goto retry5;

		/* error - reopen the connection and resend the request */
		closesocket(tmpsock);
		tmpsock = PGINVALID_SOCKET;
		goto retry2;
	}
...
PQcancel(PGcancel *cancel, char *errbuf, int errbufsize)
{
	...
	return internal_cancel(cancel, errbuf, errbufsize, false);
}

/*
 * PQrequestFinish: request query finish
 *
 * GGDB: same as PQcancel, except it sends a finish request instead of a
 * cancel request.
 */
int
PQrequestFinish(PGcancel *cancel, char *errbuf, int errbufsize)
{
	...
	return internal_cancel(cancel, errbuf, errbufsize, true);
}
