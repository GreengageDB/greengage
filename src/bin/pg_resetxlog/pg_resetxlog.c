...
main(int argc, char *argv[])
{
	...
	if (argc > 1)
	{
		...
		{
			puts("pg_resetxlog (" GP_PRODUCT_NAME ") " PG_VERSION);
			exit(0);
		}
		if (strcmp(argv[1], "--gp-version") == 0)
		{
			puts("pg_resetxlog (" GP_PRODUCT_NAME ") " GP_VERSION);
			exit(0);
		}
	}


	while ((c = getopt_long(argc, argv, "fl:m:no:r:O:u:x:e:k:", long_options, NULL)) != -1)
	{
		switch (c)
		{
			case 'f':
