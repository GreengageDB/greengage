...
check_for_data_types_usage(ClusterInfo *cluster)
{
	...
	/*
	 * GPDB: use gp_fatal_log() rather than pg_fatal() so that
	 * --continue-check-on-fatal can keep running the remaining checks instead
	 * of aborting on the first failing data type check.
	 */
	if (report)
	{
		pg_log(PG_REPORT, "fatal");
		gp_fatal_log("Data type checks failed: %s", report->data);
...
void
check_and_dump_old_cluster(char **sequence_script_file_name)
{
	/* -- OLD -- */
...
check_cluster_versions(void)
{
	...
	/*
	 * Upgrading from anything older than an 8.3 based Greengage (GPDB5) is not supported.
	 */

	if (GET_MAJOR_VERSION(old_cluster.major_version) < 803)
		pg_fatal("This utility can only upgrade from Greengage version 5 and later.");
...
check_cluster_compatibility(void)
{
	...
	if (user_opts.live_check && GET_MAJOR_VERSION(old_cluster.major_version) < 901 &&
...
check_for_incompatible_polymorphics(ClusterInfo *cluster)
{
	...
	if (report.file)
	{
		fclose(report.file);
		pg_log(PG_REPORT, "fatal");
		gp_fatal_log("Your installation contains user-defined objects that refer to internal\n"
...
check_for_appendonly_materialized_view_with_relfrozenxid(ClusterInfo *cluster)
{
	...
	for (int dbnum = 0; dbnum < cluster->dbarr.ndbs; dbnum++)
	{
		...
		if (conn == NULL)
		{
			pg_fatal("Failed to connect to database %s", active_db->db_name);
		}
