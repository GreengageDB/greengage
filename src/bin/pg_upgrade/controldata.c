				   ...
				   ControlData *newctrl)
{
	...
	/*
	 * GPDB: the upstream TOAST chunk size check is intentionally omitted.
	 * pg_controldata only reports the maximum TOAST chunk size in PostgreSQL
	 * 8.4 and later, and GPDB still supports upgrading from GPDB 5 (based on
	 * PostgreSQL 8.3), so the value may be missing in the old cluster.  This
	 * is the comment referred to by the "not mandatory in GPDB" #if 0 block
	 * in get_control_data().
	 */
