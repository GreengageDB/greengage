			  ...
			  InhInfo *inhinfo, int numInherits)
{
	...
	for (i = 0; i < numInherits; i++)
	{
		...
		/*
		 * GPDB: FIXME: In PostgreSQL, foreign tables can be inherited. But
		 * pg_dump chokes on external tables, if an external table is used
		 * as a partition, and a column has attislocal=false.  Don't set up
		 * parent links for external tables, so that they stay out of the
		 * inheritance machinery (as the old per-table code did).
		 */
		if (child->relstorage == 'x' /* RELSTORAGE_EXTERNAL */)
			continue;
	...
	for (i = 0; i < numTables; i++)
	{
		 ...
		 * used as a partition, and a column has attislocal=false.  External
		 * tables have no parent links (see above), so skip them here too:
		 * in particular, don't create a TableAttachInfo for an external
		 * partition, which would otherwise hit the invalid-number-of-parents
		 * error below.
		 */
