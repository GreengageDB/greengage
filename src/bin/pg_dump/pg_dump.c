...
dumpDatabase(Archive *fout)
{
	 ...
	 *
	 * GPDB: unlike upstream, we do not preserve relfilenodes across the
	 * upgrade (the binary_upgrade_set_next_*_relfilenode functions are not
	 * exposed in the GPDB catalog); OIDs are preserved through the
	 * preassigned-OID mechanism instead.
	 */
	if (dopt->binary_upgrade)
	{
		PGresult   *lo_res;
										 ...
										 bool include_multirange_type)
{
	PQExpBuffer upgrade_query = createPQExpBuffer();
	PGresult   *res;
	TypeInfo   *tyinfo = findTypeByOid(pg_type_oid);
	Oid			pg_type_array_oid;
	Oid			pg_type_array_ns_oid;
	char	   *pg_type_array_name;
	Oid			pg_type_multirange_oid;
	Oid			pg_type_multirange_array_oid;

	/*
	 * GPDB: the binary-upgrade preassignment functions take the namespace
	 * and object name in addition to the OID, so that the QD can dispatch
	 * the preassigned OIDs to the QEs; we also track every preassigned OID
	 * in preassigned_oids.  Take them from the TypeInfo gathered by
	 * getTypes() (which also covers table rowtypes).
	 */
	if (tyinfo == NULL)
		pg_fatal("could not find type with OID %u for binary upgrade", pg_type_oid);

	pg_type_array_oid = tyinfo->typarrayoid;
	pg_type_array_ns_oid = tyinfo->typarrayns;
	pg_type_array_name = tyinfo->typarrayname;
								 ...
								 PQExpBuffer upgrade_buffer, Oid pg_class_oid)
{
	...
	/*
	 * GPDB: the binary-upgrade preassignment functions take the namespace and
	 * object name in addition to the OID, so that the QD can dispatch the
	 * preassigned OIDs to the QEs, and we track every preassigned OID in
	 * preassigned_oids.  Unlike upstream we do not preserve relfilenodes here;
	 * the segments assign those on their own.  We still use the pre-collected
	 * class table to discover the relkind without a per-object query.
	 */
...
getTables(Archive *fout, int *numTables)
{
	...
	*numTables = ntups;
	...
	for (i = 0; i < ntups; i++)
	{
		 ...
		 * pg_dump.h
		 *
		 * GPDB: Lock all interesting tables using batched LOCK TABLE statements.
...
getTableAttrs(Archive *fout, TableInfo *tblinfo, int numTables)
{
	...
	/* GPDB: per-column ENCODING (compresstype/level/blocksize) for AO/CO tables */
	appendPQExpBufferStr(q,
...
dumpExternal(Archive *fout, const TableInfo *tbinfo, PQExpBuffer q, PQExpBuffer delq)
{
		...
		if (dopt->binary_upgrade)
			binary_upgrade_set_pg_class_oids(fout, q, tbinfo->dobj.catId.oid);
			...
			if (tbinfo->parparent && fout->remoteVersion < 100000)
			{
				...
				if (PQntuples(partres) > 0)
				{
					...
					for (i = 0; i < PQntuples(partres); i++)
					{
						...
						binary_upgrade_set_pg_class_oids(fout, q, part_oid);
						binary_upgrade_set_type_oids_by_rel(fout, q, tbinfo);
					}
				}
 ...
 *
 * Note that the result is cached and is a palloc'd copy that the caller
 * should free.
 */
static char *
