...
static const char *const backend_options = "--single -F -O -j -c gp_role=utility -c search_path=pg_catalog -c exit_on_error=true -c log_checkpoints=false";
...
#define AV_SLOTS_FOR_CONNS(nconns)	((nconns) / 6)
	...
	for (i = 0; i < connslen; i++)
	{
		...
		{
			pg_log_error("could not start a server process with max_connections=%d and shared_buffers=%d",
	...
	if (i == bufslen)
	{
		pg_log_error("could not select a suitable value for shared_buffers");
		exit(1);
	}
...
make_postgres(FILE *cmdfd)
{
	...
	/*
	 * GPDB: Make 'postgres' a template database
	 */
	...
	/*
	 * GPDB: Clean out dead rows in pg_database
	 */
