...
class Flow:
    FLOW_UNDEFINED = gdb.parse_and_eval(
        "FLOW_UNDEFINED"
    )  # used prior to calculation of type of derived flow
    FLOW_SINGLETON = gdb.parse_and_eval("FLOW_SINGLETON")  # flow has single stream
    FLOW_REPLICATED = gdb.parse_and_eval(
        "FLOW_REPLICATED"
    )  # flow is replicated across IOPs
    FLOW_PARTITIONED = gdb.parse_and_eval(
        "FLOW_PARTITIONED"
    )  # flow is partitioned across IOPs
...
def show_dispatch_info(slice, plan, pstmt):
    ...
    segments = 0
    if (
        typ == Gang.GANGTYPE_PRIMARY_WRITER
        or typ == Gang.GANGTYPE_PRIMARY_READER
        or typ == Gang.GANGTYPE_SINGLETON_READER
    ):
            ...
            while True:
                ...
                if tag == "T_Append":
                    ...
                    if aplan["appendplans"] != 0:
                        fplan = List.list_nth(aplan["appendplans"], 0).cast(
                            Plan.gdb_type
                        )
                        continue
...
class Motion(object):
    ...
    # motion types
    MOTIONTYPE_HASH = gdb.parse_and_eval(
        "MOTIONTYPE_HASH"
    )  # Use hashing to select a segindex destination
    MOTIONTYPE_FIXED = gdb.parse_and_eval(
        "MOTIONTYPE_FIXED"
    )  # Send tuples to a fixed set of segindexes
    MOTIONTYPE_EXPLICIT = gdb.parse_and_eval(
        "MOTIONTYPE_EXPLICIT"
    )  # Send tuples to the segment explicitly specified in their segid column
    ...
    def print_node(self):
        ...
        if typ == Motion.MOTIONTYPE_HASH:
            ...
            if bool(self.__mot["isBroadcast"]) == True:
                ...
                self.__plan["lefttree"]["flow"]["locustype"]
                == LocusType.CdbLocusType_Replicated
            ):
        ...
        if self.__pstmt["planGen"] == PLANGEN_PLANNER:
            ...
            if (
                self.__mot["motionType"] == Motion.MOTIONTYPE_FIXED
                and bool(self.__mot["isBroadcast"]) == False
            ):
        ...
        return "%s %d:%d %s" % (
            ...
            motion_snd,
            motion_recv,
            show_dispatch_info(self.__currentSlice, self.__plan, self.__pstmt),
        )
...
class ShareInputScan(object):
    ...
    def print_node(self):
        ...
        return "ShareInputScan (share slice:id %d:%d)" % (
            slice_id,
            int(self.__sics["share_id"]),
        )
...
class PartitionSelector(object):
    ...
    def print_node(self):
        ...
        return "PartitionSelector for %s (dynamic scan id: %d)" % (
            relname,
            int(self._ps["scanId"]),
        )
...
class PlanDump(gdb.Command):
    ...
    def __init__(self):
        super(PlanDump, self).__init__(
            "plan_dump", gdb.COMMAND_USER, gdb.COMPLETE_SYMBOL
        )
        self.__cleanup__()
    ...
    def walk_initplans(self, plans, sliceTable):
        ...
        while head != 0:
            ...
            if sliceTable != 0 and sp["qDispSliceId"] > 0:
                self.__currentSlice = List.list_nth(
                    sliceTable["slices"], sp["qDispSliceId"]
                ).cast(Slice.gdb_type)

            self.__curPlanName = sp["plan_name"]
            sp_plan = List.list_nth(
                self.__pstmt["subplans"], int(sp["plan_id"]) - 1
            ).cast(Plan.gdb_type)
    ...
    def walk_node(self, nodePtr):
        ...
        if nodeTag == "DML" or nodeTag == "ModifyTable":
            ...
            if nodeTag == "ModifyTable":
                rti = int(
                    planPtr.cast(ModifyTable.gdb_type)["resultRelations"]["head"][
                        "data"
                    ]["int_value"]
                )
            ...
            self.__currentSlice = List.list_nth(
                slices, int(planPtr.cast(Motion.gdb_type)["motionID"])
            ).cast(Slice.gdb_type)
            nodeStr = Motion(
                planPtr, self.__state, self.__pstmt, self.__currentSlice
            ).print_node()
            ...
            nodeStr = Scan(planPtr, self.__rtableMap, nodeTag).print_node(
                planPtr.cast(IndexScan.gdb_type)["indexid"]
            )
        elif nodeTag == "IndexOnlyScan":
            nodeStr = Scan(planPtr, self.__rtableMap, nodeTag).print_node(
                planPtr.cast(IndexOnlyScan.gdb_type)["indexid"]
            )
        elif nodeTag == "BitmapIndexScan":
            nodeStr = Scan(planPtr, self.__rtableMap, nodeTag).print_node(
                planPtr.cast(BitmapIndexScan.gdb_type)["indexid"]
            )
        elif nodeTag == "DynamicBitmapIndexScan":
            nodeStr = Scan(planPtr, self.__rtableMap, nodeTag).print_node(
                planPtr.cast(DynamicBitmapIndexScan.gdb_type)["indexid"]
            )
        ...
        if self.__curPlanName is not None:
            ...
            self.__result += "\t" * (self.__tabCnt - 1) + (
                "%s (%s)\n"
                % (
                    planName,
                    show_dispatch_info(self.__currentSlice, planPtr, self.__pstmt),
                )
            )
            self.__curPlanName = None
    ...
    def invoke(self, args, from_tty):
        ...
        self.__rtableMap = {}
        self.__reloidMap = {}
        rtableIter = self.__pstmt["rtable"][
            "head"
