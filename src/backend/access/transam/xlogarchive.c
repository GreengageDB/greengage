					   ...
					   bool failOnSignal, uint32 wait_event_info)
{
	 ...
	 *
	 * GPDB: in addition to upstream's %r (filename of last restartpoint), we
	 * support %c (contentId of segment).  Both values are supplied to the
	 * placeholder-replacement helper below.
	 */
	Assert(GpIdentity.segindex != UNINITIALIZED_GP_IDENTITY_VALUE);
	pg_ltoa(GpIdentity.segindex, contentid);
	xlogRecoveryCmd = replace_percent_placeholders(command, commandName, "rc",
												   lastRestartPointFname, contentid);
