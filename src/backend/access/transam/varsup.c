...
GetNewTransactionId(bool isSubXact)
{
	...
	if (Debug_burn_xids)
	{
		...
		xx = U64FromFullTransactionId(TransamVariables->nextXid);

		r = xx % page_extend_limit;
		if (r > 1 && r < (page_extend_limit - 1))
		{
			xx += page_extend_limit - r - 1;
			TransamVariables->nextXid.value = xx;
		}
	}
...
AdvanceObjectId(Oid newOid)
{
	...
	{
		int32 nextOidDifference = (int32)(newOid - TransamVariables->nextOid);
		 ...
		 * and never backwards.
		 */
		TransamVariables->nextOid = newOid;
		if (nextOidDifference >= TransamVariables->oidCount)
		{
			XLogPutNextOid(TransamVariables->nextOid + VAR_OID_PREFETCH);
			TransamVariables->oidCount = VAR_OID_PREFETCH;
		}
		else
			TransamVariables->oidCount -= nextOidDifference;
	}
	LWLockRelease(OidGenLock);
}
...
GetNewSegRelfilenodeUnderLock(void)
{
	...
	if (TransamVariables->nextRelfilenode < ((Oid) FirstNormalObjectId) &&
		IsPostmasterEnvironment)
	{
		/* wraparound in normal environment */
		TransamVariables->nextRelfilenode = FirstNormalObjectId;
		TransamVariables->relfilenodeCount = 0;
	}

	if (TransamVariables->relfilenodeCount == 0)
	{
		XLogPutNextRelfilenode(TransamVariables->nextRelfilenode + VAR_OID_PREFETCH);
		TransamVariables->relfilenodeCount = VAR_OID_PREFETCH;
	}

	result = TransamVariables->nextRelfilenode;

	(TransamVariables->nextRelfilenode)++;
	(TransamVariables->relfilenodeCount)--;
...
/*
 * Is the given Oid logically > TransamVariables->nextOid?
 */
bool
OidFollowsNextOid(Oid id)
{
	...
	diff = (int32) (id - TransamVariables->nextOid);
	return (diff > 0);
}
