					   ...
					   Buffer recent_buffer)
{
	 ...
	 */
	smgr = smgropen(rlocator, INVALID_PROC_NUMBER, SMGR_MD);
...
{
	/*
	 * GPDB: AO WAL records carry a RelFileNode; PG16's log_invalid_page() now
	 * takes a RelFileLocator (layout-identical struct), so convert field-by-
	 * field.
	 */
	RelFileLocator rlocator;
	...
	log_invalid_page(rlocator, MAIN_FORKNUM, segmentFileNum, false);
}
...
CreateFakeRelcacheEntry(RelFileLocator rlocator)
{
	 ...
	 */
	rel->rd_smgr = smgropen(rlocator, INVALID_PROC_NUMBER, SMGR_MD);
