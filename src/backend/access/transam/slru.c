 ...
 */
static void
SimpleLruTruncate_internal(SlruCtl ctl, int64 cutoffPage)
{
	SlruShared	shared = ctl->shared;
	int			prevbank;
...
{
	SimpleLruTruncate_internal(ctl, cutoffPage);
}

/*
 * Like SimpleLruTruncate, but retained for GGDB callers (e.g. distributedlog.c)
 * that already serialize truncation with a higher-level lock.  Under PG17 the
 * SLRU acquires its per-bank locks internally, so this is equivalent to
 * SimpleLruTruncate (there is no longer a single per-SLRU control lock).
 */
void
SimpleLruTruncateWithLock(SlruCtl ctl, int64 cutoffPage)
{
	SimpleLruTruncate_internal(ctl, cutoffPage);
}
