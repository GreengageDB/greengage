...
int			min_wal_size_mb = 320;	/* 320 MB (5 * 64MB segs, matches GUC boot_val) */
...
static XLogRecPtr CreateOverwriteContrecordRecord(XLogRecPtr aborted_lsn,
												  XLogRecPtr pagePtr,
												  TimeLineID newTLI);
static void UpdateCatalogForStandbyPromotion(void);
...
UpdateCatalogForStandbyPromotion(void)
{
	 ...
	 * per-backend data.
	 *
	 * MyProcNumber was already set up by InitAuxiliaryProcess.  Though this is
	 * a startup process and currently no one sends invalidation messages
	 * concurrently, we set sendOnly = false, since we have relcaches.
	 * SharedInvalBackendInit() validates MyProcNumber internally.
	 */
...
StartupXLOG(void)
{
	...
	if (ArchiveRecoveryRequested)
	{
		...
		/*
		 * Response to FTS probes after this point will not indicate that we
		 * are a mirror because the am_mirror flag is set based on existence of
		 * RECOVERY_COMMAND_FILE.  New libpq connections to the postmaster
		 * should no longer return CAC_MIRROR_READY as response because we are
		 * no longer a mirror.
		 */
	...
	if (IsNormalProcessingMode())
		elog(LOG, "latest completed transaction id is %lu and next transaction id is %u",
			 TransamVariables->latestCompletedXid.value,
...
CreateCheckPoint(int flags)
{
	 ...
	 * the TransamVariables->GxidCount variable. For the crash recovery case,
	...
	checkPoint.nextGxid = TransamVariables->nextGxid;
	if (!shutdown)
		checkPoint.nextGxid += TransamVariables->GxidCount;
	...
	checkPoint.nextRelfilenode = TransamVariables->nextRelfilenode;
	if (!shutdown)
		checkPoint.nextRelfilenode += TransamVariables->relfilenodeCount;
...
xlog_redo(XLogReaderState *record)
{
	...
	else if (info == XLOG_NEXTGXID)
	{
		...
		SpinLockAcquire(shmGxidGenLock);
		TransamVariables->nextGxid = nextGxid;
		TransamVariables->GxidCount = 0;
		SpinLockRelease(shmGxidGenLock);
	}
	else if (info == XLOG_NEXTRELFILENODE)
	{
		...
		LWLockAcquire(OidGenLock, LW_EXCLUSIVE);
		TransamVariables->nextRelfilenode = nextRelfilenode;
		TransamVariables->relfilenodeCount = 0;
		LWLockRelease(OidGenLock);
	}
				   ...
				   BackupState *state, StringInfo tblspcmapfile)
{
	...
	/*
	 * GPDB: log the backup label file contents for debugging.  The label
	 * file itself is built later from the backup state (see
	 * build_backup_content()), so reconstruct it here for the log message.
	 */
	if (debug_basebackup)
	{
		char	   *labelfile = build_backup_content(state, false);
...
do_pg_backup_stop(BackupState *state, bool waitforarchive)
{
	...
	else
	{
		...
		/* GPDB: log the stop point location for debugging. */
		elog(LOG, "Basebackup stop point is at %X/%X.",
			 LSN_FORMAT_ARGS(state->stoppoint));
