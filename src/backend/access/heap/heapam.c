...
static TM_Result heap_update_internal(Relation relation, ItemPointer otid, HeapTuple newtup,
									  ...
									  TU_UpdateIndexes *update_indexes, bool simple);
					...
					bool all_visible, bool check_serializable)
{
	...
	for (lineoff = FirstOffsetNumber; lineoff <= lines; lineoff++)
	{
		...
		theader = (HeapTupleHeader) PageGetItem(page, lpp);
		...
		{
			/*
			 * GPDB: We have a one-item cache for the common case that a lot
			 * of tuples have the same visibility info. Don't use the cache,
			 * if the tuple was ever deleted, though (i.e. if xmax is valid,
			 * and not just for tuple-locking). We could cache the xmax too,
			 * but the visibility rules get more complicated with locked-only
			 * tuples and multi-XIDs, so it seems better to just give up
			 * early.
			 */
			bool		use_cache;
