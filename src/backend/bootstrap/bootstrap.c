...
BootstrapModeMain(int argc, char *argv[], bool check_only)
{
	...
	/*
	 * MyBackendType is left as B_STANDALONE_BACKEND (set by
	 * InitStandaloneProcess above).  In PG16 the MyAuxProcType->MyBackendType
	 * mapping for real auxiliary processes lives in AuxiliaryProcessMain()
	 * (postmaster/auxprocess.c); BootstrapModeMain() is only reached for the
	 * standalone bootstrap/--check process, where MyAuxProcType is
	 * NotAnAuxProcess.  Overwriting MyBackendType here (with B_INVALID) would
	 * make the bootstrap process untracked by the pgstat IO subsystem and trip
	 * the pgstat_tracks_io_op() assertion during catalog creation.
	 */
