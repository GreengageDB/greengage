...
apply_handle_truncate(StringInfo s)
{
	 ...
	 * replication actions as the subscription owner; the run_as_table_owner
	 * argument of TruncateGuts tells it whether we want to switch to the
	 * table owner.  Those are exactly opposite conditions.
	 */
...
subxact_info_write(Oid subid, TransactionId xid)
{
	...
	fd = BufFileOpenFileSet(MyLogicalRepWorker->stream_fileset, path, O_RDWR,
							true);
	if (fd == NULL)
		fd = BufFileCreateFileSet(MyLogicalRepWorker->stream_fileset, path, NULL);
