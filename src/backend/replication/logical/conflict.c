...
static void errcode_apply_conflict(ConflictType type);
...
{
	/*
	 * GPDB: errcode() returns void here (unlike upstream's int), so this
	 * helper sets the SQLSTATE as a side effect and is void; it is invoked in
	 * the ereport() auxiliary-call list where the value would be discarded
	 * anyway.
	 */
	switch (type)
	{
