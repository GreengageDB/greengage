...
set_ps_display_with_len(const char *activity, size_t len)
{
	...
	cp = ps_buffer + ps_buffer_fixed_size;
	ep = ps_buffer + ps_buffer_size;

	Assert(cp >= ps_buffer);

	/* GPDB: Add client session's global id. */
	...
	/*
	 * GPDB: Calculate the size preceding the actual activity string start.
	...
	/*
	 * Update ps_buffer to contain both the fixed part (including the GPDB
	 * "conN segN cmdN sliceN" prefix) and the activity.
	 */
	if (real_act_prefix_size + len >= ps_buffer_size)
	{
		/* handle the case where ps_buffer doesn't have enough space */
		memcpy(ps_buffer + real_act_prefix_size, activity,
			   ps_buffer_size - real_act_prefix_size - 1);
		ps_buffer[ps_buffer_size - 1] = '\0';
		ps_buffer_cur_len = ps_buffer_size - 1;
	}
	else
	{
		memcpy(ps_buffer + real_act_prefix_size, activity, len + 1);
		ps_buffer_cur_len = real_act_prefix_size + len;
	}
	Assert(strlen(ps_buffer) == ps_buffer_cur_len);
