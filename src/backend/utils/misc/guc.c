...
/*
 * GPDB: Return the number of GUC variables currently known.
 */
int
 ...
 * gp_guc_list_init
 *
 * Builds global lists of interesting GUCs, so that we don't have to scan the
 * whole GUC hash table when we need them.
 *
...
build_guc_variables(void)
{
	 ...
	 *
	 * GPDB has its own tables of built-in variables (the "_gp" tables), which
	 * are counted and registered into the hash table alongside the upstream
	 * ones.
	 */
	...
	/*
	 * GPDB: mark each predefined GUC as needing (GUC_GPDB_NEED_SYNC) or not
	 * needing (GUC_GPDB_NO_SYNC) to be synchronized from the QD to the QEs.
	 * This must run after every built-in variable has been added to the hash
	 * table.
	 */
	guc_vars = get_guc_variables(&num_vars);
	gpdb_assign_sync_flag(guc_vars, num_vars, true /* predefine */);
	pfree(guc_vars);
}
...
/*
 * SetPGVariableOptDispatch - GPDB variant of SetPGVariable.
 *
 * Sets the GUC locally (SetPGVariable() lives in guc_funcs.c) and, when
 * gp_dispatch is true and we are the query dispatcher, propagates the
 * SET/RESET down to the QE segments.
 */
void
...
/*
 * DispatchSetPGVariable - dispatch a SET/RESET command to the QE segments.
 *
 * Exported so guc_funcs.c's ExecSetVariableStmt() (moved out of guc.c by the
 * PG16 guc split) can re-graft the GPDB QD->QE SET/RESET propagation.
 */
void
 ...
 */
bool
is_guc_modified(struct config_generic *conf)
{
	switch (conf->vartype)
	{
		case PGC_BOOL:
