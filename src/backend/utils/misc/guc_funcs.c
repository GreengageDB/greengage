...
ExecSetVariableStmt(VariableSetStmt *stmt, bool isTopLevel)
{
	...
	switch (stmt->kind)
	{
		...
		case VAR_SET_CURRENT:
			...
			(void) set_config_option(stmt->name,
									 ...
									 PGC_S_SESSION,
									 action, true, 0, false);
			/* GPDB: propagate the SET down to the QE segments */
	...
	/*
	 * GPDB: RESET/RESET ALL/SET DEFAULT must be dispatched to the QE segments
	 * separately from the normal SET path, because it can't be inside a user
	 * transaction.  (Plain SET is dispatched inline via DispatchSetPGVariable
	 * above.)
	 */
