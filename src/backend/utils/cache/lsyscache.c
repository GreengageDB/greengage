...
{
	OpIndexInterpretation *opBti;
	List	   *opBtis;

	opBtis = get_op_index_interpretation(oidOp);

	if (opBtis == NIL)
	{
		/* The operator does not belong to any ordering operator family */
		return CmptOther;
	}
	 ...
	 */
	opBti = (OpIndexInterpretation*)linitial(opBtis);

	switch(opBti->cmptype)
	{
		case COMPARE_LT:
			return CmptLT;
		case COMPARE_LE:
		...
		case COMPARE_NE:
			return CmptNEq;
		default:
			elog(ERROR, "unknown comparison type: %d", opBti->cmptype);
			return CmptOther;
	}
}
