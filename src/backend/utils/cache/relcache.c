...
AtEOXact_cleanup(Relation relation, bool isCommit)
{
	...
	{
		/*
		 * QE readers reach here outside a valid transaction, so we must not
		 * read catalogs to rebuild an entry (RelationRebuildRelation would
		 * trip its AssertCouldGetRelation/IsTransactionState check).  PG18
		 * split the old RelationClearRelation(rebuild) into separate entry
		 * points; mirror RelationFlushRelation()'s out-of-transaction
		 * handling: physically clear an unreferenced entry, otherwise just
		 * mark it invalid so it is rebuilt lazily the next time it is opened
		 * inside a real transaction.  A nailed entry always has a nonzero
		 * refcount, so it takes the invalidate path (RelationClearRelation
		 * asserts refcnt 0 and !rd_isnailed).
		 */
		if (RelationHasReferenceCountZero(relation))
			RelationClearRelation(relation);
		else
						   ...
						   char relkind)
{
	 ...
	 * already be in use as a relfilenumber of an existing relation in a
	 * segment.
	 *
	 * In binary upgrade mode, however, use the OID also as the relfilenumber.
