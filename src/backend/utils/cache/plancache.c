				 ...
				 const char *commandTag)
{
	...
	plansource->init_plangen_used = PLANGEN_PLANNER;
	plansource->plangen_switched = false;
...
/*
 * detect_plangen_switch: check if plangen switched between statements.
 *
 * Check if plangen switched for statements of one plan or between plans.
 * 'plan' should point to currently initialized plan.
 * 'plansource' is where plangen summary stored for all plans.
 */
static void
				...
				ParamListInfo boundParams, IntoClause *intoClause)
{
	...
	/* check if plangen switch appeared */
	detect_plangen_switch(plan, plansource);
...
choose_custom_plan(CachedPlanSource *plansource, ParamListInfo boundParams, IntoClause *intoClause)
{
	...
	/*
	 * It's incorrect to compare costs of Custom and Generic plans (and their
	 * statements) generated by different planners. Check if we can pass
	 * further with Custom and Generic plans cost comparison.
	 *
	 * Condition return true if there was a planner switch between multiple
	 * Custom plans generation; statements of one plan were generated using
	 * different optimizers; there was a fallback from GPORCA to Postgres
	 * planner. The fallback case indicates there was Param node(s) in plan -
	 * GPORCA doesn't support Params at all, so there's no hope of generating
	 * a generic plan here. Generally, GPORCA can generate Generic plan when
	 * there is no Param nodes, but this case was filtered by
	 * '(boundParams == NULL)' condition above. Despite of that limitation,
	 * 'plangen_switched' filled by 'detect_plangen_switch()' is ready to
	 * handle all kinds of switching.
	 */
...
CopyCachedPlan(CachedPlanSource *plansource)
{
	...
	newsource->init_plangen_used = plansource->init_plangen_used;
	newsource->plangen_switched = plansource->plangen_switched;
