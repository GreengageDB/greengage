							  ...
							  int32 shift, Numeric rbound)
{
	...
	div_var(&result, &widvar, &wrkvar, dscale, true, true);
	...
	*eq_abscissas = false;
	...
	else
	{
		...
		else 
		{
			...
			div_var(&v, &v1, &v, rscale, true, true);
		}
...
numeric_li_value(float8 f, Numeric y0, Numeric y1)
{
	...
	else
	{
		...
		(void) set_var_from_str(buf, buf, &vf, &endptr, NULL);
 ...
 *
 * GPDB: unlike upstream, this *initializes* dest (via init_alloc_var) rather
 * than reusing/freeing a previously initialized var, and it uses a local
 * digit buffer to avoid a palloc for short inputs.  Also, upstream's final
 * strip_var() call is intentionally omitted; make_result() strips leading/
 * trailing zeroes itself, as it always has in GPDB.
 */
static bool
