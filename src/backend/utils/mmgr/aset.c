...
/*
 * GPDB memory accounting: forward declaration for the mutual recursion
 * between AllocSetGetPeakUsage() and its subtree helper.  The accounting
 * method functions themselves have external linkage so that the centralized
 * method table in mcxt.c (mcxt_methods[]) can reference them.
 */
static Size AllocSetGetPeakUsage_recurse(MemoryContext parent,
...
AllocSetReset(MemoryContext context)
{
	...
	/*
	 * Make sure all children have been deleted, or the accounting data is
	 * incorrect.
	 */
...
AllocSetFree(void *pointer)
{
	...
	if (MemoryChunkIsExternal(chunk))
	{
		...
		MEMORY_ACCOUNT_DEC_ALLOCATED(set, block->endptr - (char *) pointer);
...
AllocSetRealloc(void *pointer, Size size, int flags)
{
	...
	if (oldchksize >= size)
	{
		...
		/*
		 * no need to update memory accounting summaries, since the chunk size
		 * didn't change
		 */
 ...
 *		Designate this context as an independent memory account, so that
 *		its subtree's allocations are tracked separately from its parent's.
 */
void
 ...
 *		Return the bytes currently allocated in this account's subtree.
 */
Size
 ...
 *		Return the peak bytes ever allocated in this account's subtree.
 */
Size
 ...
 *		Reset this account's peak to max(current, nbytes); return the old peak.
 */
Size
