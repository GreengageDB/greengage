					...
					TuplesortInstrumentation *stats)
{
	if (state->instrument)
	{
		...
		workmemused = MemoryContextGetPeakSpace(state->base.maincontext);
		if (state->instrument->workmemused < workmemused)
			state->instrument->workmemused = workmemused;
		state->instrument->execmemused += MemoryContextGetPeakSpace(state->base.maincontext);
	}
