			 ...
			 char *out_dbname)
{
	...
	else
	{
		...
		am_superuser = superuser();
		BackendCancelInit(MyProcNumber);
	}
	 ...
	 * walsender anyways so we should allow the connection to happen; the
	 * !am_walsender exemption below takes care of that. This may need to be
	 * reviewed later when we start supporting multiple mirrors.
	 *
