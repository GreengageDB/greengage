...
/*
 * GPDB: remember the superuser state of the authenticated user, so that
 * IsAuthenticatedUserSuperUser() (used by the dispatcher) can report it even
 * after SET SESSION AUTHORIZATION / SET ROLE change the effective user.
 */
						...
						bool bypass_login_check)
{
	...
	/*
	 * The GGDB resource-queue caching and the is_superuser GUC are handled by
	 * SetSessionAuthorization(), which is invoked via the
	 * "session_authorization" GUC assign hook set above.
	 */
...
SetSessionAuthorization(Oid userid, bool is_superuser)
{
	...
	/* SetOuterUserId also updates the is_superuser GUC. */
	if (!SetRoleIsActive)
