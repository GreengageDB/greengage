						 ...
						 bool forceBareCol, CreateStmt *stmt)
{
	 ...
	 *
	 * GPDB: external tables (forceBareCol) take bare columns only, so we
	 * do not reproduce the source table's NOT NULL constraints for them.
	 */
	if (!forceBareCol && tupleDesc->constr && tupleDesc->constr->has_not_null)
	{
		List	   *lst;
