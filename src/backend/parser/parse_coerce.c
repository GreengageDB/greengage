...
{
	/*
	 * GGDB: parser_errposition() returns void in this tree (it sets the error
	 * position as a side effect via errposition()), so we cannot return its
	 * result directly.  The effective return value of the err* helpers is
	 * always 0, so record the position and return 0 like the upstream shape.
	 */
