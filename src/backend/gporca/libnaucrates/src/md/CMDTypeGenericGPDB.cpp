...
// The newer polymorphic families (anyrange/anymultirange and the whole
// anycompatible family) must also count as ambiguous so ORCA falls back to
// the planner-resolved concrete type; keep in sync with IsPolymorphicType()
// in pg_type.h.
...
#define GPDB_ANYCOMPATIBLEARRAY_OID \
	OID(5078)  // oid of GPDB ANYCOMPATIBLEARRAY type

#define GPDB_ANYCOMPATIBLENONARRAY_OID \
	OID(5079)  // oid of GPDB ANYCOMPATIBLENONARRAY type

#define GPDB_ANYCOMPATIBLERANGE_OID \
	OID(5080)  // oid of GPDB ANYCOMPATIBLERANGE type

#define GPDB_ANYCOMPATIBLEMULTIRANGE_OID \
	OID(4538)  // oid of GPDB ANYCOMPATIBLEMULTIRANGE type
...
CMDTypeGenericGPDB::IsAmbiguous() const
{
	...
	return (GPDB_ANYELEMENT_OID == oid || GPDB_ANYARRAY_OID == oid ||
			...
			GPDB_ANYCOMPATIBLE_OID == oid ||
			GPDB_ANYCOMPATIBLEARRAY_OID == oid ||
			GPDB_ANYCOMPATIBLENONARRAY_OID == oid ||
			GPDB_ANYCOMPATIBLERANGE_OID == oid ||
