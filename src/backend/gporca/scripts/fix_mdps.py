...
def processLogFile(logFileLines):
    ...
    for line in logFileLines:
        ...
        if fileNameMatch:
            ...
            if read_plan > 0:
                print(
                    "Log file contains partial plans for %s, please update this file by hand"
                    % (current_file)
                )
                read_plan = 0
            ...
            if not re.search(r"Plan [a-z ]*comparison", line) and not re.search(
                r"Unittest", line
            ):
...
def main():
    ...
    parser.add_argument(
        "failed_tests_file",
        nargs="?",
        help="a file containing failed tests, one per line (ctest output)",
    )
    parser.add_argument(
        "--dryRun",
        action="store_true",
    ...
    parser.add_argument(
        "--logFile", help="Log file created with ctest --output-on-failure command"
    )
    ...
    if inputfile is None and logfile is None:
        ...
        print(
            "If a file with failed tests is specified, the --logFile is not allowed\n"
        )
        exit(1)
