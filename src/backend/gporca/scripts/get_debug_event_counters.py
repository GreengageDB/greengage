...
try:
    ...
    sys.exit(
        "ERROR: Cannot import modules.  Please check that you have sourced greengage_path.sh to set PYTHONPATH. "
        "Detail: " + str(e)
    )
            ...
            if len(current_run_name) > 0:
                ...
                if allruns or eof_match:
                    if not header_printed:
                        ...
                        print_or_insert_header_row(
                            "run, query num, query name, counter name, counter_type, counter_value"
                        )
                        header_printed = True
                    # print a dummy counter line that indicates the starting time of the run
                    print_or_insert_row(
                        "%d, 0, , %s, run_start_time, 0"
                        % (current_run_number, current_run_name)
                    )
...
def parseargs():
    ...
    parser.add_argument(
        "--host", default="localhost", help="Host to connect to (default is localhost)."
    )
    parser.add_argument(
        "--port", type=int, default="0", help="Port on the host to connect to"
    )
...
def main():
    ...
    if args.raw:
        ...
        if glob_use_sql:
            ...
            print(
                (
                    "Inserted a total of %d rows into table public.debug_counters."
                    % glob_sql_inserted_rows
                )
            )
