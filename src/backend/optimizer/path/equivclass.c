						 ...
						 bool create_it)
{
	...
	if (root->ec_merging_done)
	{
		...
		while ((i = bms_next_member(newec->ec_relids, i)) > 0)
		{
			...
			/*
			 * GPDB: join removal can leave RELOPT_DEADREL rels in ec_relids,
			 * and partitioned/inheritance handling can mention
			 * RELOPT_OTHER_MEMBER_REL rels here; allow both.
			 */
