...
#define EXPRKIND_WINDOW_BOUND		14
				 ...
				 ParamListInfo boundParams)
{
	...
	root = subquery_planner(glob, parse, NULL,
							false, tuple_fraction, NULL, config);
...
inheritance_planner(PlannerInfo *root)
{
	 ...
	 */
	{
		...
		grouping_planner(subroot, 0.0 /* retrieve all tuples */ , NULL);
	...
	if (grouping_is_sortable(root->processed_distinctClause))
	{
		...
		foreach(lc, input_rel->pathlist)
		{
			...
			foreach_node(List, useful_pathkeys, useful_pathkeys_list)
			{
				...
				/*
				 * GPDB: redistribute/gather the sorted path so DISTINCT is
				 * computed correctly across segments.  This Motion must be
				 * added for the distinct_pathkeys == NIL fast path too:
				 * otherwise each segment would emit its own row and the
				 * gathered result would contain one duplicate row per segment.
				 * (Upstream PostgreSQL has a single backend, so it can safely
				 * apply a bare LIMIT 1 there; we cannot.)
				 */
				sorted_path = cdb_prepare_path_for_sorted_agg(root,
															  true, /* is_sorted */
															  distinct_rel,
															  sorted_path,
				...
				else
				{
					...
					if (CdbPathLocus_IsPartitioned(sorted_path->locus))
						numDistinctRows = numDistinctRowsTotal / CdbPathLocus_NumSegments(sorted_path->locus);
					else
						  ...
						  GroupPathExtraData *extra)
{
	...
	if (can_sort)
	{
		...
		foreach(lc, input_rel->pathlist)
		{
			...
			/*
			 * GPDB: only sort the cheapest-total path (or a path that is
			 * already suitably sorted).  cdb_prepare_path_for_sorted_agg sorts
			 * the input if needed and also adds a Motion to redistribute it by
			 * the grouping keys when the data is not already colocated.
			 */
			if (path == cheapest_path || is_sorted)
			{
				...
				else if (parse->hasAggs || parse->groupClause)
				{
					 ...
					 * Make an AggPath.  (Plain GROUP BY without aggregates
					 * is also implemented as an Agg in GPDB, so Group nodes
					 * are not used.)
					 */
					add_path(grouped_rel, (Path *)
							 create_agg_path(root,
	...
	if (can_sort && cheapest_total_path != NULL)
	{
		...
		foreach(lc, input_rel->pathlist)
		{
			...
			foreach(lc2, pathkey_orderings)
			{
				...
				/*
				 * Group nodes are not used in GPDB, so always build a partial
				 * Agg (even for plain GROUP BY without aggregates).
				 */
