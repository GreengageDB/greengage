							...
							RelOptInfo *distinct_rel)
{
	...
	if (grouping_is_sortable(root->processed_distinctClause))
	{
		...
		foreach(lc, input_rel->pathlist)
		{
			 ...
			 * computed correctly across segments.  This Motion must be added
			 * for the distinct_pathkeys == NIL fast path too: otherwise each
			 * segment would emit its own row and the gathered result would
			 * contain one duplicate row per segment.  (Upstream PostgreSQL has
			 * a single backend, so it can safely apply a bare LIMIT 1 there;
			 * we cannot.)
			 */
