...
static void set_plan_references_input_asserts(PlannerInfo *root, Plan *plan)
{
	...
	foreach (lc, allVars)
	{
         ...
         *
         * PG14 added ROWID_VAR, used for the nonce row-identity variables of an
         * UPDATE/DELETE before set_plan_references() resolves them, so accept it
         * here too (otherwise distributed UPDATE/DELETE plans trip this assert).
         */
		...
		case T_WindowAgg:
			{
				...
				wplan->startOffset =
					fix_upper_expr(root, wplan->startOffset, subplan_itlist,
								   OUTER_VAR, rtoffset, NRM_EQUAL, 1);
				wplan->endOffset =
...
set_join_references(PlannerInfo *root, Join *join, int rtoffset)
{
	...
	/*
	 * GPDB: The targetlist and qpqual must not re-use any non-Var input
	 * expression that was computed in the nullable side of an outer join
	 * (on a null-extended row the expression must be recomputed from the
	 * null-extended columns, not read back pre-computed).  Upstream removed
	 * this has_non_vars hack in PG 16 because Vars above an outer join now
	 * carry varnullingrels, so such a false exact match can no longer occur
	 * for planner-built expressions.  GPDB planner code still constructs
	 * Vars without varnullingrels in places, so keep the old defensive
	 * behavior: at worst it declines a legal reuse and recomputes the
	 * expression.  (Also covers the GPDB-only JOIN_LASJ_NOTIN, which is
	 * ANTI-like.)
	 */
