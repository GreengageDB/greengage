	...
	else if (IsA(jtnode, JoinExpr))
	{
		...
		switch (j->jointype)
		{
			...
			case JOIN_SEMI:
				...
				/*
				 * GPDB: also treat the semijoin's RHS rels as participating in
				 * inner joins, so that upper outer joins won't be reordered
				 * into the semijoin's RHS (see make_outerjoininfo).
				 */
				jtitem->inner_join_rels = bms_add_members(jtitem->inner_join_rels,
														  right_item->qualscope);
				jtitem->left_rels = left_item->qualscope;
				jtitem->right_rels = right_item->qualscope;
				   ...
				   List *clause)
{
	...
	foreach(l, root->join_info_list)
	{
		 ...
		 * unsafe PHVs or the current join is a semijoin, antijoin or lasj, we
		 * must preserve ordering regardless of strictness.
		 *
						...
						List **postponed_oj_qual_list)
{
	...
	if (!bms_is_subset(relids, qualscope))
	{
		...
		for (pitem = jtitem->jti_parent; pitem; pitem = pitem->jti_parent)
		{
			 ...
			 */
			/*
			 * GPDB_94_MERGE_FIXME: In GPDB, a SEMI JOIN may come here, since
			...
			Assert(pitem->sjinfo == NULL ||
				   pitem->sjinfo->jointype == JOIN_SEMI);
		}
								...
								RestrictInfo *restrictinfo)
{
	...
	if (!bms_is_empty(relids))
	{
		...
		if (bms_get_singleton_member(relids, &relid))
		{
			...
			/*
			 * GPDB: quals that reference the outer query must be evaluated
			 * separately (in the upper part of the plan), so keep them in a
			 * dedicated list rather than the ordinary baserestrictinfo.
			 */
