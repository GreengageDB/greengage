 ...
 *
 * Once upon a time this ran only on INSERT targetlists.  In GPDB it is also
 * applied to UPDATE targetlists (via command_type == CMD_UPDATE) when the
 * UPDATE needs the full new tuple: a Split Update (delete+insert that may move
 * the row to another segment) or an append-optimized UPDATE.  The CMD_UPDATE
 * path additionally records which columns are actually changed.
 */
static List *
