				  ...
				  double outputRows)
{
	...
	/*
	 * GPDB: the new two-input SetOp requires both inputs to share a
	 * compatible locus (ensured by the caller when building the setop
	 * paths); take the resulting locus from the left input.
	 */
	pathnode->path.locus = leftpath->locus;
