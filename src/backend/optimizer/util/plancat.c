	...
	if (hasindex)
	{
		...
		foreach(l, indexoidlist)
		{
			...
			if (indexRelation->rd_rel->relkind != RELKIND_PARTITIONED_INDEX)
			{
				...
				/*
				 * GPDB: use the MPP-aware cdb_estimate_rel_size() rather than
				 * upstream's RelationGetNumberOfBlocks()/estimate_rel_size(), so
				 * the estimate reflects the whole distributed relation (including
				 * AO and external tables) rather than just the coordinator-local
				 * size.
				 */
