...
static const FormData_pg_attribute *const SysAtt[] = {&a1, &a2, &a3, &a4, &a5, &a6, &a8};
						  ...
						  const char *queryString)
{
	...
	foreach_node(Constraint, cdef, newConstraints)
	{
		...
		if (cdef->contype == CONSTR_CHECK)
		{
			...
			/*
			 * GPDB: this cooked constraint is dispatched to the QEs, which
			 * derive pg_constraint.convalidated as !skip_validation (see the
			 * cooked path in AddRelationNewConstraints and pg_constraint.c,
			 * where skip_validation is defined as !convalidated).  We must not
			 * copy cdef->skip_validation here: for a CHECK on a freshly created
			 * (empty) table both skip_validation and initially_valid are true,
			 * which would make the segments record convalidated=false while the
			 * coordinator has true, and would also violate CreateConstraintEntry's
			 * "isEnforced || !isValidated" assertion for NOT ENFORCED (hence NOT
			 * VALID) constraints.  Derive it from the constraint's validity.
			 */
			cooked->skip_validation = !cdef->initially_valid;
			cooked->is_local = is_local;
			cooked->inhcount = is_local ? 0 : 1;
