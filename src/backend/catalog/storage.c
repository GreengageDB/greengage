 ...
 */
static void
AddPendingSync(const RelFileLocator *rlocator, SMgrImpl smgr_which)
{
 ...
 */
SMgrRelation
RelationCreateStorage(RelFileLocator rlocator, char relpersistence, SMgrImpl smgr_which,
					  bool register_delete)
{
	...
	srel = smgropen(rlocator, procNumber, smgr_which);
	...
	if (register_delete)
	{
		...
		pending->relnode.node = rlocator;
		pending->relnode.isTempRelation = procNumber == ProcNumberForTempRelations();
	...
	if (relpersistence == RELPERSISTENCE_PERMANENT && !XLogIsNeeded())
	{
		...
		AddPendingSync(&rlocator, smgr_which);
	}
...
RelationDropStorage(Relation rel)
{
	...
	pending = (PendingRelDelete *)
		MemoryContextAlloc(TopMemoryContext, sizeof(PendingRelDelete));
	pending->relnode.node = rel->rd_locator;
	pending->relnode.isTempRelation = rel->rd_backend == ProcNumberForTempRelations();
...
RelationPreserveStorage(RelFileLocator rlocator, bool atCommit)
{
	...
	{
		next = pending->next;
		if (RelFileLocatorEquals(rlocator, pending->relnode.node)
...
RestorePendingSyncs(char *startAddress)
{
	...
	for (relnode = (RelFileNodePendingSync *) startAddress;
		 relnode->node.relNumber != 0; relnode++)
	...
	for (pending = pendingDeletes; pending != NULL; pending = next)
	{
		...
		else
		{
			...
			if (pending->atCommit == isCommit)
			{
				...
				/* GPDB: backend can only be ProcNumberForTempRelations() or
				 * INVALID_PROC_NUMBER for a given relfile since we don't tie temp
...
smgrDoPendingSyncs(bool isCommit, bool isParallelWorker)
{
	...
	for (pending = pendingDeletes; pending != NULL; pending = pending->next)
		if (pending->atCommit)
			(void) hash_search(pendingSyncHash, (void *) &pending->relnode.node,
							   HASH_REMOVE, NULL);
...
smgrGetPendingDeletes(bool forCommit, RelFileNodePendingDelete **ptr)
{
	...
	for (pending = pendingDeletes; pending != NULL; pending = pending->next)
	{
		if (pending->nestLevel >= nestLevel && pending->atCommit == forCommit
			 ...
			 * prepared.
			 */
			/* && pending->relnode.backend == INVALID_PROC_NUMBER) */
				)
			nrels++;
	}
	if (nrels == 0)
...
smgr_redo(XLogReaderState *record)
{
	...
	if (info == XLOG_SMGR_CREATE)
	{
		...
		reln = smgropen(xlrec->rlocator, INVALID_PROC_NUMBER, xlrec->impl);
	...
	else if (info == XLOG_SMGR_TRUNCATE)
	{
		 ...
		 */
		reln = smgropen(xlrec->rlocator, INVALID_PROC_NUMBER, SMGR_MD);
