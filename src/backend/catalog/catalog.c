					...
					char **dir, char **filename)
{
	...
	/* PG18: relpathbackend() now returns a RelPathStr by value. */
	rpath = relpathbackend(rlocator, backend, forknum);
	path = rpath.str;
...
aorelpathbackend(RelFileNode node, ProcNumber backend, int32 segno)
{
	...
	/* PG18: relpathbackend() now returns a RelPathStr by value. */
	rpath = relpathbackend(rlocator, backend, MAIN_FORKNUM);
	path = rpath.str;
	if (segno == 0)
...
IsInplaceUpdateOid(Oid relid)
{
	return (relid == RelationRelationId ||
			...
			relid == FastSequenceRelationId);	/* GPDB: gp_fastsequence is
												 * updated in place */
}
