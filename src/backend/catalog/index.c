			 ...
			 Oid *constraintId)
{
	...
	if (!OidIsValid(indexRelationId))
	{
		...
		/*
		 * GPDB: the pg_class OID is assigned by GetNewOidForRelation() above.
		 * During binary upgrade it comes from the preassigned-OID mechanism
		 * (binary_upgrade_set_next_index_pg_class_oid() feeds it there), not
		 * from binary_upgrade_next_index_pg_class_oid, so we don't apply the
		 * upstream OID override here.  We still honor the upstream PG16
		 * binary-upgrade override for the index relfilenumber.
		 */
		if (IsBinaryUpgrade)
		{
			/*
				   ...
				   double reltuples)
{
	 ...
	 *
	 * GPDB: Also skip on the dispatcher (QD), where the relation holds no
	 * data; the real statistics come from the segments.
	 */
	...
	if (update_stats)
	{
		 ...
		 * tables, but disable them for now by leaving relallvisible (and
		 * relallfrozen) at 0.
		 */
