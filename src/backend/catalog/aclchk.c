...
/* GPDB: external protocols live in a GPDB-specific catalog (pg_extprotocol) */
static void ExecGrant_ExtProtocol(InternalGrant *istmt);
...
objectNamesToOids(ObjectType objtype, List *objnames, bool is_grant)
{
	...
	switch (objtype)
	{
		...
		case OBJECT_LARGEOBJECT:
			...
			/*
			 * GPDB: large objects are not supported; reject GRANT/REVOKE on
			 * them explicitly rather than letting the default get_object_address
			 * path handle them.
			 */
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
			...
			break;
		case OBJECT_EXTPROTOCOL:
			/* GPDB-specific object type not known to get_object_address(). */
			foreach(cell, objnames)
			{
				 ...
				 void (*object_check) (InternalGrant *istmt, HeapTuple tuple))
{
	...
	foreach(cell, istmt->objects)
	{
		...
		/*
		 * GPDB: MPP-6929: metadata tracking.  In PG15 and before, this was
		 * done in the per-class ExecGrant_Database/Namespace/Tablespace
		 * functions, which were folded into ExecGrant_common in PG16; keep
		 * tracking exactly those object classes.
		 */
		if (Gp_role == GP_ROLE_DISPATCH &&
			(classid == DatabaseRelationId ||
			 classid == NamespaceRelationId ||
			 classid == TableSpaceRelationId))
			MetaTrackUpdObject(classid,
							   objectid,
							   GetUserId(), /* not grantorId, */
							   "PRIVILEGE",
							   (istmt->is_grant) ? "GRANT" : "REVOKE"
...
/*
 * GPDB: Exported routine for examining a user's privileges for an external
 * protocol.
 */
AclMode
...
/*
 * GPDB: Exported routine for checking a user's access privileges to an
 * external protocol
 */
AclResult
