...
/*
 * GPDB: hand-written read (text) support for nodes marked custom_read_write.
 * Must mirror the matching _out* functions field-for-field, in order.  The
 * binary QD->QE path is in readfast.c; SerializedParams/TupleDescNode/
 * QueryDispatchDesc are no_read (never text-read), so no read func here.
 */

/* Read a placeholder dummy field written by WRITE_DUMMY_FIELD; set to fldvalue */
#define READ_DUMMY_FIELD(fldname, fldvalue) \
	do { \
		token = pg_strtok(&length);		/* skip :fldname */ \
		token = pg_strtok(&length);		/* skip dummy value */ \
		(void) token; \
		local_node->fldname = (fldvalue); \
	} while (0)
...
_readSliceTable(void)
{
	...
	for (int i = 0; i < local_node->numSlices; i++)
	{
		...
		READ_NODE_FIELD(slices[i].children);
		READ_ENUM_FIELD(slices[i].gangType, GangType);
		READ_NODE_FIELD(slices[i].segments);
		READ_DUMMY_FIELD(slices[i].primaryGang, NULL);
		READ_NODE_FIELD(slices[i].primaryProcesses);
