...
# GPDB: abstract node types that nevertheless need a concrete nodetag.
# A bare Plan node is instantiated as the Adaptive-NestLoop fake outer
# subplan, and T_Scan/T_Join are referenced as defensive switch labels and
# in IsA() tests.  They get a nodetag but no generated copy/equal/out/read
# support (GPDB provides those by hand where needed).
my @gpdb_abstract_with_tag = qw(Plan Scan Join);
...
# GGDB-specific by-value field types used in scanned node structs.  These are
# copied by assignment; their out/read handling is verified once generation
# runs (CdbLocusType is an enum, EstimatedBytes a double typedef, ParentStmtType
# a uint8 typedef, ItemPointerData a POD struct copied by assignment).
push @scalar_types, qw(CdbLocusType EstimatedBytes ParentStmtType ItemPointerData);
...
{
	next if (elem $n, @abstract_types) and not (elem $n, @gpdb_abstract_with_tag);
...
" unless $struct_no_equal;
	...
	foreach my $f (@{ $node_type_info{$n}->{fields} })
	{
		...
		# GPDB: CdbPathLocus is a by-value struct (holds a List*). It only
		# appears in no_equal/no_read nodes (Path/PlannerInfo), so copy via
		# shallow assignment (like QualCost) and leave equal/read to those
		# node-level attrs.
		elsif ($t eq 'CdbPathLocus')
		{
			print $cff "\tnewnode->$f = from->$f;\n" unless $copy_ignore;
		}
...
";
	...
	foreach my $f (@{ $node_type_info{$n}->{fields} })
	{
		...
		# GPDB: fixed-width integer typedefs used in node fields
		elsif ($t eq 'int8' || $t eq 'int16' || $t eq 'int32')
		{
			print $off "\tWRITE_INT_FIELD($f);\n";
			print $rff "\tREAD_INT_FIELD($f);\n" unless $no_read;
		}
		...
		# GPDB scalar typedefs (registered in @scalar_types above): give them
		# out/read formats here (copy/equal handle them via the @scalar_types path)
		elsif ($t eq 'ParentStmtType')
		{
			print $off "\tWRITE_UINT_FIELD($f);\n";
			print $rff "\tREAD_UINT_FIELD($f);\n" unless $no_read;
		}
		elsif ($t eq 'EstimatedBytes')
		{
			print $off "\tWRITE_FLOAT_FIELD($f);\n";
			print $rff "\tREAD_FLOAT_FIELD($f);\n" unless $no_read;
		}
		elsif ($t eq 'CdbLocusType')
		{
			print $off "\tWRITE_ENUM_FIELD($f, CdbLocusType);\n";
			print $rff "\tREAD_ENUM_FIELD($f, CdbLocusType);\n" unless $no_read;
		}
...
! unless $no_read;
		...
		# GPDB: out-only serialization of CdbPathLocus (Path/PlannerInfo are
		# no_read, so no read side). _outCdbPathLocus is hand-written in the
		# outfuncs stub.
		elsif ($t eq 'CdbPathLocus')
		{
			print $off "\t_outCdbPathLocus(str, \"$f\", &node->$f);\n";
		}
		else
		{
			die
