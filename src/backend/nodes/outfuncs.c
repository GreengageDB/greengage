...
#include "cdb/cdbpathlocus.h"		/* GPDB: _outCdbPathLocus() */
...
/* GPDB: forward decl — _outCdbPathLocus is called from the generated
 * outfuncs.funcs.c (Path.locus is inlined into Path-subtype out funcs). */
static void _outCdbPathLocus(StringInfo str, const char *fldname,
							const CdbPathLocus *node);

#include "outfuncs.funcs.c"

/*
 * GPDB: hand-written out (text) support for nodes marked custom_read_write.
 * Text output is debug-only; the real QD->QE serialization is the binary path
 * in outfast.c.  Some of these nodes are no_read (SerializedParams/TupleDescNode/
 * QueryDispatchDesc) since they are never text-read (stringToNode).
 */

/* Write a placeholder for a non-serializable runtime pointer (e.g. a Gang*) */
#define WRITE_DUMMY_FIELD(fldname) \
	(appendStringInfo(str, " :" CppAsString(fldname) " "), \
	 outToken(str, NULL))

static void
_outCdbPathLocus(StringInfo str, const char *fldname, const CdbPathLocus *node)
{
	appendStringInfo(str, " :%s", fldname);
...
_outSerializedParams(StringInfo str, const SerializedParams *node)
{
	...
	for (int i = 0; i < node->nExternParams; i++)
	{
		...
		if (!node->externParams[i].isnull)
			outDatum(str, node->externParams[i].value,
					 node->externParams[i].plen, node->externParams[i].pbyval);
	}
	...
	for (int i = 0; i < node->nExecParams; i++)
	{
		...
		if (node->execParams[i].isvalid && !node->execParams[i].isnull)
			outDatum(str, node->execParams[i].value,
					 node->execParams[i].plen, node->execParams[i].pbyval);
	}

	/*
	 * transientTypes (List of TupleDescNode) is serialized only on the binary
	 * QD->QE path (outfast.c); text out is debug-only and TupleDescNode is
	 * no_read, so it is intentionally omitted here.
	 */
}
