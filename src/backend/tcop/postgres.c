			   ...
			   const char * serializedQueryDispatchDesc, int serializedQueryDispatchDesclen)
{
	...
	if ( slice )
	{
		...
		if (sliceTable->localSlice != slice->rootIndex)
		{
			 ...
			 * GPDB: PG16 moved requiredPerms out of RangeTblEntry into
			 * RTEPermissionInfo (PlannedStmt.permInfos), which only exist for
			 * relations that need permission checks.
			 */
			foreach(rtcell, plan->permInfos)
			{
				perminfo = (RTEPermissionInfo *) lfirst(rtcell);
				if (0 != (perminfo->requiredPerms & removeperms))
					perminfo->requiredPerms &= ~removeperms;
			}
		}
	}
						  ...
						  const char **dbname)
{
	 ...
	 */
	while ((flag = getopt(argc, argv, "B:bC:c:D:d:EeFf:h:ijk:lMmN:nOPp:r:S:sTt:v:W:-:")) != -1)
	{
		switch (flag)
		{
			case 'B':
