...
static void ProcessCancelRequestPacket(Port *port, void *pkt, int pktlen, MsgType code);
...
retry:
	...
	{
		/*
		 * GPDB: proto may be CANCEL_REQUEST_CODE or the GPDB-specific
		 * FINISH_REQUEST_CODE (graceful query finish).  Pass the code down so
		 * the packet handler can route a finish request to
		 * PROCSIG_QUERY_FINISH instead of a plain SIGINT cancel.
		 */
		ProcessCancelRequestPacket(port, buf, len, proto);
 ...
 */
static void
ProcessCancelRequestPacket(Port *port, void *pkt, int pktlen, MsgType code)
{
	...
	/*
	 * GPDB: a FINISH_REQUEST_CODE packet asks the target backend to finish the
	 * current query gracefully (PROCSIG_QUERY_FINISH) rather than to cancel it
	 * with SIGINT.  Both share the same backend PID + cancel key matching, so
	 * route through SendQueryFinishRequest which mirrors SendCancelRequest but
	 * delivers PROCSIG_QUERY_FINISH.
	 */
	if (code == FINISH_REQUEST_CODE)
		SendQueryFinishRequest(pg_ntoh32(canc->backendPID), canc->cancelAuthCode, len);
	else
