...
static int64 sendDir(bbsink *sink, const char *path, int basepathlen, bool sizeonly,
					 ...
					 IncrementalBackupInfo *ib, HTAB *exclude);
					...
					IncrementalBackupInfo *ib)
{
	...
	/*
	 * GPDB: do_pg_backup_start() has just created the backup checkpoint.  This
			...
			foreach(lc, state.tablespaces)
			{
				...
				if (tmp->path == NULL)
					tmp->size = sendDir(sink, ".", 1, true, state.tablespaces,
										true, NULL, InvalidOid, NULL, opt->exclude);
				else
		...
		foreach(lc, state.tablespaces)
		{
			...
			if (ti->path == NULL)
			{
				...
				sendDir(sink, ".", 1, false, state.tablespaces,
						sendtblspclinks, &manifest, InvalidOid, ib, opt->exclude);
...
sendDir(bbsink *sink, const char *path, int basepathlen, bool sizeonly,
		...
		Oid spcoid, IncrementalBackupInfo *ib, HTAB *exclude)
{
	DIR		   *dir;
	struct dirent *de;
		...
		else if (S_ISDIR(statbuf.st_mode))
		{
			...
			if (!skip_this_dir)
				size += sendDir(sink, pathbuf, basepathlen, sizeonly, tablespaces,
								sendtblspclinks, manifest, spcoid, ib, exclude);
		}
