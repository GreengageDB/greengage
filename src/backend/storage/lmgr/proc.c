 ...
 * GetAwaitedLock()/ResetAwaitedLock() accessors.
 */
...
FastPathLockShmemSize(void)
{
	 ...
	 *
	 * GPDB: fpHoldTillEndXactBits (GDD) is parallel to fpLockBits, one uint64
	 * word per fast-path group, so it needs an array of the same size.
	 */
	fpLockBitsSize = MAXALIGN(FastPathLockGroupsPerBackend * sizeof(uint64));
	fpHoldBitsSize = MAXALIGN(FastPathLockGroupsPerBackend * sizeof(uint64));
	fpRelIdSize = MAXALIGN(FastPathLockSlotsPerBackend() * sizeof(Oid));

	size = add_size(size, mul_size(TotalProcs,
								   (fpLockBitsSize + fpHoldBitsSize + fpRelIdSize)));
...
InitProcGlobal(void)
{
	...
	for (i = 0; i < TotalProcs; i++)
	{
		...
		/* GPDB: HoldTillEndXactBits (GDD), parallel to fpLockBits. */
		proc->fpHoldTillEndXactBits = (uint64 *) fpPtr;
		fpPtr += fpHoldBitsSize;
...
InitProcess(void)
{
	...
	MyTmGxact = &ProcGlobal->allTmGxact[MyProcNumber];
...
ResProcSleep(LOCKMODE lockmode, LOCALLOCK *locallock, void *incrementSet)
{
	...
	do
	{
		...
		if (log_lock_waits && deadlock_state != DS_NOT_YET_CHECKED)
		{
			...
			{
				PROCLOCK   *proclock = dlist_container(PROCLOCK, lockLink,
													   proclock_iter.cur);
...
ResLockWaitCancel(void)
{
	...
	partitionLock = LockHashPartitionLock(awaitedLock->hashcode);
	LWLockAcquire(partitionLock, LW_EXCLUSIVE);
	...
	if (MyProc->links.next != NULL)
	{
		...
		Assert(LOCALLOCK_LOCKMETHOD(*awaitedLock) == RESOURCE_LOCKMETHOD);
		...
		ResRemoveFromWaitQueue(MyProc, awaitedLock->hashcode);
	}
