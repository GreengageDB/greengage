...
/*
 * fpHoldTillEndXactBits is used for GDD.  Like fpLockBits it is now split
 * into per-group words (one uint64 per fast-path group), so index the word
 * with FAST_PATH_GROUP(n) and shift within it by FAST_PATH_INDEX(n).
 * "bits" is the result of FAST_PATH_GET_BITS(proc, n); we simply set the same
 * bits in the hold-till-end-of-xact word as we do in fpLockBits.
 */
#define FAST_PATH_HOLD_BITS(proc, n)	(proc)->fpHoldTillEndXactBits[FAST_PATH_GROUP(n)]
#define FAST_PATH_SET_HOLD_TILL_END_XACT(proc, n, bits) \
	 FAST_PATH_HOLD_BITS(proc, n) |= (((uint64) (bits)) & FAST_PATH_MASK) << (FAST_PATH_BITS_PER_SLOT * FAST_PATH_INDEX(n))
#define FAST_PATH_GET_HOLD_TILL_END_XACT_BITS(proc, n) \
	((FAST_PATH_HOLD_BITS(proc, n) >> (FAST_PATH_BITS_PER_SLOT * FAST_PATH_INDEX(n))) & FAST_PATH_MASK)
/*
					...
					bool logLockFailure)
{
	...
	{
		/*
		 * GGDB: emit a diagnostic when a gang member is about to block on a
		 * lock.  found_conflict is set, so unless JoinWaitQueue() can grant
		 * the lock immediately we are going to wait; log only when a real
		 * blocking wait is requested to preserve prior behavior.
		 */
		if (!dontWait && IS_QUERY_EXECUTOR_BACKEND())
		{
			if (!Gp_is_writer)
				   ...
				   PROCLOCK *proclock)
{
	...
	if (mppSessionId != InvalidGpSessionId)
	{
		...
		for (i = 1; i <= numLockModes; i++)
		{
			 ...
			 * may be this process itself.  (reuses the function-level
			 * proclock_iter declared above)
			 */
...
GetLockStatusData(void)
{
	...
	while ((proclock = (PROCLOCK *) hash_seq_search(&seqstat)))
	{
		...
		TMGXACT	   *tmGxact = &ProcGlobal->allTmGxact[GetNumberFromPGProc(proc)];
...
setFPHoldTillEndXact(Oid relid)
{
	...
	for (f = 0; f < FastPathLockSlotsPerBackend(); ++f)
	{
		uint32 lockbits;
