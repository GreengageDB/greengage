...
WaitReadBuffers(ReadBuffersOperation *operation)
{
	...
	/*
	 * Temp tables in Greenplum use shared buffers so that backends executing
	 * multiple slices of the same query can share them.  Because of that all
	 * buffers handled here are shared buffers, never local ones.
	 */
	...
	for (int i = 0; i < nblocks; ++i)
	{
		...
		if (!isLocalBuf)
			for (int j = 0; j < io_buffers_len; ++j)
				BufferMProtect(GetBufferDescriptor(io_buffers[j] - 1),
