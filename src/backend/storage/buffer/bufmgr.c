						  ...
						  BufferAccessStrategy strategy, bool permanent)
{
	 ...
	 * shared buffers
	 */
	SMgrRelation smgr = smgropen(rlocator, InvalidBackendId, 0);
						...
						uint32 *extended_by)
{
	...
	for (int i = 0; i < extend_by; i++)
	{
		...
		else
		{
			...
			else if (eb.relpersistence == RELPERSISTENCE_TEMP)
				buf_state |= BM_TEMP;
							   ...
							   ForkNumber forkNum, bool permanent)
{
	...
	nblocks = smgrnblocks(smgropen(srclocator, InvalidBackendId, 0),
						  forkNum);
	...
	smgrextend(smgropen(dstlocator, InvalidBackendId, 0), forkNum, nblocks - 1,
			   buf.data, true);
						  ...
						  RelFileLocator dst_rlocator, bool permanent)
{
	...
	{
		if (smgrexists(smgropen(src_rlocator, InvalidBackendId, 0), forkNum))
		{
			smgrcreate(smgropen(dst_rlocator, InvalidBackendId, 0), forkNum, false);
