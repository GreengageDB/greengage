						  ...
						  BufferAccessStrategy strategy, bool permanent)
{
	 ...
	 * shared buffers
	 */
	SMgrRelation smgr = smgropen(rlocator, INVALID_PROC_NUMBER, 0);
						...
						uint32 *extended_by)
{
	 ...
	 * tags the buffers with BM_TEMP instead.
	 */
			...
			IOContext io_context)
{
	...
	if (reln == NULL)
	{
		...
		reln = smgropen(BufTagGetRelFileLocator(&buf->tag),
						istemp ? TempRelBackendId : INVALID_PROC_NUMBER, 0);
	}
							   ...
							   ForkNumber forkNum, bool permanent)
{
	...
	nblocks = smgrnblocks(smgropen(srclocator, INVALID_PROC_NUMBER, 0),
						  forkNum);
	...
	smgrextend(smgropen(dstlocator, INVALID_PROC_NUMBER, 0), forkNum, nblocks - 1,
			   buf.data, true);
	...
	src_smgr = smgropen(srclocator, INVALID_PROC_NUMBER, SMGR_MD);
	...
	for (blkno = 0; blkno < nblocks; blkno++)
	{
		 ...
		 * this in PG16; the read_stream loop copies all blocks so block
						  ...
						  RelFileLocator dst_rlocator, bool permanent)
{
	...
	src_rel = smgropen(src_rlocator, INVALID_PROC_NUMBER, 0);
	dst_rel = smgropen(dst_rlocator, INVALID_PROC_NUMBER, 0);
	 ...
	 */
	RelationCreateStorage(dst_rlocator, relpersistence, SMGR_MD, false);
...
AbortBufferIO(Buffer buffer)
{
	...
	else
	{
		...
		if (buf_state & BM_IO_ERROR)
		{
			...
			/*
			 * GPDB: a temporary relation (BM_TEMP) lives in shared buffers, so
			 * its path must be reported with the temp backend sentinel rather
			 * than as a permanent relation.
			 */
			RelPathStr	path = relpathbackend(BufTagGetRelFileLocator(&buf_hdr->tag),
											  (buf_state & BM_TEMP) ?
											  TempRelBackendId : INVALID_PROC_NUMBER,
...
shared_buffer_write_error_callback(void *arg)
{
	...
	if (bufHdr != NULL)
	{
		...
		/*
		 * GPDB: a temporary relation (BM_TEMP) lives in shared buffers, so its
		 * path must be reported with the temp backend sentinel rather than as
		 * a permanent relation.
		 */
		RelPathStr	path = relpathbackend(BufTagGetRelFileLocator(&bufHdr->tag),
										  (buf_state & BM_TEMP) ?
										  TempRelBackendId : INVALID_PROC_NUMBER,
