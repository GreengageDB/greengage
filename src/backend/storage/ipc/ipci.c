...
CreateSharedMemoryAndSemaphores(void)
{
	...
	/*
	 * GPDB: request our built-in named LWLock tranches.  RequestNamedLWLockTranche()
	 * is only allowed while process_shmem_requests_in_progress is set (normally
	 * during the postmaster's shmem_request_hook).  Doing it here covers the
	 * bootstrap/single-user paths too (which never run that hook) and happens
	 * before CalculateShmemSize() so the computed size accounts for these locks.
	 */
