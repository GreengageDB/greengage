...
/*
 * SendQueryFinishRequest
 *		GPDB: mirror of SendCancelRequest, but instead of interrupting the
 *		target backend with SIGINT it delivers PROCSIG_QUERY_FINISH, asking the
 *		backend to finish the current query gracefully.  Used to service a
 *		FINISH_REQUEST_CODE startup packet, which carries the same backend PID
 *		and cancel key as a cancel request.
 */
void
SendQueryFinishRequest(int backendPID, const uint8 *cancel_key, int cancel_key_len)
{
	Assert(backendPID != 0);

	/*
	 * See SendCancelRequest for a discussion of the races involved in reading
	 * pss_pid / pss_cancel_key.
	 */
	for (int i = 0; i < NumProcSignalSlots; i++)
	{
		ProcSignalSlot *slot = &ProcSignal->psh_slot[i];
		...
		else
		{
			...
			if (match)
			{
				/* Found a match; ask that backend to finish its query */
				ereport(DEBUG2,
						(errmsg_internal("query finish request to process %d",
										 backendPID)));

				SendProcSignal(backendPID, PROCSIG_QUERY_FINISH,
							   INVALID_PROC_NUMBER);
			}
			else
			{
