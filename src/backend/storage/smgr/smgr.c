...
/*
 * GPDB: unlike upstream, the f_smgr struct definition lives in smgr.h so
 * that smgr_hook implementations can provide their own function tables.
 */
...
const f_smgr *
smgr_standard(ProcNumber backend, RelFileLocator rlocator, SMgrImpl which)
{
	// for md.c
	return &smgrsw[which];
}
...
const f_smgr *
smgr(ProcNumber backend, RelFileLocator rlocator, SMgrImpl which)
{
	const f_smgr *result;

	if (smgr_hook)
 	{
		result = (*smgr_hook)(backend, rlocator, which);
 	}
	else
		result = smgr_standard(backend, rlocator, which);
 ...
 */
SMgrRelation
smgropen(RelFileLocator rlocator, ProcNumber backend, SMgrImpl which)
{
	RelFileLocatorBackend brlocator;
	SMgrRelation reln;
	bool		found;

	/* GPDB: don't support MyProcNumber as a possible backend. */
	Assert(backend == INVALID_PROC_NUMBER || backend == ProcNumberForTempRelations());
	...
	if (!found)
	{
		...
		reln->storageManager = smgr(backend, rlocator, which);
...
smgrexists(SMgrRelation reln, ForkNumber forknum)
{
	...
	HOLD_INTERRUPTS();
	ret = (*reln->storageManager).smgr_exists(reln, forknum);
	RESUME_INTERRUPTS();
...
smgrcreate_ao(RelFileNodeBackend rnode, int32 segmentFileNum, bool isRedo)
{
	...
	{
		/*
		 * GPDB: the AO storage layer still deals in RelFileNode terms;
		 * convert to the RelFileLocator form the hook expects.
		 */
...
smgrdounlinkall(SMgrRelation *rels, int nrels, bool isRedo)
{
	...
	{
		for (forknum = 0; forknum <= MAX_FORKNUM; forknum++)
			(*rels[i]->storageManager).smgr_unlink(rlocators[i], forknum, isRedo);
	}
			   ...
			   int nblocks, bool skipFsync)
{
	...
	(*reln->storageManager).smgr_zeroextend(reln, forknum, blocknum,
											nblocks, skipFsync);
			 ...
			 int nblocks)
{
	...
	HOLD_INTERRUPTS();
	ret = (*reln->storageManager).smgr_prefetch(reln, forknum, blocknum, nblocks);
	RESUME_INTERRUPTS();
...
{
	HOLD_INTERRUPTS();
	(*reln->storageManager).smgr_readv(reln, forknum, blocknum, buffers,
									   nblocks);
	RESUME_INTERRUPTS();
}
...
{
	HOLD_INTERRUPTS();
	(*reln->storageManager).smgr_writev(reln, forknum, blocknum,
			 ...
			 BlockNumber *old_nblocks, BlockNumber *nblocks)
{
	...
	for (i = 0; i < nforks; i++)
	{
		...
		(*reln->storageManager).smgr_truncate(reln, forknum[i],
...
smgr_aio_reopen(PgAioHandle *ioh)
{
	...
	reln = smgropen(sd->smgr.rlocator, procno, SMGR_MD);
