...
ForgetDatabaseSyncRequests(Oid dbid)
{
	 ...
	 * pending entries.
	 *
	 * GPDB: the AO sync handler (INIT_FILETAG) still keys off RelFileNode, so
	 * build one matching the RelFileLocator above (only dbOid is compared by
	 * mdfiletagmatches()).
	 */
	rnode.dbNode = dbid;
	rnode.spcNode = 0;
...
DropRelationFiles(RelFileNodePendingDelete *delrels, int ndelrels, bool isRedo)
{
	 ...
	 * (Since PG17 the smgr owner model is gone and smgrclose() merely
	 * releases resources, but keep the reverse order for symmetry.)
	 */
...
{
	SMgrRelation reln = smgropen(ftag->rlocator, INVALID_PROC_NUMBER, SMGR_MD);
	File		file;
	instr_time	io_start;
	bool		need_to_close;
	int			result,
...
{
	SMgrRelation reln = smgropen(ftag->rlocator, INVALID_PROC_NUMBER, SMGR_AO);
	char	   *p;
