 ...
 * temp_spill_files_tablespaces. (Some entries may be InvalidOid,
...
GetTempTablespaces(Oid *tableSpaces, int numSpaces)
{
	 ...
	 */
	if (tempSpillFilesTableSpaces.numTempTableSpaces > 0) {
		Assert(TempFileTablespacesAreSet() || IsAbortInProgress());
		for (i = 0; i < tempSpillFilesTableSpaces.numTempTableSpaces && i < numSpaces; ++i)
			tableSpaces[i] = tempSpillFilesTableSpaces.tempTableSpaces[i];
	}
	else {
		Assert(TempTablespacesAreSet() || IsAbortInProgress());
		for (i = 0; i < tempTableSpaces.numTempTableSpaces && i < numSpaces; ++i)
			tableSpaces[i] = tempTableSpaces.tempTableSpaces[i];
	}
 ...
 * closed. We also forget any transaction-local temp_tablespace and
 * temp_spill_files_tablespaces lists.
 *
