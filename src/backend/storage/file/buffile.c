...
struct BufFile
{
	...
	/*
	 * GPDB: PG upstream uses PGAlignedBlock here (with an XXX note that it
	 * should ideally use PGIOAlignedBlock, but might need a way to avoid
	 * wasting per-file alignment padding when some users create many files).
	 * We use a palloc'd buffer instead.
	 */
	FakeAlignedBlock buffer;
