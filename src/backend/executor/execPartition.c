...
get_partition_for_tuple(PartitionKey key, PartitionDesc partdesc, Datum *values, bool *isnull)
{
	...
	/*
	 * GPDB: get_partition_for_tuple() is exported and can be called (e.g. from
	 * tablecmds_gp.c) with a partdesc that has no partitions.  Bail out early
	 * in that case so we don't dereference an empty/NULL boundinfo below.
	 */
	if (partdesc->nparts == 0)
		return part_index;
...
ExecDoInitialPruning(EState *estate)
{
	...
	foreach(lc, estate->es_part_prune_infos)
	{
		...
		if (prunestate->do_initial_prune)
			validsubplans = ExecFindMatchingSubPlans(prunestate, estate, -1,
													 NIL, true,
													 &validsubplan_rtis);
		else
 ...
 * and subplan-map fixup that ExecInitPartitionExecPruning performs.  The
 * caller must already have set up the node's ExprContext
 * (ExecAssignExprContext); ExecPartitionSelector stores each probe row in that
 * ExprContext's ecxt_outertuple.
 *
 * PG18 split exec-pruning context setup out of CreatePartitionPruneState()
 * into InitExecPartitionPruneContexts(), which is normally reached via
 * ExecInitPartitionExecPruning().  Since the PartitionSelector path never goes
 * through ExecInitPartitionExecPruning(), we finalize the exec contexts here.
 * The selector evaluates its join expressions against the outer tuple, so the
 * exec contexts must reference planstate->ps_ExprContext (not the throwaway
 * ExprContext that CreatePartitionPruneState() creates internally).
 */
PartitionPruneState *
							  ...
							  PartitionPruneInfo *partitionpruneinfo)
{
	...
	prunestate = CreatePartitionPruneState(planstate->state,
										   partitionpruneinfo,
										   &all_leafpart_rtis);

	/*
	 * Initialize the exec-pruning contexts, wiring them to the selector's own
	 * ExprContext so ExecPartitionSelector's ecxt_outertuple is visible during
	 * pruning.  There are no parent subplans to re-sequence, so pass an empty
	 * initially_valid_subplans / zero total to skip the subplan-map fixup.
	 */
	if (prunestate->do_exec_prune)
	{
		prunestate->econtext = planstate->ps_ExprContext;
		InitExecPartitionPruneContexts(prunestate, planstate, NULL, 0);
	}
...
ExecAddMatchingSubPlans(PartitionPruneState *prunestate, Bitmapset *result)
{
	...
	thisresult = ExecFindMatchingSubPlans(prunestate, NULL, -1, NIL, false, NULL);
