...
ldelete:
	 ...
	 *
	 * In a split update, the processed rows are returned by the INSERT of the
	 * new row, not the DELETE of the old one.
	 *
	 * If this is part of a cross-partition UPDATE, and the RETURNING list
					   ...
					   bool canSetTag)
{
	...
	slot = ExecInsert(context, resultRelInfo, slot, canSetTag,
			...
			case CMD_UPDATE:
				...
				else
				{
					...
					result = ExecUpdateAct(context, resultRelInfo, tupleid,
										   NULL, newslot, GpIdentity.segindex,
										   canSetTag, &updateCxt);
		...
		switch (operation)
		{
			...
			case CMD_UPDATE:
				...
				if (AttributeNumberIsValid(action_attno))
				{
					if (action == DML_INSERT)
					{
						...
						else
						{
							...
							slot = ExecGetInsertNewTuple(insertRelInfo, context.planSlot);
						}
						estate->es_result_relation_info = insertRelInfo;
						slot = ExecSplitUpdate_Insert(&context, slot,
													  node->canSetTag);
					}
					else
					{
						...
						slot = ExecDelete(&context, resultRelInfo, tupleid, segid,
										  oldtuple,
										  false,	/* processReturning */
										  false,	/* changingPart */
										  false,	/* canSetTag */
										  true,		/* splitUpdate */
										  NULL /* tmresult */, NULL, NULL);
					}
					break;
				}
				...
				slot = ExecUpdate(&context, resultRelInfo, tupleid, oldtuple,
								  oldSlot, slot, segid, node->canSetTag);
				if (tuplock)
			...
			case CMD_DELETE:
				slot = ExecDelete(&context, resultRelInfo, tupleid, segid,
								  ...
								  false /* splitUpdate */, NULL /* tmresult */,
								  NULL, NULL);
				break;
...
ExecInitModifyTable(ModifyTable *node, EState *estate, int eflags)
{
	...
	{
		/*
		 * GPDB: node->isSplitUpdates carries one flag per node->resultRelations
		 * (the unpruned list), so size and validate against total_nrels rather
		 * than the pruned nrels.
		 */
		mtstate->mt_isSplitUpdates = (bool *) palloc0(total_nrels * sizeof(bool));
		if (node->isSplitUpdates)
		{
			if (list_length(node->isSplitUpdates) != total_nrels)
