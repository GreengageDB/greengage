...
{
	/*
	 * Release slots and tuplestore resources (also clears the per-function
	 * scan slots). ExecFreeExprContext is gone in PG17; the ExprContext is
	 * released by the executor framework.
	 */
	ExecEagerFreeFunctionScan(node);

	/*
	 * GPDB: destroy the shared-input tuplestore reader if it exists.
	 */
