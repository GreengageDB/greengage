					 ...
					 bool ereport_on_violation)
{
	...
	foreach(l, rangeTable)
	{
		...
		if (rte->perminfoindex != 0)
		{
			 ...
			 */
			/*
			 * GPDB: a view (or relation) RTE that the planner pulled up is
			 * marked RTE_VOID but keeps its perminfoindex so the object's ACL
			 * is still checked (ExecCheckOneRelPerms uses the RTEPermissionInfo,
			 * not the RTE), so allow RTE_VOID here as well.
			 */
	...
	foreach(l, rteperminfos)
	{
		...
		/*
		 * GPDB: on non-root slices the dispatcher clears the write-permission
		 * bits (see the sliceTable handling in PostgresMain), which can leave a
		 * perminfo with requiredPerms == 0.  Such an entry needs no permission
		 * check here (the coordinator already checked), and ExecCheckOneRelPerms
		 * would assert on it, so skip it.
		 */
					...
					List *mergeActions)
{
	...
	switch (resultRel->rd_rel->relkind)
	{
		...
		case RELKIND_VIEW:
			 ...
			 *
			 * GPDB: DML on views is unconditionally rejected, so upstream's
			 * view_has_instead_trigger()/error_view_not_updatable() path (PG17)
			 * is intentionally not reached.
			 */
			ereport(ERROR,
