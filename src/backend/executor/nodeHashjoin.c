		...
		switch (node->hj_JoinState)
		{
			case HJ_BUILD_HASHTABLE:
				...
				if (!node->prefetch_inner)
				{
					 ...
					 * win to check this when the outer relation's startup cost
					 * is less than the projected cost of building the hash
					 * table.  Otherwise it's best to build the hash table first
					 * and see if the inner relation is empty.  (When it's a
					 * left join, we should always make this check, since we
					 * aren't going to be able to skip the join on the strength
					  ...
					  MemoryContext bfCxt)
{
	 ...
	 * the bfCxt context, NOT in the batchCxt.
	 *
