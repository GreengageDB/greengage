	...
	)
{
	...
	return GPOS_NEW(context->m_mp) CTranslatorQueryToDXL
		(
		context,
		md_accessor,
...
CTranslatorQueryToDXL::TranslateInsertQueryToDXL()
{
	...
	for (ULONG ul = 0; ul < num_table_columns; ul++)
	{
		...
		CDXLNode *project_elem_dxlnode = CTranslatorUtils::CreateDXLProjElemConstNULL(m_mp, m_md_accessor, m_context->m_colid_counter, mdcol);
...
//	List *col_names = into_clause->colNames;
	...
	for (ULONG ul = 0; ul < num_columns; ul++)
	{
		...
		CDXLColDescr *dxl_col_descr = GPOS_NEW(m_mp) CDXLColDescr
											...
											m_mp,
											md_colname,
											m_context->m_colid_counter->next_id(),
											resno /* attno */,
											mdid,
	...
	)
{
	...
	ForEach (lc, window_clause)
	{
		...
		if (NULL != wc->name)
		{
			CWStringDynamic *alias_str = CDXLUtils::CreateDynamicStringFromCharArray(m_mp, wc->name);
		...
		if (0 < gpdb::ListLength(wc->orderClause))
		{
			...
			CDXLNodeArray *dxl_sort_cols = TranslateSortColumsToDXL(wc->orderClause, sort_col_attno_to_colid_mapping);
	...
	const
{
	...
	ForEach (lc, sort_clause)
	{
		...
		SortGroupClause *sort_group_clause = (SortGroupClause *) node_sort_clause;
	...
	)
{
	...
	CDXLNodeArray *dxl_sort_cols = TranslateSortColumsToDXL(sort_clause, grpcols_to_colid_mapping);
	const ULONG size = dxl_sort_cols->Size();
	for (ULONG ul = 0; ul < size; ul++)
	{
	...
	)
{
	...
	CDXLLogicalTVF *tvf_dxlop = CTranslatorUtils::ConvertToCDXLLogicalTVF(m_mp, m_md_accessor, m_context->m_colid_counter, rte);
	...
	ForEach (lc, funcexpr->args)
	{
		...
		CDXLNode *func_expr_arg_dxlnode =
				m_scalar_translator->TranslateScalarToDXL((Expr *) arg_node, m_var_to_colid_map);
...
{
	CDXLNode *scalar_dxlnode = m_scalar_translator->TranslateScalarToDXL(expr, m_var_to_colid_map);
	...
	)
{
	...
	ForEach (lc, cte_list)
	{
		...
		CDXLLogicalCTEProducer *lg_cte_prod_dxlop = GPOS_NEW(m_mp) CDXLLogicalCTEProducer(m_mp, m_context->m_cte_id_counter->next_id(), colid_array);
	...
	const
{
	...
	for (ULONG ul = 0; ul < size; ul++)
	{
		colid_array->Append(GPOS_NEW(mp) ULONG(m_context->m_colid_counter->next_id()));
	}
