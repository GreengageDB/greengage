											  ...
											  bool can_set_tag)
{
	...
	planned_stmt->queryPartOids =
		m_dxl_to_plstmt_context->GetPartitionedTablesList();
	planned_stmt->canSetTag = can_set_tag;
	planned_stmt->relationOids = oids_list;
	planned_stmt->numSelectorsPerScanId =
		m_dxl_to_plstmt_context->GetNumPartitionSelectorsList();

	plan->nMotionNodes = m_dxl_to_plstmt_context->GetCurrentMotionId() - 1;
	planned_stmt->nMotionNodes =
	...
	if (CMD_SELECT == m_cmd_type && NULL != dxlnode->GetDXLDirectDispatchInfo())
	{
		...
		{
			List *motion_node_list =
				gpdb::ExtractNodesPlan(planned_stmt->planTree, T_Motion,
			...
			ForEach(lc, motion_node_list)
			{
				...
				motion->plan.directDispatch.contentIds =
					plan->directDispatch.contentIds;
			}
		}
	}
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	return (this->*dxlnode_to_logical_funct)(dxlnode, output_context,
											 ctxt_translation_prev_siblings);
}
...
CTranslatorDXLToPlStmt::SetInitPlanVariables(PlannedStmt *planned_stmt)
{
	...
	{
		subplan_list =
			gpdb::ExtractNodesPlan((Plan *) lfirst(lc), T_SubPlan, true);
		ListCell *lc2;
	...
	if (subplan->is_initplan)
	{
		...
		if (1 < m_dxl_to_plstmt_context->GetCurrentMotionId())
		{
			subplan->qDispSliceId =
				m_dxl_to_plstmt_context->GetCurrentMotionId() +
				subplan->plan_id - 1;
		}
		else
		{
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	{
		const IMDRelationExternal *md_rel_ext =
			dynamic_cast<const IMDRelationExternal *>(md_rel);
		...
		ext_scan->uriList =
			gpdb::GetExternalScanUriList(ext_table_entry, &isMasterOnly);
		...
		GPOS_ASSERT((IMDRelation::EreldistrMasterOnly ==
					 md_rel_ext->GetRelDistribution()) == isMasterOnly);
	...
	TranslatePlanCosts(CDXLPhysicalProperties::PdxlpropConvert(
						   tbl_scan_dxlnode->GetProperties())
						   ->GetDXLOperatorCost(),
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	// translate operator costs
	TranslatePlanCosts(CDXLPhysicalProperties::PdxlpropConvert(
						   index_scan_dxlnode->GetProperties())
						   ->GetDXLOperatorCost(),
					   &(plan->startup_cost), &(plan->total_cost),
					   &(plan->plan_rows), &(plan->plan_width));
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	// translate operator costs
	TranslatePlanCosts(
		CDXLPhysicalProperties::PdxlpropConvert(limit_dxlnode->GetProperties())
			->GetDXLOperatorCost(),
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	// translate operator costs
	TranslatePlanCosts(
		CDXLPhysicalProperties::PdxlpropConvert(hj_dxlnode->GetProperties())
			->GetDXLOperatorCost(),
		&(plan->startup_cost), &(plan->total_cost), &(plan->plan_rows),
		&(plan->plan_width));
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	// translate operator costs
	TranslatePlanCosts(
		CDXLPhysicalProperties::PdxlpropConvert(tvf_dxlnode->GetProperties())
			->GetDXLOperatorCost(),
		&(plan->startup_cost), &(plan->total_cost), &(plan->plan_rows),
		&(plan->plan_width));
				...
				right_dxl_translate_ctxt.GetParamIdMappingElement(ulColid))
			{
				CMappingElementColIdParamId *pmecolidparamid =
					GPOS_NEW(m_mp) CMappingElementColIdParamId(
						ulColid, m_dxl_to_plstmt_context->GetNextParamId(),
						pmdid, iTypeModifier);
#ifdef GPOS_DEBUG
				BOOL fInserted =
#endif
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	// translate operator costs
	TranslatePlanCosts(CDXLPhysicalProperties::PdxlpropConvert(
						   merge_join_dxlnode->GetProperties())
						   ->GetDXLOperatorCost(),
					   &(plan->startup_cost), &(plan->total_cost),
					   &(plan->plan_rows), &(plan->plan_width));
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	// translate operator costs
	TranslatePlanCosts(
		CDXLPhysicalProperties::PdxlpropConvert(motion_dxlnode->GetProperties())
			->GetDXLOperatorCost(),
		&(plan->startup_cost), &(plan->total_cost), &(plan->plan_rows),
		&(plan->plan_width));
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	// translate operator costs
	TranslatePlanCosts(
		CDXLPhysicalProperties::PdxlpropConvert(agg_dxlnode->GetProperties())
			->GetDXLOperatorCost(),
		&(plan->startup_cost), &(plan->total_cost), &(plan->plan_rows),
		&(plan->plan_width));
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	TranslatePlanCosts(
		CDXLPhysicalProperties::PdxlpropConvert(window_dxlnode->GetProperties())
			->GetDXLOperatorCost(),
		&(plan->startup_cost), &(plan->total_cost), &(plan->plan_rows),
		&(plan->plan_width));
	...
	if (size == 1)
	{
		...
		TranslateSortCols(sort_col_list_dxlnode, &child_context,
						  window->ordColIdx, window->ordOperators, NULL,
						  is_nulls_first);
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	// translate operator costs
	TranslatePlanCosts(
		CDXLPhysicalProperties::PdxlpropConvert(sort_dxlnode->GetProperties())
			->GetDXLOperatorCost(),
		&(plan->startup_cost), &(plan->total_cost), &(plan->plan_rows),
		&(plan->plan_width));
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	// translate operator costs
	TranslatePlanCosts(CDXLPhysicalProperties::PdxlpropConvert(
						   subquery_scan_dxlnode->GetProperties())
						   ->GetDXLOperatorCost(),
					   &(plan->startup_cost), &(plan->total_cost),
					   &(plan->plan_rows), &(plan->plan_width));
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	// translate operator costs
	TranslatePlanCosts(
		CDXLPhysicalProperties::PdxlpropConvert(result_dxlnode->GetProperties())
			->GetDXLOperatorCost(),
		&(plan->startup_cost), &(plan->total_cost), &(plan->plan_rows),
		&(plan->plan_width));
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	const ULONG num_of_levels =
		partition_selector_dxlop->GetPartitioningLevel();
	...
	partition_selector->relid =
		CMDIdGPDB::CastMdid(partition_selector_dxlop->GetRelMdId())->Oid();
	partition_selector->selectorId = m_partition_selector_counter++;

	// translate operator costs
	TranslatePlanCosts(CDXLPhysicalProperties::PdxlpropConvert(
						   partition_selector_dxlnode->GetProperties())
						   ->GetDXLOperatorCost(),
					   &(plan->startup_cost), &(plan->total_cost),
					   &(plan->plan_rows), &(plan->plan_width));
	...
	CDXLNode *project_list_dxlnode =
		(*partition_selector_dxlnode)[EdxlpsIndexProjList];
	CDXLNode *eq_filters_dxlnode =
		(*partition_selector_dxlnode)[EdxlpsIndexEqFilters];
	CDXLNode *filters_dxlnode =
		(*partition_selector_dxlnode)[EdxlpsIndexFilters];
	CDXLNode *residual_filter_dxlnode =
		(*partition_selector_dxlnode)[EdxlpsIndexResidualFilter];
	CDXLNode *proj_expr_dxlnode =
		(*partition_selector_dxlnode)[EdxlpsIndexPropExpr];
	...
	partition_selector->levelEqExpressions = TranslateDXLFilterList(
		eq_filters_dxlnode, NULL /* base_table_context */, child_contexts,
		output_context, true /* is_eq_filters */,
		false /* is_eq_filter_elems */);
	...
	partition_selector->levelExpressions = TranslateDXLFilterList(
		...
		output_context, false /* is_eq_filters */,
		false /* is_eq_filter_elems */);
	...
	CMappingColIdVarPlStmt colid_var_mapping = CMappingColIdVarPlStmt(
		...
		m_dxl_to_plstmt_context);
	if (!m_translator_dxl_to_scalar->HasConstTrue(residual_filter_dxlnode,
												  m_md_accessor))
	{
		partition_selector->residualPredicate =
			(Node *) m_translator_dxl_to_scalar->TranslateDXLToScalar(
				residual_filter_dxlnode, &colid_var_mapping);
	}

	//translate propagation expression
	if (!m_translator_dxl_to_scalar->HasConstNull(proj_expr_dxlnode))
	{
		partition_selector->propagationExpression =
			(Node *) m_translator_dxl_to_scalar->TranslateDXLToScalar(
				proj_expr_dxlnode, &colid_var_mapping);
	}
	...
	bool is_eq_filter_elems)
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		if (!is_eq_filter_elems && m_translator_dxl_to_scalar->HasConstTrue(
		...
		if (is_eq_filters)
		{
			List *eq_filter_elems = TranslateDXLFilterList(
				child_filter_dxlnode, base_table_context, child_contexts,
				output_context, false /* is_eq_filters */,
				true /* is_eq_filter_elems */);
			filters_list = gpdb::LAppend(filters_list, eq_filter_elems);
		}
		else
		{
			Expr *filter_expr =
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	// translate operator costs
	TranslatePlanCosts(
		CDXLPhysicalProperties::PdxlpropConvert(append_dxlnode->GetProperties())
			->GetDXLOperatorCost(),
		&(plan->startup_cost), &(plan->total_cost), &(plan->plan_rows),
		&(plan->plan_width));
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	// translate operator costs
	TranslatePlanCosts(CDXLPhysicalProperties::PdxlpropConvert(
						   materialize_dxlnode->GetProperties())
						   ->GetDXLOperatorCost(),
					   &(plan->startup_cost), &(plan->total_cost),
					   &(plan->plan_rows), &(plan->plan_width));
	...
	if (materialize_dxlop->IsSpooling())
	{
		...
		materialize->share_type =
			(0 < materialize_dxlop->GetNumConsumerSlices())
				? SHARE_MATERIAL_XSLICE
				: SHARE_MATERIAL;
	}
	else
	{
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	TranslatePlanCosts(CDXLPhysicalProperties::PdxlpropConvert(
						   cte_producer_dxlnode->GetProperties())
						   ->GetDXLOperatorCost(),
					   &(plan->startup_cost), &(plan->total_cost),
					   &(plan->plan_rows), &(plan->plan_width));
	...
	if (!IsA(child_plan, Material) && !IsA(child_plan, Sort))
	{
		...
		materialize_plan->plan_node_id =
			m_dxl_to_plstmt_context->GetNextPlanId();
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	// translate operator costs
	TranslatePlanCosts(CDXLPhysicalProperties::PdxlpropConvert(
						   cte_consumer_dxlnode->GetProperties())
						   ->GetDXLOperatorCost(),
					   &(plan->startup_cost), &(plan->total_cost),
					   &(plan->plan_rows), &(plan->plan_width));
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	// translate operator costs
	TranslatePlanCosts(CDXLPhysicalProperties::PdxlpropConvert(
						   sequence_dxlnode->GetProperties())
						   ->GetDXLOperatorCost(),
					   &(plan->startup_cost), &(plan->total_cost),
					   &(plan->plan_rows), &(plan->plan_width));
	...
	Plan *last_child_plan = TranslateDXLOperatorToPlan(
		last_child_dxlnode, &child_context, ctxt_translation_prev_siblings);
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	// translate operator costs
	TranslatePlanCosts(CDXLPhysicalProperties::PdxlpropConvert(
						   dyn_tbl_scan_dxlnode->GetProperties())
						   ->GetDXLOperatorCost(),
					   &(plan->startup_cost), &(plan->total_cost),
					   &(plan->plan_rows), &(plan->plan_width));
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	dyn_idx_scan->partIndexPrintable =
		dyn_index_scan_dxlop->GetPartIndexIdPrintable();
	...
	dyn_idx_scan->logicalIndexInfo =
		gpdb::GetLogicalIndexInfo(rte->relid, index_oid);
	...
	// translate operator costs
	TranslatePlanCosts(CDXLPhysicalProperties::PdxlpropConvert(
						   dyn_idx_scan_dxlnode->GetProperties())
						   ->GetDXLOperatorCost(),
					   &(plan->startup_cost), &(plan->total_cost),
					   &(plan->plan_rows), &(plan->plan_width));
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	if (IMDRelation::EreldistrMasterOnly !=
		m_md_accessor->RetrieveRel(mdid_target_table)->GetRelDistribution())
	{
	...
	// translate operator costs
	TranslatePlanCosts(
		CDXLPhysicalProperties::PdxlpropConvert(dml_dxlnode->GetProperties())
			->GetDXLOperatorCost(),
		&(plan->startup_cost), &(plan->total_cost), &(plan->plan_rows),
		&(plan->plan_width));
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	const TargetEntry *te_ctid_col =
		output_context->GetTargetEntry(phy_split_dxlop->GetCtIdColId());
	...
	// translate operator costs
	TranslatePlanCosts(
		CDXLPhysicalProperties::PdxlpropConvert(split_dxlnode->GetProperties())
			->GetDXLOperatorCost(),
		&(plan->startup_cost), &(plan->total_cost), &(plan->plan_rows),
		&(plan->plan_width));
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	// translate operator costs
	TranslatePlanCosts(
		CDXLPhysicalProperties::PdxlpropConvert(assert_dxlnode->GetProperties())
			->GetDXLOperatorCost(),
		&(plan->startup_cost), &(plan->total_cost), &(plan->plan_rows),
		&(plan->plan_width));
...
Plan *
CTranslatorDXLToPlStmt::TranslateDXLRowTrigger(
	const CDXLNode *row_trigger_dxlnode, CDXLTranslateContext *output_context,
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	CDXLPhysicalRowTrigger *phy_row_trigger_dxlop =
		CDXLPhysicalRowTrigger::Cast(row_trigger_dxlnode->GetOperator());
	...
	Oid relid_oid =
		CMDIdGPDB::CastMdid(phy_row_trigger_dxlop->GetRelMdId())->Oid();
	...
	{
		row_trigger->oldValuesColIdx = CTranslatorUtils::ConvertColidToAttnos(
			colids_old_array, &child_context);
	}
	...
	{
		row_trigger->newValuesColIdx = CTranslatorUtils::ConvertColidToAttnos(
			colids_new_array, &child_context);
	}
	...
	// translate operator costs
	TranslatePlanCosts(CDXLPhysicalProperties::PdxlpropConvert(
						   row_trigger_dxlnode->GetProperties())
						   ->GetDXLOperatorCost(),
					   &(plan->startup_cost), &(plan->total_cost),
					   &(plan->plan_rows), &(plan->plan_width));
...
void
CTranslatorDXLToPlStmt::TranslatePlanCosts(
	const CDXLOperatorCost *dxl_operator_cost, Cost *startup_cost_out,
	Cost *total_cost_out, Cost *cost_rows_out, INT *width_out)
{
	...
	*width_out =
		CTranslatorUtils::GetIntFromStr(dxl_operator_cost->GetWidthStr());
}
	...
	BOOL is_resjunk)
{
	...
	TargetEntry *te_new =
		gpdb::MakeTargetEntry((Expr *) var, resno, resname_str, is_resjunk);
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	// translate operator costs
	TranslatePlanCosts(
		CDXLPhysicalProperties::PdxlpropConvert(ctas_dxlnode->GetProperties())
			->GetDXLOperatorCost(),
		&(plan->startup_cost), &(plan->total_cost), &(plan->plan_rows),
		&(plan->plan_width));
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	// translate operator costs
	TranslatePlanCosts(CDXLPhysicalProperties::PdxlpropConvert(
						   bitmapscan_dxlnode->GetProperties())
						   ->GetDXLOperatorCost(),
					   &(plan->startup_cost), &(plan->total_cost),
					   &(plan->plan_rows), &(plan->plan_width));
	...
	BitmapHeapScan *bitmap_tbl_scan)
{
	...
	if (IsA(bitmap_tbl_scan, DynamicBitmapHeapScan))
	{
		...
		dyn_bitmap_idx_scan->partIndex =
			((DynamicBitmapHeapScan *) bitmap_tbl_scan)->partIndex;
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	// translate operator costs
	TranslatePlanCosts(CDXLPhysicalProperties::PdxlpropConvert(
						   value_scan_dxlnode->GetProperties())
						   ->GetDXLOperatorCost(),
					   &(plan->startup_cost), &(plan->total_cost),
					   &(plan->plan_rows), &(plan->plan_width));
