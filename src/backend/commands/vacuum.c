...
static void dispatchVacuum(VacuumParams *params, Oid relid,
						   VacuumStatsContext *ctx, MemoryContext vac_context);
				  ...
				  int options)
{
	...
	else
	{
		 ...
		 * required privileges.
		 */
		if (vacuum_is_permitted_for_relation(relid, classForm, options) && !skip_this)
		{
			oldcontext = MemoryContextSwitchTo(vac_context);
		...
		if ((options & VACOPT_VACUUM) && include_parts && !include_children)
			ereport(WARNING,
		   ...
		   bool recursing)
{
	...
	if (params->options & VACOPT_PROCESS_MAIN)
	{
		 ...
		 *
		 * GPDB: append-optimized tables are never rewritten through
		 * cluster_rel(); even under VACUUM FULL their space is reclaimed by
		 * the AO vacuum phases orchestrated below, so route them through
		 * table_relation_vacuum() regardless of the FULL flag.
		 */
	...
	if (is_appendoptimized && ao_vacuum_phase == 0)
	{
		...
		vacuum_rel(relid, this_rangevar, params, bstrategy, vac_context, false);
	...
	if (toast_relid != InvalidOid)
		vacuum_rel(toast_relid, NULL, params, bstrategy, vac_context, true);

	/* do the same for an AO segments table, if any */
	if (aoseg_relid != InvalidOid)
		vacuum_rel(aoseg_relid, NULL, params, bstrategy, vac_context, true);

	/* do the same for an AO block directory table, if any */
	if (aoblkdir_relid != InvalidOid)
		vacuum_rel(aoblkdir_relid, NULL, params, bstrategy, vac_context, true);
		...
		(!is_appendoptimized || ao_vacuum_phase))
	{
		...
		dispatchVacuum(params, relid, &stats_context, vac_context);
...
vacuum_params_to_options_list(VacuumParams *params)
{
	...
	/*
	 * VACOPT_PROCESS_MAIN (new in PG16) is also set by default; pass it
	 * through the same way so the segment agrees with the coordinator on
	 * whether to process the main relation.
	 */
	options = lappend(options, makeDefElem("process_main",
										   (Node *) makeInteger((optmask & VACOPT_PROCESS_MAIN) ? 1 : 0),
										   -1));
	...
	foreach (lc, updated_stats)
	{
		...
		if (stats_context->nsegs > 0 && stats->count == stats_context->nsegs)
		{
			...
			vac_update_relstats(rel,
								...
								stats->relallvisible,
								/*
								 * GPDB does not dispatch relallfrozen back from
								 * the QEs, so preserve the current on-disk value.
								 */
