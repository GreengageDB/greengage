			...
			BufferAccessStrategy bstrategy, gp_acquire_sample_rows_context *ctx)
{
	 ...
	 * autostats.
	 *
	 * In the first case, we always want to make sure the user has the
	 * required privileges on the table.  In the autovacuum case, it will be
	 * called as superuser so we don't really care, but the privilege check
	 * should always succeed.  For autostats, we only do the check if
	 * gp_autostats_allow_nonowner=false, otherwise we can proceed with the
	 * analyze.
	 *
			   ...
			   int elevel, gp_acquire_sample_rows_context *ctx)
{
	...
	else /* funny indentation to avoid re-indenting upstream code */
  {
	...
	if (!inh)
	{
		...
		{
			/* AO/AOCS relations have no visibility map */
			relallvisible = 0;
			relallfrozen = 0;
		}
		else if (RELKIND_HAS_STORAGE(onerel->rd_rel->relkind))
...
examine_attribute(Relation onerel, int attnum, Node *index_expr, int elevel)
{
	...
	stats->elevel = elevel;
	/*
	 * GPDB: keep a copy of the pg_attribute fixed part so the MPP
	 * statistics-merging code (merge_leaf_stats, HLL helpers, ...) can reach
	 * attname/attrelid/attnum from just the VacAttrStats.  Note that in PG17
	 * attstattarget is no longer part of the fixed pg_attribute record, so it
	 * is carried separately in stats->attstattarget below.
	 */
					...
					double *totalrows, double *totaldeadrows)
{
	 ...
	 * above).  The read stream would drive md into the append-optimized
	 ...
	 *
	 * For heap, it is safe to use batching, as block_sampling_read_stream_next
	 * never blocks.
	 */
	if (RelationIsAppendOptimized(onerel))
		stream = NULL;
				 ...
				 double totalrows)
{
	...
	if (ndistinct > -1 && OidIsValid(eqopr))
	{
		...
		mcvpairArray = aggregate_leaf_partition_MCVs(
			...
			stats->attstattarget, ndistinct, &num_mcv, &rem_mcv,
			resultMCV);
		MemoryContextSwitchTo(old_context);
	...
	if (OidIsValid(eqopr) && OidIsValid(ltopr))
	{
		...
		int num_hist = aggregate_leaf_partition_histograms(
			stats->attr->attrelid, stats->attr->attnum,
			numPartitions, heaptupleStats, relTuples,
			stats->attstattarget, mcvpairArray + num_mcv,
