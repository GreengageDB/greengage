...
DefineRange(ParseState *pstate, CreateRangeStmt *stmt)
{
	...
	bool		multirangeNameAutoGenerated = false;
	Oid			rangeArrayOid;
	Oid			multirangeOid;
	...
	/*
	 * GPDB: the multirange and multirange-array type OIDs must be pre-assigned
	 * on the QD keyed by (name, namespace) and dispatched to the segments, so
	 * resolve the auto-generated multirange name here, before OID assignment,
	 * rather than just before TypeCreate as upstream does.  The probed names
	 * do not collide with the types being created, so computing them earlier
	...
	if (multirangeTypeName == NULL)
	{
		...
		multirangeNameAutoGenerated = true;
	}
	multirangeArrayName = makeArrayTypeName(multirangeTypeName, multirangeNamespace);
	...
	multirangeArrayOid = AssignTypeMultirangeArrayOid(multirangeArrayName,
													  multirangeNamespace);
	...
	/*
	 * Create the multirange that goes with it.  The name was resolved above
	 * (GPDB pre-assigns its OID before this point).  For a user-specified
	 * multirange name, check whether it already exists.
	 */
	if (!multirangeNameAutoGenerated)
	{
		Oid			old_typoid;
	...
	/* Create the multirange's array type (name resolved above) */
...
AssignTypeMultirangeOid(char *multirangeTypeName, Oid typeNamespace)
{
	...
	else
	{
		...
		/*
		 * GPDB: use the OID-dispatch-aware allocator so the segments reuse the
		 * OID pre-assigned on the QD (keyed by name+namespace), as for the
		 * range and array types.
		 */
		type_multirange_oid = GetNewOidForType(pg_type, TypeOidIndexId,
											   Anum_pg_type_oid,
											   multirangeTypeName, typeNamespace);
		table_close(pg_type, AccessShareLock);
	}
 ...
 */
Oid
AssignTypeMultirangeArrayOid(char *multirangeArrayName, Oid typeNamespace)
{
	...
	else
	{
		...
		/* GPDB: OID-dispatch-aware allocation, see AssignTypeMultirangeOid(). */
		type_multirange_array_oid = GetNewOidForType(pg_type, TypeOidIndexId,
													 Anum_pg_type_oid,
													 multirangeArrayName,
													 typeNamespace);
		table_close(pg_type, AccessShareLock);
...
AlterTypeSetDefaultEnc(AlterTypeStmtSetDefaultEnc *stmt)
{
	...
	/* check permissions on type */
	if (!object_ownercheck(TypeRelationId, typid, GetUserId()))
