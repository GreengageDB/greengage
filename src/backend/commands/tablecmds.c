 ...
 * serializable Nodes for dispatch to QE segments.
 */
...
static List *MergeAttributes(List *columns, const List *supers, char relpersistence,
							 ...
							 List **supnotnulls, bool gp_style_alter_part);
...
static RangeVar *make_temp_table_name(Relation rel, ProcNumber id);
			   ...
			   bool dispatch, bool useChangedOpts, GpPolicy *intoPolicy)
{
	 ...
	 *
	 * Note: the access method (including for partitioned tables) has already
	 * been selected and stored in accessMethodId earlier in this function,
	 * because GPDB needs the AM to extract/validate the AO/AOCS reloptions.
	 */
		...
		(Gp_role != GP_ROLE_EXECUTE || !gp_dispatch_utility_statement))
	{
		 ...
		 * gp_dispatch_utility_statement == false is not dispatched and the QE
		 * builds its own copy, so it must compute the AOCS column encodings
		 * itself -- otherwise the new partition has no pg_attribute_encoding
		 * rows and AOCO scans/inserts fail.
		 */
		/*
	...
	if (Gp_role != GP_ROLE_EXECUTE && stmt->constraints)
	{
		...
		/*
		 * Remember the names of the CHECK constraints we just created, so the
		 * not-null constraints below can avoid colliding with them (upstream
		 * REL_18_4 fix).
		 */
		foreach_ptr(CookedConstraint, cons, newCookedDefaults)
		{
			if (cons->name != NULL)
...
ExecuteTruncate(TruncateStmt *stmt)
{
	...
	ExecuteTruncateGuts(rels, relids, relids_logged,
						stmt->behavior, stmt->restart_seqs, false, stmt);
	...
	switch (cmd->subtype)
	{
		...
		case AT_AlterColumnType:	/* ALTER COLUMN TYPE */
			...
			cmd = ATParseTransformCmd(wqueue, tab, rel, cmd, recurse, lockmode,
									  AT_PASS_UNSET, context, &cmd->prepStmts);
			Assert(cmd != NULL);
...
ATAocsWriteNewColumns(AlteredTableInfo *tab)
{
	...
	if (Gp_role != GP_ROLE_DISPATCH && scancol != -1)
	{
		...
		for (segi = 0; segi < nseg; ++segi)
		{
			 ...
			 * appendonly segment.
			 */
			aocs_headerscan_opensegfile(sdesc, segInfos[segi], basepath.str);
			...
			/* GPDB: AO segfiles use RelFileNode; convert from PG16 rd_locator */
			rnode.node.spcNode = rel->rd_locator.spcOid;
			rnode.node.dbNode = rel->rd_locator.dbOid;
			rnode.node.relNode = rel->rd_locator.relNumber;
				...
				AlterTableUtilityContext *context)
{
	...
	if (RELKIND_HAS_STORAGE(relkind))
	{
		...
		/*
		 * GPDB: A column of a domain type with constraints cannot use the
		 * missing-value optimization; force a rewrite so the CoerceToDomain
		 * default is evaluated for every existing row.
		 */
							 ...
							 bool hasperiod)
{
	...
	/*
	 * Greengage Database: foreign keys are not enforced, so skip the initial
...
ATExecSetAccessMethodNoStorage(Relation rel, Oid newAccessMethodId)
{
	...
	/*
	 * GPDB: remove the pg_attribute_encoding entries when we are changing an
	 * AOCO table to some other AM.
	 */
	if (oldAccessMethodId == AO_COLUMN_TABLE_AM_OID)
		RemoveAttributeEncodingsByRelid(reloid);
...
index_copy_data(Relation rel, RelFileLocator newrlocator)
{
	 ...
	 */
	dstrel = RelationCreateStorage(newrlocator, rel->rd_rel->relpersistence, smgr_which, true);
		 ...
		 forkNum <= MAX_FORKNUM; forkNum++)
	{
		if (smgrexists(RelationGetSmgr(rel), forkNum))
		{
			...
			if (RelationIsPermanent(rel) ||
				...
				log_smgrcreate(&newrlocator, forkNum, smgr_which);
					 ...
					 bool useExistingColumnAttributes)
{
	...
	DestReceiver *dest;
	QueryDesc *queryDesc;
	RangeVar *tmprel = make_temp_table_name(rel, MyProcNumber);
	TupleDesc	tupdesc;
	int			attno;
	bool		pre_built;
...
static RangeVar *
make_temp_table_name(Relation rel, ProcNumber id)
{
	char	   *nspname;
