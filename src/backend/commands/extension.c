 ...
 */
static void
execute_sql_string_internal(const char *sql, const char *filename)
{
...
/*
 * GPDB: exported 1-argument entry point (see extension.h).  Greenplum callers
 * such as tablecmds_gp.c run generated SQL strings that have no originating
 * script file, so no filename is available for error context.
 */
void
execute_sql_string(const char *sql)
{
	execute_sql_string_internal(sql, "");
}
						 ...
						 const char *schemaName)
{
	...
	PG_TRY();
	{
		...
		execute_sql_string_internal(c_sql, filename);
	}
	PG_FINALLY();
	{
		/*
