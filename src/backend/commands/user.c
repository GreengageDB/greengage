...
AlterRole(ParseState *pstate, AlterRoleStmt *stmt)
{
	...
	/* GPDB: remember whether the role is currently a superuser */
	bWas_super = authform->rolsuper;
	...
	{
		/*
		 * Things an unprivileged user certainly can't do.  The GPDB-specific
		 * options (resource queue/group, external table permissions and DENY
		 * rules) are as much off-limits as the upstream role attributes.
		 */
		if (dinherit || dcreaterole || dcreatedb || dcanlogin || dconnlimit ||
			dvalidUntil || disreplication || dbypassRLS ||
			dresqueue || dresgroup || exttabcreate || exttabnocreate ||
			addintervals || dropintervals)
			ereport(ERROR,
					(errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
					 errmsg("permission denied to alter role"),
	...
	if (dissuper)
	{
		...
		if (!isNull)
		{
			 ...
			 */
			if (!should_be_super && roleResgroup == ADMINRESGROUP_OID)
			{
				new_record[Anum_pg_authid_rolresgroup - 1] = ObjectIdGetDatum(DEFAULTRESGROUP_OID);
				new_record_repl[Anum_pg_authid_rolresgroup - 1] = true;
			}
			else if (should_be_super && roleResgroup == DEFAULTRESGROUP_OID)
			{
				new_record[Anum_pg_authid_rolresgroup - 1] = ObjectIdGetDatum(ADMINRESGROUP_OID);
				new_record_repl[Anum_pg_authid_rolresgroup - 1] = true;
			}
