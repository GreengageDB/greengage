...
RegisterBackgroundWorker(BackgroundWorker *worker)
{
	 ...
	 * process.
	 *
	 * GGDB: auxiliary background workers (FTS et al.) are registered directly
	 * from load_auxiliary_libraries() rather than through
	 * shared_preload_libraries, including a re-registration from
	 * SubPostmasterMain() under EXEC_BACKEND.  Exempt them from the
	 * postmaster-only restriction below, matching the historical
	 * "&& !auxworker" guard.
	 */
	if (!auxworker && (IsUnderPostmaster || !IsPostmasterEnvironment))
	{
		/*
	 ...
	 * BackgroundWorkerShmemInit().  GGDB: auxiliary workers may be
	 * (re-)registered after shmem init (EXEC_BACKEND SubPostmasterMain), so
	 * exempt them here as well.
	 */
	if (!auxworker && BackgroundWorkerData != NULL)
