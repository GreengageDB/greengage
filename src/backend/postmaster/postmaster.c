 ...
 */
void
processCancelRequest(int backendPID, int32 cancelAuthCode, MsgType code)
{
	Backend    *bp;
...
BackendStartup(ClientSocket *client_sock)
{
	 ...
	 * intentionally not treated as dead_end so they can still be served.
	 */
	bn->dead_end = (startup_data.canAcceptConnections != CAC_OK &&
					startup_data.canAcceptConnections != CAC_SUPERUSER &&
					startup_data.canAcceptConnections != CAC_MIRROR_READY);
