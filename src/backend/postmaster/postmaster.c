...
PostmasterMain(int argc, char *argv[])
{
	 ...
	 */
	while ((opt = getopt(argc, argv, "B:bC:c:D:d:EeFf:h:ijk:lMmN:OPp:r:S:sTt:W:-:")) != -1)
	{
		switch (opt)
		{
			case 'B':
...
checkPgDir(const char *dir)
{
	...
	if (stat(buf, &st) != 0)
	{
		...
		/*
		 * Quit all processes and exit.  In PG16 the postmaster signal handler
		 * pmdie() was split into handle_pm_shutdown_request_signal() (sets a
		 * pending flag) and process_pm_shutdown_request() (does the work).
		 * Request an immediate shutdown, the equivalent of the old
		 * pmdie(SIGQUIT), by setting the pending flag and processing it here.
		 */
...
canAcceptConnections(BackendType backend_type)
{
	...
	if (pmState != PM_RUN && pmState != PM_HOT_STANDBY)
	{
		 ...
		 * CAC_NOTHOTSTANDBY branch below would shadow the mirror-ready state
		 * for good.  CAC_NOTHOTSTANDBY has no FTS exemption in
		 ...
		 * upstream fail-fast CAC_NOTHOTSTANDBY behavior for genuine hot
		...
		case SmartShutdown:
			...
			if (pmState == PM_STARTUP)
			{
				...
				if (StartupPMChild != NULL)
					signal_child(StartupPMChild, SIGTERM);
				if (WalReceiverPMChild != NULL)
					signal_child(WalReceiverPMChild, SIGTERM);
...
PostmasterStateMachine(void)
{
	 ...
	 */
	if (pmState == PM_STARTUP && StartupPMChild == NULL &&
		WalReceiverPMChild == NULL)
	{
		UpdatePMState(PM_WAIT_DEAD_END);
	}
	...
	if (pmState == PM_STOP_BACKENDS || pmState == PM_WAIT_BACKENDS)
	{
		...
		targetMask = btmask_add(targetMask,
								...
								B_BG_WORKER,
		/*
		 * GPDB: the FTS and fault-injection handler processes are
		 * backend-like (they serve a single probe/request connection), so
		 * they must be terminated together with regular backends.  Listing
		 * them here also keeps the BTYPE_MASK_ALL completeness assertion below
		 * satisfied now that the BackendType enum carries GPDB-specific types.
		 */
	...
	if (WalReceiverRequested)
	{
			...
			Shutdown <= SmartShutdown)
		{
			...
			if (WalReceiverPMChild != NULL)
			{
				...
				/*
				 * GPDB: the wal receiver has been launched at least once, so a
				 * mirror can now report itself as ready to FTS (see
				 * canAcceptConnections / CAC_MIRROR_READY).
				 */
				SetMirrorReadyFlag();
			}
...
BackendStartup(ClientSocket *client_sock)
{
	...
	/*
	 * GPDB: superuser-reserved and mirror-ready (FTS probe) connections are
	 * intentionally not treated as dead-end children so they can still be
	 * served; give them a real backend child slot too.
	 */
	if (cac == CAC_OK || cac == CAC_SUPERUSER || cac == CAC_MIRROR_READY)
	{
...
process_pm_pmsignal(void)
{
		...
		pmState == PM_STARTUP && Shutdown == NoShutdown)
	{
		...
		/*
		 * GPDB: if a promote signal has been requested we don't wish to convey
		 * PM_STATUS_STANDBY, instead wish pg_ctl -w to wait till
		 * connections can be actually accepted by the database.
		 *
		 * PG16 removed the promote_trigger_file GUC; promotion is now
		 * signal-based, so consult CheckPromoteSignal() instead of stat()'ing
		 * a trigger file.
		 */
	...
	{
		/*
		 * GPDB: the FTS probe process is a background worker tracked via its
		 * shared-memory PID rather than a PMChild handle, so wake it with a
		 * direct kill() instead of signal_child().
		 */
		kill(FtsProbePID(), SIGINT);
	}
	...
	{
		/*
		 * GPDB: like the FTS probe process, the DTX recovery process is a
		 * background worker tracked via its shared-memory PID, so wake it with
		 * a direct kill() instead of signal_child().
		 */
		kill(DtxRecoveryPID(), SIGINT);
	}
