				   ...
				   char **logdetail)
{
	...
	switch (get_password_type(shadow_pass))
	{
		...
		case PASSWORD_TYPE_PLAINTEXT:
			if (strlen(client_pass) == strlen(shadow_pass) &&
				timingsafe_bcmp(client_pass, shadow_pass, strlen(shadow_pass)) == 0)
				return STATUS_OK;
			else
			{
				*logdetail = psprintf(_("Password does not match for user \"%s\"."),
		...
		case PASSWORD_TYPE_SHA256:
			...
			if (strlen(crypt_client_pass2) == strlen(shadow_pass) &&
				timingsafe_bcmp(crypt_client_pass2, shadow_pass, strlen(shadow_pass)) == 0)
				return STATUS_OK;
			else
			{
				*logdetail = psprintf(_("Password does not match for user \"%s\"."),
